...
const EXEC_EXTS: &[&str] = &[
    "exe", "scr", "bat", "com", "cmd", "vbs", "vbe", "js", "jse", "ws", "wsh",
    "wsf", "ps1", "psm1", "psd1", "msi", "msp", "mst", "msc", "cpl", "reg",
    "lnk", "pif", "scf", "inf", "hta", "gadget", "jar", "appx", "appxbundle",
    "app", "dmg", "pkg", "deb", "rpm", "apk", "ipa",
    "sh", "bash", "zsh", "ksh", "csh", "fish", "py", "pyc", "rb", "pl", "php",
    "dll", "so", "dylib", "sys",
];
...
const WEB_EXTS: &[&str] = &["html", "htm", "xhtml", "shtml", "xml", "svg", "mht", "mhtml", "url"];
...
fn all_exts(name: &str) -> Vec<String> {
    name.split('.').skip(1).map(|p| p.to_ascii_lowercase()).collect()
}
...
fn check_double_extension(name: &str, last_ext: &str) -> Option<String> {
    ...
    let common_doc = is_in(DOC_EXTS_SAFE, pre_last)
        || is_in(IMAGE_EXTS_SAFE, pre_last)
        || pre_last == "txt";
...
fn check_lure_name(name: &str, ext: &str) -> Option<String> {
    ...
    let lures = [
        "fattura", "invoice", "ricevuta", "receipt", "ordine", "order",
        "dhl", "ups", "fedex", "tnt", "bartolini", "brt", "gls", "poste",
        "amazon", "tracking", "spedizione", "pagamento", "payment",
        "remittance", "bonifico", "verifica", "verify", "account", "password",
        "documento", "scansione", "scan",
    ];
    let hits: Vec<&str> = lures.iter().copied().filter(|l| lower.contains(l)).collect();
    if !hits.is_empty() && (is_in(EXEC_EXTS, ext) || is_in(ARCHIVE_EXTS, ext) || is_in(DISK_EXTS, ext)) {
        return Some(format!(
            "Nome attira-click ({}) abbinato a tipo {} — pattern tipico di phishing.",
            hits.join(", "),
            if is_in(EXEC_EXTS, ext) { "eseguibile" }
            else if is_in(DISK_EXTS, ext) { "immagine disco" }
            else { "archivio" }
        ));
    }
    None
...
fn check_mime_mismatch(content_type: &str, ext: &str) -> Option<String> {
    ...
    if claims_pdf && ext != "pdf" && !ext.is_empty() {
        return Some(format!("MIME dichiara 'application/pdf' ma estensione è '.{ext}'."));
    }
...
pub fn analyze(
    ...
    let safe_name = if filename.trim().is_empty() { "(senza nome)" } else { filename };
    ...
    let mut level = if is_in(EXEC_EXTS, &ext) {
        ...
        suggestions.push("NON aprire. Gli eseguibili via email sono il vettore numero 1 di malware.".into());
        ThreatLevel::Danger
    } else if is_in(MACRO_EXTS, &ext) {
        reasons.push(format!("Estensione '.{ext}' = documento Office con macro VBA."));
        suggestions.push("Apri solo se atteso da un mittente fidato; disabilita le macro all'apertura.".into());
        ThreatLevel::Caution
    } else if is_in(DISK_EXTS, &ext) {
        reasons.push(format!("Estensione '.{ext}' = immagine disco, usata per bypassare l'antimalware."));
        suggestions.push("Non montare se il mittente non è chiaramente atteso.".into());
        ThreatLevel::Caution
    } else if is_in(ARCHIVE_EXTS, &ext) {
        reasons.push(format!("Estensione '.{ext}' = archivio compresso (può contenere eseguibili)."));
        suggestions.push("Apri solo se atteso; scansiona il contenuto prima di eseguire qualunque cosa.".into());
        ThreatLevel::Caution
    } else if is_in(WEB_EXTS, &ext) {
        reasons.push(format!("Estensione '.{ext}' = pagina web (può richiamare risorse remote / fingere un login)."));
    ...
    if let Some(r) = check_double_extension(safe_name, &ext) {
        reasons.push(r);
        suggestions.push("Mai aprire file con doppia estensione: è il pattern classico di un trojan.".into());
    ...
    if let Some(r) = check_lure_name(safe_name, &ext) {
        reasons.push(r);
        if matches!(level, ThreatLevel::Safe) { level = ThreatLevel::Caution; }
        if is_in(EXEC_EXTS, &ext) { level = ThreatLevel::Danger; }
    }
    ...
    if let Some(b) = bytes {
        ...
        if reasons.iter().any(|r| {
            ...
            level = ThreatLevel::Danger;
            suggestions.push("Contenuto attivo rilevato dentro il file: alto rischio di payload nascosto.".into());
        }
    }
...
pub fn deep_scan(filename: &str, bytes: &[u8]) -> Vec<String> {
    ...
    if matches!(ext.as_str(), "docx" | "docm" | "xlsx" | "xlsm" | "pptx" | "pptm" | "odt" | "ods" | "odp" | "zip" | "epub")
    ...
    if ext == "svg" {
        if contains_ignore_case(bytes, b"<script") {
            findings.push("SVG contiene <script>: può eseguire JavaScript se aperto in browser.".into());
        }
...
fn scan_pdf(bytes: &[u8]) -> Vec<String> {
    ...
    let mut counters: Vec<(&str, &str)> = vec![
        ("/JavaScript", "JavaScript embedded (rischio exploit reader)"),
        ("/JS",         "Riferimento JS abbreviato"),
        ("/OpenAction", "OpenAction (codice eseguito automaticamente all'apertura)"),
...
fn scan_zip(bytes: &[u8]) -> Vec<String> {
    ...
    for i in 0..zip.len() {
        ...
        if name.ends_with("vbaproject.bin") { has_vba = true; }
        if name.contains("vbaproject")      { has_vba = true; }
        if name.contains("macros/")         { has_vba = true; }
        if name.contains("xl/macrosheets/") { has_xl_macro = true; }
        if name.ends_with(".exe") || name.ends_with(".dll") || name.ends_with(".scr")
            || name.ends_with(".bat") || name.ends_with(".cmd") || name.ends_with(".vbs")
            || name.ends_with(".ps1") || name.ends_with(".js") {
...
fn contains_ignore_case(haystack: &[u8], needle_lower: &[u8]) -> bool {
    ...
    haystack.windows(needle_lower.len()).any(|w| {
        w.iter().zip(needle_lower.iter()).all(|(a, b)| a.to_ascii_lowercase() == *b)
    })
}
...
mod tests {
    ...
    fn macro_doc_is_caution() {
        let r = analyze("budget.xlsm", "application/vnd.ms-excel.sheet.macroEnabled.12", 9000, None);
