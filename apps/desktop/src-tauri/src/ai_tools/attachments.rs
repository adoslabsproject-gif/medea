...
fn extract_pdf(bytes: &[u8]) -> Result<String> {
    pdf_extract::extract_text_from_mem(bytes)
        .map_err(|e| anyhow!("PDF parse fallito: {e}"))
}
...
fn extract_docx(bytes: &[u8]) -> Result<String> {
    ...
    let mut zip = zip::ZipArchive::new(cursor)
        .map_err(|e| anyhow!("DOCX non è uno ZIP valido: {e}"))?;
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(e)) => {
                let name = e.name();
                if name.as_ref() == b"w:t" { in_w_t = true; }
                if name.as_ref() == b"w:p" || name.as_ref() == b"w:br" { out.push('\n'); }
            }
            Ok(Event::End(e)) => {
                if e.name().as_ref() == b"w:t" { in_w_t = false; }
                if e.name().as_ref() == b"w:p" { out.push('\n'); }
            }
            Ok(Event::Text(t)) => {
                if in_w_t {
                    if let Ok(s) = t.unescape() { out.push_str(&s); }
                }
            }
            Ok(Event::Eof) => break,
...
fn strip_block_ci(input: &str, open: &str, close: &str) -> String {
    ...
    loop {
        ...
        let Some(oi) = oi else { out.push_str(rest); break; };
...
fn collapse_ws(input: &str, repl: &str) -> String {
    ...
    for ch in input.chars() {
        if ch.is_whitespace() {
            if !prev_ws { out.push_str(repl); prev_ws = true; }
        } else {
            out.push(ch);
            prev_ws = false;
        }
...
mod tests {
    ...
    #[test]
    fn html_strips_tags() {
        let r = extract_text("page.html", b"<html><body><p>Ciao <b>mondo</b></p><script>x</script></body></html>").unwrap();
