...
function insertLegacyNode(id: string, opts: { compiled?: string | null; executor?: string } = {}): void {
  ...
  `).run(id, `slug-${id}`, `Node ${id}`, opts.executor ?? 'exec', opts.compiled === undefined ? 'bundle' : opts.compiled, at, at);
}
...
function flagSet(): boolean {
  return conn.prepare('SELECT value FROM system_flags WHERE key = ?').get(INTEGRITY_ENFORCED_FLAG) !== undefined;
}
...
describe('🚨 backfillCustomNodeIntegrity', () => {
  it('firma i nodi compilati legacy con record VERIFICABILE + setta il flag', () => {
    ...
    const verdict = verifyPackageIntegrity(
      {
        slug: 'slug-n1', semver: '0.1.0',
        sourceExecutor: 'exec', sourceDefinition: 'def', sourceSchema: 'schema',
