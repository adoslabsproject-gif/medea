      ...
      item: [
        {
          ...
          request: {
            ...
            url: {
              raw: 'https://api.acme.test/v2/charges?limit=10',
              host: ['api', 'acme', 'test'],
              path: ['v2', 'charges'],
        ...
        },
        {
          ...
          request: {
            method: 'GET',
            url: {
              raw: 'https://api.acme.test/v2/charges/:id',
              host: ['api', 'acme', 'test'],
              path: ['v2', 'charges', ':id'],
            },
          },
        },
        {
          ...
          request: {
            method: 'POST',
            url: {
              raw: 'https://api.acme.test/v2/charges',
              host: ['api', 'acme', 'test'],
              path: ['v2', 'charges'],
            },
...
describe('parsing — rifiuti', () => {
  ...
  it('zero request → errore (no spec vuoto)', () => {
    expect(() =>
      postmanToOpenApi({ info: { name: 'x' }, item: [{ name: 'folder', item: [] }] }),
    ).toThrow(/non contiene request/);
  });
});
...
describe('mapping collection → openapi', () => {
  ...
  it('auth bearer → securitySchemes http bearer', () => {
    const { spec } = postmanToOpenApi(COLLECTION);
    const schemes = (
      spec.components as { securitySchemes: Record<string, { type: string; scheme?: string }> }
    ).securitySchemes;
  ...
  it('apikey auth in header → securitySchemes apiKey', () => {
    const { spec } = postmanToOpenApi({
      ...
      auth: {
        type: 'apikey',
        apikey: [
          { key: 'key', value: 'X-Acme' },
          { key: 'in', value: 'header' },
        ],
      },
      item: [
        { name: 'ping', request: { method: 'GET', url: { host: ['a', 'test'], path: ['ping'] } } },
      ],
    });
    const schemes = (
      spec.components as {
      ...
      item: [
        {
          ...
          item: [
            {
              name: 'List v1',
              request: { method: 'GET', url: { host: ['a', 'test'], path: ['charges'] } },
            },
          ],
        },
        {
...
describe('🚨 CATENA end-to-end: postman → openapi → nodo eseguibile', () => {
  it("genera un custom node valido e l'executor costruisce la request giusta", async () => {
    ...
    const def = JSON.parse(
      node.sourceDefinition.replace('export const definition = ', '').replace(/;\s*$/, ''),
    ) as {
    ...
    const idKey = def.config.find(
      (f) => f.showIf?.equals === getOneId && f.key.endsWith('id'),
    )!.key;

    const executor = loadExecutor(node.sourceExecutor);
    const fetchMock = vi
      .fn()
      .mockResolvedValue({ status: 200, ok: true, text: async () => '{"id":"ch_9"}' });
  ...
  it('determinismo: stessa collection → stesso spec', () => {
    expect(JSON.stringify(postmanToOpenApi(COLLECTION).spec)).toBe(
      JSON.stringify(postmanToOpenApi(COLLECTION).spec),
    );
  });
});
