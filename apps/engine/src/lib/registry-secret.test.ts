 ...
 * FLOWFORGE_INTERNAL_TOKEN, che onboarding.ts rigenera (`randomBytes(32)`) a
...
const ENV_KEYS = [
  'FLOWFORGE_REGISTRY_SECRET',
  'FLOWFORGE_SSO_SECRET',
  'FLOWFORGE_TENANT_ID',
  'FLOWFORGE_INTERNAL_TOKEN',
] as const;
...
const SSO = 'a'.repeat(64); // FLOWFORGE_SSO_SECRET in prod è lungo (≥16)
...
describe('registrySecret — derivazione STABILE da SSO_SECRET', () => {
  it('🔒 REGRESSIONE STREAMMY: cambiare INTERNAL_TOKEN NON cambia il secret', () => {
    process.env.FLOWFORGE_SSO_SECRET = SSO;
    process.env.FLOWFORGE_TENANT_ID = TENANT;

    process.env.FLOWFORGE_INTERNAL_TOKEN = 'token-prima-del-recreate';
    const before = registrySecret();
    // recreate del container: onboarding rigenera randomBytes(32)
    process.env.FLOWFORGE_INTERNAL_TOKEN = 'token-DOPO-il-recreate-completamente-diverso';
  ...
  it('🔒 ISOLAMENTO: tenant diversi (stesso SSO) → secret DIVERSI', () => {
    process.env.FLOWFORGE_SSO_SECRET = SSO;
    process.env.FLOWFORGE_TENANT_ID = 'tenant-A';
    const a = registrySecret();
    process.env.FLOWFORGE_TENANT_ID = 'tenant-B';
  ...
  it('🔒 SSO diversi (stesso tenant) → secret DIVERSI (non si forgia senza SSO_SECRET)', () => {
    process.env.FLOWFORGE_TENANT_ID = TENANT;
    process.env.FLOWFORGE_SSO_SECRET = 'b'.repeat(64);
    const a = registrySecret();
    process.env.FLOWFORGE_SSO_SECRET = 'c'.repeat(64);
    const b = registrySecret();
    expect(a).not.toBe(b);
  });

  it('override esplicito FLOWFORGE_REGISTRY_SECRET vince su tutto (rotation ops)', () => {
    process.env.FLOWFORGE_REGISTRY_SECRET = 'rotazione-controllata-ops';
    process.env.FLOWFORGE_SSO_SECRET = SSO;
    process.env.FLOWFORGE_TENANT_ID = TENANT;
    process.env.FLOWFORGE_INTERNAL_TOKEN = 'qualsiasi';
    expect(registrySecret()).toBe('rotazione-controllata-ops');
  });

  it('SSO troppo corto (<16) → NON deriva, fallback a INTERNAL_TOKEN', () => {
    process.env.FLOWFORGE_SSO_SECRET = 'corto'; // <16
    process.env.FLOWFORGE_TENANT_ID = TENANT;
    process.env.FLOWFORGE_INTERNAL_TOKEN = 'fallback-token';
  ...
  it('soglia esatta: SSO di 16 char (≥16) → deriva, non fa fallback', () => {
    process.env.FLOWFORGE_SSO_SECRET = 'x'.repeat(16);
    process.env.FLOWFORGE_TENANT_ID = TENANT;
    process.env.FLOWFORGE_INTERNAL_TOKEN = 'NON-deve-uscire-questo';
