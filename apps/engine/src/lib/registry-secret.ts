 ...
 * deve derivare da `MEDEA_INTERNAL_TOKEN` — quello è `randomBytes(32)`
 ...
 * node def"). Ora il secret è DERIVATO da `MEDEA_SSO_SECRET` (stabile,
...
export function registrySecret(): string {
  ...
  const explicit = process.env.MEDEA_REGISTRY_SECRET;
  ...
  const sso = process.env.MEDEA_SSO_SECRET ?? '';
  const tenant = process.env.MEDEA_TENANT_ID ?? '';
