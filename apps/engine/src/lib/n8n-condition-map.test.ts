...
describe('mapN8nIfConditions — n8n v2 (combinator + conditions[])', () => {
    ...
    const r = mapN8nIfConditions({
      conditions: {
        ...
        conditions: [
          ...
          { leftValue: '={{ $json.age }}', rightValue: 18, operator: { type: 'number', operation: 'gt' } },
        ],
      },
    });
    const parsed = rules(r.config);
    expect(parsed.combinator).toBe('AND');
    expect(parsed.rules[0]).toEqual({ left: 'input.status', op: 'equals', type: 'string', right: 'active' });
    expect(parsed.rules[1]).toEqual({ left: 'input.age', op: 'gt', type: 'number', right: '18' });
  });

  it('combinator OR rispettato', () => {
    const r = mapN8nIfConditions({
      conditions: { combinator: 'or', conditions: [{ leftValue: '={{ $json.a }}', rightValue: '1', operator: { operation: 'equals' } }] },
    });
  ...
  it('operatore UNARIO (isEmpty) → niente right', () => {
    const r = mapN8nIfConditions({
      conditions: { conditions: [{ leftValue: '={{ $json.note }}', operator: { type: 'string', operation: 'isEmpty' } }] },
    });
    expect(rules(r.config).rules[0]).toEqual({ left: 'input.note', op: 'is-empty', type: 'string' });
  });

  it('operatore SCONOSCIUTO → warning, regola saltata', () => {
    const r = mapN8nIfConditions({
      conditions: { conditions: [{ leftValue: '={{ $json.x }}', operator: { operation: 'quantumEntangles' } }] },
    });
...
describe('mapN8nIfConditions — n8n v1 (string/number/boolean[])', () => {
  it('legacy {string:[{value1,operation,value2}]} → rules', () => {
    const r = mapN8nIfConditions({
      conditions: { string: [{ value1: '={{ $json.role }}', operation: 'contains', value2: 'admin' }] },
    });
    expect(rules(r.config).rules[0]).toEqual({ left: 'input.role', op: 'contains', type: 'string', right: 'admin' });
  });
...
describe('mapN8nIfConditions — operandi', () => {
  it('$json["key"] → input.key; letterale invariato', () => {
    const r = mapN8nIfConditions({
      conditions: { conditions: [{ leftValue: '={{ $json["userId"] }}', rightValue: 'abc', operator: { operation: 'equals' } }] },
    });
  ...
  it('riferimento ad altro nodo ($node) → warning', () => {
    const r = mapN8nIfConditions({
      conditions: { conditions: [{ leftValue: '={{ $node["Prev"].json.x }}', rightValue: '1', operator: { operation: 'equals' } }] },
    });
  ...
  it('OR: role contains admin OPPURE level>=5 → valutazione corretta', () => {
    const { config } = mapN8nIfConditions({
      conditions: {
        combinator: 'or',
        conditions: [
          { leftValue: '={{ $json.role }}', rightValue: 'admin', operator: { type: 'string', operation: 'contains' } },
          { leftValue: '={{ $json.level }}', rightValue: '5', operator: { type: 'number', operation: 'gte' } },
        ],
      },
    });
    const rs = parseRuleset(config.conditionRules)!;
