...
import {
  ...
  type VisionLlmTarget,
} from './vision-tools.service.js';
...
export function resolveWorkflowName(
  name: string,
): { workflow: WorkflowSummary; confidence: number } | null {
...
const BARE_DOMAIN_REGEX =
  /\b([a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.[a-z]{2,24}(?:\.[a-z]{2,24})?)\b/gi;
...
export function detectIntents(userMessage: string): DetectedIntent[] {
  ...
  for (const rx of LIST_WORKFLOWS_TRIGGERS) {
    if (rx.test(msg)) {
      intents.push({ type: 'list_workflows', args: {}, reason: 'list workflows trigger' });
      break;
    }
  }
  for (const rx of RUN_WORKFLOW_TRIGGERS) {
    ...
    if (m?.[1]) {
      intents.push({
        type: 'run_workflow',
        args: { workflowName: m[1].trim() },
        reason: `run trigger: "${m[1].trim()}"`,
      });
      break;
    }
  }
  for (const rx of ENABLE_WORKFLOW_TRIGGERS) {
    ...
    if (m?.[1] && !intents.some((i) => i.type === 'run_workflow')) {
      intents.push({
        ...
        args: { workflowName: m[1].trim() },
        reason: `enable trigger: "${m[1].trim()}"`,
      });
      break;
    }
  }
  for (const rx of DISABLE_WORKFLOW_TRIGGERS) {
    ...
    if (m?.[1] && !intents.some((i) => i.type === 'run_workflow' || i.type === 'enable_workflow')) {
      intents.push({
        ...
        args: { workflowName: m[1].trim() },
        reason: `disable trigger: "${m[1].trim()}"`,
      });
      break;
    }
  }
  for (const rx of CONFIGURE_NODE_TRIGGERS) {
    ...
    if (m?.[1] && m[2] && m[3]) {
      intents.push({
        ...
        args: {
          nodeId: m[1].trim(),
          workflowName: m[2].trim(),
          configPatch: parseConfigPatch(m[3].trim()),
        },
  ...
  // 1. URL espliciti
  const explicitUrls = Array.from(msg.matchAll(URL_REGEX))
    .map((m) => m[1])
  ...
  if (hasFetchTrigger && bareDomains.length > 0) {
    for (const dom of bareDomains.slice(0, 2)) {
      intents.push({
        type: 'fetch_url',
        args: { url: `https://${dom}` },
        reason: `Bare domain + verbo fetch: ${dom}`,
      });
    }
  }
  ...
  if (hasSearchTrigger) {
    ...
    const query = msg
      .replace(
        /\b(cerca(?:mi|temi)?|cerchi|cercare|trova(?:mi|temi)?|trovare|google|googla(?:mi|temi)?|search|find|lookup)\b/gi,
        '',
      )
    ...
    if (query.length >= 2) {
      intents.push({
        type: 'web_search',
        args: { query },
        reason: `Trigger search keyword: "${query}"`,
      });
    }
    ...
    if (dom) {
      intents.push({
        type: 'web_search',
        args: { query: dom },
        reason: `Bare domain in messaggio info-seeking: ${dom}`,
      });
    }
  }
      ...
      switch (intent.type) {
        ...
        case 'run_workflow': {
          ...
          if (!resolved) {
            results.push({
              intent,
              data: { ok: false, error: `Workflow "${name}" non trovato.` },
          ...
          const ran = await runWorkflow({
            ...
            triggerInput: intent.args.triggerInput,
          });
          results.push({
            intent: { ...intent, args: { ...intent.args, workflowId: resolved.workflow.id } },
            data: ran,
          });
          break;
        }
        ...
        case 'disable_workflow': {
          ...
          const toggled = setWorkflowEnabled({
            workflowId: resolved.workflow.id,
            enabled: intent.type === 'enable_workflow',
          });
          results.push({
            intent: { ...intent, args: { ...intent.args, workflowId: resolved.workflow.id } },
            data: toggled,
        ...
        case 'configure_node': {
          ...
          if (
            !intent.args.nodeId ||
            !intent.args.configPatch ||
            Object.keys(intent.args.configPatch).length === 0
          ) {
            results.push({
              intent,
              data: { ok: false, error: 'configure_node richiede nodeId + key=value.' },
              error: 'invalid_args',
            });
            break;
          }
        ...
        case 'analyze_image': {
          ...
          if (!visionTarget) {
            results.push({
              ...
              data: {
                ok: false,
                error: 'Lettura immagini non disponibile: nessun provider AI risolto.',
                elapsedMs: 0,
              },
          ...
          results.push({
            ...
            data: await analyzeImage(
              intent.args.imageBase64,
              intent.args.imagePrompt,
              visionTarget,
            ),
          });
          break;
        ...
        case 'extract_document': {
          ...
          results.push({
            intent,
            data: await extractDocument(intent.args.documentBase64, intent.args.documentMime),
          });
          break;
        }
      }
  ...
  for (const r of results) {
    ...
    if (r.intent.type === 'fetch_url') {
      ...
      if (d.ok && (d.text ?? '').trim().length > 0) {
        ...
        parts.push(slice);
        if (d.structured)
          parts.push(`Dati strutturati: ${JSON.stringify(d.structured).slice(0, 2_000)}`);
        totalChars += slice.length + 500;
      } else {
        parts.push(
          `[L'utente HA allegato un'immagine ma l'analisi visiva non è disponibile ora${d.error ? `: ${d.error}` : ''}. NON negare che l'allegato esista: chiedi di riprovare o di descriverlo.]`,
        );
        totalChars += 300;
      }
      ...
      if (d.ok && (d.text ?? '').trim().length > 0) {
        const slice = d.text!.slice(0, 8_000);
        parts.push(
          `Testo estratto dal documento allegato${d.pages ? ` (${d.pages.toString()} pagine)` : ''}:`,
        );
        parts.push(slice);
        totalChars += slice.length + 500;
      } else {
        parts.push(
          `[L'utente HA allegato un documento ma l'estrazione non è riuscita${d.error ? `: ${d.error}` : ''}. NON negare che l'allegato esista.]`,
        );
        totalChars += 300;
      }
    } else {
