...
const WAKE_EXTRACTED =
  'Avvio del workspace in corso… Il tuo ambiente si era messo in pausa. Riparte in pochi secondi — questa pagina si aggiorna da sola.';
...
describe('isWorkspaceWakePage', () => {
  ...
  it('🔒 NO falso positivo: articolo che cita "avvio del workspace" senza pausa, status 200 → false', () => {
    expect(isWorkspaceWakePage('Guida: avvio del workspace su Linux, passo per passo.', 200)).toBe(
      false,
    );
  });
  it('🔒 NO falso positivo: contenuto normale di un sito → false', () => {
    expect(
      isWorkspaceWakePage('Benvenuto nel nostro e-commerce di scarpe. Spedizione gratuita.'),
    ).toBe(false);
  });
});
...
describe('fetchUrl — wake page detection (end-to-end via mock fetch)', () => {
  it('🚨 URL che torna la wake page (200 text/html) → WorkspaceWakingError, NON la passa', async () => {
    global.fetch = vi
      .fn()
      .mockResolvedValue(mockFetchResponse({ body: WAKE_HTML, contentType: 'text/html' })) as never;
    await expect(fetchUrl('https://nicola-cucurachi.app.automazionezeli.com/x')).rejects.toThrow(
      WorkspaceWakingError,
    );
  });
  ...
  it('🚨 wake servita con 503 @waking → WorkspaceWakingError', async () => {
    global.fetch = vi
      .fn()
      .mockResolvedValue(
        mockFetchResponse({ status: 503, body: WAKE_HTML, contentType: 'text/html' }),
      ) as never;
    await expect(fetchUrl('https://slug.app.automazionezeli.com/y')).rejects.toThrow(
      /wake|in pausa/i,
    );
  });
  ...
  it('✅ ANTI-REGRESSIONE: una pagina NORMALE NON viene scambiata per wake → ritorna il contenuto', async () => {
    const html =
      '<html><body><article><h1>Ricette</h1><p>' +
      'Pasta al pomodoro: ingredienti e preparazione passo per passo. '.repeat(5) +
      '</p></article></body></html>';
    global.fetch = vi
      .fn()
      .mockResolvedValue(mockFetchResponse({ body: html, contentType: 'text/html' })) as never;
