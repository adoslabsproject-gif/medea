...
function seedRun(args: {
  ...
  const steps = args.failedStep
    ? [
        { nodeId: 'n-trigger', defId: 'trigger_manual', status: 'success', output: 'ok', durationMs: 1 },
        { nodeId: args.failedStep.nodeId, defId: args.failedStep.defId, status: 'error', error: args.failedStep.error, durationMs: 472 },
      ]
...
describe('AiExplainService', () => {
  describe('error cases', () => {
    it('throws RunNotFoundError when the run id is unknown', async () => {
      ...
      await expect(
        svc.explain({ tenantId: 'acme', runId: 'nonexistent' }),
    ...
    it('throws RunSucceededError when the run status is success', async () => {
      ...
      await expect(svc.explain({ tenantId: 'acme', runId })).rejects.toBeInstanceOf(RunSucceededError);
    ...
    it('throws NoFailedStepError when status != success but no error step', async () => {
      ...
      await expect(svc.explain({ tenantId: 'acme', runId })).rejects.toBeInstanceOf(NoFailedStepError);
    });
    ...
    it('throws LlmResponseError when LLM returns non-JSON', async () => {
      ...
      await expect(svc.explain({ tenantId: 'acme', runId })).rejects.toBeInstanceOf(LlmResponseError);
    });
  ...
  describe('happy path', () => {
    it('returns explanation + fix + patch + interactionId on a partial run', async () => {
      const dispatch: LlmDispatcher = async () => JSON.stringify({
      ...
      const runId = seedRun({
        workflowId: wfId,
        status: 'partial',
        failedStep: { nodeId: 'n-mail', defId: 'action_send_email', error: '550 Sender not allowed' },
      });
  ...
  describe('dispatcher input', () => {
    it('passes the system prompt and workflow context to the dispatcher', async () => {
      const dispatch = vi.fn<LlmDispatcher>(async () => JSON.stringify({ explanation: 'e', fix: 'f' }));
      const svc = new AiExplainService(dispatch);
      const wfId = 'wf8';
      seedWorkflow(wfId);
      const runId = seedRun({
