...
describe('fetchUrl — URL validation', () => {
  ...
  it('safe-fetch validateUrlForFetch fail → throw SSRF guard', async () => {
    safeFetchMock.validateUrlForFetch.mockReturnValueOnce({
      ok: false,
      reason: 'dns_rebind',
      detail: 'CNAME to 127.0.0.1',
    } as never);
...
describe('fetchUrl — FireCrawl provider', () => {
    ...
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          ...
          body: 'Title: Jina Page\n\nJina markdown content rich enough to bypass 50 char threshold xxx',
        }),
      ) as never;
    const r = await fetchUrl('https://example.com/page');
  ...
  it('FireCrawl success:false → fallback Jina', async () => {
    ...
    global.fetch = vi
      .fn()
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'application/json', jsonBody: { success: false } }),
      )
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'text/markdown',
          body: 'Jina content threshold rich enough abc def ghi jkl mno pqr stu vwx yz0 123',
        }),
      ) as never;
    const r = await fetchUrl('https://example.com/page');
    expect(r.extractor).toBe('jina-reader');
  ...
  it('FireCrawl throw → catch + fallback', async () => {
    ...
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'text/markdown',
          body: 'Jina fallback content with enough chars to exceed the 50 char threshold of the parser path',
        }),
      ) as never;
    const r = await fetchUrl('https://example.com/page');
    expect(r.extractor).toBe('jina-reader');
...
describe('fetchUrl — Jina Reader', () => {
    ...
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          ...
          body: '<html><body><main>Main content here</main></body></html>',
        }),
      ) as never;
    const r = await fetchUrl('https://example.com/page');
  ...
  it('Jina !ok → null → fallback', async () => {
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'text/html', body: '<html><body>fallback</body></html>' }),
      ) as never;
    const r = await fetchUrl('https://example.com/page');
    expect(r.extractor).toBe('cheerio');
  });
  ...
  it('Jina throw → null → fallback', async () => {
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'text/html', body: '<html><body>x</body></html>' }),
      ) as never;
    const r = await fetchUrl('https://example.com/page');
    expect(r.extractor).toBe('cheerio');
  });
...
describe('fetchUrl — manual fetch + cheerio', () => {
  ...
  it('no Content-Type header → default text/plain', async () => {
    (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      new Response('some text body', { status: 200, headers: new Headers() }),
    );
    const r = await fetchUrl('https://example.com/');
...
describe('fetchUrl — redirect handling (manual SSRF-safe)', () => {
  it('301 redirect a URL safe → segue', async () => {
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'text/html',
          body: '<html><body><main>Final</main></body></html>',
        }),
      ) as never;
  ...
  it('301 redirect SENZA Location → break loop, usa response', async () => {
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        new Response('redirect no loc', { status: 301, headers: new Headers() }),
      ) as never;
    const r = await fetchUrl('https://example.com/');
  ...
  it('redirect a URL SSRF-blocked (private IP) → throw', async () => {
    global.fetch = vi
      ...
      .mockResolvedValueOnce(mockFetchResponse({ body: 'tiny' }))
      .mockResolvedValueOnce(
        new Response('', { status: 302, headers: new Headers({ location: 'http://127.0.0.1/' }) }),
      ) as never;
    ...
    await expect(fetchUrl('https://example.com/r')).rejects.toThrow(
      /redirect.*private_ip|fetch fallito/,
    );
  });
  ...
  it('Exa !ok → fallback', async () => {
    ...
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'application/json',
          jsonBody: ['q', ['Hit'], ['Desc'], ['https://en.wikipedia.org/wiki/Hit']],
        }),
      ) // en.wikipedia opensearch
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'application/json',
          jsonBody: { extract: 'Encyclopedia entry' },
        }),
      ) as never;
  ...
  it('Exa results con snippet undefined → fallback su text.slice(0,300)', async () => {
    ...
    global.fetch = vi.fn().mockResolvedValueOnce(
      mockFetchResponse({
        contentType: 'application/json',
        jsonBody: {
          results: [{ title: 'X', url: 'https://x.com', text: 'long text '.repeat(100) }],
        },
      }),
    ) as never;
  ...
  it('Tavily !ok → throw → fallback', async () => {
    ...
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'application/json',
          jsonBody: ['q', ['T'], ['D'], ['https://w.com']],
        }),
      )
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'application/json', jsonBody: { extract: 'ext' } }),
      ) as never;
...
describe('webSearch — Brave', () => {
  ...
  it('Brave !ok → fallback', async () => {
    ...
    global.fetch = vi
      ...
      .mockResolvedValueOnce(mockFetchResponse({ status: 500 }))
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'application/json', jsonBody: ['q', [], [], []] }),
      )
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'application/json', jsonBody: ['q', [], [], []] }),
      )
  ...
  it('Jina result senza description → fallback content.slice(0,300)', async () => {
    ...
    global.fetch = vi.fn().mockResolvedValueOnce(
      mockFetchResponse({
        contentType: 'application/json',
        jsonBody: {
          data: [{ title: 'J', url: 'https://j.com', content: 'long content '.repeat(50) }],
        },
      }),
    ) as never;
...
describe('webSearch — Searx', () => {
  ...
  it('Searx istanza ritorna 0 results → continue alla prossima', async () => {
    global.fetch = vi
      .fn()
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'application/json', jsonBody: { results: [] } }),
      )
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'application/json',
          jsonBody: { results: [{ title: 'OK', url: 'https://ok.com', content: '' }] },
        }),
      ) as never;
  ...
  it('DDG html fallback !ok → throw "DuckDuckGo X" → wikipedia', async () => {
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'application/json',
          jsonBody: { extract: 'Wikipedia answer' },
        }),
      ) as never;
...
describe('webSearch — Wikipedia', () => {
  it('it.wikipedia success → ritorna answer + results', async () => {
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'text/html', body: '<html><body>noting</body></html>' }),
      )
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'text/html', body: '<html><body>noting</body></html>' }),
      )
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'application/json',
          jsonBody: { extract: 'Galileo Galilei era un astronomo italiano.' },
        }),
      ) as never;
  ...
  it('it.wikipedia 0 results → prova en.wikipedia', async () => {
    global.fetch = vi
      ...
      .mockResolvedValueOnce(mockFetchResponse({ status: 500 }))
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'text/html', body: '<html><body></body></html>' }),
      )
      .mockResolvedValueOnce(
        mockFetchResponse({ contentType: 'text/html', body: '<html><body></body></html>' }),
      )
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'application/json',
          jsonBody: ['q', ['EnHit'], ['EnDesc'], ['https://en.wikipedia.org/wiki/EnHit']],
        }),
      )
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'application/json',
          jsonBody: { extract: 'English extract' },
        }),
      ) as never;
  ...
  it('Wikipedia summary throw → answer undefined', async () => {
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockFetchResponse({
          contentType: 'application/json',
          jsonBody: ['q', ['Hit'], ['Desc'], ['https://w.it']],
        }),
      )
      // wiki summary throw
...
describe('buildSearxInstances — SearXNG interno first (2026-07-08)', () => {
  ...
  it('🚨 SEARXNG_URL vuoto/undefined → solo le 3 pubbliche', () => {
    expect(buildSearxInstances(undefined)).toEqual([
      'https://searx.be',
