...
const m = vi.hoisted(() => ({
  ...
  redactText: vi.fn((s: string) => ({ redacted: `R(${s.slice(0, 20)})`, classes: s.includes('@') ? ['email'] : [] })),
  redactJson: vi.fn((j: unknown) => ({
    ...
    redacted: j === null
      ? null
      : Array.isArray(j) ? j : { redacted: true, original: j },
    classes: [] as string[],
  })),
}));
...
describe('isCaptureEnabled / capture preferences', () => {
  ...
  it('🚨 getCaptureSettings default: OFF + 90 days + nessun consenso', () => {
    ...
    expect(s).toEqual({ captureEnabled: false, retentionDays: 90, consentAt: null, consentByUserId: null, decided: false });
  });
...
describe('🚨 Track A — ai_signals registrato SEMPRE (anche opt-out)', () => {
  it('🚨 tenant OPT-OUT: insert→null MA il segnale non-personale è registrato', () => {
    ...
    const sig = m.sqlite.prepare('SELECT * FROM ai_signals WHERE tenant_id = ?').get('t1') as Record<string, unknown>;
    ...
    expect(m.sqlite.prepare('SELECT COUNT(*) AS c FROM ai_interactions WHERE tenant_id = ?').get('t1')).toEqual({ c: 0 });
  });
  ...
  it('🚨 updateOutcome riflette l\'esito ANCHE nel segnale', () => {
    ...
    const sig = m.sqlite.prepare('SELECT outcome FROM ai_signals WHERE id = ?').get(id) as { outcome: string };
...
describe('🚨 insert — PII redaction + retention', () => {
  it('insert happy path: ritorna id + INSERT eseguito', () => {
    ...
    const row = m.sqlite.prepare('SELECT * FROM ai_interactions WHERE id = ?').get(id) as Record<string, unknown>;
  ...
  it('🚨 PII classes union: union di tutti gli array redactor → pii_classes_json', () => {
    ...
    const id = svc.insert(baseInsert());
    const row = m.sqlite.prepare('SELECT pii_classes_json FROM ai_interactions WHERE id = ?').get(id) as { pii_classes_json: string };
  ...
  it('🚨 tenant opted-out → return null + nessun INSERT', () => {
    ...
    const id = svc.insert(baseInsert());
    expect(id).toBeNull();
    const count = (m.sqlite.prepare('SELECT COUNT(*) AS c FROM ai_interactions').get() as { c: number }).c;
    expect(count).toBe(0);
  });
  ...
  it('🚨 retention_until = NOW + retentionDays', () => {
    ...
    const id = svc.insert(baseInsert());
    const row = m.sqlite.prepare('SELECT retention_until FROM ai_interactions WHERE id = ?').get(id) as { retention_until: string };
...
describe('🚨 updateOutcome — tenant scope', () => {
  it('updateOutcome happy: cambia outcome + outcome_at + returns true', () => {
    ...
    const ok = svc.updateOutcome({
      interactionId: id!, tenantId: 't1', outcome: 'accepted',
      followupRunId: 'run-99', followupRunStatus: 'success',
    });
    expect(ok).toBe(true);
    const row = m.sqlite.prepare('SELECT * FROM ai_interactions WHERE id = ?').get(id) as Record<string, unknown>;
  ...
  it('🚨 cross-tenant update → returns false (no change)', () => {
    const id = svc.insert(baseInsert());
    const ok = svc.updateOutcome({
      interactionId: id!, tenantId: 't2-OTHER', outcome: 'accepted',
    });
    expect(ok).toBe(false);
    const row = m.sqlite.prepare('SELECT outcome FROM ai_interactions WHERE id = ?').get(id) as { outcome: string };
  ...
  it('updateOutcome con patch applied → redacted in DB', () => {
    const id = svc.insert(baseInsert());
    svc.updateOutcome({
      interactionId: id!, tenantId: 't1', outcome: 'edited',
...
describe('🚨 updateReview — quality score validation', () => {
  it('quality 0-5 ok, training_split assigned', () => {
    ...
    const ok = svc.updateReview({
      interactionId: id!, tenantId: 't1', qualityScore: 4,
      reviewerUserId: 'reviewer-1', reviewerNotes: 'good',
      trainingSplit: 'train',
    });
    expect(ok).toBe(true);
    const row = m.sqlite.prepare('SELECT * FROM ai_interactions WHERE id = ?').get(id) as Record<string, unknown>;
  ...
  it('🚨 qualityScore > 5 → throw "must be 0-5"', () => {
    const id = svc.insert(baseInsert());
    expect(() => svc.updateReview({
      interactionId: id!, tenantId: 't1', qualityScore: 6,
      reviewerUserId: 'r',
  ...
  it('🚨 qualityScore < 0 → throw', () => {
    const id = svc.insert(baseInsert());
    expect(() => svc.updateReview({
      interactionId: id!, tenantId: 't1', qualityScore: -1,
      reviewerUserId: 'r',
    })).toThrow(/must be 0-5/u);
  });
  ...
  it('🚨 cross-tenant review → returns false', () => {
    const id = svc.insert(baseInsert());
    const ok = svc.updateReview({
      interactionId: id!, tenantId: 't2', qualityScore: 5,
      reviewerUserId: 'r',
    });
    expect(ok).toBe(false);
  });
...
describe('🚨 exportJsonl — fine-tuning dataset', () => {
  function setupSampleData(): { idAccepted: string; idRejected: string; idPending: string } {
    const idAccepted = svc.insert(baseInsert({
    ...
    svc.updateReview({
      interactionId: idAccepted, tenantId: 't1', qualityScore: 5,
      reviewerUserId: 'r', trainingSplit: 'train',
    });
    svc.updateReview({
      interactionId: idRejected, tenantId: 't1', qualityScore: 5,
      reviewerUserId: 'r', trainingSplit: 'train',
    });
  ...
  it('🚨 export solo quality >= minQuality (default 3)', () => {
    ...
    svc.updateOutcome({ interactionId: id, tenantId: 't1', outcome: 'accepted' });
    svc.updateReview({
      interactionId: id, tenantId: 't1', qualityScore: 2, // sotto soglia default 3
      reviewerUserId: 'r', trainingSplit: 'train',
    });
  ...
  it('🚨 export richiede training_split NOT NULL', () => {
    ...
    svc.updateOutcome({ interactionId: id, tenantId: 't1', outcome: 'accepted' });
    svc.updateReview({
      interactionId: id, tenantId: 't1', qualityScore: 5,
  ...
  it('exportJsonl format: messages chat-template HuggingFace', () => {
    setupSampleData();
    const jsonl = svc.exportJsonl({ tenantId: 't1' });
    const obj = JSON.parse(jsonl.trim()) as { messages: { role: string; content: string }[]; metadata: { quality: number; model: string } };
  ...
  it('exportJsonl filtro per split', () => {
    ...
    svc.updateReview({
      interactionId: id, tenantId: 't1', qualityScore: 5,
      reviewerUserId: 'r', trainingSplit: 'val',
    });
...
describe('🚨 sweep — retention purge', () => {
  it('sweep delete rows past retention_until', () => {
    ...
    m.sqlite.prepare("UPDATE ai_interactions SET retention_until = '2020-01-01T00:00:00.000Z' WHERE id = ?").run(id);
    const res = svc.sweep();
...
describe('🚨 stats — per tenant aggregations', () => {
  ...
  it('stats: piiDetections conta solo classes non vuoti', () => {
    ...
    m.redactText.mockImplementation((s: string) => ({
      redacted: s, classes: s === 'with-pii' ? ['email'] : [],
    }));
    m.redactJson.mockReturnValue({ redacted: null, classes: [] });

    svc.insert(baseInsert({
      request: { prompt: 'plain' }, response: { message: 'plain', model: 'm', latencyMs: 1 },
    }));
    svc.insert(baseInsert({
      request: { prompt: 'with-pii' }, response: { message: 'plain', model: 'm', latencyMs: 1 },
    }));
    const s = svc.stats('t1');
