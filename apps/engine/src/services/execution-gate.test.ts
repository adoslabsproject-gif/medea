...
describe('assertTenantCanExecute', () => {
  ...
  it('🚨 trial scaduto / suspended → THROW (fail-closed: il run NON parte)', () => {
    assertActive.mockImplementation(() => {
      throw new TenantNotActiveError('t-1', 'trial', 'trial scaduto');
    });
    expect(() => {
      assertTenantCanExecute('t-1');
    }).toThrow(TenantNotActiveError);
  });

  it('tenant NON in tabella → passa (fail-open BENIGNO) e NON emette metrica', () => {
    assertActive.mockImplementation(() => {
      throw new TenantNotFoundError('t-x');
    });
    expect(() => {
      assertTenantCanExecute('t-x');
    }).not.toThrow();
