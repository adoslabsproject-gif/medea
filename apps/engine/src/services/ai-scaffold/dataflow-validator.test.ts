...
import { validateDataflow, healDataflowReachability, type DfNode, type DfEdge } from './dataflow-validator.js';

const N = (id: string, defId: string, config: Record<string, unknown> = {}): DfNode => ({ id, defId, config });

describe('validateDataflow — REACHABILITY', () => {
  it('riferimento a un nodo A MONTE (predecessore raggiungibile) → OK', () => {
    const nodes = [N('a', 'trigger_manual'), N('b', 'action_http', { url: '{{$node.a.json.url}}' })];
  ...
  it('riferimento transitivo (a→b→c, c usa a) → OK (a è raggiungibile)', () => {
    const nodes = [N('a', 'trigger_manual'), N('b', 'action_http'), N('c', 'action_send_email', { body: '{{$node.a.json.x}}' })];
    const edges: DfEdge[] = [{ from: 'a', to: 'b' }, { from: 'b', to: 'c' }];
    expect(validateDataflow(nodes, edges)).toEqual([]);
  });

  it('riferimento a un nodo NON a monte (scollegato) → fail', () => {
    const nodes = [N('a', 'trigger_manual'), N('b', 'action_http', { url: '{{$node.orphan.json}}' }), N('orphan', 'action_http')];
    const edges: DfEdge[] = [{ from: 'a', to: 'b' }];
  ...
  it('sintassi {{node.X.json}} (senza $) riconosciuta come $node.X.json', () => {
    ...
    const nodes = [N('a', 'trigger_manual'), N('b', 'action_http', { url: '{{node.ghost.json}}' }), N('ghost', 'action_http')];
    const edges: DfEdge[] = [{ from: 'a', to: 'b' }];
    const issues = validateDataflow(nodes, edges);
...
describe('validateDataflow — CATENA json_extract (caso reale IMAP CRM email→dominio)', () => {
  it('estrai .domain dall\'output di un json_extract che ha estratto .email → WARN', () => {
    const nodes = [
      N('trig', 'trigger_imap'),
      N('sender', 'action_json_extract', { sourceExpression: '$node.trig.json[0].from', path: '$.email' }),
      N('domain', 'action_json_extract', { sourceExpression: '$node.sender.json', path: '$.domain' }),
    ];
    const edges: DfEdge[] = [{ from: 'trig', to: 'sender' }, { from: 'sender', to: 'domain' }];
    const issues = validateDataflow(nodes, edges);
  ...
  it('json_extract che estrae un OGGETTO (path root $) → nessun warn', () => {
    ...
    const edges: DfEdge[] = [{ from: 'x', to: 'a' }, { from: 'a', to: 'b' }];
...
describe('healDataflowReachability — AUTO-HEAL (l\'automazione, non solo detection)', () => {
  it('db_insert referenzia un nodo non-a-monte → AGGIUNGE l\'edge che lo collega', () => {
    ...
    const edges: DfEdge[] = [{ from: 'a', to: 'domain' }, { from: 'a', to: 'db' }];
...
describe('validateDataflow — catena json_extract con wrapper {{ }} (bug fresh-gen IMAP CRM)', () => {
  it('sourceExpression WRAPPATA {{$node.X.json}} + path nested → WARN (prima sfuggiva)', () => {
    const nodes = [
      N('trig', 'trigger_imap'),
      N('sender', 'action_json_extract', { sourceExpression: '{{$node.trig.json}}', path: '$.from' }),
      N('domain', 'action_json_extract', { sourceExpression: '{{$node.sender.json}}', path: '$.address' }),
    ];
    const edges: DfEdge[] = [{ from: 'trig', to: 'sender' }, { from: 'sender', to: 'domain' }];
    const warn = validateDataflow(nodes, edges).find((i) => i.nodeId === 'domain' && i.status === 'warn');
