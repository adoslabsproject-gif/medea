...
import { extractMissingCapabilities, buildCoverageFeedback, buildCapabilityInjection } from './requirement-coverage.js';
...
describe('requirement-coverage — enforcement "fa quello che è chiesto"', () => {
  it('grafico richiesto + NESSUN chart node → manca chart (suggerisce action_generate_chart)', () => {
    const missing = extractMissingCapabilities('mandami un report email con grafico storico', ['action_send_email', 'db_query']);
  ...
  it('grafico richiesto + chart node PRESENTE → coperto (no missing)', () => {
    const missing = extractMissingCapabilities('report con grafico', ['action_generate_chart', 'action_send_email']);
    expect(missing.map((m) => m.id)).not.toContain('chart');
  });

  it('confronto storico → richiede db_query', () => {
    const missing = extractMissingCapabilities('confronto storico vs settimana scorsa', ['action_send_email']);
    expect(missing.map((m) => m.id)).toContain('db_query');
  });
  it('confronto storico + db_query presente → coperto', () => {
    expect(extractMissingCapabilities('confronto storico settimana scorsa', ['db_query']).map((m) => m.id)).not.toContain('db_query');
  });

  it('invio email richiesto + nessun nodo email → manca email', () => {
    expect(extractMissingCapabilities('invia una email di riepilogo', ['action_http']).map((m) => m.id)).toContain('email');
  });
  it('email presente (anche email_send_tracked) → coperto', () => {
    expect(extractMissingCapabilities('mandami una mail', ['action_email_send_tracked']).map((m) => m.id)).not.toContain('email');
  });

  it('pdf richiesto → action_pdf_generate', () => {
    expect(extractMissingCapabilities('genera un pdf della fattura', []).map((m) => m.id)).toContain('pdf');
  });

  it('prompt SENZA capability speciali → nessun missing (no falsi positivi)', () => {
    expect(extractMissingCapabilities('quando arriva un webhook chiama una API', ['trigger_webhook', 'action_http'])).toEqual([]);
  });

  it('caso reale Sitemap Crawler: grafico + db_query mancanti, email presente', () => {
    const goal = 'crawla la sitemap, fai audit SEO, mandami report email HTML con tabella e grafico storico vs settimana scorsa (db_query per confronto)';
  ...
  it('buildCoverageFeedback nomina i nodi da aggiungere + parte con "Workflow rejected — quality gate"', () => {
    const fb = buildCoverageFeedback([{ id: 'chart', label: 'grafico', suggestNode: 'action_generate_chart' }]);
...
describe('buildCapabilityInjection — inject deterministico (refinement "l\'ho fatto io")', () => {
  it('chart: inietta action_generate_chart cablato sul nodo TERMINALE (no edge uscenti)', () => {
    const nodes = [{ id: 'trig', defId: 'trigger_manual' }, { id: 'agg', defId: 'action_aggregate' }];
  ...
  it('email/pdf/db_persist: TUTTI iniettati (niente warning), cablati sul terminale', () => {
    ...
    expect(buildCapabilityInjection('email', nodes, [])!.node.config.body).toBe('{{$node.src.json}}');
  });
