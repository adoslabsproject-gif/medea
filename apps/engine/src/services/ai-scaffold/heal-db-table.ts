...
export interface DbHealNode { id: string; defId: string; config: Record<string, unknown> }
export interface DbHealDatabase { id: string; columns: Record<string, string[]> }
export interface HealColumn { name: string; type: string; primaryKey?: boolean; nullable?: boolean }
export interface HealTableSpec { databaseId?: string; name: string; description: string; columns: HealColumn[] }
export interface DbHealResult { tablesToCreate: HealTableSpec[]; notes: string[] }
...
function inferType(col: string): HealColumn {
  ...
  if (/(^|_)(price|amount|total|median_price|cost|importo)($|_)/.test(c)) return { name: col, type: 'decimal' };
  if (/(count|_id|hop|qty|quantita|number|num)$/.test(c) || /^(count|num)/.test(c)) return { name: col, type: 'integer' };
  if (/(_at$|date|time|timestamp)/.test(c)) return { name: col, type: 'datetime' };
  if (/^(is_|has_)|(_flag$|detected$|enabled$|ok$|valid$)/.test(c)) return { name: col, type: 'boolean' };
...
function tableNameFromGoal(goal: string, fallback: string): string {
  const slug = goal.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '')
 ...
 */
export function healDbTableReferences(nodes: readonly DbHealNode[], databases: readonly DbHealDatabase[], goal: string): DbHealResult {
  ...
  for (const node of nodes) {
    ...
    // marker verrebbe trattato come nome tabella e CREATA una tabella
    // letteralmente chiamata "__USE_PICKER__").
    if (!dbId || !table || dbId.includes('{{') || table.includes('{{') || dbId === '__USE_PICKER__' || table === '__USE_PICKER__') continue;
    ...
    const isSubset = (cols: string[] | undefined): boolean => !!cols && reqArr.every((c) => cols.includes(c));
    ...
    if (match) {
      ...
      notes.push(`🔧 db node "${node.id}": ripuntato da tabella "${table}" → "${match[0]}" (colonne ${reqArr.join(', ')} corrispondono).`);
      continue;
    }
    ...
    if (!createdNames.has(newName)) {
      ...
      const cols: HealColumn[] = reqArr.some((c) => c.toLowerCase() === 'id') ? [] : [{ name: 'id', type: 'uuid', primaryKey: true }];
      for (const c of reqArr) cols.push(inferType(c));
      tablesToCreate.push({ name: newName, description: `Tabella creata automaticamente per ${node.defId} (${goal.slice(0, 80)})`, columns: cols });
    }
    if (newName !== table) {
      node.config[spec.tableField] = newName;
      notes.push(`🆕 db node "${node.id}": tabella "${table}" non adatta → creata + ripuntata a "${newName}" (colonne ${reqArr.join(', ')}).`);
    } else {
      notes.push(`🆕 db node "${node.id}": tabella "${newName}" dichiarata in tablesToCreate (colonne ${reqArr.join(', ')}).`);
    }
  }
