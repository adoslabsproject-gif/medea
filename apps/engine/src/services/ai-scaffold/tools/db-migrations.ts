...
export function createDatabaseHandler(
  ...
  if (!name)
    return Promise.resolve({
      ok: false,
      error: 'create_database richiede "name" (es. "workflow_data")',
    });
  if (!/^[a-z][a-z0-9_]{0,62}$/i.test(name)) {
    return Promise.resolve({
      ok: false,
      error: 'name deve iniziare con lettera, max 63 char, solo [a-zA-Z0-9_]',
    });
  }
  const description =
    typeof args.description === 'string' ? args.description : 'Database creato da AI scaffold';
  try {
    ...
    logger.info(
      { databaseId: created.id, name, tenantId: session.tenantId },
      '[SCAFFOLD] database created',
    );
    return Promise.resolve({ ok: true, data: { databaseId: created.id, name: created.name } });
  } catch (e) {
    return Promise.resolve({
      ok: false,
      error: `create_database fallita: ${e instanceof Error ? e.message : String(e)}`,
    });
  }
}
...
export async function createTableHandler(
  ...
  if (!databaseId || !table || typeof table !== 'object') {
    return {
      ok: false,
      error: 'create_table richiede databaseId + table { name, columns: [...] }',
    };
  }
  if (typeof table.name !== 'string')
    return { ok: false, error: 'table.name mancante o non stringa' };
  if (!Array.isArray(table.columns) || table.columns.length === 0)
    return { ok: false, error: 'table.columns deve essere un array non vuoto' };
  const tableName: string = table.name;
  try {
    await session.dbStudio.applyMigration(
      databaseId,
      [
        {
          kind: 'create_table',
    ...
    return {
      ok: false,
      error: `create_table fallita: ${e instanceof Error ? e.message : String(e)}`,
    };
  }
}

export async function addColumnHandler(
  session: ScaffoldSession,
  args: Record<string, unknown>,
): Promise<ToolResult> {
  const databaseId = coerceString(args.databaseId ?? '');
  ...
  if (!databaseId || !tableName || !column || typeof column.name !== 'string') {
    return {
      ok: false,
      error: 'add_column richiede databaseId, table, column { name, type, constraints? }',
    };
  }
  try {
    await session.dbStudio.applyMigration(
    ...
    return {
      ok: false,
      error: `add_column fallita: ${e instanceof Error ? e.message : String(e)}`,
    };
  }
}

export async function dropColumnHandler(
  session: ScaffoldSession,
  args: Record<string, unknown>,
): Promise<ToolResult> {
  const databaseId = coerceString(args.databaseId ?? '');
  ...
  if (!databaseId || !tableName || !columnName)
    return { ok: false, error: 'drop_column richiede databaseId, table, columnName.' };
  try {
    await session.dbStudio.applyMigration(
      databaseId,
      [{ kind: 'drop_column', tableName, columnName }],
      session.tenantId,
    );
    logger.info(
      { tenantId: session.tenantId, databaseId, tableName, columnName },
      'ai-scaffold: drop_column',
    );
    return { ok: true, data: { dropped: `${tableName}.${columnName}` } };
  } catch (e) {
    return {
      ok: false,
      error: `drop_column fallito: ${e instanceof Error ? e.message : String(e)}`,
    };
  }
}

export async function dropTableHandler(
  session: ScaffoldSession,
  args: Record<string, unknown>,
): Promise<ToolResult> {
  const databaseId = coerceString(args.databaseId ?? '');
  ...
  if (!databaseId || !tableName)
    return { ok: false, error: 'drop_table richiede databaseId, tableName.' };
  if (confirm !== tableName)
    return {
      ok: false,
      error: `Per drop_table devi passare confirmTableName="${tableName}" — è una operazione DISTRUTTIVA non reversibile.`,
    };
  try {
    await session.dbStudio.applyMigration(
      databaseId,
      [{ kind: 'drop_table', tableName }],
      session.tenantId,
    );
    ...
    return {
      ok: false,
      error: `drop_table fallito: ${e instanceof Error ? e.message : String(e)}`,
    };
  }
}
...
export function renameColumnHandler(
  ...
  return {
    ok: false,
    error:
      'rename_column non supportato uniformemente da tutti gli engine DB. Workaround: usa add_column con il nuovo nome, esegui un workflow di copia (db_query → db_insert), poi drop_column del nome vecchio.',
  };
}

export async function addIndexHandler(
  session: ScaffoldSession,
  args: Record<string, unknown>,
): Promise<ToolResult> {
  const databaseId = coerceString(args.databaseId ?? '');
  ...
  if (columns.length === 0)
    return { ok: false, error: 'columns deve essere un array di nomi colonna stringa.' };
  try {
    await session.dbStudio.applyMigration(
      databaseId,
      [
