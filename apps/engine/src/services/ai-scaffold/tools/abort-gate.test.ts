...
describe('evaluateAbort — hallucinated defId rejection', () => {
  it('BUG 2026-05-30 esatto: "community_telegram non disponibile" mentre IS in catalog → REJECT', () => {
    const r = evaluateAbort({
      reason:
        'Necessario nodo Telegram non disponibile. Installare community_telegram o abilitare http_request per notifiche Telegram.',
  ...
  it('Reason genuino (goal impossibile) → ACCEPT', () => {
    ...
    const r = evaluateAbort({
      reason:
        'Il goal richiede attuazione fisica nel mondo reale (consegna fisica drone). Nessun workflow software può risolverlo.',
      installedDefIds: INSTALLED,
    });
  ...
  it('Hallucinati MULTIPLI nel reason → tutti listati nella response', () => {
    const r = evaluateAbort({
      reason:
        'community_telegram, community_slack e action_http tutti mancanti, non posso continuare',
      installedDefIds: INSTALLED,
    });
    expect(r.reject).toBe(true);
    if (r.reject) {
