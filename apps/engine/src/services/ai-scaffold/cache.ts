...
import {
  templateCache,
  type RetrieveResult,
} from '@/services/ai-scaffold/template-cache/template.service.js';
 ...
 */
export function serveCachedWorkflow(
  cacheRetrieve: RetrieveResult,
  goal: string,
  start: number,
): AiScaffoldResult | null {
  ...
  try {
    ...
    logger.warn(
      ...
      '[SINGLESHOT] cached workflow_json non parse — fallback singleshot',
    );
    return null;
  }
    ...
    logger.info(
      {
        ...
        fixes: cf.appliedFixes.map((f) => f.type),
        dataflowFails: cachedDfFails.length,
      },
      '[SINGLESHOT] template cache entry EVICTED (heal strutturale o data-flow error = bassa qualità) → fresh generation',
    );
    return null;
  }
    ...
    cachedWf.nodes = cf.nodes.map((fn) => {
      ...
      return (
        orig
          ...
          : { id: fn.id, defId: fn.defId, config: fn.config, x: 0, y: 0 }
      ) as (typeof cachedWf.nodes)[number];
    });
    cachedWf.edges = cf.edges.map((e) => ({ from: e.from, to: e.to }));
    logger.info(
      { templateId: cacheRetrieve.template.id, fixes: cf.appliedFixes.length },
      '[SINGLESHOT] cached workflow auto-fixed on retrieval (triviale)',
    );
  }
  ...
  try {
    templateCache.save({
      ...
      workflow: {
        name: cachedWf.name,
        nodes: cachedWf.nodes.map((n) => ({ id: n.id, defId: n.defId })),
        edges: cachedWf.edges.map((e) => ({ from: e.from, to: e.to })),
      },
