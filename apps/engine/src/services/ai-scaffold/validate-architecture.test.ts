...
const N = (id: string, defId: string, config: Record<string, unknown> = {}) => ({
  id,
  defId,
  config,
});
...
describe('validateArchitecture — 4 check anti-bug', () => {
  it('workflow valido (trigger→action) → nessun issue', () => {
    const issues = validateArchitecture(
      [N('t', 'trigger_webhook'), N('a', 'action_http')],
      [{ from: 't', to: 'a' }],
      CAT,
    );
  ...
  it('2. nodo non-trigger SENZA edge entrante → orfano', () => {
    const issues = validateArchitecture(
      [N('t', 'trigger_webhook'), N('orphan', 'action_http')],
      [],
      CAT,
    );
  ...
  it('3. switch con case dichiarato ma SENZA edge fromPort → issue', () => {
    const nodes = [
      N('t', 'trigger_webhook'),
      N('sw', 'logic_switch', { cases: [{ case: 'a' }, { case: 'b' }] }),
      N('x', 'action_http'),
    ];
    const edges = [
      { from: 't', to: 'sw' },
      { from: 'sw', to: 'x', fromPort: 'a' },
    ]; // manca fromPort 'b'
  ...
  it('4. anti-pigrizia: 3+ rami switch TUTTI verso community_slack → issue', () => {
    const nodes = [
      N('t', 'trigger_webhook'),
      N('sw', 'logic_switch'),
      N('s1', 'community_slack'),
      N('s2', 'community_slack'),
      N('s3', 'community_slack'),
    ];
    const edges = [
      { from: 't', to: 'sw' },
      { from: 'sw', to: 's1', fromPort: 'a' },
      { from: 'sw', to: 's2', fromPort: 'b' },
      { from: 'sw', to: 's3', fromPort: 'c' },
    ];
    const issues = validateArchitecture(nodes, edges, CAT);
