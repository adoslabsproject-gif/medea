...
toolRegistry.register(
  defineTool({
    name: 'list_databases',
    description:
      'Elenca i database connection del tenant con table count + engine. Use case: decidere DOVE persistere dati ("save nell\'esistente vs nuovo DB"). Output lean: no credentials, no full schema.',
    schema: EmptyArgs,
...
toolRegistry.register(
  defineTool({
    name: 'read_db_schema',
    description:
      'Schema completo (tabelle + colonne + tipi + PK + nullable) di un database. Use case: prima di add_node che usa db_query/db_insert, verificare le colonne reali.',
...
toolRegistry.register(
  defineTool({
    ...
    description:
      'Lista workflow esistenti del tenant (id/name/desc/enabled, no full nodes). Use case: ispirarsi a workflow simili, evitare duplicati.',
    schema: EmptyArgs,
...
toolRegistry.register(
  defineTool({
    name: 'read_workflow',
    description:
      'Workflow completo (nodes + edges + nodeDefs) per analisi o cloning. Tenant-scoped.',
    schema: z.object({
...
toolRegistry.register(
  defineTool({
    ...
    description:
      'Catalogo nodi runtime. Senza args: lista breve. Con defId: full config-field schema (tipi/enum/regex). Per add_node corretto.',
...
toolRegistry.register(
  defineTool({
    ...
    description:
      'Account email IMAP/SMTP pre-configurati dal tenant admin. Referenziati nei nodi via systemAccountId. Critico per evitare credenziali hardcoded.',
    schema: EmptyArgs,
...
toolRegistry.register(
  defineTool({
    name: 'list_secrets',
    description:
      'NOMI dei secret disponibili (mai i valori). L\'agente li referenzia con {{secrets.NOME}} nei config — boundary "dati sensibili a parte".',
    schema: EmptyArgs,
...
toolRegistry.register(
  defineTool({
    ...
    description:
      'LLM provider con API key configurata nel tenant. Use case: scegliere provider per agent_extractor/agent_* nodes invece di hardcodare vendor.',
    schema: EmptyArgs,
...
toolRegistry.register(
  defineTool({
    ...
    description:
      "Stato corrente del draft (nodes + edges). Aiuta l'agente a riorientarsi mid-loop dopo molti add_node/connect_nodes.",
    schema: EmptyArgs,
...
toolRegistry.register(
  defineTool({
    ...
    description:
      'Ultimi N run del tenant (id/status/durata/errori, NO step output). Tenant-scoped. Use case: diagnostic "perché il workflow X ha fallito ieri".',
    schema: z.object({
...
toolRegistry.register(
  defineTool({
    name: 'read_run',
    description:
      'Dettaglio singolo run: steps con status/duration/error REDACTED (no PII, no payload base64). Per output completo serve UI Run Inspector.',
...
toolRegistry.register(
  defineTool({
    ...
    description:
      "Riepilogo settings del tenant (email/secrets/LLM/DB count). L'agente decide se chiedere all'utente di configurare X prima di costruire il workflow.",
    schema: EmptyArgs,
...
toolRegistry.register(
  defineTool({
    ...
    description:
      'Crea un nuovo database SQLite locale del tenant. Usalo quando list_databases ritorna lista vuota e il workflow richiede storage. Restituisce { databaseId, name } da usare in create_table successivo. NO credentials richieste — SQLite vive nel volume tenant.',
    schema: z.object({
      name: z
        .string()
        .min(1)
        .max(63)
        .regex(/^[a-z][a-z0-9_]*$/i, 'name: solo [a-zA-Z0-9_], inizio con lettera'),
...
toolRegistry.register(
  defineTool({
    name: 'create_table',
    description:
      'Crea una nuova tabella con colonne tipizzate. Cache schema aggiornata automaticamente.',
    schema: z.object({
...
toolRegistry.register(
  defineTool({
    name: 'drop_column',
    description:
      'DROP COLUMN. Last-resort destructive op — usata per correggere errori di create_table.',
    schema: z.object({
      databaseId: z.string().min(1),
      table: z.string().min(1),
...
toolRegistry.register(
  defineTool({
    name: 'drop_table',
    description:
      'DROP TABLE — DISTRUTTIVO. Richiede confirmTableName === tableName come safety gate (stesso pattern del SqlEditor).',
    schema: z.object({
      databaseId: z.string().min(1),
...
toolRegistry.register(
  defineTool({
    name: 'rename_column',
    description:
      'NON SUPPORTATO uniformemente cross-engine. Ritorna error con workaround suggerito (add_column + copy via workflow + drop_column).',
    schema: z.object({
      databaseId: z.string().min(1),
      table: z.string().min(1),
      from: z.string().min(1),
...
toolRegistry.register(
  defineTool({
    name: 'add_index',
    description:
      'Crea un index (B-tree) su una o più colonne. Use case: ottimizzare db_query frequenti su colonne specifiche.',
    schema: z.object({
      databaseId: z.string().min(1),
      table: z.string().min(1),
        ...
        .array(
          z.object({
            ...
            defId: z
              .string()
              .min(1)
              .describe('defId esistente nel catalogo (vedi list_node_catalog)'),
      ...
      edges: z.array(
        z.object({
          ...
          fromPort: z
            .string()
            .optional()
            .describe('"true"|"false" per logic_if; case name per logic_switch'),
        }),
      ),
    }),
    handler: (session: ScaffoldSession, args: unknown) =>
...
toolRegistry.register(
  defineTool({
    name: 'add_node',
    description:
      'Aggiunge un nodo al draft. Config validato per-field via schema del node catalog (enum/regex/required).',
    schema: z.object({
      id: z.string().min(1),
...
toolRegistry.register(
  defineTool({
    name: 'update_node',
    description:
      'Patch merge sui field del nodo. Re-valida contro lo schema. Merge-semantic: unspecified keys mantengono il valore precedente.',
    schema: z.object({
      id: z.string().min(1),
      config: z.record(z.string(), z.unknown()).optional(),
      label: z.string().optional(),
...
toolRegistry.register(
  defineTool({
    name: 'delete_node',
    description:
      'Rimuove nodo + edges dipendenti (cascade). Mantiene invariante: no orphan edges nel draft.',
...
toolRegistry.register(
  defineTool({
    ...
    description:
      'Rimuove edge (idempotente, no error se gone). Identificato da (from, to, fromPort?) triple.',
    schema: z.object({
      from: z.string().min(1),
      to: z.string().min(1),
      fromPort: z.string().optional(),
...
toolRegistry.register(
  defineTool({
    ...
    description:
      'Chiude e ritorna il workflow. Chiamato SOLO dopo TUTTI i nodi+edge. Double-check orphan edges.',
    schema: z.object({
      id: z.string().min(1),
      name: z.string().min(1),
