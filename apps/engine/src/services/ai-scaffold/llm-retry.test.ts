...
describe('🚨 transient retries', () => {
  it.each(['429 Too Many Requests', '502 Bad Gateway', '503', '504 Gateway Timeout'])(
    '🚨 "%s" → retry attempted',
    async (errMsg) => {
      const fn = vi.fn().mockRejectedValueOnce(new Error(errMsg)).mockResolvedValueOnce('success');
  ...
  it.each(['ECONNRESET', 'ENOTFOUND', 'ETIMEDOUT', 'fetch failed', 'network error'])(
    '🚨 network "%s" → retry',
    async (errMsg) => {
      const fn = vi.fn().mockRejectedValueOnce(new Error(errMsg)).mockResolvedValueOnce('ok');
  ...
  it('🚨 exhausted maxAttempts → last error re-thrown', async () => {
    ...
    await expect(callLlmWithRetry(fn, { maxAttempts: 3, baseDelayMs: 1 })).rejects.toThrow(/503/u);
...
describe('🚨 fail-fast on auth/4xx', () => {
  it.each(['401 Unauthorized', '403 Forbidden', '404 Not Found'])(
    '🚨 "%s" → NO retry, throw immediately',
    async (errMsg) => {
      const fn = vi.fn().mockRejectedValueOnce(new Error(errMsg));
      await expect(callLlmWithRetry(fn, { maxAttempts: 5, baseDelayMs: 1 })).rejects.toThrow(
        errMsg,
      );
      expect(fn).toHaveBeenCalledTimes(1);
    },
...
describe('🚨 non-Error throwables', () => {
  it('🚨 throw string → wrap in Error', async () => {
    ...
    await expect(callLlmWithRetry(fn, { maxAttempts: 1, baseDelayMs: 1 })).rejects.toThrow(
      'plain-string-error',
    );
  });
});
