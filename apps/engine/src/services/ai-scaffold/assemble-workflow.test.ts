...
const baseNode = (id: string, defId: string, extra: Record<string, unknown> = {}) => ({
  id,
  defId,
  config: {},
  ...extra,
...
describe('assembleWorkflow — parsed→Workflow', () => {
  it('posizionamento default: x=idx*220, y=200 se la LLM non li dà', () => {
    const wf = assembleWorkflow({
      name: 'W',
      nodes: [baseNode('a', 'trigger_webhook'), baseNode('b', 'action_http')],
      edges: [{ from: 'a', to: 'b' }],
    });
  ...
  it('rispetta x/y espliciti della LLM', () => {
    const wf = assembleWorkflow({
      name: 'W',
      nodes: [baseNode('a', 'trigger_webhook', { x: 999, y: 7 }), baseNode('b', 'action_http')],
      edges: [],
    });
  ...
  it('serializza i config non-string in JSON (string passa intatta)', () => {
    const wf = assembleWorkflow({
      name: 'W',
      nodes: [
        baseNode('a', 'action_http', { config: { url: 'x', retries: 3, opts: { a: 1 } } }),
        baseNode('b', 'action_http'),
      ],
      edges: [],
    });
  ...
  it('label → name sul nodo', () => {
    const wf = assembleWorkflow({
      name: 'W',
      nodes: [baseNode('a', 'action_http', { label: 'Chiamata' }), baseNode('b', 'action_http')],
      edges: [],
    });
