...
const { createTenantDatabase, CreateDatabaseError, isEmbeddedEngine } =
  await import('./create-database.js');
...
describe('createTenantDatabase — MANAGED (sidecar nel tenant)', () => {
  ...
  it('🚨 provisioning fallito (ManagedDbError) → propagato, nessun DB creato (no riga orfana)', async () => {
    provisionMock.mockRejectedValueOnce(
      new ManagedDbError('PULL_FAILED', 'immagine non scaricata'),
    );
    await expect(
      createTenantDatabase({
        tenantId: 't',
