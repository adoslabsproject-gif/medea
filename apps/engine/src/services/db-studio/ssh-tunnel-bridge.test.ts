...
function conn(over: Partial<Database['connection']> = {}): Database['connection'] {
  return {
    ...
    hostname: '127.0.0.1',
    port: 5432,
    database: 'nothumanallowed',
    username: 'flowforge_ro',
    passwordSecretRef: 'pw',
    sshTunnel: {
      host: '91.98.131.3',
      port: 22,
      user: 'root',
      privateKeySecretRef: 'KEYDATA',
...
beforeEach(() => {
  connectMock.mockReset().mockResolvedValue({ localPort: 55432, close: vi.fn() });
});
...
describe('openDbStudioSshTunnel', () => {
  it('mappa ssh+db nel config e ritorna il tunnel (localPort)', async () => {
    ...
    expect(cfg).toMatchObject({
      ssh: {
        ...
        user: 'root',
        hostKeyFingerprint: FP,
        auth: { method: 'key', privateKeySecretRef: 'KEYDATA' },
      },
  ...
  it('engine fuori-policy (mongodb) → db.engine placeholder postgres (tunnel TCP-only)', async () => {
    ...
    expect(connectMock.mock.calls[0]![0]).toMatchObject({
      db: { engine: 'postgres', port: 27017 },
    });
  });

  it('passphrase opzionale inclusa quando presente', async () => {
    await openDbStudioSshTunnel(
      conn({
        sshTunnel: {
          host: 'h',
          port: 22,
          ...
          hostKeyFingerprint: FP,
          passphraseSecretRef: 'pp',
        },
      }),
    );
    expect(
      (connectMock.mock.calls[0]![0] as { ssh: { auth: { passphraseSecretRef?: string } } }).ssh
        .auth.passphraseSecretRef,
    ).toBe('pp');
  });

  it('🔒 fingerprint mancante → la policy (parseSshDbConfig reale) RIFIUTA', async () => {
    await expect(
      openDbStudioSshTunnel(
        conn({
          sshTunnel: {
            host: 'h',
            port: 22,
  ...
  it('senza sshTunnel → throw', async () => {
    await expect(openDbStudioSshTunnel(conn({ sshTunnel: undefined }))).rejects.toThrow(
      /sshTunnel/u,
    );
  });
});
...
describe('resolver segreti (no fallback letterale su ref vault malformato)', () => {
  it('ref vault: non risolvibile → la connect riceve un resolver che throwa', async () => {
    ...
    const resolve = (
      connectMock.mock.calls[0]![1] as { resolveSecret: (r: string) => Promise<string> }
    ).resolveSecret;
