...
vi.mock('@/lib/internal-service-fetch.js', () => ({
  internalAwareFetch: (...a: unknown[]) => fetchMock(...a) as unknown,
}));
...
describe('isManagedEngine', () => {
  it('riconosce i sidecar, rifiuta embedded', () => {
    for (const e of ['postgres', 'mysql', 'mongodb', 'redis', 'mssql', 'qdrant', 'pgvector'])
      expect(isManagedEngine(e)).toBe(true);
    for (const e of ['sqlite', 'duckdb', 'vector-embedded', 'oracle'])
      expect(isManagedEngine(e)).toBe(false);
  });
});
...
describe('provisionManagedDb', () => {
  it('POST /internal/tenant-db/provision con token + body, ritorna connection', async () => {
    ...
    expect((init as { headers: Record<string, string> }).headers['X-Internal-Token']).toBe(
      'shared-portal-token',
    );
  ...
  it('risposta ok:false → ManagedDbError col code', async () => {
    fetchMock.mockResolvedValue(
      new Response(JSON.stringify({ ok: false, code: 'NOT_READY', error: 'non pronto' }), {
        status: 504,
      }),
    );
