...
import { extractDbTableRefs, dedicatedTablesToDrop, WRITE_DB_DEFIDS, ANY_DB_DEFIDS } from './workflow-db-tables.js';

const n = (defId: string, databaseId: string, table: string) => ({ defId, config: { databaseId, table } });
...
describe('extractDbTableRefs', () => {
  it('estrae solo i defId richiesti + dedup + skip espressioni {{}}', () => {
    ...
    expect(extractDbTableRefs(nodes, WRITE_DB_DEFIDS)).toEqual([{ databaseId: 'd1', table: 'price_monitoring' }]);
...
describe('dedicatedTablesToDrop — anti-dataloss', () => {
  it('tabella scritta SOLO da questo workflow → droppabile', () => {
    ...
    expect(dedicatedTablesToDrop(target, others)).toEqual([{ databaseId: 'd1', table: 'price_monitoring' }]);
  });
  ...
  it('stesso nome ma DB diverso → tabelle distinte', () => {
    ...
    expect(dedicatedTablesToDrop([n('db_insert', 'dA', 'logs')], others)).toEqual([{ databaseId: 'dA', table: 'logs' }]);
  });
});
