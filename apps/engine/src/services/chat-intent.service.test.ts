...
describe('detectIntents — workflow control', () => {
  it('"esegui workflow orders-hot" → run_workflow', () => {
    expect(detectIntents('esegui workflow orders-hot').some((i) => i.type === 'run_workflow')).toBe(
      true,
    );
  });
  ...
  it('"disattiva orders-hot" → disable', () => {
    expect(detectIntents('disattiva orders-hot').some((i) => i.type === 'disable_workflow')).toBe(
      true,
    );
  });
  it('"lista i miei workflow" → list', () => {
    expect(detectIntents('lista i miei workflow').some((i) => i.type === 'list_workflows')).toBe(
      true,
    );
  });
  it('"show me all workflows" → list', () => {
    expect(detectIntents('show me all workflows').some((i) => i.type === 'list_workflows')).toBe(
      true,
    );
  });
  it('"configura nodo n1 del workflow orders-hot con path=/new method=POST"', () => {
    const intents = detectIntents(
      'configura nodo n1 del workflow orders-hot con path="/new" method=POST.',
    );
...
describe('detectIntents — regression web tools', () => {
  it('URL fetch still works', () => {
    expect(detectIntents('dammi https://example.com').some((i) => i.type === 'fetch_url')).toBe(
      true,
    );
  });
  ...
  it('reflexive "si trova" does NOT trigger web_search (UI location question)', () => {
    expect(
      detectIntents('e dove si trova?? non vedo i nodi nella lavagna!').some(
        (i) => i.type === 'web_search',
      ),
    ).toBe(false);
    expect(
      detectIntents('dove si trova il pannello impostazioni?').some((i) => i.type === 'web_search'),
    ).toBe(false);
  });
  it('imperative "trova/trovami X" still triggers web_search', () => {
    expect(
      detectIntents('trovami una valvola Parker 3/8').some((i) => i.type === 'web_search'),
    ).toBe(true);
    expect(
      detectIntents('trova le specifiche del CETOP 5').some((i) => i.type === 'web_search'),
    ).toBe(true);
  });
...
describe('executeIntents — end-to-end', () => {
  ...
  it('enable + disable persist', async () => {
    const re = await executeIntents([
      { type: 'enable_workflow', args: { workflowName: 'email-summary' }, reason: 't' },
    ]);
    expect((re[0]?.data as { enabled?: boolean }).enabled).toBe(true);
    const rd = await executeIntents([
      { type: 'disable_workflow', args: { workflowName: 'orders' }, reason: 't' },
    ]);
  ...
  it('run_workflow not_found error', async () => {
    const r = await executeIntents([
      { type: 'run_workflow', args: { workflowName: 'nope' }, reason: 't' },
    ]);
  ...
  it('analyze_image dispatched (col provider risolto)', async () => {
    const r = await executeIntents(
      [{ type: 'analyze_image', args: { imageBase64: 'b64' }, reason: 't' }],
      VISION_TARGET,
    );
      ...
      [
        {
          ...
          args: { documentBase64: 'b64', documentMime: 'application/pdf', documentName: 'a.pdf' },
          reason: 't',
        },
      ],
      VISION_TARGET,
...
describe('🛡️ formatToolResultsForPrompt — allegati immagine/documento (fix 2026-06-22)', () => {
  ...
  const imgIntent = {
    type: 'analyze_image',
    args: { imageBase64: 'x' },
    reason: 'image attachment: foto.png',
  };
  const docIntent = {
    type: 'extract_document',
    args: { documentBase64: 'x', documentMime: 'application/pdf', documentName: 'a.pdf' },
    reason: 'document attachment: a.pdf',
  };
  ...
  it('🚨 analyze_image OK → il testo estratto ENTRA nel prompt (pre-fix: perso, "Query: undefined")', () => {
    const out = mod.formatToolResultsForPrompt([
      {
        intent: imgIntent,
        data: { ok: true, text: 'Un gatto rosso su un divano blu', confidence: 0.9, elapsedMs: 10 },
      } as unknown as R,
    ]);
  ...
  it('analyze_image con structured → JSON incluso', () => {
    const out = mod.formatToolResultsForPrompt([
      {
        intent: imgIntent,
        data: { ok: true, text: 'fattura', structured: { totale: 120 }, elapsedMs: 1 },
      } as unknown as R,
    ]);
  ...
  it("🚨 analyze_image KO (ok:false senza throw) → avviso ESPLICITO, NON nega l'allegato", () => {
    const out = mod.formatToolResultsForPrompt([
      {
        intent: imgIntent,
        data: { ok: false, error: 'vision_offline', elapsedMs: 2 },
      } as unknown as R,
    ]);
  ...
  it('🚨 extract_document OK → testo del documento nel prompt', () => {
    const out = mod.formatToolResultsForPrompt([
      {
        intent: docIntent,
        data: { ok: true, text: 'Contratto di fornitura art. 5', pages: 3, elapsedMs: 7 },
      } as unknown as R,
    ]);
