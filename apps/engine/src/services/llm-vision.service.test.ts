...
function mockFetch(body: unknown, init: { status?: number } = {}): ReturnType<typeof vi.fn> {
  const fn = vi
    .fn()
    .mockResolvedValue(new Response(JSON.stringify(body), { status: init.status ?? 200 }));
...
describe('dispatchLLMVision — formato per provider (BYOK)', () => {
  it('OpenAI-compat (openai): content[].image_url data-URL + Bearer', async () => {
    ...
    const r = await dispatchLLMVision(
      { provider: 'openai', apiKey: 'sk-1', model: 'gpt-4o' },
      'leggi',
      [IMG],
    );
    ...
    expect(
      (fn.mock.calls[0]![1] as { headers: Record<string, string> }).headers.Authorization,
    ).toBe('Bearer sk-1');
    const body = lastBody(fn);
    const content = (
      body.messages as { content: { type: string; image_url?: { url: string } }[] }[]
    )[0]!.content;
  ...
  it('Anthropic: content[].image.source(base64) + header x-api-key', async () => {
    ...
    const r = await dispatchLLMVision(
      { provider: 'anthropic', apiKey: 'ak-1', model: '' },
      'leggi',
      [IMG],
    );
    expect(r).toEqual({ ok: true, text: 'TESTO-CLAUDE' });
    expect((fn.mock.calls[0]![1] as { headers: Record<string, string> }).headers['x-api-key']).toBe(
      'ak-1',
    );
    const content = (
      lastBody(fn).messages as {
  ...
  it('Gemini: parts[].inline_data + model+key in URL', async () => {
    ...
    const r = await dispatchLLMVision(
      { provider: 'gemini', apiKey: 'gk-1', model: 'gemini-2.0-flash' },
      'leggi',
      [IMG],
    );
    ...
    const parts = (
      lastBody(fn).contents as { parts: { inline_data?: { mime_type: string; data: string } }[] }[]
    )[0]!.parts;
  ...
  it('Ollama: message.images[] (base64 puro)', async () => {
    ...
    const r = await dispatchLLMVision(
      { provider: 'ollama', apiKey: '', model: 'llava', baseUrl: 'http://localhost:11434' },
      'leggi',
      [IMG],
    );
  ...
  it('🚨 provider che non supporta vision → ok:false, NESSUNA fetch', async () => {
    const fn = mockFetch({});
    const r = await dispatchLLMVision({ provider: 'cohere', apiKey: 'x', model: '' }, 'leggi', [
      IMG,
    ]);
