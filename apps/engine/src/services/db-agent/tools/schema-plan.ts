...
const PLAN_PARAM = {
  type: 'array',
  description:
    'lista di azioni: {op:"create_table",table,columns} | {op:"add_column",table,column} | {op:"drop_column",table,columnName} | {op:"drop_table",table} | {op:"add_index",table,indexName,columns,unique?} | {op:"add_foreign_key",name,fromTable,fromColumn,toTable,toColumn,onDelete?}',
  items: { type: 'object' },
} as const;
...
const previewSchemaPlanTool: DbAgentToolDef = {
  ...
  description:
    "Anteprima: ritorna l'SQL che un piano-schema eseguirebbe, SENZA toccare il database. Usalo prima di apply_schema_plan per mostrare il diff.",
  parameters: {
    type: 'object',
  ...
  handler: async (ctx, args) => {
    ...
    const sql = await ctx.dbStudio.previewMigration(
      a.databaseId,
      buildMigrationActions(a.plan),
      ctx.tenantId,
    );
...
const applySchemaPlanTool: DbAgentToolDef = {
  ...
  description:
    "Applica un piano-schema in UN'UNICA migration atomica (tutto o niente). Se il piano contiene op distruttive (drop_table/drop_column) serve confirmDestructive=true.",
  parameters: {
    type: 'object',
    properties: {
      ...
      confirmDestructive: {
        type: 'boolean',
        description: 'obbligatorio true se il piano contiene drop_table/drop_column',
      },
    },
    required: ['databaseId', 'plan'],
    additionalProperties: false,
  ...
  handler: async (ctx, args) => {
    ...
    if (planHasDestructive(a.plan) && a.confirmDestructive !== true) {
      throw new ConfirmationRequiredError(
        `Il piano contiene operazioni distruttive (${destructiveSummary(a.plan)}): passa confirmDestructive=true per applicarlo.`,
      );
    }
    loadOwnedDatabase(ctx, a.databaseId);
    const result = await ctx.dbStudio.applyMigration(
      a.databaseId,
      buildMigrationActions(a.plan),
      ctx.tenantId,
    );
    return {
      applied: a.plan.length,
      affectedTables: (result as { affectedTables?: string[] }).affectedTables ?? [],
    };
  },
};
