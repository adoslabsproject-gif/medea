...
const VIEW_CAPABLE_ENGINES = new Set([
  'sqlite',
  'postgres',
  'pgvector',
  'mysql',
...
function assertRelational(engine: string): void {
  if (!VIEW_CAPABLE_ENGINES.has(engine)) {
    throw new ToolValidationError(
      `Le view non sono supportate sull'engine "${engine}" (solo database relazionali: sqlite/postgres/mysql/mssql/duckdb).`,
    );
  }
}
...
function assertReadOnlySelect(sql: string): void {
  try {
    ...
    throw new ToolValidationError(
      err instanceof Error ? err.message : 'SQL non valido per una view.',
    );
  }
  const kind = classifyStatement(sql);
  if (kind !== 'select') {
    throw new ToolValidationError(
      `La definizione della view deve essere una SELECT di sola lettura (rilevato: ${kind}).`,
    );
  }
}
...
const createViewTool: DbAgentToolDef = {
  name: 'create_view',
  description:
    'Crea una VIEW (query SELECT salvata) in un database relazionale del workspace. select = una singola SELECT di sola lettura.',
  parameters: {
    type: 'object',
    properties: {
      ...
      select: {
        type: 'string',
        description: 'una SELECT di sola lettura (no DML/DDL, no multi-statement)',
      },
    },
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.dbStudio.applyMigration(
      a.databaseId,
      [{ kind: 'create_view', view: { name: a.name, select: a.select } }],
      ctx.tenantId,
    );
...
const dropViewTool: DbAgentToolDef = {
  name: 'drop_view',
  description:
    'Elimina una VIEW (DISTRUTTIVO). confirmViewName deve combaciare ESATTAMENTE con viewName.',
  parameters: {
    type: 'object',
    properties: {
      databaseId: { type: 'string' },
  ...
  handler: async (ctx, args) => {
    ...
    if (a.confirmViewName !== a.viewName) {
      throw new ConfirmationRequiredError(
        `drop_view è distruttivo: passa confirmViewName="${a.viewName}" per confermare.`,
      );
    }
    const db = loadOwnedDatabase(ctx, a.databaseId);
    assertRelational(db.connection.engine);
    await ctx.dbStudio.applyMigration(
      a.databaseId,
      [{ kind: 'drop_view', viewName: a.viewName }],
      ctx.tenantId,
    );
