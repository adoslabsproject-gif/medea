...
import { nextRetryDecision, OUTBOX_MAX_ATTEMPTS, OUTBOX_BASE_DELAY_MS, OUTBOX_MAX_DELAY_MS } from './retry-policy.js';
...
function evt(over: Partial<ErrorOutboxInsert> & { id: string; runId: string; channel: ErrorOutboxChannel }): ErrorOutboxInsert {
  return {
    workflowId: 'wf-1', tenantId: 't-1', errorNodeId: 'n', errorMessage: 'boom', errorHash: 'h',
    durationMs: 1, startedAt: '2026-06-19T10:00:00.000Z', triggerType: 'manual', triggerInputJson: null,
...
const boom: ChannelDispatcher = async () => { throw new Error('dispatch failed'); };
...
describe('retry-policy — backoff + dead-letter (#2)', () => {
  ...
  it('backoff esponenziale crescente + cap', () => {
    ...
    if (dCap.kind === 'retry') {
      expect(new Date(dCap.nextAttemptAt).getTime() - NOW.getTime()).toBeLessThanOrEqual(OUTBOX_MAX_DELAY_MS);
...
describe('worker.runOutboxSweepOnce', () => {
  ...
  const dispatchers = (over: Partial<Record<ErrorOutboxChannel, ChannelDispatcher>> = {}): Record<ErrorOutboxChannel, ChannelDispatcher> =>
    ({ fanout: ok, webhook: ok, email: ok, ...over });

  beforeEach(() => { raw = new Database(':memory:'); raw.exec(ERROR_OUTBOX_DDL); sqlite = wrap(raw); });
  const statusOf = (id: string): string => (raw.prepare("SELECT status FROM error_outbox WHERE id=?").get(id) as { status: string }).status;
  const attemptsOf = (id: string): number => (raw.prepare("SELECT attempts FROM error_outbox WHERE id=?").get(id) as { attempts: number }).attempts;
  ...
  it('dispatch fallisce (attempts bassi) → retry, attempts+1, pending', async () => {
    enqueueErrorEvent(sqlite, evt({ id: 'a', runId: 'r1', channel: 'webhook' }));
    const res = await runOutboxSweepOnce({ sqlite, dispatchers: dispatchers({ webhook: boom }), now: () => NOW });
  ...
  it('🚨 BUG-BOUNTY #2: evento poison a attempts=MAX-1 che fallisce → DEAD (non ritenta in eterno)', async () => {
    ...
    raw.prepare("UPDATE error_outbox SET attempts=? WHERE id='poison'").run(OUTBOX_MAX_ATTEMPTS - 1);
    const res = await runOutboxSweepOnce({ sqlite, dispatchers: dispatchers({ webhook: boom }), now: () => NOW });
    expect(res.dead).toBe(1);
    expect(statusOf('poison')).toBe('dead');
    // e una sweep successiva NON lo ri-claima (coda non bloccata)
    const res2 = await runOutboxSweepOnce({ sqlite, dispatchers: dispatchers({ webhook: boom }), now: () => NOW });
  ...
  it('claim solo i dovuti (futuri esclusi)', async () => {
    enqueueErrorEvent(sqlite, evt({ id: 'due', runId: 'r1', channel: 'webhook', nextAttemptAt: '2026-06-19T09:00:00.000Z' }));
    enqueueErrorEvent(sqlite, evt({ id: 'future', runId: 'r2', channel: 'webhook', nextAttemptAt: '2026-06-19T23:00:00.000Z' }));
    const res = await runOutboxSweepOnce({ sqlite, dispatchers: dispatchers(), now: () => NOW });
  ...
  it('canale senza dispatcher → dead (non loop infinito)', async () => {
    ...
    const partial = { webhook: ok, email: ok } as unknown as Record<ErrorOutboxChannel, ChannelDispatcher>;
