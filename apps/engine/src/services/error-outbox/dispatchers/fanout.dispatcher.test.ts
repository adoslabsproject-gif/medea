...
import { makeFanoutDispatcher, type FanoutDispatcherDeps, type FanoutStartParams } from './fanout.dispatcher.js';
import type { ErrorOutboxRecord } from '../repository.js';

function rec(over: Partial<ErrorOutboxRecord> = {}): ErrorOutboxRecord {
  return {
    id: 'o1', runId: 'r1', channel: 'fanout', workflowId: 'wf-src', tenantId: 't-1',
    errorNodeId: 'node-3', errorMessage: 'boom', errorHash: 'h', durationMs: 5,
    startedAt: '2026-06-19T10:00:00.000Z', triggerType: 'manual', triggerInputJson: '{"a":1}',
...
describe('fanout dispatcher', () => {
  it('avvia l\'error-handler con il payload re-risolto (config corrente)', async () => {
    ...
    expect(startErrorHandler.mock.calls[0]![0]).toMatchObject({
      ...
      runId: 'r1', failedNodeId: 'node-3', error: 'boom', triggerInputJson: '{"a":1}',
    });
  });

  it('nessun handler configurato (resolve → null) → no-op, nessun avvio (done)', async () => {
    const startErrorHandler = vi.fn((_p: FanoutStartParams) => Promise.resolve());
    const d = makeFanoutDispatcher(deps({ resolveErrorWorkflowId: async () => null, startErrorHandler }));
    await d(rec());
  ...
  it('🚨 anti-self: errWfId === workflow sorgente → NON avvia (no auto-loop)', async () => {
    const startErrorHandler = vi.fn((_p: FanoutStartParams) => Promise.resolve());
    const d = makeFanoutDispatcher(deps({ resolveErrorWorkflowId: async () => 'wf-src', startErrorHandler }));
  ...
  it('🚨 rate-limit superato → DROP (done), NESSUN avvio e NESSUN throw (no retry-storm)', async () => {
    ...
    const d = makeFanoutDispatcher(deps({ checkRateLimit: () => false, startErrorHandler, onRateLimited }));
  ...
  it('startErrorHandler throwa → il dispatcher throwa (→ retry/dead nel worker)', async () => {
    const d = makeFanoutDispatcher(deps({ startErrorHandler: vi.fn(() => Promise.reject(new Error('start failed'))) }));
