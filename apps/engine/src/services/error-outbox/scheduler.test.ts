...
function wrap(db: Database.Database): SqliteCompatProxy {
  return {
    prepare: (sql) => { const s = db.prepare(sql); return { run: (...p) => s.run(...p), get: (...p) => s.get(...p), all: (...p) => s.all(...p) }; },
...
function seed(channel: ErrorOutboxChannel, runId: string, nextAttemptAt: string): void {
  enqueueErrorEvent(sqlite, {
    id: `${channel}-${runId}`, runId, channel, workflowId: 'wf', tenantId: 't', errorNodeId: 'n', errorMessage: 'e',
    errorHash: 'h', durationMs: 1, startedAt: '2026-06-19T10:00:00.000Z', triggerType: 'manual',
    triggerInputJson: null, nextAttemptAt,
  });
}
...
describe('startOutboxScheduler', () => {
  it('sweepNow dispatcha gli eventi due e li marca done', async () => {
    ...
    const dispatchers = { ...noopDispatchers, webhook: ((ev) => { dispatched.push(ev.runId); return Promise.resolve(); }) as ChannelDispatcher };
    const h = startOutboxScheduler({ sqlite, dispatchers, now: () => Date.parse('2026-06-19T10:00:00.000Z') });
    await h.sweepNow();
    h.stop();
    expect(dispatched).toEqual(['r1']);
    const row = db.prepare('SELECT status FROM error_outbox WHERE run_id = ?').get('r1') as { status: string };
  ...
  it('🚨 anti-overlap: un secondo sweep mentre il primo è in volo non parte', async () => {
    ...
    let calls = 0;
    const dispatchers = { ...noopDispatchers, webhook: async () => { calls++; await gate; } };
    const h = startOutboxScheduler({ sqlite, dispatchers, now: () => Date.parse('2026-06-19T10:00:00.000Z') });
  ...
  it('gcNow elimina done/dead oltre la retention, conserva i recenti e i pending', () => {
    ...
    db.prepare("UPDATE error_outbox SET status='done', updated_at='2026-06-01T00:00:00.000Z' WHERE run_id='old'").run();
    // riga done recente → conservata
    seed('webhook', 'fresh', '2026-06-19T09:00:00.000Z');
    db.prepare("UPDATE error_outbox SET status='done', updated_at='2026-06-19T09:59:00.000Z' WHERE run_id='fresh'").run();
    // riga pending → MAI eliminata
    seed('webhook', 'pending', '2026-06-19T09:00:00.000Z');

    const h = startOutboxScheduler({
      sqlite, dispatchers: noopDispatchers,
    ...
    const left = (db.prepare('SELECT run_id FROM error_outbox ORDER BY run_id').all() as { run_id: string }[]).map((r) => r.run_id);
  ...
  it('🚨 fail-soft: un dispatcher che esplode NON propaga da sweepNow', async () => {
    ...
    const row = db.prepare('SELECT status, attempts FROM error_outbox WHERE run_id=?').get('r1') as { status: string; attempts: number };
  ...
  it('il timer periodico invoca lo sweep; stop() lo ferma', async () => {
    seed('webhook', 'r1', '2026-06-19T09:00:00.000Z');
    let calls = 0;
    const dispatchers = { ...noopDispatchers, webhook: () => { calls++; return Promise.resolve(); } };
    const h = startOutboxScheduler({ sqlite, dispatchers, sweepIntervalMs: 1000, now: () => Date.parse('2026-06-19T10:00:00.000Z') });
