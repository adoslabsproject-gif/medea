...
function seedFixture(tenantId = 't-1', { runs = 0, errors = 0, success = 0 } = {}) {
  ...
  for (let i = 0; i < runs - errors - success; i++) {
    sqliteInst.prepare(`INSERT INTO runs (id, tenant_id, workflow_id, status, started_at, total_duration_ms, error_count) VALUES (?, ?, 'wf', 'partial', ?, 100, 0)`).run(`r-${tenantId}-${i}`, tenantId, old);
  }
  for (let i = 0; i < errors; i++) {
    sqliteInst.prepare(`INSERT INTO runs (id, tenant_id, workflow_id, status, started_at, total_duration_ms, error_count) VALUES (?, ?, 'wf', 'error', ?, 100, 1)`).run(`re-${tenantId}-${i}`, tenantId, old);
  }
  for (let i = 0; i < success; i++) {
    sqliteInst.prepare(`INSERT INTO runs (id, tenant_id, workflow_id, status, started_at, total_duration_ms, ended_at, error_count) VALUES (?, ?, 'wf', 'success', ?, 200, ?, 0)`).run(`rs-${tenantId}-${i}`, tenantId, old, new Date(now).toISOString());
  }
}
...
describe('🚨 instance', () => {
  it('🚨 zero data → all zero + successRate=100', () => {
    ...
    expect(r).toEqual({
      tenants: 0, users: 0, workflows: 0, activeWorkflows: 0,
      runsLast24h: 0, runsLast7d: 0, errorsLast24h: 0, successRate7d: 100,
    });
  });

  it('🚨 cross-tenant aggregate count', () => {
    sqliteInst.exec(`INSERT INTO users VALUES ('u1', 't-A', 1), ('u2', 't-A', 1), ('u3', 't-B', 1)`);
...
describe('🚨 tenantDashboard', () => {
  ...
  it('🚨 recentRuns limit 20 + ordered DESC', () => {
    for (let i = 0; i < 25; i++) {
      sqliteInst.prepare(`INSERT INTO runs (id, tenant_id, workflow_id, status, started_at) VALUES (?, 't-1', 'wf-1', 'success', ?)`).run(`r-extra-${i}`, new Date(Date.now() - i * 1000).toISOString());
    }
    const r = new AdminStatsService().tenantDashboard('t-1');
  ...
  it('🚨 currentlyRunning = ended_at NULL only', () => {
    sqliteInst.prepare(`INSERT INTO runs (id, tenant_id, workflow_id, status, started_at, ended_at) VALUES ('inflight-1', 't-1', 'wf-1', 'running', ?, NULL)`).run(new Date().toISOString());
    sqliteInst.prepare(`INSERT INTO runs (id, tenant_id, workflow_id, status, started_at, ended_at) VALUES ('done-1', 't-1', 'wf-1', 'success', ?, ?)`).run(new Date().toISOString(), new Date().toISOString());
    const r = new AdminStatsService().tenantDashboard('t-1');
