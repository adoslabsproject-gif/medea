  ...
  },
  {
    ...
    handler: (ctx, args) => {
      ...
      return {
        ...
        ...(entry.actions
          ? { actions: entry.actions.map((a) => ({ id: a.id, label: a.label })) }
          : {}),
      };
    },
  },
...
export function executeWorkflowTool(
  ...
  if (!tool)
    return {
      ok: false,
      error: `Tool sconosciuto: "${name}". Disponibili: ${[...BY_NAME.keys()].join(', ')}.`,
    };
  const parsed = tool.schema.safeParse(args ?? {});
  if (!parsed.success) {
    return {
      ok: false,
      error: `Argomenti non validi per ${name}: ${parsed.error.issues.map((i) => `${i.path.join('.')} ${i.message}`).join('; ')}`,
    };
  }
  try {
