...
describe('modifyWorkflow — aggiungi nodo + collega (il caso che il vecchio chatter NON faceva)', () => {
  it('🚨 add email + connect al webhook esistente → patch addNodes + addEdges', async () => {
    ...
    expect(r.patch.addNodes).toEqual([
      { id: 'email', defId: 'action_send_email', config: { to: 'a@b.it' } },
    ]);
  ...
  it('🚨 set_config su nodo esistente → patch updateNodes con config completa', async () => {
    ...
    const r = await modifyWorkflow(
      base({
        ...
        llmTurn: scripted([
          call('1', 'set_config', { nodeId: 'h', config: { method: 'POST' }, merge: true }),
          call('2', 'finish', {}),
        ]),
      }),
    );
    expect(r.patch.updateNodes).toEqual([
      { id: 'h', patch: { config: { url: 'https://old', method: 'POST' } } },
    ]);
  });
});
      ...
      base({
        ...
        llmTurn: scripted([
          call('1', 'add_node', {
            defId: 'action_send_email',
            id: 'email',
            config: { to: 'a@b.it', apiKey: '{{secrets.EMAIL_API_KEY}}' },
          }),
    ...
    const r = await modifyWorkflow(
      base({
        ...
        llmTurn: scripted([
          ...
          {
            kind: 'final',
            text: 'Non esiste un nodo adatto nel catalogo: crealo nell\'IDE (tab "I Miei Nodi").',
          },
        ]),
      }),
    );
...
describe('modifyWorkflow — read-before-edit / no-op', () => {
  it('🚨 nessuna operazione del modello → patch vuoto + il workflow esistente è PRESERVATO', async () => {
    ...
    const r = await modifyWorkflow(
      base({
        currentWorkflow: wf,
        request: 'che fa questo workflow?',
        llmTurn: scripted([{ kind: 'final', text: 'Spiegazione...' }]),
      }),
    );
      ...
      base({
        ...
        llmTurn: scripted([
          call('1', 'add_node', {
            defId: 'action_http_request',
            id: 'h',
            config: { url: 'https://x/health', method: 'GET' },
          }),
