...
import { evaluateLockAcquire, isLockAlive, type LockState, type LockDecision } from './workflow-lock.logic.js';
...
interface LockRow { user_id: string; user_name: string; heartbeat_at: number }

export class WorkflowLockService {
  private read(workflowId: string): LockState | null {
    const row = getDatabase().sqlite
      .prepare('SELECT user_id, user_name, heartbeat_at FROM workflow_locks WHERE workflow_id = ?')
  ...
  acquire(workflowId: string, userId: string, userName: string, now: number = Date.now()): LockDecision {
  ...
  heartbeat(workflowId: string, userId: string, now: number = Date.now()): boolean {
    const res = getDatabase().sqlite
      .prepare('UPDATE workflow_locks SET heartbeat_at = ? WHERE workflow_id = ? AND user_id = ?')
  ...
  release(workflowId: string, userId: string): void {
    getDatabase().sqlite
      .prepare('DELETE FROM workflow_locks WHERE workflow_id = ? AND user_id = ?')
  ...
  status(workflowId: string, userId: string, now: number = Date.now()): LockStatus {
    ...
    return { locked: true, mine: current.userId === userId, by: { userId: current.userId, userName: current.userName } };
  }
}
