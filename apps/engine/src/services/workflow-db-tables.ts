 ...
 */
export interface DbTableRef { databaseId: string; table: string }
...
export const ANY_DB_DEFIDS = ['db_insert', 'db_insert_batch', 'db_update', 'db_delete', 'db_query'] as const;
 ...
 */
export function dedicatedTablesToDrop(targetNodes: unknown, otherWorkflowsNodes: readonly unknown[]): DbTableRef[] {
  const written = extractDbTableRefs(targetNodes, WRITE_DB_DEFIDS);
  if (written.length === 0) return [];
  const usedByOthers = new Set<string>();
  for (const nodes of otherWorkflowsNodes) {
    for (const r of extractDbTableRefs(nodes, ANY_DB_DEFIDS)) usedByOthers.add(`${r.databaseId}::${r.table}`);
  }
