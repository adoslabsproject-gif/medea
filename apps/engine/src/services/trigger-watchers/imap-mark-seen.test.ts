...
const baseParams: MarkSeenParams = {
  workflowId: 'wf-1', host: 'imap.test', port: 993, tlsMode: 'tls',
  username: 'u', password: 'p', mailbox: 'INBOX', uid: 42, mode: 'on-success',
};
...
function makeFakeClient(over: Partial<FakeClientControl> = {}): { client: ImapMarkSeenClient; ctrl: FakeClientControl; seenOpts?: ImapConnectOptions } {
  ...
  const client: ImapMarkSeenClient = {
    connect: async () => { ctrl.calls.push('connect'); await ctrl.connect(); },
    getMailboxLock: async (mb: string) => {
      ctrl.calls.push(`lock:${mb}`);
      return { release: () => { ctrl.released = true; ctrl.calls.push('release'); } };
    },
    messageFlagsAdd: async (range, flags, opts) => {
      ctrl.calls.push(`flags:${JSON.stringify({ range, flags, opts })}`);
      return ctrl.messageFlagsAdd();
    },
    logout: async () => { ctrl.calls.push('logout'); await ctrl.logout(); },
  };
  return { client, ctrl };
}

function depsWith(client: ImapMarkSeenClient, capture?: (o: ImapConnectOptions) => void, timeoutMs = 5000): MarkSeenDeps {
  return {
    createClient: (opts) => { capture?.(opts); return client; },
    timeoutMs,
  };
}
...
describe('markSeenWithFreshConnection', () => {
  it('🚨 happy: sequenza connect→lock→flags→logout, args esatti, secure da tlsMode', async () => {
    ...
    await markSeenWithFreshConnection(baseParams, depsWith(client, (o) => { opts = o; }));
    ...
    expect(opts).toMatchObject({ host: 'imap.test', port: 993, secure: true, auth: { user: 'u', pass: 'p' } });
  });

  it('tlsMode != "tls" → secure false', async () => {
    const { client } = makeFakeClient();
    let opts: ImapConnectOptions | undefined;
    await markSeenWithFreshConnection({ ...baseParams, tlsMode: 'starttls' }, depsWith(client, (o) => { opts = o; }));
    expect(opts!.secure).toBe(false);
  });

  it('🚨 errore IMAP su messageFlagsAdd → lock RILASCIATO, warn, NESSUN throw', async () => {
    const { client, ctrl } = makeFakeClient({ messageFlagsAdd: async () => { throw new Error('IMAP boom'); } });
    const { logger } = await import('@/lib/logger.js');
    const warn = vi.spyOn(logger, 'warn').mockImplementation(() => logger);
    const info = vi.spyOn(logger, 'info').mockImplementation(() => logger);

    await expect(markSeenWithFreshConnection(baseParams, depsWith(client))).resolves.toBeUndefined();
  ...
  it('🚨 TIMEOUT: connect appeso + timeoutMs piccolo → warn, NO throw', async () => {
    const { client } = makeFakeClient({ connect: () => new Promise<void>(() => { /* mai */ }) });
    const { logger } = await import('@/lib/logger.js');
    const warn = vi.spyOn(logger, 'warn').mockImplementation(() => logger);
    await expect(markSeenWithFreshConnection(baseParams, depsWith(client, undefined, 10))).resolves.toBeUndefined();
  ...
  it('🚨 non lancia MAI: anche se pure il logout di cleanup rigetta', async () => {
    const { client } = makeFakeClient({
      messageFlagsAdd: async () => { throw new Error('boom'); },
      logout: async () => { throw new Error('logout also fails'); },
    });
    vi.spyOn((await import('@/lib/logger.js')).logger, 'warn').mockImplementation(function (this: unknown) { return this as never; });
    await expect(markSeenWithFreshConnection(baseParams, depsWith(client))).resolves.toBeUndefined();
  });
