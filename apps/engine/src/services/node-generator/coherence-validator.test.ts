...
describe('validateCoherence', () => {
  it('🚨 config.<key> non dichiarata → unknown_config_key (error)', () => {
    ...
    const v = validateCoherence(
      def,
      'async function execute(config,i,x){ return { a: config.apiKey, b: config.url }; }',
    );
    expect(
      v
        .filter((x) => x.kind === 'unknown_config_key')
        .map((x) => x.message)
        .join(),
    ).toMatch(/apiKey/u);
    // url È dichiarata → NON deve comparire
    expect(v.some((x) => x.kind === 'unknown_config_key' && x.message.includes('config.url'))).toBe(
      false,
    );
  });
  ...
  it('🚨 secret usato ma non dichiarato come field secret → undeclared_secret', () => {
    ...
    const v = validateCoherence(
      def,
      'async function execute(c,i,ctx){ return { t: ctx.secrets["TOKEN"] }; }',
    );
  ...
  it('secret correttamente dichiarato (type secret) → nessuna violazione', () => {
    ...
    const v = validateCoherence(
      def,
      'async function execute(c,i,ctx){ return { t: ctx.secrets["API_KEY"] }; }',
    );
  ...
  it('select CON options → nessun warning', () => {
    const def: CoherenceDefView = {
      configFields: [{ key: 'mode', type: 'select', options: ['a', 'b'] }],
    };
