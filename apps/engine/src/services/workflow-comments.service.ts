...
function rowToComment(r: CommentRow): WorkflowComment {
  ...
  if (r.mentions_json) {
    try {
      mentions = JSON.parse(r.mentions_json) as string[];
    } catch {
      mentions = [];
    }
  }
...
export class WorkflowCommentsService {
  ...
  list(workflowId: string, nodeId?: string | null): WorkflowComment[] {
    ...
    const rows = (
      nodeId === undefined
        ? sqlite
            .prepare(
              'SELECT * FROM workflow_comments WHERE workflow_id = ? ORDER BY created_at ASC',
            )
            .all(workflowId)
        : sqlite
            .prepare(
              'SELECT * FROM workflow_comments WHERE workflow_id = ? AND node_id IS ? ORDER BY created_at ASC',
            )
  ...
  countsByNode(workflowId: string): Record<string, number> {
    const rows = getDatabase()
      .sqlite.prepare(
        'SELECT node_id, COUNT(*) AS n FROM workflow_comments WHERE workflow_id = ? AND node_id IS NOT NULL AND resolved = 0 GROUP BY node_id',
      )
  ...
  setResolved(id: string, resolved: boolean, workflowId: string): boolean {
    const res = getDatabase()
      .sqlite.prepare('UPDATE workflow_comments SET resolved = ? WHERE id = ? AND workflow_id = ?')
  ...
  remove(id: string, userId: string, workflowId: string): boolean {
    const res = getDatabase()
      .sqlite.prepare(
        'DELETE FROM workflow_comments WHERE id = ? AND user_id = ? AND workflow_id = ?',
      )
