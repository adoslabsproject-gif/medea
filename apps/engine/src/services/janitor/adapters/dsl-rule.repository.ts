...
export class DslRuleRepository {
  ...
  async get(id: string): Promise<DslRule | null> {
    const { db } = getDatabase();
    const rows = await db.select().from(janitorDslRules).where(eq(janitorDslRules.id, id)).limit(1);
    const row = rows[0];
  ...
  async delete(id: string, tenantId: string): Promise<void> {
    const { db } = getDatabase();
    await db
      .delete(janitorDslRules)
      .where(and(eq(janitorDslRules.id, id), eq(janitorDslRules.tenantId, tenantId)));
  }
}
...
function rowToDsl(row: JanitorDslRuleRow): DslRule {
  ...
  try {
    ...
    if (Array.isArray(parsed))
      tags = Object.freeze(parsed.filter((t): t is string => typeof t === 'string'));
  } catch {
    /* malformed → empty */
  }
