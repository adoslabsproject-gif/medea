...
describe('🚨 detect', () => {
  ...
  it('🚨 run senza step running → skip', async () => {
    executeRawMock.mockResolvedValueOnce({ rows: [{
      id: 'r-1', workflow_id: 'wf', tenant_id: 't', status: 'success',
  ...
  it('🚨 run con 1+ step running → detected', async () => {
    executeRawMock.mockResolvedValueOnce({ rows: [{
      id: 'r-bad', workflow_id: 'wf', tenant_id: 't-1', status: 'success',
      started_at: 'a', ended_at: '2026-06-07',
  ...
  it('🚨 tenant_id null → tenantId NON nel result', async () => {
    executeRawMock.mockResolvedValueOnce({ rows: [{
      id: 'r-no-tenant', workflow_id: 'wf', tenant_id: null, status: 'success',
      started_at: 'a', ended_at: 'b',
  ...
  it('🚨 steps_json malformato → skip (no throw)', async () => {
    executeRawMock.mockResolvedValueOnce({ rows: [{
      id: 'r-broken', workflow_id: 'wf', tenant_id: null,
      status: 'success', started_at: 'a', ended_at: 'b',
      steps_json: 'not-json{',
      error_count: 0,
    }] });
    const r = await runsTruncatedStepsRule.detect!(baseCtx as any);
  ...
  it('🚨 maxRows cap output (anche se SQL LIMIT più alto)', async () => {
    const rows = Array.from({ length: 200 }, (_, i) => ({
      id: `r-${i}`, workflow_id: 'wf', tenant_id: null,
      status: 'success', started_at: 'a', ended_at: 'b',
