...
describe('callAiAssist', () => {
  ...
  it('passa workspaceId come header X-FF-Workspace', async () => {
    ...
    await callAiAssist({
      ...
      sources: { executor: '', definition: '', schema: '' },
    });
  ...
  it('💬 action=chat: prompt conversazionale (istruzione "applica solo se chiesto" + sorgenti come contesto)', async () => {
    ...
    await callAiAssist({
      ...
      workspaceId: 'ws-1',
      prompt: 'ciao, cosa fa?',
      sources: { executor: 'EXEC_SRC', definition: 'D', schema: 'S' },
    });
    const userMsg = JSON.parse((mockFetch.mock.calls[0]![1] as { body: string }).body).messages.at(
      -1,
    ).content as string;
  ...
  it('💬 chat SENZA codice in risposta → conversazione pura, NESSUNA patch', async () => {
    mockFetch.mockResolvedValue(
      mockGatewayResponse('Risponde a "ciao" e ritorna un saluto. Vuoi che aggiunga un campo?'),
    );
    const res = await callAiAssist({
      action: 'chat',
      workspaceId: 'ws-1',
      prompt: 'spiegami a parole',
    });
  ...
  it('crossSurfaceContext → iniettato come SECONDO system message (Liara "ricorda" le altre schede)', async () => {
    ...
    expect(sent.messages[1]).toEqual({
      role: 'system',
      content: '[CONTESTO DA ALTRE SCHEDE]\n• Workflow editor: monitoraggio prezzi',
    });
  ...
  it('HTTP error gateway → throws', async () => {
    ...
    await expect(callAiAssist({ action: 'explain', workspaceId: 'ws-1' })).rejects.toThrow(/503/u);
  });
  ...
  it('response senza usage → tokens undefined', async () => {
    mockFetch.mockResolvedValue({
      ...
      text: () =>
        Promise.resolve(JSON.stringify({ choices: [{ message: { content: 'no usage' } }] })),
    } as unknown as Response);
