...
export async function rateGeneration(
  ...
  if (rating !== null && !RATINGS.includes(rating))
    throw new Error(`rating non valido: ${String(rating)}`);
...
export async function listConversations(deps: RepoDeps): Promise<ConversationSummary[]> {
  const sql =
    `SELECT conversation_id AS id, COUNT(*) AS n, MAX(created_at) AS last, MAX(prompt) AS title ` +
  ...
  const res = await deps.dbStudio.executeRaw(
    deps.dbId,
    sql,
    { dryRun: false, rowLimit: 200 },
    deps.tenantId,
  );
...
export async function getConversation(deps: RepoDeps, convId: string): Promise<ConversationItem[]> {
  ...
  const sql =
    `SELECT id, created_at, kind, prompt, media_ref, mime, rating FROM ${GENERATIONS_TABLE} ` +
...
export async function deleteConversation(deps: RepoDeps, convId: string): Promise<void> {
  ...
  await deps.dbStudio.executeRaw(
    deps.dbId,
    `DELETE FROM ${GENERATIONS_TABLE} WHERE conversation_id = '${id}'`,
