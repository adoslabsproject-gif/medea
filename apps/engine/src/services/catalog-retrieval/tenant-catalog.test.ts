...
vi.mock('@/services/ai-scaffold/node-catalog.js', () => ({
  buildNodeCatalog: () => [
    { defId: 'trigger_webhook', type: 'trigger', label: 'Webhook', description: 'Avvia da HTTP.', fields: [] },
    { defId: 'action_send_email', type: 'action', label: 'Email', description: 'Invia email.', fields: [] },
  ],
}));
...
beforeEach(() => { listCustomNodesMock.mockReset(); });

function customNode(slug: string, status: string, displayName = slug, description = 'desc'): Record<string, unknown> {
  return { id: `id-${slug}`, slug, displayName, description, category: 'Custom', semver: '1.0.0', status };
}

describe('listRunnableCustomEntries', () => {
  it('solo RUNNABLE (published_priv / marketplace_published) — draft/candidate esclusi', async () => {
    listCustomNodesMock.mockResolvedValue({ items: [
...
describe('buildTenantCatalog', () => {
  it('base (sistema) + custom runnable, fusi', async () => {
    listCustomNodesMock.mockResolvedValue({ items: [customNode('mio_nodo', 'published_priv')], total: 1 });
