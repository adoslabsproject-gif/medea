...
const run = (config: Record<string, unknown>) =>
  janitorCleanupExecutor(config as never, {} as never, ctx);
...
describe('single — branching su esito rule', () => {
  ...
  it('success + quarantined > 0 → branch DETECTION', async () => {
    janitor.executeRule.execute.mockResolvedValue({
      report: ruleReport({ rowsQuarantined: 3, rowsDetected: 3 }),
    });
  ...
  it('report.success=false → branch ERROR con report propagato', async () => {
    janitor.executeRule.execute.mockResolvedValue({
      report: ruleReport({ success: false, error: 'lock contention' }),
    });
    const res = await run({ mode: 'single', ruleId: 'rule-1' });
    expect(res.branch).toBe('error');
  ...
  it('overrides: paramsOverride object + maxRowsPerRun string → passati a executeRule', async () => {
    ...
    await run({
      mode: 'single',
      ruleId: 'rule-1',
      paramsOverride: { soglia: 5 },
      maxRowsPerRun: '250',
    });
    const arg = janitor.executeRule.execute.mock.calls[0]![0] as {
      overrides: { params: unknown; maxRowsPerRun: number };
    };
...
describe('cycle — aggregazione multi-rule', () => {
  ...
  it('tutte pulite → branch CLEAN', async () => {
    janitor.executeCycle.execute.mockResolvedValue(
      cycleReport([
  ...
  it('somma quarantined > 0 → DETECTION con totali aggregati', async () => {
    ...
    expect(res.branch).toBe('detection');
    expect(res.output as { rowsQuarantined: number; rowsDetected: number }).toMatchObject({
      rowsQuarantined: 5,
      rowsDetected: 7,
    });
  });
  ...
  it('tagFilter CSV → split e passato a executeCycle (vuoto → omesso)', async () => {
    janitor.executeCycle.execute.mockResolvedValue(
      cycleReport([{ success: true, rowsQuarantined: 0, rowsDetected: 0 }]),
    );
    await run({ mode: 'cycle', tagFilter: 'orders, gdpr , ' });
    expect(janitor.executeCycle.execute.mock.calls[0]![0]).toMatchObject({
      tagFilter: ['orders', 'gdpr'],
    });
  });
});
