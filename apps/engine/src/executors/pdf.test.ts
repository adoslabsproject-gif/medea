...
describe('🚨 pdfParseExecutor — input resolution', () => {
  ...
  it('path input dal tenant namespace', async () => {
    ...
    const gen = await pdfGenerateExecutor(
      { title: 'TestDoc', sectionsJson: '[{"heading":"H","body":"B"}]' },
      null,
      ctx,
    );
    // ref-primario: pdf_generate emette SEMPRE un handle; senza store = inline base64.
    const buf = Buffer.from(
      (gen.output as { binary: { data?: string } }).binary.data ?? '',
      'base64',
    );
  ...
  it('🚨 missing input → throw', async () => {
    await expect(pdfParseExecutor({}, null, ctx)).rejects.toThrow(
      /almeno uno tra config\.path o config\.base64/u,
    );
  });

  it('🚨 path traversal denied', async () => {
    await expect(pdfParseExecutor({ path: '../../../etc/passwd.pdf' }, null, ctx)).rejects.toThrow(
  ...
  it('🚨 size > 32MB → throw', async () => {
    ...
    await expect(pdfParseExecutor({ path: 'big.pdf' }, null, ctx)).rejects.toThrow(
      /troppo grande.*Max 32 MB/u,
    );
  });
  ...
  it('🚨 size > 32MB via base64 (input in-memory upstream) → throw PRIMA di pdf-parse', async () => {
    ...
    await expect(
      pdfParseExecutor({ base64: bigB64, mode: 'pdf-parse-only' }, null, ctx),
...
describe('🚨 mode router validation', () => {
  it('🚨 mode invalido → throw', async () => {
    ...
    ).rejects.toThrow(/mode "invalid-mode" non valido/u);
  });
...
describe('🚨 LLM-vision fallback', () => {
  ...
  it('🚨 mode=llm-only senza Anthropic key + senza Liara → throw esplicito', async () => {
    ...
    ).rejects.toThrow(/Anthropic API key.*Liara enabled/u);
  });
  ...
  it('🚨 mode=llm-only senza Anthropic + Liara enabled → throw "non supporta PDF vision"', async () => {
    ...
    ).rejects.toThrow(/Liara non supporta ancora input PDF vision/u);
  });
  ...
  it('mode=llm-only con Anthropic key → chiama API + ritorna text estratto', async () => {
    ...
    m.safeFetch.mockResolvedValue({
      ...
      json: async () => ({
        content: [
          { type: 'text', text: 'Estratto da LLM: FATTURA n.999 totale 250 euro contenuto reale' },
        ],
      }),
    });
    const r = await pdfParseExecutor(
  ...
  it('🚨 LLM API HTTP error → throw con status code', async () => {
    ...
    ).rejects.toThrow(/LLM-vision \(Anthropic 429\)/u);
  });
    ...
    m.safeFetch.mockResolvedValue({
      ...
      body: {
        getReader: () => ({
          read: async () => {
            reads += 1;
            return reads <= 500 ? { done: false, value: chunk } : { done: true, value: undefined };
          },
          cancel: cancelSpy,
        }),
      },
      text: async () => {
        throw new Error('🚨 res.text() = body errore intero in RAM (OOM)');
      },
    });
    await expect(
      pdfParseExecutor({ base64: buf.toString('base64'), mode: 'llm-only' }, null, ctx),
  ...
  it('🚨 LLM response vuota → throw', async () => {
    ...
    ).rejects.toThrow(/risposta vuota/u);
  });
});
...
describe('🚨 pdfGenerateExecutor — output reale via PDFKit', () => {
  ...
  it('🚨 title mancante → throw', async () => {
    await expect(pdfGenerateExecutor({}, null, ctx)).rejects.toThrow(/"title".*obbligatorio/u);
  });
  ...
  it('🚨 sectionsJson non array → throw', async () => {
    await expect(
      pdfGenerateExecutor(
        {
          title: 'X',
          sectionsJson: '{"not":"an array"}',
        },
        null,
        ctx,
      ),
    ).rejects.toThrow(/sectionsJson deve essere un array/u);
  });
  ...
  it('🚨 tableJson non array → throw', async () => {
    ...
    ).rejects.toThrow(/tableJson deve essere un array/u);
  });
  ...
  it('🚨 tabella > 10k righe → throw', async () => {
    ...
    ).rejects.toThrow(/tabella troppo grande/u);
  });
  ...
  it('table render con multiple pages (output size grows linearly)', async () => {
    ...
    const rowsLarge = JSON.stringify(
      Array.from({ length: 200 }, (_, i) => ({
        n: i,
        descrizione: `Riga ${String(i)} con testo descrittivo`,
        totale: i * 10,
      })),
    );
...
describe('🚨 GAP2 FLIP — pdfGenerateExecutor emette SEMPRE un handle BinaryData (ref-primario)', () => {
  ...
  it('🚨 CORRETTEZZA: il blob binario è un PDF COMPLETO e integro (header + trailer, no corruzione)', async () => {
    ...
    // dei due mancherebbe.
    const { c, store } = await ctxStore();
    const res = await pdfGenerateExecutor(
      { title: 'Doc', sectionsJson: '[{"heading":"Capitolo","body":"corpo"}]' },
      null,
      c,
    );
  ...
  it("🚨 NIENTE LEGACY: l'output non ha MAI il campo base64 (né con store né senza)", async () => {
    ...
    const a = (await pdfGenerateExecutor({ title: 'A' }, null, withStore.c)).output as Record<
      string,
      unknown
    >;
    const b = (await pdfGenerateExecutor({ title: 'B' }, null, ctx)).output as Record<
      string,
      unknown
    >;
...
describe('🚨 GAP2 capstone — resolveBinaryInline (risoluzione BinaryData senza core-schema)', () => {
  ...
  it('🚨 ref SENZA readBinary → errore esplicito (no byte vuoti silenziosi)', async () => {
    await expect(resolveBinaryInline(refBin('z'.repeat(64), 10))).rejects.toThrow(
      /context\.readBinary assente/u,
    );
  });
});
...
describe('🚨 GAP2 capstone — pdfParse ROUTING dei byte in input (failure-deterministica)', () => {
  ...
  it('🚨 input BinaryData inline → i SUOI byte raggiungono pdf-parse (garbage → throw)', async () => {
    ...
    await expect(
      pdfParseExecutor(
        { mode: 'pdf-parse-only' },
        binGarbage(Buffer.from('NOT-A-PDF').toString('base64')),
        ctx,
      ),
    ).rejects.toThrow(/estrazione PDF locale fallita/u);
  });
