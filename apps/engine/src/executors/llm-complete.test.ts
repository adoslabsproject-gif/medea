...
describe('🚨 prompt validation', () => {
  it('🚨 prompt vuoto → throw obbligatorio', async () => {
    await expect(llmCompleteExecutor({ prompt: '' } as never, {} as never, ctx())).rejects.toThrow(
      /"prompt"/u,
    );
  });
...
describe('🚨 Provider whitelist + BYOK resolution', () => {
  ...
  it('🚨 provider sconosciuto "cohere" → fallback liara (non in ALLOWED_PROVIDERS)', async () => {
    await llmCompleteExecutor({ prompt: 'x', provider: 'cohere' } as never, {} as never, ctx());
    expect(resolverMock).not.toHaveBeenCalled();
  ...
  it('🚨 provider=gemini → ora SUPPORTATO (BYOK): resolver call, NON fallback liara', async () => {
    ...
    await llmCompleteExecutor({ prompt: 'x', provider: 'gemini' } as never, {} as never, ctx());
  ...
  it('🚨 provider=perplexity → ora SUPPORTATO (BYOK): resolver call + apiKey', async () => {
    ...
    await llmCompleteExecutor({ prompt: 'x', provider: 'perplexity' } as never, {} as never, ctx());
  ...
  it('🚨 provider=anthropic → resolver call + apiKey injected', async () => {
    ...
    await llmCompleteExecutor({ prompt: 'x', provider: 'anthropic' } as never, {} as never, ctx());
  ...
  it('🚨 resolver baseUrl → forwardato a dispatchLLMChat', async () => {
    ...
    await llmCompleteExecutor({ prompt: 'x', provider: 'openai' } as never, {} as never, ctx());
  ...
  it('🚨 NoLlmProviderError → helpful message Settings', async () => {
    ...
    await expect(
      llmCompleteExecutor({ prompt: 'x', provider: 'anthropic' } as never, {} as never, ctx()),
  ...
  it('🚨 generic error in resolver → re-throw as-is', async () => {
    ...
    await expect(
      llmCompleteExecutor({ prompt: 'x', provider: 'openai' } as never, {} as never, ctx()),
  ...
  it('🚨 BYOK custom model hint sovrascrive default', async () => {
    ...
    await llmCompleteExecutor(
      { prompt: 'x', provider: 'openai', model: 'gpt-4-turbo' } as never,
      {} as never,
      ctx(),
    );
...
describe('🚨 Clamp temperatura / maxTokens / timeoutMs', () => {
  ...
  it('🚨 temperature NaN → default 0.7', async () => {
    await expect(
      llmCompleteExecutor(
        { prompt: 'x', temperature: 'invalid' as never } as never,
        {} as never,
        ctx(),
      ),
    ).resolves.toBeDefined();
  ...
  it('🚨 maxTokens 999999 → clamp 32768 (env var verifica)', async () => {
    await llmCompleteExecutor({ prompt: 'x', maxTokens: 999_999 } as never, {} as never, ctx());
  ...
  it('🚨 maxTokens negativo → default 2048', async () => {
    await llmCompleteExecutor({ prompt: 'x', maxTokens: -500 } as never, {} as never, ctx());
    expect(dispatchMock).toHaveBeenCalled();
  });

  it('🚨 timeoutMs 999999 → clamp 300_000', async () => {
    await llmCompleteExecutor({ prompt: 'x', timeoutMs: 999_999 } as never, {} as never, ctx());
    expect(dispatchMock).toHaveBeenCalled();
  });

  it('🚨 timeoutMs NaN → default 60_000', async () => {
    await llmCompleteExecutor(
      { prompt: 'x', timeoutMs: 'never' as never } as never,
      {} as never,
      ctx(),
    );
    expect(dispatchMock).toHaveBeenCalled();
...
describe('🚨🚨 timeout/maxTokens via opts ESPLICITO — niente mutazione process.env (anti-race)', () => {
  ...
  it('🚨 NON muta MAI process.env (né durante né dopo, neppure su throw)', async () => {
    ...
    await llmCompleteExecutor(
      { prompt: 'x', timeoutMs: 30_000, maxTokens: 1024 } as never,
      {} as never,
      ctx(),
    );
    ...
    await expect(llmCompleteExecutor({ prompt: 'x' } as never, {} as never, ctx())).rejects.toThrow(
      'LLM down',
    );
    expect(process.env.MEDEA_LIARA_TIMEOUT_MS).toBe('PRE-EXISTING');
    expect(process.env.MEDEA_LIARA_MAX_TOKENS).toBe('PRE-MAX');
  ...
  it('🚨 timeout/maxTokens/temperature clampati passati in opts a dispatchLLMChat', async () => {
    await llmCompleteExecutor(
      { prompt: 'x', maxTokens: 999_999, timeoutMs: 999_999, temperature: 1.3 } as never,
      {} as never,
      ctx(),
    );
  ...
  it('🚨🚨 RACE: due esecuzioni concorrenti → ciascuna porta il PROPRIO maxTokens in opts', async () => {
    ...
    const seen = dispatchMock.mock.calls
      .map((c) => (c[9] as { maxTokens?: number }).maxTokens)
      .sort();
...
describe('🚨 responseFormat=json + system addendum', () => {
  it('🚨 responseFormat=text → no addendum JSON istruzioni', async () => {
    await llmCompleteExecutor(
      { prompt: 'x', systemPrompt: 'You are helpful.' } as never,
      {} as never,
      ctx(),
    );
...
describe('🚨 JSON parse output cleaning', () => {
  it('🚨 valid JSON output → jsonParsed populated', async () => {
    ...
    const r = await llmCompleteExecutor(
      { prompt: 'x', responseFormat: 'json' } as never,
      {} as never,
      ctx(),
    );
  ...
  it('🚨 responseFormat=text → jsonParsed NEVER attempted', async () => {
    ...
    const r = await llmCompleteExecutor(
      { prompt: 'x', responseFormat: 'text' } as never,
      {} as never,
      ctx(),
    );
...
describe('🚨 finishReason heuristic — length vs stop', () => {
  it('🚨 completion termina con "." → stop', async () => {
    ...
    const r = await llmCompleteExecutor(
      { prompt: 'x', maxTokens: 1000 } as never,
      {} as never,
      ctx(),
    );
...
describe('🚨 Output shape contract', () => {
  ...
  it('🚨 _llm standard presente ACCANTO a tokensUsed (mai al posto di)', async () => {
    const r = await llmCompleteExecutor(
      { prompt: 'x', provider: 'liara' } as never,
      {} as never,
      ctx(),
    );
  ...
  it('🚨 _llm coerente con tokensUsed anche quando il listener non scatta (usage zero)', async () => {
    ...
    const r = await llmCompleteExecutor({ prompt: 'x' } as never, {} as never, ctx());
    const out = r.output as {
      _llm: { inputTokens: number; outputTokens: number; fromApi: boolean };
    };
...
describe('degradazione quota — fallback BYOK', () => {
  const run = (cfg: Record<string, unknown>) =>
    llmCompleteExecutor(cfg as never, {} as never, ctx());
    ...
    dispatchMock
      ...
      .mockImplementationOnce(
        async (
          ...
          listener({ input: 10, output: 20, fromApi: true });
          return 'BYOK output.';
        },
      );
    fallbackMock.mockReturnValue({
      provider: 'anthropic',
      apiKey: 'sk-ant',
      model: 'claude-sonnet-4-6',
    });

    const res = (await run({ prompt: 'ciao' })) as {
      output: { completion: string; provider: string; degraded?: string };
    };
