...
export function coerceChartData(raw: unknown, labelField: string, valueField: string): { label: unknown; value: unknown }[] {
  let data = raw;
  ...
  return (data as unknown[]).slice(0, MAX_POINTS).map((row, i) => {
    if (Array.isArray(row)) { const arr = row as unknown[]; return { label: arr[0] ?? i, value: arr[1] }; } // [label, value]
...
export const chartGenerateExecutor: NodeExecutor = (config, input, _context) => {
  ...
  const type = (CHART_TYPES.has(asString(config.chartType)) ? asString(config.chartType) : 'bar') as ChartType;
  ...
  if (points.length === 0) warnings.push('Nessun dato valido: generato un grafico placeholder. Controlla dataJson / labelField / valueField.');
