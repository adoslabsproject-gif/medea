...
describe('memory_note — operation: set + get', () => {
  ...
  it('🚨 SECURITY: set valore oltre 1MB → throw (anti-DoS storage)', async () => {
    ...
    await expect(
      memoryNoteExecutor({ operation: 'set', key: 'big', value: huge }, null, ctx()),
  ...
  it('🚨 SECURITY: append che supera 1MB → throw (no crescita illimitata)', async () => {
    ...
    await expect(
      memoryNoteExecutor({ operation: 'append', key: 'log', value: half }, null, ctx()),
    ).rejects.toThrow(/cap di 1000000|1 MB/);
  });

  it('set object → serialized JSON in value', async () => {
    await memoryNoteExecutor(
      { operation: 'set', key: 'k2', value: { foo: 42, bar: 'x' } },
      null,
      ctx(),
    );
...
describe('memory_note — operation: append', () => {
  ...
  it('append con separator custom', async () => {
    ...
    await memoryNoteExecutor(
      { operation: 'append', key: 'csv', value: 'b', appendSeparator: ',' },
      null,
      ctx(),
    );
    await memoryNoteExecutor(
      { operation: 'append', key: 'csv', value: 'c', appendSeparator: ',' },
      null,
      ctx(),
    );
...
describe('memory_note — TTL', () => {
  it('TTL 1s → dopo 1.1s la key scade (get → exists=false)', async () => {
    await memoryNoteExecutor(
      { operation: 'set', key: 'temp', value: 'soon-gone', ttlSeconds: 1 },
      null,
      ctx(),
    );
  ...
  it('TTL 60s → key vive (exists=true)', async () => {
    await memoryNoteExecutor(
      { operation: 'set', key: 'long', value: 'still-here', ttlSeconds: 60 },
      null,
      ctx(),
    );
...
describe('memory_note — validation errors', () => {
  it('operation invalida → throw', async () => {
    await expect(memoryNoteExecutor({ operation: 'rmrf', key: 'x' }, null, ctx())).rejects.toThrow(
      /non valida/,
    );
  });

  it('get senza key → throw', async () => {
    await expect(memoryNoteExecutor({ operation: 'get', key: '' }, null, ctx())).rejects.toThrow(
      /"key" e\\` obbligatoria/,
    );
  });

  it('set senza value → throw', async () => {
    await expect(memoryNoteExecutor({ operation: 'set', key: 'x' }, null, ctx())).rejects.toThrow(
      /"value" obbligatorio/,
    );
  });
});
...
describe('memory_note — get con valore di default', () => {
  it('🚨 key assente + default → ritorna il default, exists=false, usedDefault=true', async () => {
    const r = await memoryNoteExecutor(
      { operation: 'get', key: 'mancante', default: 'fallback' },
      null,
      ctx(),
    );
  ...
  it('key presente → usedDefault=false, ignora il default + espone expiresAt', async () => {
    await memoryNoteExecutor(
      { operation: 'set', key: 'k', value: 'reale', ttlSeconds: 3600 },
      null,
      ctx(),
    );
    const r = await memoryNoteExecutor(
      { operation: 'get', key: 'k', default: 'fallback' },
      null,
      ctx(),
    );
