...
interface TranscriptSegment { tStart: number; tEnd: number; text: string }
interface TranscriptResult { transcript: string; segments?: TranscriptSegment[]; language?: string }
interface SceneDescription { tStart: number; tEnd: number; description: string }
...
async function postJson<T>(url: string, body: unknown, timeoutMs: number, scope: { allow: InternalServiceKey }): Promise<T> {
...
export const videoSummarizerExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  const enableTranscription = cfg.enableTranscription === true
    || coerceString(cfg.enableTranscription ?? 'false') === 'true';
  ...
  const langInstruction = summaryLanguage === 'auto'
    ? `Lingua output = lingua del transcript (${transcriptRes.language ?? 'detected'}).`
    : `Lingua output = ${summaryLanguage}.`;
  const fusionSystem = 'Sei un summarizer multimodale. Rispondi SOLO con JSON valido, niente prosa attorno.';
  const fusionUser =
    `Dato:\n` +
    `TRANSCRIPT: ${transcriptRes.transcript.slice(0, 8000)}\n\n` +
    `SCENES (description per intervallo):\n${visionRes.scenes.slice(0, 50).map((s) => `[${String(s.tStart)}s-${String(s.tEnd)}s] ${s.description}`).join('\n')}\n\n` +
  ...
  try {
    ...
    const raw = (await dispatchLLMChat(
      ...
      fusionSystem, fusionUser, resolved.baseUrl, [],
      (u) => { llmUsage = u; }, undefined, { maxTokens: 800, timeoutMs: 60_000 },
    )).trim();
    // Fase 3 (#15): prompt di fusione completo + risposta → StepLog 'llm'.
    logLlmExchange(context, { provider: llmProvider, model: llmModel, system: fusionSystem, user: fusionUser, response: raw, phase: 'fusion' });
    const jsonMatch = /\{[\s\S]*\}/.exec(raw);
    if (jsonMatch) {
      const parsed = JSON.parse(jsonMatch[0]) as Partial<SummaryResult>;
      summary = {
        tldr: typeof parsed.tldr === 'string' ? parsed.tldr : '',
        bullets: Array.isArray(parsed.bullets) ? parsed.bullets.filter((x) => typeof x === 'string').slice(0, 10) : [],
        chapters: Array.isArray(parsed.chapters) ? parsed.chapters.filter((c) => c && typeof c.tStart === 'number' && typeof c.title === 'string').slice(0, 30) : [],
      };
    } else {
  ...
  return {
    output: {
      ...
      summary,
      warnings,
      durationSec: visionRes.scenes.length > 0 ? Math.max(...visionRes.scenes.map((s) => s.tEnd)) : 0,
