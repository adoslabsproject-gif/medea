...
interface MemoryRow { value: string; updated_at: number; expires_at: number | null }
...
export const memoryNoteExecutor: NodeExecutor = (rawConfig, _input, context) => {
  ...
  if (!['get', 'set', 'append', 'delete', 'list'].includes(op)) {
    return Promise.reject(new Error(`memory_note: operation "${op}" non valida (get/set/append/delete/list)`));
  }
  const key = coerceString(cfg.key ?? '').trim();
  const workflowId = context.workflowId;
  if (op !== 'list' && !key) {
    return Promise.reject(new Error('memory_note: "key" e\\` obbligatoria per get/set/append/delete'));
  }
  ...
  switch (op) {
    case 'get': {
      ...
      if (!row || isExpired(row)) {
        if (row && isExpired(row)) {
          sqlite.prepare('DELETE FROM workflow_memory WHERE workflow_id = ? AND key = ?')
            .run(workflowId, key);
        }
        ...
        const hasDefault = cfg.default !== undefined && cfg.default !== null && coerceString(cfg.default) !== '';
        return Promise.resolve({
          output: { exists: false, value: hasDefault ? coerceString(cfg.default) : null, usedDefault: hasDefault, operation: 'get' },
          durationMs: Date.now() - start,
        });
      }
      return Promise.resolve({
        output: { exists: true, value: row.value, updatedAt: row.updated_at, expiresAt: row.expires_at, usedDefault: false, operation: 'get' },
        durationMs: Date.now() - start,
      });
    }
    ...
    case 'set': {
      ...
      const prev = sqlite.prepare(
        'SELECT value, expires_at FROM workflow_memory WHERE workflow_id = ? AND key = ?'
      ...
      return Promise.resolve({
        output: { exists: true, value: v, oldValue, changed: oldValue !== v, updatedAt: now, expiresAt, operation: 'set' },
        durationMs: Date.now() - start,
      });
    }
    ...
    case 'delete': {
      const info = sqlite.prepare(
        'DELETE FROM workflow_memory WHERE workflow_id = ? AND key = ?'
    ...
    case 'list': {
      ...
      return Promise.resolve({
        output: { keys: rows.map((r) => r.key), count: rows.length, pattern: pattern || null, operation: 'list' },
        durationMs: Date.now() - start,
      });
    }
  }
  ...
  return Promise.resolve({ output: {}, durationMs: Date.now() - start } satisfies NodeExecutionResult);
};

export const __test__ = {
  resetSchema: (): void => { _schemaReady = false; },
};
