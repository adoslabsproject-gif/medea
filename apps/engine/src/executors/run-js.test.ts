...
describe('run-js executor', () => {
  ...
  it('rejecta code vuoto', async () => {
    await expect(
      runJsExecutor({ code: '' }, null, baseContext),
    ).rejects.toThrow(/code.*obbligatorio/i);
  });

  it('rejecta code > 50KB', async () => {
    await expect(
      runJsExecutor({ code: 'a'.repeat(50_001) }, null, baseContext),
    ).rejects.toThrow(/troppo lungo/i);
  });

  it('rejecta sintassi non valida', async () => {
    await expect(
      runJsExecutor({ code: 'return ((' }, null, baseContext),
    ).rejects.toThrow(/sintassi/i);
  });

  it('blocca timeout su infinite loop', async () => {
    await expect(
      runJsExecutor(
        { code: 'while(true){} return 1;', timeoutMs: 200 },
        null,
        baseContext,
      ),
  ...
  it('clamp memoryLimitMb a min 16', async () => {
    const r = await runJsExecutor(
      { code: 'return "ok";', memoryLimitMb: 1 },
      null,
      baseContext,
    );
  ...
  it('non blocca l\'event-loop durante uno script CPU-bound (async run)', async () => {
    let ticks = 0;
    const timer = setInterval(() => { ticks++; }, 15);
    try {
      const r = await runJsExecutor(
        ...
        { code: 'let s = 0; for (let i = 0; i < 5e8; i++) { s += i; } return s;', timeoutMs: 30_000 },
        null,
        baseContext,
      );
