...
function niceBounds(values: readonly number[]): { min: number; max: number } {
  ...
  if (max < 0) max = 0;
  if (min === max) { max = min + 1; } // evita divisione per 0
  return { min, max };
}

function svgHeader(width: number, height: number): string {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${sx(width)}" height="${sx(height)}" viewBox="0 0 ${sx(width)} ${sx(height)}" font-family="system-ui,Arial,sans-serif">`
...
function placeholder(opts: ChartRenderOptions): string {
  return svgHeader(opts.width, opts.height) + titleEl(opts.title, opts.width)
...
function plotArea(opts: ChartRenderOptions): { x: number; y: number; w: number; h: number } {
  return { x: PAD.left, y: PAD.top, w: Math.max(1, opts.width - PAD.left - PAD.right), h: Math.max(1, opts.height - PAD.top - PAD.bottom) };
}

function axes(a: { x: number; y: number; w: number; h: number }, min: number, max: number): string {
  const zeroY = a.y + a.h - ((0 - min) / (max - min)) * a.h;
  return `<line x1="${sx(a.x)}" y1="${sx(a.y)}" x2="${sx(a.x)}" y2="${sx(a.y + a.h)}" stroke="#d4d4d8"/>`
...
function xLabels(points: readonly ChartPoint[], a: { x: number; y: number; w: number; h: number }): string {
  const n = points.length;
  const step = a.w / n;
  // diluisce le label se troppe (max ~12 visibili) per non sovrapporle
  const every = Math.max(1, Math.ceil(n / 12));
  return points.map((p, i) => (i % every !== 0 ? '' :
    `<text x="${sx(a.x + step * (i + 0.5))}" y="${sx(a.y + a.h + 18)}" text-anchor="middle" font-size="11" fill="#52525b">${escapeXml(p.label)}</text>`)).join('');
}
...
function renderBars(points: readonly ChartPoint[], opts: ChartRenderOptions): string {
  ...
  return svgHeader(opts.width, opts.height) + titleEl(opts.title, opts.width) + axes(a, min, max) + bars + xLabels(points, a) + '</svg>';
}

function renderLine(points: readonly ChartPoint[], opts: ChartRenderOptions, fill: boolean): string {
  const a = plotArea(opts);
  const { min, max } = niceBounds(points.map((p) => p.value));
  ...
  return svgHeader(opts.width, opts.height) + titleEl(opts.title, opts.width) + axes(a, min, max) + area + line + dots + xLabels(points, a) + '</svg>';
}
...
function renderPie(points: readonly ChartPoint[], opts: ChartRenderOptions): string {
  ...
  const legend = points.map((p, i) =>
    ...
    + `<text x="${sx(opts.width - 132)}" y="${sx(PAD.top + i * 20 + 11)}" font-size="11" fill="#52525b">${escapeXml(p.label)}</text>`).join('');
  return svgHeader(opts.width, opts.height) + titleEl(opts.title, opts.width) + slices + legend + '</svg>';
}
 ...
 */
export function renderChartSvg(rawPoints: readonly { label: unknown; value: unknown }[], opts: ChartRenderOptions): string {
  const points: ChartPoint[] = rawPoints.map((p) => ({ label: toLabel(p.label), value: num(p.value) }));
  if (points.length === 0) return placeholder(opts);
  switch (opts.type) {
    case 'pie': return renderPie(points, opts);
    case 'line': return renderLine(points, opts, false);
    case 'area': return renderLine(points, opts, true);
    case 'bar':
    default: return renderBars(points, opts);
  }
}
