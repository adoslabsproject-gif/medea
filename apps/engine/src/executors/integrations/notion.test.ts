...
describe('🚨 operation validation', () => {
  it('🚨 operation missing → INVALID_PAYLOAD', async () => {
    await expect(notionExecutor({ operation: '' } as never, null, ctx())).rejects.toThrow(
...
describe('🚨 createPage', () => {
  it('🚨 parentId obbligatorio', async () => {
    await expect(notionExecutor({ operation: 'createPage' } as never, null, ctx())).rejects.toThrow(
      /parentId/u,
    );
  });
...
describe('🚨 updatePage / getPage', () => {
  it('🚨 updatePage pageId obbligatorio', async () => {
    await expect(notionExecutor({ operation: 'updatePage' } as never, null, ctx())).rejects.toThrow(
      /pageId/u,
    );
  });
  ...
  it('🚨 getPage pageId obbligatorio', async () => {
    await expect(notionExecutor({ operation: 'getPage' } as never, null, ctx())).rejects.toThrow(
      /pageId/u,
    );
  });
...
describe('🚨 queryDatabase', () => {
  ...
  it('🚨 pageSize default 50', async () => {
    safeFetchMock.mockResolvedValueOnce(
      mockRes({ results: [], has_more: false, next_cursor: null }),
    );
    await notionExecutor(
      {
...
describe('🚨 error handling + retry', () => {
  ...
  it('🚨 429 → retryable=true', async () => {
    safeFetchMock.mockResolvedValue(mockRes('Rate limit', { status: 429, ok: false }));
    try {
      await notionExecutor(
        {
          operation: 'createPage',
  ...
  it('🚨 500 → retryable=true', async () => {
    safeFetchMock.mockResolvedValue(mockRes('ISE', { status: 500, ok: false }));
    try {
      await notionExecutor(
        {
          operation: 'createPage',
