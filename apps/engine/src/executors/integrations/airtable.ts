...
export const airtableExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  if (!baseId || !tableName)
    throw new IntegrationError({
      ...
      message: 'baseId e tableName obbligatori',
    });
      ...
      switch (operation) {
        case 'listRecords': {
          ...
          do {
            ...
            const r = await jsonFetch<{ records: unknown[]; offset?: string }>(
              'airtable',
        ...
        case 'createRecord': {
          ...
          record = await jsonFetch('airtable', base, token, {
            method: 'POST',
  ...
  return {
    output: { ok: true, records, count: records.length, record, deleted },
