      ...
      switch (operation) {
        ...
        case 'createSharedLink': {
          ...
          const r = await jsonFetch<{ url?: string }>(
            'dropbox',
            `${RPC}/sharing/create_shared_link_with_settings`,
            null,
            { method: 'POST', body: { path }, headers },
          );
        ...
        default:
          throw new IntegrationError({
            provider: 'dropbox',
            code: 'INVALID_PAYLOAD',
            message: `operation "${operation}" non supportata (listFolder/createFolder/getMetadata/createSharedLink/deletePath)`,
          });
      }
    },
