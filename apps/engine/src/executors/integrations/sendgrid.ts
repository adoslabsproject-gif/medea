...
interface SendgridCreds { apiKey?: string }

function assertEmail(v: string, field: string): string {
  const e = v.trim();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)) {
    throw new IntegrationError({ provider: 'sendgrid', code: 'INVALID_PAYLOAD', message: `${field} non è un'email valida: "${v}"` });
  }
  return e;
}
...
export const sendgridExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  const integ = requireIntegration({ provider: 'sendgrid', tenantId: context.tenantId, label: getIntegrationLabel(cfg) });
  const apiKey = (integ.credentials as SendgridCreds).apiKey;
  if (!apiKey) throw new IntegrationError({ provider: 'sendgrid', code: 'INVALID_CREDENTIALS', message: 'apiKey assente nel vault' });
  ...
  if (operation === 'sendTemplate') {
    ...
    if (!templateId) throw new IntegrationError({ provider: 'sendgrid', code: 'INVALID_PAYLOAD', message: 'templateId obbligatorio per sendTemplate' });
    ...
    if (!subject) throw new IntegrationError({ provider: 'sendgrid', code: 'INVALID_PAYLOAD', message: 'subject obbligatorio per sendEmail' });
    const content = coerceString(cfg.body ?? '').trim();
    if (!content) throw new IntegrationError({ provider: 'sendgrid', code: 'INVALID_PAYLOAD', message: 'body obbligatorio per sendEmail' });
    const type = coerceString(cfg.contentType ?? 'text/plain') === 'text/html' ? 'text/html' : 'text/plain';
    body.subject = subject;
    body.content = [{ type, value: content }];
  } else {
    throw new IntegrationError({ provider: 'sendgrid', code: 'INVALID_PAYLOAD', message: `operation "${operation}" non supportata (sendEmail/sendTemplate)` });
  }

  await withRetry(async () => {
    // SendGrid /mail/send → 202 Accepted senza body. jsonFetch ritorna {} su 2xx no-content.
    await jsonFetch<unknown>(
      'sendgrid', 'https://api.sendgrid.com/v3/mail/send', null,
      { method: 'POST', body, headers },
    );
  }, { label: `sendgrid.${operation}` });

  return { output: { ok: true, operation, accepted: true, to, from }, durationMs: Date.now() - start };
};
