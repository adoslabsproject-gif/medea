...
function shopBase(domain: string): string {
  ...
  if (!/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/i.test(d)) {
    throw new IntegrationError({
      ...
      message: `shopDomain non valido: "${domain}" (atteso <store>.myshopify.com)`,
    });
  }
...
export const shopifyExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  if (!creds.shopDomain || !creds.accessToken) {
    throw new IntegrationError({
      ...
      message: 'shopDomain o accessToken assente nel vault',
    });
  }
      ...
      switch (operation) {
        case 'getOrder': {
          ...
          if (!orderId)
            throw new IntegrationError({
              provider: 'shopify',
              code: 'INVALID_PAYLOAD',
              message: 'orderId obbligatorio per getOrder',
            });
          const r = await jsonFetch<{ order?: Record<string, unknown> }>(
            'shopify',
            `${base}/orders/${encodeURIComponent(orderId)}.json`,
            null,
            { method: 'GET', headers },
          );
        ...
        case 'createOrder': {
          ...
          if (Object.keys(orderBody).length === 0)
            throw new IntegrationError({
              provider: 'shopify',
              code: 'INVALID_PAYLOAD',
              message: 'orderJson obbligatorio per createOrder',
            });
        ...
        case 'listProducts': {
          ...
          const r = await jsonFetch<{ products?: unknown[] }>(
            'shopify',
            `${base}/products.json?limit=${String(limit)}`,
            null,
            { method: 'GET', headers },
          );
        ...
        case 'createProduct': {
          ...
          if (Object.keys(productBody).length === 0)
            throw new IntegrationError({
              provider: 'shopify',
              code: 'INVALID_PAYLOAD',
              message: 'productJson obbligatorio per createProduct',
            });
        ...
        default:
          throw new IntegrationError({
            provider: 'shopify',
            code: 'INVALID_PAYLOAD',
            message: `operation "${operation}" non supportata (getOrder/createOrder/listProducts/createProduct)`,
          });
      }
    },
