...
export const typeformExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  await withRetry(
    async () => {
      switch (operation) {
        case 'listForms': {
          const r = await jsonFetch<{ items: unknown[]; total_items: number }>(
            'typeform',
        ...
        case 'getForm': {
          ...
          if (!id)
            throw new IntegrationError({
              ...
              message: 'formId obbligatorio',
            });
          form = await jsonFetch(
            'typeform',
            `https://api.typeform.com/forms/${encodeURIComponent(id)}`,
            token,
          );
          break;
        }
        case 'listResponses': {
          ...
          const r = await jsonFetch<{ items: unknown[]; total_items: number }>(
            'typeform',
            `https://api.typeform.com/forms/${encodeURIComponent(id)}/responses?${params.toString()}`,
            token,
          );
          responses = r.items;
          total = r.total_items;
        ...
        case 'getResponse': {
          ...
          if (!id || !rid)
            throw new IntegrationError({
              provider: 'typeform',
              code: 'INVALID_PAYLOAD',
              message: 'formId + responseId obbligatori',
            });
          const r = await jsonFetch<{ items: unknown[] }>(
            'typeform',
            `https://api.typeform.com/forms/${encodeURIComponent(id)}/responses?included_response_ids=${encodeURIComponent(rid)}`,
            token,
          );
          responses = r.items;
          break;
  ...
  return {
    output: { ok: true, forms, form, responses, count: responses.length, total },
