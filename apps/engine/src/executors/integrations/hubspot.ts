...
export const hubspotExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  if (!operation)
    throw new IntegrationError({
      provider: 'hubspot',
      code: 'INVALID_PAYLOAD',
      message: 'community_hubspot: "operation" obbligatorio',
    });
    ...
    async () => {
      switch (operation) {
        case 'createContact': {
          ...
          const r = await hsFetch<{ id: string; properties: Record<string, unknown> }>(
            creds.accessToken,
        ...
        case 'updateContact': {
          ...
          if (!id && email) {
            // Lookup per email
            const lookup = await hsFetch<{ id?: string }>(
              creds.accessToken,
        ...
        case 'listContacts': {
          ...
          const r = await hsFetch<{ results: unknown[] }>(
            creds.accessToken,
            `/crm/v3/objects/contacts?limit=${limit.toString()}`,
            { ...(context.abortSignal ? { signal: context.abortSignal } : {}) },
          );
          data = r.results;
