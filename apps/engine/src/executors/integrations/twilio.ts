...
interface TwilioCreds { accountSid?: string; authToken?: string; fromNumber?: string }
...
function assertE164(n: string, field: string): string {
  ...
  if (!/^\+[1-9]\d{6,14}$/.test(core)) {
    throw new IntegrationError({ provider: 'twilio', code: 'INVALID_PAYLOAD', message: `${field} non in formato E.164 (es. +393331234567): "${n}"` });
  }
  return v;
}
...
export const twilioExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  const integ = requireIntegration({ provider: 'twilio', tenantId: context.tenantId, label: getIntegrationLabel(cfg) });
  const creds = integ.credentials as TwilioCreds;
  if (!creds.accountSid || !creds.authToken) {
    throw new IntegrationError({ provider: 'twilio', code: 'INVALID_CREDENTIALS', message: 'accountSid o authToken assente nel vault' });
  }
  if (!/^AC[0-9a-f]{32}$/i.test(creds.accountSid)) {
    throw new IntegrationError({ provider: 'twilio', code: 'INVALID_CREDENTIALS', message: 'accountSid non valido (atteso "AC" + 32 hex)' });
  }
  ...
  await withRetry(async () => {
    switch (operation) {
      ...
      case 'sendWhatsapp': {
        ...
        if (!body) throw new IntegrationError({ provider: 'twilio', code: 'INVALID_PAYLOAD', message: 'body (testo messaggio) obbligatorio' });
        ...
        if (!rawFrom) throw new IntegrationError({ provider: 'twilio', code: 'INVALID_PAYLOAD', message: 'fromNumber obbligatorio (config o vault)' });
        if (!rawTo) throw new IntegrationError({ provider: 'twilio', code: 'INVALID_PAYLOAD', message: 'to (destinatario) obbligatorio' });
        const from = isWa ? `whatsapp:${assertE164(rawFrom, 'fromNumber').replace(/^whatsapp:/, '')}` : assertE164(rawFrom, 'fromNumber');
        const to = isWa ? `whatsapp:${assertE164(rawTo, 'to').replace(/^whatsapp:/, '')}` : assertE164(rawTo, 'to');
        const form = new URLSearchParams({ To: to, From: from, Body: body }).toString();
        const r = await jsonFetch<{ sid?: string; status?: string }>(
          'twilio', `${base}/Messages.json`, null,
      ...
      case 'getMessage': {
        ...
        if (!sid) throw new IntegrationError({ provider: 'twilio', code: 'INVALID_PAYLOAD', message: 'messageSid obbligatorio per getMessage' });
        const r = await jsonFetch<{ sid?: string; status?: string; error_code?: number }>(
          'twilio', `${base}/Messages/${encodeURIComponent(sid)}.json`, null,
          { method: 'GET', headers },
        );
        output = { messageSid: r.sid ?? null, status: r.status ?? null, errorCode: r.error_code ?? null };
        break;
      }
      default:
        throw new IntegrationError({ provider: 'twilio', code: 'INVALID_PAYLOAD', message: `operation "${operation}" non supportata (sendSms/sendWhatsapp/getMessage)` });
    }
