...
vi.mock('@/services/integrations/store.js', () => ({
  getIntegration: vi.fn().mockReturnValue({
    provider: 'slack', tenantId: 't', label: null,
...
const ctx = {
  ...
  defId: 'integration_slack_post', secrets: {}, llmProviders: [], nodeOutputs: {},
...
describe('slackExecutor', () => {
  ...
  it('postMessage successo → output.ts', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({
      ok: true, ts: '1717599999.123456', channel: 'C123',
  ...
  it('blocks JSON parsed e passato a Slack', async () => {
    ...
    const body = JSON.parse((fetchMock.mock.calls[0]![1] as { body: string }).body) as { blocks: unknown[] };
    expect(body.blocks).toEqual([{ type: 'section', text: { type: 'mrkdwn', text: '*x*' } }]);
  });

  it('threadTs → thread_ts nel payload', async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({ ok: true, ts: '2.0' }), { status: 200 }));
    globalThis.fetch = fetchMock;
    await slackExecutor({ channel: 'C1', text: 't', threadTs: '1.0' }, null, ctx);
    const body = JSON.parse((fetchMock.mock.calls[0]![1] as { body: string }).body) as { thread_ts: string };
  ...
  it('Slack error ok:false → throw con error code', async () => {
    ...
    await expect(slackExecutor({ channel: 'C-no-exists', text: 't' }, null, ctx))
      .rejects.toThrow(/channel_not_found/);
  });

  it('channel vuoto → throw INVALID_PAYLOAD', async () => {
    await expect(slackExecutor({ channel: '', text: 't' }, null, ctx))
      .rejects.toThrow(/"channel" e\\` obbligatorio/);
  });

  it('text vuoto → throw INVALID_PAYLOAD', async () => {
    await expect(slackExecutor({ channel: 'C1', text: '' }, null, ctx))
      .rejects.toThrow(/"text" e\\` obbligatorio/);
  });

  it('blocks JSON malformato → throw', async () => {
    await expect(slackExecutor({ channel: 'C1', text: 'fb', blocks: 'not-json' }, null, ctx))
  ...
  it('HTTP 500 → throw API_HTTP_ERROR dopo retry', async () => {
    ...
    await expect(slackExecutor({ channel: 'C1', text: 't' }, null, ctx))
      .rejects.toThrow(/Slack HTTP 500/);
