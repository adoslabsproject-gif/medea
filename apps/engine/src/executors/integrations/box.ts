...
function reqId(v: unknown, field: string): string {
  ...
  if (!s)
    throw new IntegrationError({
      ...
      message: `${field} obbligatorio`,
    });
  if (!/^\d+$/.test(s))
    throw new IntegrationError({
      provider: 'box',
      code: 'INVALID_PAYLOAD',
      message: `${field} deve essere un ID numerico Box: "${s}"`,
    });
  return s;
}
...
export const boxExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  if (!token)
    throw new IntegrationError({
      ...
      message: 'accessToken assente nel vault',
    });
    ...
    async () => {
      switch (operation) {
        case 'listFolder': {
          ...
          if (!/^\d+$/.test(folderId))
            throw new IntegrationError({
              provider: 'box',
              code: 'INVALID_PAYLOAD',
              message: 'folderId deve essere numerico (0 = root)',
            });
          ...
          const r = await jsonFetch<{ entries?: Record<string, unknown>[]; total_count?: number }>(
            'box',
            `${BASE}/folders/${folderId}/items?limit=${String(limit)}`,
            null,
            { method: 'GET', headers },
          );
        ...
        case 'createFolder': {
          ...
          if (!name)
            throw new IntegrationError({
              provider: 'box',
              code: 'INVALID_PAYLOAD',
              message: 'name (nome cartella) obbligatorio',
            });
        ...
        case 'createSharedLink': {
          ...
          const access = VALID_ACCESS.has(coerceString(cfg.access ?? 'open'))
            ? coerceString(cfg.access ?? 'open')
            : 'open';
          const r = await jsonFetch<{ shared_link?: { url?: string; download_url?: string } }>(
            'box',
            `${BASE}/files/${fileId}?fields=shared_link`,
            null,
            { method: 'PUT', body: { shared_link: { access } }, headers },
          );
          output = {
            sharedUrl: r.shared_link?.url ?? null,
            downloadUrl: r.shared_link?.download_url ?? null,
          };
          break;
        }
        case 'deleteFile': {
          const fileId = reqId(cfg.itemId, 'itemId');
          await jsonFetch<unknown>('box', `${BASE}/files/${fileId}`, null, {
            method: 'DELETE',
            headers,
          });
        ...
        default:
          throw new IntegrationError({
            provider: 'box',
            code: 'INVALID_PAYLOAD',
            message: `operation "${operation}" non supportata (listFolder/createFolder/getItem/createSharedLink/deleteFile)`,
          });
      }
    },
    { label: `box.${operation}` },
