...
export const telegramExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  if (!chatId)
    throw new IntegrationError({
      ...
      message: 'integration_telegram_send: "chatId" e\\` obbligatorio',
    });
    ...
    async () => {
      ...
      const body =
        await readJsonCapped<
          TgResponse<{ message_id: number; chat: { id: number }; date: number }>
        >(res);
      if (!body.ok) {
        throw new IntegrationError({
          provider: 'telegram',
          ...
          retryable:
            body.error_code === 429 || (body.error_code !== undefined && body.error_code >= 500),
        });
      }
      return body.result;
    },
