...
export const discordExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  if (!body.content && !body.embeds) {
    throw new IntegrationError({
      ...
      message: 'content o embedJson richiesto',
    });
  }
      ...
      if (mode === 'webhook') {
        ...
        if (!channel)
          throw new IntegrationError({
            provider: 'discord',
            code: 'INVALID_PAYLOAD',
            message: 'channelId obbligatorio per mode=bot',
          });
        const r = await jsonFetch<{ id?: string; channel_id?: string }>(
          'discord',
        ...
        messageId = r.id ?? null;
        channelId = r.channel_id ?? channel;
      }
    },
    { label: 'discord.send' },
  );
  ...
  return {
    output: { ok: true, messageId, channelId, sentAt: new Date().toISOString() },
