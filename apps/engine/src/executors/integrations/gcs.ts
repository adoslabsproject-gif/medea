...
interface GcsCreds { accessToken?: string }
...
function assertBucket(v: string): string {
  ...
  if (!/^[a-z0-9][a-z0-9._-]{1,61}[a-z0-9]$/.test(b)) {
    throw new IntegrationError({ provider: 'gcs', code: 'INVALID_PAYLOAD', message: `bucket non valido (3-63 char, lowercase): "${v}"` });
  }
  return b;
}
...
export const gcsExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  const integ = requireIntegration({ provider: 'gcs', tenantId: context.tenantId, label: getIntegrationLabel(cfg) });
  const token = (integ.credentials as GcsCreds).accessToken;
  if (!token) throw new IntegrationError({ provider: 'gcs', code: 'INVALID_CREDENTIALS', message: 'accessToken (OAuth2) assente nel vault' });
    ...
    switch (operation) {
      case 'listObjects': {
        ...
        const r = await jsonFetch<{ items?: Record<string, unknown>[]; nextPageToken?: string }>(
          'gcs', `${BASE}/b/${bucket}/o?${qs.toString()}`, null, { method: 'GET', headers },
        );
      ...
      case 'getObjectMetadata': {
        ...
        if (!objectName) throw new IntegrationError({ provider: 'gcs', code: 'INVALID_PAYLOAD', message: 'objectName obbligatorio' });
        const r = await jsonFetch<Record<string, unknown>>(
          'gcs', `${BASE}/b/${bucket}/o/${encodeURIComponent(objectName)}`, null, { method: 'GET', headers },
        );
      ...
      case 'deleteObject': {
        ...
        await jsonFetch<unknown>(
          'gcs', `${BASE}/b/${bucket}/o/${encodeURIComponent(objectName)}`, null, { method: 'DELETE', headers },
        );
        output = { deleted: true, objectName };
        break;
      }
      default:
        throw new IntegrationError({ provider: 'gcs', code: 'INVALID_PAYLOAD', message: `operation "${operation}" non supportata (listObjects/getObjectMetadata/deleteObject)` });
    }
  }, { label: `gcs.${operation}` });
