...
interface MailchimpCreds { apiKey?: string }

function datacenter(apiKey: string): string {
  const dc = apiKey.split('-').pop() ?? '';
  if (!/^[a-z]{2}\d{1,3}$/i.test(dc)) {
    throw new IntegrationError({ provider: 'mailchimp', code: 'INVALID_CREDENTIALS', message: 'apiKey senza datacenter valido (atteso suffisso tipo "-us21")' });
  }
  return dc;
}
...
export const mailchimpExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  if (!listId) throw new IntegrationError({ provider: 'mailchimp', code: 'INVALID_PAYLOAD', message: 'listId (Audience ID) obbligatorio' });

  const integ = requireIntegration({ provider: 'mailchimp', tenantId: context.tenantId, label: getIntegrationLabel(cfg) });
  const apiKey = (integ.credentials as MailchimpCreds).apiKey;
  if (!apiKey) throw new IntegrationError({ provider: 'mailchimp', code: 'INVALID_CREDENTIALS', message: 'apiKey assente nel vault' });
  ...
  await withRetry(async () => {
    switch (operation) {
      case 'addMember': {
        ...
        if (!email) throw new IntegrationError({ provider: 'mailchimp', code: 'INVALID_PAYLOAD', message: 'email obbligatoria per addMember' });
        ...
        const r = await jsonFetch<{ id?: string; status?: string }>(
          'mailchimp', `${base}/lists/${listId}/members/${subscriberHash(email)}`, null,
          { method: 'PUT', body: { email_address: email, status_if_new: statusVal, status: statusVal, merge_fields: mergeFields }, headers },
        );
      ...
      case 'getMember': {
        const email = coerceString(cfg.email ?? '').trim();
        if (!email) throw new IntegrationError({ provider: 'mailchimp', code: 'INVALID_PAYLOAD', message: 'email obbligatoria per getMember' });
      ...
      case 'addTag': {
        ...
        if (!email || !tag) throw new IntegrationError({ provider: 'mailchimp', code: 'INVALID_PAYLOAD', message: 'email e tag obbligatori per addTag' });
        await jsonFetch<unknown>(
          'mailchimp', `${base}/lists/${listId}/members/${subscriberHash(email)}/tags`, null,
      ...
      default:
        throw new IntegrationError({ provider: 'mailchimp', code: 'INVALID_PAYLOAD', message: `operation "${operation}" non supportata (addMember/getMember/addTag)` });
    }
