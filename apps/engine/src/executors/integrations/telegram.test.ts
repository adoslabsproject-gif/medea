...
vi.mock('@/services/integrations/store.js', () => ({
  getIntegration: vi.fn().mockReturnValue({
    provider: 'telegram', tenantId: 't', label: null,
...
const ctx = {
  ...
  defId: 'integration_telegram_send', secrets: {}, llmProviders: [], nodeOutputs: {},
...
describe('telegramExecutor', () => {
  ...
  it('sendMessage text → ok + messageId', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({
      ok: true, result: { message_id: 42, chat: { id: -1001234567890 }, date: 1717599999 },
    }), { status: 200 }));
  ...
  it('sendPhoto se photoUrl valorizzato', async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({
      ok: true, result: { message_id: 43, chat: { id: 100 }, date: 0 },
    }), { status: 200 }));
    globalThis.fetch = fetchMock;
    const r = await telegramExecutor({
      chatId: '100', text: 'Caption', photoUrl: 'https://example.com/img.png',
    }, null, ctx);
  ...
  it('parseMode invalido → throw INVALID_PAYLOAD', async () => {
    await expect(telegramExecutor({
      chatId: '1', text: 't', parseMode: 'WrongMode',
    }, null, ctx)).rejects.toThrow(/parseMode.*non supportato/);
  });

  it('chatId vuoto → throw', async () => {
    await expect(telegramExecutor({ chatId: '', text: 't' }, null, ctx))
      .rejects.toThrow(/"chatId" e\\` obbligatorio/);
  });

  it('no text + no photoUrl → throw', async () => {
    await expect(telegramExecutor({ chatId: '1' }, null, ctx))
      .rejects.toThrow(/serve almeno uno tra "text" e "photoUrl"/);
  });
  ...
  it('Telegram API error → throw con descrizione', async () => {
    ...
    await expect(telegramExecutor({ chatId: 'invalid', text: 't' }, null, ctx))
  ...
  it('disableNotification → payload include disable_notification', async () => {
    ...
    const body = JSON.parse((fetchMock.mock.calls[0]![1] as { body: string }).body) as { disable_notification: boolean };
