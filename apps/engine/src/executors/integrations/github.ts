...
async function ghFetch<T>(
  ...
  path: string,
  opts: FetchOpts = {},
): Promise<{ data: T; rateLimit: number | null }> {
    ...
    async () => {
      switch (operation) {
        case 'createIssue': {
          ...
          if (assignees)
            body.assignees = assignees
              ...
              .map((s) => s.trim())
              .filter(Boolean);
          const r = await ghFetch<Record<string, unknown>>(
            creds.token,
            `/repos/${ownerEnc}/${repoEnc}/issues`,
            {
              method: 'POST',
              body,
        ...
        case 'listIssues': {
          ...
          const r = await ghFetch<unknown[]>(
            creds.token,
            `/repos/${ownerEnc}/${repoEnc}/issues?state=${encodeURIComponent(state)}&per_page=${perPage.toString()}`,
        ...
        case 'getIssue': {
          ...
          const r = await ghFetch<Record<string, unknown>>(
            creds.token,
            `/repos/${ownerEnc}/${repoEnc}/issues/${encodeURIComponent(issueNumber)}`,
            { ...(context.abortSignal ? { signal: context.abortSignal } : {}) },
          );
          data = r.data;
          rateLimit = r.rateLimit;
        ...
        case 'addComment': {
          ...
          const r = await ghFetch<Record<string, unknown>>(
            creds.token,
            `/repos/${ownerEnc}/${repoEnc}/issues/${encodeURIComponent(issueNumber)}/comments`,
            {
              method: 'POST',
              body: { body },
              signal: context.abortSignal,
        ...
        case 'listCommits': {
          ...
          const r = await ghFetch<unknown[]>(
            creds.token,
            `/repos/${ownerEnc}/${repoEnc}/commits?per_page=${perPage.toString()}`,
            { ...(context.abortSignal ? { signal: context.abortSignal } : {}) },
          );
          data = r.data;
          count = r.data.length;
