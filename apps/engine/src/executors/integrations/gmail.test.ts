...
const ctx = { tenantId: 'ten-1', runId: 'run-1', nodeId: 'node-1' } as unknown as Parameters<typeof gmailExecutor>[2];

/** Ultima chiamata jsonFetch che matcha un pezzo di URL. */
function callFor(urlPart: string): { url: string; token: string | null; opts: { method?: string; body?: { raw?: string } } } {
  ...
  return { url: String(hit[1]), token: hit[2] as string | null, opts: (hit[3] ?? {}) as { method?: string; body?: { raw?: string } } };
}
...
beforeEach(() => {
  ...
  m.requireIntegration.mockReturnValue({
    ...
    provider: 'gmail',
    tenantId: 'ten-1',
    credentials: { accessToken: 'stale', refreshToken: 'refresh', scope: 'gmail.send', userEmail: 'me@gmail.com' },
    expiresAt: Date.now() + 3600_000,
  });
  m.buildOAuthClient.mockResolvedValue({ clientId: 'cid', clientSecret: 'sec', defaultRedirectUri: undefined });
...
describe('🚨 gmail send — costruzione MIME + auth', () => {
  ...
  it('🚨 subject non-ASCII → MIME encoded-word (=?UTF-8?B?…?=)', async () => {
    ...
    await gmailExecutor({ operation: 'send', to: 'd@x.it', subject: 'Però è così — €', bodyText: 'x' }, null, ctx);
  ...
  it('text + html → multipart/alternative', async () => {
    m.jsonFetch.mockResolvedValueOnce({ id: 'm1', threadId: 't1' });
    await gmailExecutor({ operation: 'send', to: 'd@x.it', subject: 's', bodyText: 'plain', bodyHtml: '<b>rich</b>' }, null, ctx);
    const raw = decodeRaw(callFor('/messages/send').opts.body!.raw!);
  ...
  it('con allegato → multipart/mixed + Content-Disposition attachment', async () => {
    ...
    await gmailExecutor({
      operation: 'send', to: 'd@x.it', subject: 's', bodyText: 'body',
      attachmentsJson: JSON.stringify([{ filename: 'report.pdf', content: b64, mimeType: 'application/pdf' }]),
    }, null, ctx);
    const raw = decodeRaw(callFor('/messages/send').opts.body!.raw!);
  ...
  it('accetta allegati anche in forma { name, base64, contentType } e come array già parsato', async () => {
    ...
    await gmailExecutor({
      operation: 'send', to: 'd@x.it', subject: 's', bodyText: 'b',
  ...
  it('send senza "to" → INVALID_PAYLOAD, nessuna chiamata send', async () => {
    await expect(gmailExecutor({ operation: 'send', bodyText: 'x' }, null, ctx)).rejects.toThrow(/destinatario/i);
  });

  it('send senza corpo → INVALID_PAYLOAD', async () => {
    await expect(gmailExecutor({ operation: 'send', to: 'd@x.it' }, null, ctx)).rejects.toThrow(/bodyText.*bodyHtml/);
  });
});
...
describe('gmail list — arricchimento metadata', () => {
  it('lista + recupera from/subject/date/snippet per ogni messaggio', async () => {
    m.jsonFetch
      ...
      .mockResolvedValueOnce({ id: 'a', threadId: 'ta', snippet: 'anteprima', labelIds: ['INBOX'], payload: { headers: [
        { name: 'From', value: 'mittente@x.it' }, { name: 'Subject', value: 'Oggetto X' }, { name: 'Date', value: 'Wed, 3 Jul 2026' },
      ] } });
    const r = await gmailExecutor({ operation: 'list', query: 'is:unread', maxResults: 5 }, null, ctx);
    const out = r.output as { count: number; messages: { from: string; subject: string; snippet: string }[] };
    expect(out.count).toBe(1);
    expect(out.messages[0]).toMatchObject({ from: 'mittente@x.it', subject: 'Oggetto X', snippet: 'anteprima' });
...
describe('gmail get — parsing corpo + allegati', () => {
  it('estrae bodyText, bodyHtml e allegati da payload multipart', async () => {
    m.jsonFetch.mockResolvedValueOnce({
      id: 'g1', threadId: 'tg', snippet: 's', labelIds: ['INBOX'],
      payload: {
        headers: [{ name: 'From', value: 'a@b.it' }, { name: 'Subject', value: 'Sub' }],
        parts: [
          { mimeType: 'text/plain', body: { data: Buffer.from('ciao testo', 'utf8').toString('base64url') } },
          { mimeType: 'text/html', body: { data: Buffer.from('<p>ciao</p>', 'utf8').toString('base64url') } },
          { mimeType: 'application/pdf', filename: 'f.pdf', body: { attachmentId: 'att1', size: 1234 } },
        ],
      },
    });
    const r = await gmailExecutor({ operation: 'get', messageId: 'g1' }, null, ctx);
    const out = r.output as { bodyText: string; bodyHtml: string; subject: string; attachments: { filename: string; size: number }[] };
    expect(out.bodyText).toBe('ciao testo');
    expect(out.bodyHtml).toBe('<p>ciao</p>');
    expect(out.subject).toBe('Sub');
    expect(out.attachments).toEqual([{ filename: 'f.pdf', mimeType: 'application/pdf', attachmentId: 'att1', size: 1234 }]);
  });
...
describe('gmail — auth e operazioni', () => {
  it('🚨 credenziali senza refreshToken → INVALID_CREDENTIALS (no chiamata)', async () => {
    m.requireIntegration.mockReturnValueOnce({
      id: 'int-1', provider: 'gmail', tenantId: 'ten-1',
      credentials: { accessToken: 'x', scope: 'gmail.send' }, expiresAt: null,
    });
    await expect(gmailExecutor({ operation: 'send', to: 'd@x.it', bodyText: 'b' }, null, ctx)).rejects.toThrow(/Riconnetti Google/);
    expect(m.jsonFetch).not.toHaveBeenCalled();
  });

  it('operazione ignota → INVALID_PAYLOAD', async () => {
    await expect(gmailExecutor({ operation: 'archive' }, null, ctx)).rejects.toThrow(/non supportata/);
  });
});
