...
export const googleSheetsExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  if (!spreadsheetId)
    throw new IntegrationError({
      ...
      message: 'spreadsheetId obbligatorio',
    });
    ...
    async () => {
      switch (operation) {
        case 'getValues': {
          if (!range)
            throw new IntegrationError({
              provider: 'google_sheets',
              code: 'INVALID_PAYLOAD',
              message: 'range obbligatorio',
            });
          const r = await jsonFetch<{ values?: unknown[][]; range: string }>(
            'google_sheets',
        ...
        case 'appendValues': {
          ...
          data = r;
          updatedCells = r.updates?.updatedCells ?? 0;
          break;
        }
        case 'batchGet': {
          const ranges = range
  ...
  return {
    output: { ok: true, data, rows, count: rows.length, updatedCells, spreadsheetId },
