...
describe('linearExecutor', () => {
  ...
  it('team non trovato → throw TEAM_NOT_FOUND', async () => {
    ...
    await expect(linearExecutor({ teamKey: 'XYZ', title: 'Bug' }, null, ctx)).rejects.toThrow(
      /team "XYZ" non trovato/,
    );
  });

  it('teamKey vuoto → throw', async () => {
    await expect(linearExecutor({ teamKey: '', title: 'Bug' }, null, ctx)).rejects.toThrow(
      /"teamKey" obbligatorio/,
    );
  });

  it('title vuoto → throw', async () => {
    await expect(linearExecutor({ teamKey: 'ENG', title: '' }, null, ctx)).rejects.toThrow(
      /"title" obbligatorio/,
    );
  });
  ...
  it('assignee non trovato → warning ma issue creata', async () => {
    globalThis.fetch = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ data: { users: { nodes: [] } } }), { status: 200 }),
      )
      .mockResolvedValueOnce(new Response(JSON.stringify(issueCreateResp), { status: 200 }));
  ...
  it('labels comma → resolve a id', async () => {
    ...
    await linearExecutor(
      { teamKey: 'ENG', title: 'Bug', labelNames: 'bug, production' },
      null,
      ctx,
    );
    // 3a fetch (issueCreate) deve avere labelIds nel variables
    const lastCall = JSON.parse((fetchMock.mock.calls[2]![1] as { body: string }).body) as {
  ...
  it('GraphQL errors → throw', async () => {
    ...
    await expect(linearExecutor({ teamKey: 'ENG', title: 'Bug' }, null, ctx)).rejects.toThrow(
      /Invalid token/,
    );
  });

  it('apiKey non valido (no prefix) → throw INVALID_CREDENTIALS', async () => {
    const mod = (await import('@/services/integrations/store.js')) as unknown as {
