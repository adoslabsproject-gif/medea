...
export const notionExecutor: NodeExecutor = async (rawConfig, _input, context) => {
  ...
  if (!operation)
    throw new IntegrationError({
      provider: 'notion',
      code: 'INVALID_PAYLOAD',
      message: 'community_notion: "operation" obbligatorio',
    });
        ...
        case 'createPage': {
          ...
          if (!parentId)
            throw new IntegrationError({
              provider: 'notion',
              code: 'INVALID_PAYLOAD',
              message: 'community_notion: createPage richiede parentId',
            });
          const parentType = coerceString(cfg.parentType ?? 'database_id');
          const parent =
            parentType === 'page_id' ? { page_id: parentId } : { database_id: parentId };
          const r = await notionFetch<{ id: string; properties: unknown }>(
            creds.integrationToken,
            '/pages',
            {
              method: 'POST',
        ...
        case 'updatePage': {
          ...
          if (!id)
            throw new IntegrationError({
              provider: 'notion',
              code: 'INVALID_PAYLOAD',
              message: 'community_notion: updatePage richiede pageId',
            });
          const r = await notionFetch<{ id: string; properties: unknown }>(
            creds.integrationToken,
        ...
        case 'getPage': {
          ...
          if (!id)
            throw new IntegrationError({
              provider: 'notion',
              code: 'INVALID_PAYLOAD',
              message: 'community_notion: getPage richiede pageId',
            });
          const r = await notionFetch<{ id: string; properties: unknown }>(
            creds.integrationToken,
            `/pages/${encodeURIComponent(id)}`,
        ...
        case 'queryDatabase': {
          ...
          if (!id)
            throw new IntegrationError({
              provider: 'notion',
              code: 'INVALID_PAYLOAD',
              message: 'community_notion: queryDatabase richiede databaseId',
            });
