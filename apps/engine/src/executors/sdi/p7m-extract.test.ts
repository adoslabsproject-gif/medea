...
const P7M_B64 = readFileSync(
  join(__dirname, '__fixtures__', 'fattura-test.p7m.b64'),
  'utf8',
).trim();
...
describe('p7mExtractExecutor', () => {
  ...
  it('🚨 contenuto né XML né p7m → errore chiaro', async () => {
    await expect(run({ content: 'ciao questo è testo qualunque non un p7m' })).rejects.toThrow(
      /non è né XML né un p7m/u,
    );
  });
      ...
      attachments: [
        {
          ...
          binary: { __ffBinary: true as const, encoding: 'ref', ref: 'txt-ref' },
        },
        {
          filename: 'IT01234567890_00042.xml.p7m',
          binary: { __ffBinary: true as const, encoding: 'ref', ref: 'p7m-ref' },
        },
      ],
    };
  ...
  it('attachments[] con un solo allegato non-.p7m → usa comunque il primo (fallback)', async () => {
    const input = {
      attachments: [
        {
          filename: 'fattura.xml.base64',
          binary: { __ffBinary: true as const, encoding: 'base64', data: P7M_B64 },
        },
      ],
    };
    const out = await run({}, input);
