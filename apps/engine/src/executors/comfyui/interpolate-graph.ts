...
function typedValue(raw: string): string | number | boolean {
  ...
  if (/^-?\d+(\.\d+)?([eE][+-]?\d+)?$/.test(raw.trim()) && raw.trim() !== '')
...
export function substituteGraphVariables(
  ...
  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new ComfyError(
      'Grafo non valido',
      'Atteso un oggetto { "<id>": { "class_type": ... } } (formato API di ComfyUI).',
    );
  }
  ...
  if (
    ...
    throw new ComfyError(
      'Grafo non valido',
      'Ogni nodo deve avere un "class_type" stringa (formato API di ComfyUI).',
    );
  }
