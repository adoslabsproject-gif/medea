...
function collectFrom(node: unknown, kindHint: 'image' | 'video' | null): ComfyMedia[] {
  ...
  for (const [key, val] of Object.entries(node as Record<string, unknown>)) {
    ...
    const keyKind: 'image' | 'video' | null =
      key === 'gifs' || key === 'videos' ? 'video' : key === 'images' ? 'image' : kindHint;
  ...
  if (id) {
    if (!(id in outputs)) {
      throw new ComfyError(
        'Nodo di output non trovato',
        `id "${id}" assente negli output (${Object.keys(outputs).join(', ') || 'nessuno'}).`,
      );
    }
...
export function firstMedia(outputs: Record<string, unknown>, outputNodeId?: string): ComfyMedia {
  ...
  if (media.length === 0) {
    throw new ComfyError(
      'Nessun media prodotto',
      'Il grafo è stato eseguito ma non ha generato immagini/video scaricabili (manca un nodo SaveImage/Save Video?).',
    );
  }
  return media[0]!;
}
