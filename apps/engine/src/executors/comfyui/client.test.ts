...
function res(opts: { ok?: boolean; status?: number; body?: unknown; text?: string; bytes?: Buffer; contentType?: string }): Response {
...
describe('ComfyClient.submitPrompt', () => {
  ...
  it('rigetta se node_errors non vuoto', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(res({ body: { prompt_id: 'a', node_errors: { '3': 'bad' } } })));
    await expect(new ComfyClient('http://h:8188').submitPrompt({}, 'c')).rejects.toThrow(/node_errors/);
  });

  it('rigetta su HTTP non ok col motivo', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(res({ ok: false, status: 400, text: 'nope' })));
    await expect(new ComfyClient('http://h:8188').submitPrompt({}, 'c')).rejects.toThrow(/rifiutato il grafo/);
  });

  it('rigetta su errore di rete con messaggio "non raggiungibile"', async () => {
    vi.stubGlobal('fetch', vi.fn().mockRejectedValue(new Error('ECONNREFUSED')));
    await expect(new ComfyClient('http://h:8188').submitPrompt({}, 'c')).rejects.toThrow(/non raggiungibile/);
  });

  it('rigetta se manca prompt_id', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(res({ body: {} })));
    await expect(new ComfyClient('http://h:8188').submitPrompt({}, 'c')).rejects.toThrow(/prompt_id/);
  });
});
...
describe('ComfyClient.waitForOutputs (polling)', () => {
  ...
  it('ritorna gli outputs al primo poll utile', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(res({ body: { p1: { outputs: { '9': { images: [{ filename: 'a.png' }] } } } } })));
    const c = new ComfyClient('http://h:8188');
  ...
  it('rigetta se lo status del backend è error', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(res({ body: { p1: { status: { status_str: 'error', messages: ['boom'] } } } })));
    const c = new ComfyClient('http://h:8188');
    const promise = c.waitForOutputs('p1', 5000);
...
describe('ComfyClient.fetchMedia', () => {
  ...
  it('scarica i byte e usa il content-type dell\'header', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(res({ bytes: Buffer.from('PNG'), contentType: 'image/png' })));
    const m = await new ComfyClient('http://h:8188').fetchMedia({ filename: 'a.png', subfolder: '', type: 'output', kind: 'image' });
  ...
  it('fallback mime dall\'estensione se manca l\'header', async () => {
    ...
    const m = await new ComfyClient('http://h:8188').fetchMedia({ filename: 'clip.mp4', subfolder: '', type: 'output', kind: 'video' });
  ...
  it('rigetta su HTTP non ok', async () => {
    ...
    await expect(new ComfyClient('http://h:8188').fetchMedia({ filename: 'a.png', subfolder: '', type: 'output', kind: 'image' })).rejects.toThrow(/Download/);
  });
});
