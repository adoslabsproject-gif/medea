...
const HOSTNAME_BLOCKLIST =
  /^(localhost|.*\.local|.*\.internal|.*\.lan|.*\.localdomain|host\.docker\.internal|metadata\.google\.internal|host\.containers\.internal)$/i;
...
async function assertPublicDomain(domain: string): Promise<string[]> {
  if (HOSTNAME_BLOCKLIST.test(domain)) {
    throw new Error(
      `action_security_audit: dominio "${domain}" non auditabile (hostname interno/loopback bloccato per SSRF safety).`,
    );
  }
  let ips: string[];
  try {
    ...
    throw new Error(`action_security_audit: dominio "${domain}" non risolvibile (no A record)`);
  }
  if (ips.length === 0)
    throw new Error(`action_security_audit: dominio "${domain}" risolve a 0 A record.`);
  for (const ip of ips) {
    if (isPrivateOrReserved(`${ip}/32`)) {
      throw new Error(
        `action_security_audit: dominio "${domain}" risolve a IP privato/reserved (${ip}) — SSRF bloccato.`,
      );
    }
  }
  return ips;
  ...
  try {
    const records = await Promise.race([
      ...
      new Promise<never>((_, rej) =>
        signal.addEventListener('abort', () => rej(new Error('timeout'))),
      ),
    ]);
...
async function inspectSsl(
  ...
  return await new Promise<SslResult | null>((resolve) => {
    let settled = false;
    const finish = (v: SslResult | null): void => {
      if (!settled) {
        settled = true;
        resolve(v);
      }
      ...
      const sock: TLSSocket = tlsConnect(
        ...
        () => {
          ...
          const san = sanRaw
            ...
            .filter(Boolean);
          const sanMatches = san.some(
            (d) => d === domain || (d.startsWith('*.') && domain.endsWith(d.slice(1))),
          );
          sock.destroy();
...
function buildFindings(input: {
  ...
  if (!input.spf.found) {
    f.push({
      severity: 'high',
      framework: 'spf',
      title: 'SPF record assente',
      remediation: `Aggiungi TXT a ${input.domain}: "v=spf1 include:_spf.tuoprovider.it ~all"`,
    });
  } else if (input.spf.allMode === '+all') {
    f.push({
      severity: 'critical',
      framework: 'spf',
      title: 'SPF +all = permissivo (spoofing aperto)',
      remediation: 'Cambia "+all" in "-all" (strict) o "~all" (softfail)',
    });
  } else if (input.spf.allMode === '?all') {
    f.push({
      severity: 'medium',
      framework: 'spf',
      title: 'SPF ?all = neutral (anti-spoofing debole)',
      remediation: 'Cambia in "-all" o "~all"',
    });
  }
  ...
  if (input.dkim.selectorsTried.length > 0 && input.dkim.selectorsFound.length === 0) {
    f.push({
      severity: 'high',
      framework: 'dkim',
      title: 'DKIM nessun selettore valido trovato',
      remediation: `Configura DKIM dal tuo email provider e pubblica TXT <selector>._domainkey.${input.domain}`,
    });
  }
  ...
  if (!input.dmarc.found) {
    f.push({
      severity: 'high',
      framework: 'dmarc',
      title: 'DMARC record assente',
      remediation: `Aggiungi TXT _dmarc.${input.domain}: "v=DMARC1; p=quarantine; rua=mailto:dmarc@${input.domain}; pct=100"`,
    });
  } else {
    if (input.dmarc.policy === 'none') {
      f.push({
        ...
        remediation: 'Dopo 30gg di monitoring sano passa a p=quarantine, poi p=reject',
      });
    }
    if (input.dmarc.pct !== undefined && input.dmarc.pct < 100) {
      f.push({
        severity: 'low',
        framework: 'dmarc',
        title: `DMARC pct=${String(input.dmarc.pct)} (parziale)`,
        remediation: 'Quando il monitoring è clean, alza pct=100',
      });
    }
    if (!input.dmarc.rua) {
      f.push({
        severity: 'low',
        framework: 'dmarc',
        title: 'DMARC rua= mancante (no aggregate report)',
        remediation: `Aggiungi rua=mailto:dmarc@${input.domain}`,
      });
    }
  }
  ...
  if (input.sslChecked) {
    if (!input.ssl) {
      f.push({
        severity: 'critical',
        framework: 'ssl',
        title: 'SSL/TLS connessione fallita su :443',
        remediation: "Verifica firewall + cert installato. Usa Let's Encrypt o cert commerciale.",
      });
    } else {
      if (input.ssl.daysUntilExpiry !== undefined && input.ssl.daysUntilExpiry < 0) {
        f.push({
          ...
          remediation: `Renew immediato (cert expired ${String(-input.ssl.daysUntilExpiry)} giorni fa)`,
        });
      } else if (input.ssl.daysUntilExpiry !== undefined && input.ssl.daysUntilExpiry < 30) {
        f.push({
          severity: 'high',
          framework: 'ssl',
          title: `Certificato SSL scade in ${String(input.ssl.daysUntilExpiry)} giorni`,
          remediation: 'Schedule renewal o verifica certbot autorenew',
        });
      }
      if (input.ssl.sanMatches === false) {
        f.push({
          severity: 'high',
          framework: 'ssl',
          title: 'Certificato SAN non matcha il dominio',
          remediation: `Reissue cert con ${input.domain} nei SAN`,
        });
      }
      if (input.ssl.protocol && /^TLSv1(\.0|\.1)?$/.test(input.ssl.protocol)) {
        f.push({
          severity: 'high',
          framework: 'ssl',
          title: `Protocollo SSL obsoleto: ${input.ssl.protocol}`,
          remediation: 'Disabilita TLS<1.2 nel webserver (nginx ssl_protocols TLSv1.2 TLSv1.3)',
        });
      }
    }
  }
  ...
  if (input.redirectsChecked) {
    const startHost = (() => {
      try {
        return new URL(`http://${input.domain}`).host;
      } catch {
        return input.domain;
      }
    })();
      ...
      try {
        ...
        if (toHost !== startHost && !toHost.endsWith(`.${startHost.replace(/^www\./, '')}`)) {
          f.push({
            severity: 'medium',
            framework: 'redirect',
            title: `Redirect cross-domain: ${startHost} → ${toHost}`,
            remediation: 'Verifica intenzionale (no open-redirect / phishing risk)',
          });
          break;
        }
      } catch {
    ...
    if (input.redirects.length >= 10) {
      f.push({
        severity: 'medium',
        framework: 'redirect',
        title: 'Redirect chain ≥10 hop (loop probabile)',
        remediation: 'Refactor catena redirect, mantieni <=2 hop',
      });
    }
  }
  ...
  if (!input.headers.hsts)
    f.push({
      severity: 'medium',
      framework: 'headers',
      title: 'HSTS header assente',
      remediation:
        'Aggiungi "Strict-Transport-Security: max-age=63072000; includeSubDomains; preload"',
    });
  if (!input.headers.csp)
    f.push({
      ...
      remediation: "Definisci CSP restrittivo (default-src 'self')",
    });
  if (!input.headers.xfo)
    f.push({
      severity: 'low',
      framework: 'headers',
      title: 'X-Frame-Options assente (clickjacking)',
      remediation: 'Aggiungi "X-Frame-Options: SAMEORIGIN" o CSP frame-ancestors',
    });
  if (!input.headers.xcto)
    f.push({
      severity: 'low',
      framework: 'headers',
      title: 'X-Content-Type-Options=nosniff assente',
      remediation: 'Aggiungi "X-Content-Type-Options: nosniff"',
    });
  if (!input.headers.referrer)
    f.push({
      severity: 'low',
      framework: 'headers',
      title: 'Referrer-Policy assente',
      remediation: 'Aggiungi "Referrer-Policy: strict-origin-when-cross-origin"',
    });
  ...
  for (const x of f) {
    score -=
      x.severity === 'critical' ? 25 : x.severity === 'high' ? 15 : x.severity === 'medium' ? 7 : 3;
  }
...
export const securityAuditExecutor: NodeExecutor = async (rawConfig, _input, _context) => {
  ...
  let domain = coerceString(cfg.domain ?? '')
    .trim()
    .toLowerCase();
  ...
  if (!domainIsValid(domain))
    throw new Error(`action_security_audit: dominio non valido "${domain}".`);

  const timeoutMs = clampTimeout(cfg.timeoutMs);
  const dkimSelectors = coerceString(cfg.dkimSelectors ?? '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  const checkSsl = cfg.checkSsl !== false && coerceString(cfg.checkSsl ?? 'true') !== 'false';
  const checkRedirects =
    cfg.checkRedirects !== false && coerceString(cfg.checkRedirects ?? 'true') !== 'false';
  ...
  try {
    const [spf, dkim, dmarc, ssl, redirects, headers] = await Promise.all([
      ...
      dkimSelectors.length > 0
        ? lookupDkim(domain, dkimSelectors, ctrl.signal)
        : Promise.resolve({ selectorsFound: [], selectorsTried: [] }),
      lookupDmarc(domain),
