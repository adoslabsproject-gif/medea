...
describe('newsDisplayExecutor end-to-end', () => {
  ...
  it('fetcha RSS + restituisce items', async () => {
    ...
    const r = await newsDisplayExecutor(
      { feedUrl: 'https://example.com/rss', limit: 5 },
      null,
      ctx,
    );
  ...
  it('🚨 ATTACCO: feed ENORME → lettura cappata in streaming (no res.text() integrale)', async () => {
    ...
    globalThis.fetch = vi
      ...
      .mockResolvedValueOnce(
        new Response(huge, { status: 200, headers: { 'content-type': 'application/xml' } }),
      );
    ...
    await newsDisplayExecutor(
      { feedUrl: 'https://example.com/huge-rss', limit: 5 },
      null,
      ctx,
    ).catch(() => undefined);
  ...
  it('renderFormat=markdown → campo markdown popolato', async () => {
    ...
    const r = await newsDisplayExecutor(
      { feedUrl: 'https://example.com/rss', renderFormat: 'markdown' },
      null,
      ctx,
    );
  ...
  it('renderFormat=html → campo html popolato', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce(new Response(RSS_SAMPLE, { status: 200 }));
    const r = await newsDisplayExecutor(
      { feedUrl: 'https://example.com/rss', renderFormat: 'html' },
      null,
      ctx,
    );
  ...
  it('limit applica cap (RSS con 2 items, limit 1 → 1 item)', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce(new Response(RSS_SAMPLE, { status: 200 }));
    const r = await newsDisplayExecutor(
      { feedUrl: 'https://example.com/rss', limit: 1 },
      null,
      ctx,
    );
  ...
  it('feedUrl vuoto → throw', async () => {
    await expect(newsDisplayExecutor({ feedUrl: '' }, null, ctx)).rejects.toThrow(
      /feedUrl.*obbligatorio/,
    );
  });
  ...
  it('HTTP 500 → throw informativo', async () => {
    ...
    await expect(
      newsDisplayExecutor({ feedUrl: 'https://example.com/rss' }, null, ctx),
  ...
  it('XML invalido → throw "non riconosciuto"', async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce(new Response('<not-a-feed/>', { status: 200 }));
    await expect(
      newsDisplayExecutor({ feedUrl: 'https://example.com/rss' }, null, ctx),
  ...
  it('sinceHours filtra items vecchi (cut-off 0.001h = 3.6s — tutti scartati)', async () => {
    ...
    const r = await newsDisplayExecutor(
      { feedUrl: 'https://example.com/rss', sinceHours: 1 },
      null,
      ctx,
    );
