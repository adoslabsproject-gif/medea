...
vi.mock('node:tls', () => ({
  connect: vi.fn((_opts: unknown, cb?: () => void) => {
    ...
    const sock = {
      getPeerCertificate: () => ({
        valid_to: futureDate, subjectaltname: 'DNS:pinned.test',
        issuer: { O: 'TestCA' }, subject: { CN: 'pinned.test' },
      }),
      getProtocol: () => 'TLSv1.3',
      destroy: vi.fn(),
      once: vi.fn(),
...
describe('security-audit executor — validation', () => {
  it('rejecta dominio vuoto', async () => {
    await expect(
      securityAuditExecutor({ domain: '' }, null, baseContext),
  ...
  it('strip protocol/path/port automatico', async () => {
    ...
    const r = await securityAuditExecutor(
      { domain: 'https://example.com:8443/some/path?q=1', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' },
      null,
      baseContext,
    );
  ...
  it('domain non risolvibile → error chiaro', async () => {
    ...
    await expect(
      securityAuditExecutor({ domain: 'invalid.test', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' }, null, baseContext),
...
describe('security-audit executor — CONTRACT: opera SOLO su config.domain, ignora l\'input upstream', () => {
  it('un domain nell\'INPUT NON fa da fallback quando config.domain è vuoto', async () => {
    ...
    await expect(
      securityAuditExecutor({ domain: '' }, { domain: 'iniettato.example.com', json: { domain: 'iniettato.example.com' } } as never, baseContext),
    ).rejects.toThrow(/obbligatorio/i);
  });

  it('input arbitrario/sporco non altera il risultato: conta solo config.domain', async () => {
    mockResolveTxt.mockResolvedValue([]);
    const cfg = { domain: 'example.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' };
    const withNull = await securityAuditExecutor(cfg, null, baseContext);
    const withDirtyInput = await securityAuditExecutor(cfg, { evil: '<script>', domain: 'altro.com', items: [1, 2] } as never, baseContext);
...
describe('security-audit executor — SSRF guard (CVE-fix consulente 2026-06-05)', () => {
  it('hostname "localhost" → reject SSRF', async () => {
    await expect(
      securityAuditExecutor({ domain: 'localhost', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' }, null, baseContext),
    ).rejects.toThrow(/SSRF safety|interno\/loopback/i);
  });

  it('hostname "*.local" → reject SSRF (Bonjour/mDNS)', async () => {
    await expect(
      securityAuditExecutor({ domain: 'mac-mini.local', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' }, null, baseContext),
    ).rejects.toThrow(/SSRF safety|interno\/loopback/i);
  });

  it('hostname "host.docker.internal" → reject SSRF (container escape)', async () => {
    await expect(
      securityAuditExecutor({ domain: 'host.docker.internal', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' }, null, baseContext),
    ).rejects.toThrow(/SSRF safety|interno\/loopback/i);
  });

  it('hostname "metadata.google.internal" → reject SSRF (cloud metadata)', async () => {
    await expect(
      securityAuditExecutor({ domain: 'metadata.google.internal', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' }, null, baseContext),
    ).rejects.toThrow(/SSRF safety|interno\/loopback/i);
  });
  ...
  it('dominio pubblico ma A record privato (10.x.x.x) → reject SSRF', async () => {
    ...
    await expect(
      securityAuditExecutor({ domain: 'attacker-controlled.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' }, null, baseContext),
  ...
  it('dominio pubblico ma A record loopback (127.0.0.1) → reject SSRF', async () => {
    ...
    await expect(
      securityAuditExecutor({ domain: 'pin-loopback.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' }, null, baseContext),
    ).rejects.toThrow(/IP privato.*SSRF/i);
  });
  ...
  it('dominio pubblico ma A record link-local (169.254.169.254 AWS metadata) → reject SSRF', async () => {
    ...
    await expect(
      securityAuditExecutor({ domain: 'aws-metadata.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' }, null, baseContext),
    ).rejects.toThrow(/IP privato.*SSRF/i);
  });
  ...
  it('multi A record con almeno 1 privato → reject SSRF (no partial leak)', async () => {
    ...
    await expect(
      securityAuditExecutor({ domain: 'mixed-leak.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' }, null, baseContext),
    ).rejects.toThrow(/IP privato.*SSRF/i);
  });
  ...
  it('dominio pubblico legittimo (8.8.8.8) → passa la SSRF guard', async () => {
    ...
    mockResolveTxt.mockResolvedValue([]);
    const r = await securityAuditExecutor(
      { domain: 'public.example.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' },
      null,
      baseContext,
    );
  ...
  it('🚨🚨 ANTI DNS-REBINDING: la TLS connect è PINNATA all\'IP validato, NON al dominio', async () => {
    ...
    const opts = mockTlsConnect.mock.calls[0]![0] as { host?: string; servername?: string; port?: number };
  ...
  it('🚨 DNS risolve UNA sola volta (no seconda risoluzione sfruttabile per rebinding)', async () => {
    ...
    mockResolveTxt.mockResolvedValue([]);
    await securityAuditExecutor(
      { domain: 'single-resolve.com', checkSsl: 'true', checkRedirects: 'false', dkimSelectors: '' },
      null, baseContext,
    );
...
describe('security-audit executor — SPF', () => {
  ...
  it('SPF +all (spoofing aperto) → CRITICAL', async () => {
    ...
    const out = r.output as { findings: { framework: string; severity: string; title: string }[]; spf: { allMode: string } };
  ...
  it('SPF -all + mechanisms parsati', async () => {
    ...
    const out = r.output as { spf: { found: boolean; mechanisms: string[]; allMode: string }; findings: { framework: string }[] };
...
describe('security-audit executor — DKIM', () => {
  it('DKIM selettori tutti vuoti → finding HIGH', async () => {
    mockResolveTxt.mockResolvedValue([]);
    const r = await securityAuditExecutor(
      { domain: 'example.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: 'google,default' },
      null,
      baseContext,
    );
    const out = r.output as { findings: { framework: string; severity: string }[]; dkim: { selectorsTried: string[]; selectorsFound: string[] } };
  ...
  it('DKIM almeno 1 selettore valido → no finding', async () => {
    mockResolveTxt.mockImplementation(async (name: string) => {
      if (name === 'google._domainkey.example.com') return [['v=DKIM1; k=rsa; p=MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQ...']];
      return [];
    });
    const r = await securityAuditExecutor(
      { domain: 'example.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: 'google,default' },
    ...
    const out = r.output as { findings: { framework: string }[]; dkim: { selectorsFound: string[] } };
  ...
  it('DKIM skip se selectors vuoto', async () => {
    ...
    const out = r.output as { findings: { framework: string }[]; dkim: { selectorsTried: string[] } };
...
describe('security-audit executor — DMARC', () => {
  ...
  it('DMARC p=none → finding MEDIUM', async () => {
    mockResolveTxt.mockImplementation(async (name: string) => {
      if (name === '_dmarc.example.com') return [['v=DMARC1; p=none; pct=100; rua=mailto:dmarc@example.com']];
      return [];
    });
    const r = await securityAuditExecutor(
      { domain: 'example.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' },
    ...
    const out = r.output as { findings: { framework: string; severity: string; title: string }[]; dmarc: { policy: string; pct: number; rua: string } };
    expect(out.dmarc.policy).toBe('none');
    expect(out.dmarc.pct).toBe(100);
    expect(out.dmarc.rua).toBe('mailto:dmarc@example.com');
    expect(out.findings.find((f) => f.framework === 'dmarc' && f.title.includes('policy=none'))?.severity).toBe('medium');
  });
  ...
  it('DMARC pct<100 + senza rua → 2 findings (low ognuno)', async () => {
    ...
    expect(dmarcFinds.some((f) => /rua.*mancante/i.test(f.title) && f.severity === 'low')).toBe(true);
  });

  it('DMARC p=reject pct=100 con rua → no DMARC finding', async () => {
    mockResolveTxt.mockImplementation(async (name: string) => {
      if (name === '_dmarc.example.com') return [['v=DMARC1; p=reject; pct=100; rua=mailto:d@example.com']];
      return [];
    });
    const r = await securityAuditExecutor(
      { domain: 'example.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: '' },
...
describe('security-audit executor — Headers', () => {
  it('HSTS+CSP+XFO+nosniff+referrer mancanti → 5 findings', async () => {
    ...
    const out = r.output as { findings: { framework: string }[]; headers: { hsts: boolean; csp: boolean; xfo: boolean; xcto: boolean; referrer: boolean } };
  ...
  it('tutti security headers presenti → 0 header findings', async () => {
    ...
    const out = r.output as { findings: { framework: string }[]; headers: { hsts: boolean; csp: boolean } };
...
describe('security-audit executor — Score', () => {
  it('dominio perfect (SPF -all + DKIM trovato + DMARC reject + tutti headers) → score ≥85', async () => {
    ...
    const r = await securityAuditExecutor(
      { domain: 'example.com', checkSsl: 'false', checkRedirects: 'false', dkimSelectors: 'google' },
      null,
      baseContext,
    );
