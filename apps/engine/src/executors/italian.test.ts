...
const __fixtCertPem = readFileSync(
  join(__sdiDir, '__fixtures__', 'test-signing-cert.pem'),
  'utf-8',
);
...
describe('🚨 pecArubaSendExecutor — SMTP default path', () => {
  ...
  it('🚨 missing required fields → throw esplicito', async () => {
    await expect(pecArubaSendExecutor({ username: 'me' }, null, ctx)).rejects.toThrow(
      /username\/password\/to\/subject required/u,
    );
  });
  ...
  it('🚨 attachments JSON invalid → throw', async () => {
    ...
    ).rejects.toThrow(/attachmentsJson is not valid JSON/u);
  });
  ...
  it('attachments base64 → content Buffer', async () => {
    ...
    const sendCall = m.sendMail.mock.calls[0]?.[0] as {
    ...
    await pecArubaSendExecutor(
      {
        ...
        attachmentsJson: JSON.stringify([
          { filename: 'x.pdf', binary: bin, base64: Buffer.from('LEGACY').toString('base64') },
        ]),
      },
      null,
      ctx,
      ...
      pecArubaSendExecutor(
        {
          ...
          subject: 's',
          attachmentsJson: JSON.stringify([
            { filename: 'leak.pdf', url: 'http://169.254.169.254/imds' },
          ]),
        },
        null,
        ctx,
    ...
    await pecArubaSendExecutor(
      {
        ...
        subject: 's',
        attachmentsJson: JSON.stringify([
          { filename: 'doc.pdf', url: 'https://cdn.example.com/d.pdf' },
        ]),
      },
      null,
      ctx,
...
describe('🚨 pecArubaSendExecutor — SOAP legacy path', () => {
  ...
  it('🚨 SOAP HTTP error → throw con status code', async () => {
    ...
    ).rejects.toThrow(/PEC Aruba \(SOAP\) 500/u);
  });
      ...
      pecArubaSendExecutor(
        {
          ...
          body: 'b',
          transport: 'soap',
          endpoint: 'https://attacker.com/PecService',
        },
        null,
        ctx,
      ),
    ).rejects.toThrow(/host non consentito|host/u);
...
describe('🚨 zucchettiPayrollExecutor', () => {
  it('happy: POST /payroll/jobs + Bearer + body', async () => {
    ...
    const r = await zucchettiPayrollExecutor(
      {
        baseUrl: 'https://api.zucchetti.com',
        apiToken: 'tok-1',
        companyCode: 'AC01',
        period: '2026-05',
        dryRun: true,
    ...
    const body = JSON.parse((m.safeFetch.mock.calls[0]?.[1] as { body: string }).body) as Record<
      string,
      unknown
    >;
  ...
  it('🚨 missing required → throw', async () => {
    await expect(zucchettiPayrollExecutor({ baseUrl: 'x' }, null, ctx)).rejects.toThrow(
      /baseUrl\/apiToken\/companyCode\/period required/u,
    );
  });
  ...
  it('🚨 HTTP error → throw con status', async () => {
    ...
    ).rejects.toThrow(/Zucchetti 401/u);
  });
  ...
  it('🚨🚨 ANTI-ESFILTRAZIONE: baseUrl=attacker.com → blocco, Bearer MAI spedito', async () => {
    ...
    ).rejects.toThrow(/host non consentito|non ammesso|host/u);
    expect(m.safeFetch).not.toHaveBeenCalled();
  });
...
describe('🚨 sdiSendInvoiceExecutor', () => {
  ...
  const baseInvoiceXml =
    '<?xml version="1.0"?><FatturaElettronica><HeaderXX/><BodyXX/></FatturaElettronica>';
  ...
  beforeEach(() => {
    m.safeFetch.mockResolvedValue({
      ...
      status: 200,
      text: async () =>
        '<r><IdentificativoSdI>sdi-99</IdentificativoSdI><DataOraRicezione>2026-06-06T10:00:00</DataOraRicezione></r>',
    });
  });
  ...
  it('skipSigning=true → bypass signing, upload as-is', async () => {
    const r = await sdiSendInvoiceExecutor(
      {
        invoiceXml: baseInvoiceXml,
        validateXsd: false,
        sdiUsername: 'u',
        sdiPassword: 'p',
        skipSigning: true,
        fileName: 'IT12345_F0.xml',
      },
      null,
      ctx,
    );
  ...
  it('🚨 invoiceXml mancante → throw', async () => {
    ...
    ).rejects.toThrow(/invoiceXml richiesto/u);
  });
  ...
  it('🚨 validateXsd default ON: XML con root FatturaElettronica ma contenuto NON conforme → RIFIUTATO pre-invio, NESSUN upload', async () => {
    ...
    ).rejects.toThrow(/NON è conforme allo schema XSD FatturaPA/u);
  ...
  it('🚨 validateXsd:false → bypassa la validazione (XML non conforme viene caricato lo stesso)', async () => {
    const xsdInvalid =
      '<?xml version="1.0"?><FatturaElettronica><Bogus>non conforme</Bogus></FatturaElettronica>';
    const r = await sdiSendInvoiceExecutor(
      {
        invoiceXml: xsdInvalid,
        sdiUsername: 'u',
  ...
  it("🚨 fattura ufficiale VALIDA + validateXsd ON → passa la validazione e procede all'upload", async () => {
    const validInvoice = readFileSync(
      join(__sdiDir, '__fixtures__', 'IT01234567890_FPR01.xml'),
      'utf-8',
    );
    const r = await sdiSendInvoiceExecutor(
      {
        invoiceXml: validInvoice,
        sdiUsername: 'u',
        sdiPassword: 'p',
        skipSigning: true,
  ...
  it('🚨 credentials mancanti → throw', async () => {
    ...
    ).rejects.toThrow(/sdiUsername.*sdiPassword/u);
  });
  ...
  it('🚨 HTTP error SDI → throw con status', async () => {
    ...
    ).rejects.toThrow(/SDI upload 503/u);
  });
  ...
  it('🚨 signing path: cert+key da PEM inline → XAdES envelope generato + IssuerSerial REALE', async () => {
    ...
    expect(decoded).toContain(
      '<ds:X509SerialNumber xmlns:ds="http://www.w3.org/2000/09/xmldsig#">305419896</ds:X509SerialNumber>',
    );
  ...
  it('🚨 signing con cert PEM NON parsabile → fail-fast esplicito (no firma con metadata fasulli)', async () => {
    ...
    ).rejects.toThrow(/certificato X\.509 non parsabile/u);
  });
  ...
  it('🚨 signing su XML senza root FatturaElettronica → throw', async () => {
    ...
    ).rejects.toThrow(/root <FatturaElettronica> non trovata/u);
  });
  ...
  it('🚨 signing senza cert/key → throw esplicito', async () => {
    ...
    ).rejects.toThrow(/Missing MEDEA_SDI/u);
  });
...
describe('🚨 sdiCheckStatusExecutor', () => {
  ...
  it('🚨 missing required → throw', async () => {
    await expect(sdiCheckStatusExecutor({ fileName: 'x' }, null, ctx)).rejects.toThrow(
      /fileName\/sdiUsername\/sdiPassword required/u,
    );
  });
  ...
  it('🚨 HTTP error → throw con status', async () => {
    ...
    ).rejects.toThrow(/SDI status 404/u);
  });
