...
const ctx = {
  ...
  defId: 'weather_node', secrets: {}, llmProviders: [], nodeOutputs: {},
} as unknown as Parameters<typeof weatherExecutor>[2];

describe('parseLocation', () => {
  it('parses "lat,lon" decimali', () => {
    expect(__test__.parseLocation('45.4642,9.1900')).toEqual({ mode: 'coords', lat: 45.4642, lon: 9.1900 });
  });
  it('parses "lat, lon" con spazi + negativi', () => {
    expect(__test__.parseLocation(' -33.8688 , 151.2093 ')).toEqual({ mode: 'coords', lat: -33.8688, lon: 151.2093 });
  });
...
describe('weatherExecutor end-to-end', () => {
  ...
  it('fetcha forecast per coords (no geocoding)', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce(
      new Response(JSON.stringify({
        current: { temperature_2m: 18.5, apparent_temperature: 17.0, relative_humidity_2m: 60, wind_speed_10m: 12, wind_direction_10m: 180, weather_code: 0 },
        daily: { time: ['2026-06-05', '2026-06-06'], temperature_2m_max: [22, 24], temperature_2m_min: [14, 15], weather_code: [1, 2], precipitation_sum: [0, 0.5] },
        timezone: 'Europe/Rome',
      }), { status: 200 }),
    );
    const r = await weatherExecutor({ location: '45.46,9.19', forecastDays: 2 }, null, ctx);
    const out = r.output as { location: { lat: number; timezone: string }; current: { tempC: number; condition: string }; forecast: { date: string; minC: number; maxC: number }[] };
  ...
  it('city name → geocoding → forecast (2 fetch chiamate)', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({
        results: [{ name: 'Roma', latitude: 41.9, longitude: 12.5, country: 'Italy', timezone: 'Europe/Rome' }],
      }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({
        current: { temperature_2m: 25, apparent_temperature: 26, relative_humidity_2m: 50, wind_speed_10m: 5, wind_direction_10m: 90, weather_code: 0 },
        daily: { time: [], temperature_2m_max: [], temperature_2m_min: [], weather_code: [], precipitation_sum: [] },
        timezone: 'Europe/Rome',
      }), { status: 200 }));
  ...
  it('cache: 2 chiamate consecutive stesse coords → 1 sola fetch', async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({
      current: { temperature_2m: 20, apparent_temperature: 20, relative_humidity_2m: 50, wind_speed_10m: 10, wind_direction_10m: 0, weather_code: 0 },
      timezone: 'UTC',
    }), { status: 200 }));
    globalThis.fetch = fetchMock;
  ...
  it('imperial units: tempC restituito in °F', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({
      current: { temperature_2m: 0, apparent_temperature: 0, relative_humidity_2m: 50, wind_speed_10m: 100, wind_direction_10m: 0, weather_code: 0 },
      timezone: 'UTC',
    }), { status: 200 }));
    const r = await weatherExecutor({ location: '0,0', forecastDays: 0, units: 'imperial' }, null, ctx);
  ...
  it('language en → condition in inglese', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({
      current: { temperature_2m: 10, apparent_temperature: 10, relative_humidity_2m: 80, wind_speed_10m: 0, wind_direction_10m: 0, weather_code: 61 },
      timezone: 'UTC',
    }), { status: 200 }));
    const r = await weatherExecutor({ location: '0,0', forecastDays: 0, language: 'en' }, null, ctx);
    const out = r.output as { current: { condition: string } };
    expect(out.current.condition).toBe('slight rain');
  });

  it('location vuoto → throw', async () => {
    await expect(weatherExecutor({ location: '' }, null, ctx)).rejects.toThrow(/"location" è obbligatorio/);
  });

  it('geocoding miss → throw informativo', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({ results: [] }), { status: 200 }));
    await expect(weatherExecutor({ location: 'Xyzabc1234' }, null, ctx)).rejects.toThrow(/non trovata/);
  });

  it('forecast API 500 → throw', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce(new Response('boom', { status: 500 }));
    await expect(weatherExecutor({ location: '0,0' }, null, ctx)).rejects.toThrow(/forecast API fallita/);
  });

  it('forecastDays clamp [0,7]: input 999 → cappato a 7', async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({
      current: { temperature_2m: 1, apparent_temperature: 1, relative_humidity_2m: 1, wind_speed_10m: 1, wind_direction_10m: 1, weather_code: 0 },
      daily: { time: ['d1','d2','d3','d4','d5','d6','d7'], temperature_2m_max: [1,1,1,1,1,1,1], temperature_2m_min: [0,0,0,0,0,0,0], weather_code: [0,0,0,0,0,0,0], precipitation_sum: [0,0,0,0,0,0,0] },
      timezone: 'UTC',
    }), { status: 200 }));
    globalThis.fetch = fetchMock;
