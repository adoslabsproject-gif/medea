...
import { executeRemoteSshDbQuery, makeVaultResolverFrom, type RemoteSshDeps, type PgRunner } from './remote-ssh-db-query.js';
...
const RAW = {
  ssh: {
    host: '8.8.8.8', port: 22, user: 'tunnel',
    hostKeyFingerprint: 'SHA256:abcdEFGH1234567890abcdefghIJKLmnopqrstuvwx',
    auth: { method: 'key', privateKeySecretRef: 'vault://ssh/key' },
  },
  db: { engine: 'postgres', host: '127.0.0.1', port: 5432, database: 'app', userSecretRef: 'vault://db/user', passwordSecretRef: 'vault://db/pw' },
};

function deps(over: Partial<RemoteSshDeps> = {}): {
  deps: RemoteSshDeps; close: ReturnType<typeof vi.fn>; openTunnel: ReturnType<typeof vi.fn>; pgRun: ReturnType<typeof vi.fn>;
} {
  const close = vi.fn().mockResolvedValue(undefined);
  const tunnel: SshTunnel = { localPort: 49999, close };
  const openTunnel = vi.fn<(o: SshTunnelTransportOptions) => Promise<SshTunnel>>().mockResolvedValue(tunnel);
...
describe('executeRemoteSshDbQuery', () => {
  it('SELECT → apre tunnel, esegue pgRun su 127.0.0.1:localPort con creds DB risolte, chiude', async () => {
    ...
    expect(conn).toEqual({ host: '127.0.0.1', port: 49999, database: 'app', user: 'RES(vault://db/user)', password: 'RES(vault://db/pw)' });
  ...
  it('🚨 query NON read-only → SshPolicyError PRIMA di aprire il tunnel', async () => {
    ...
    await expect(executeRemoteSshDbQuery(RAW, 'DROP TABLE clienti', 100, d)).rejects.toThrow(SshPolicyError);
    expect(openTunnel).not.toHaveBeenCalled();
  });

  it('🚨 SELECT con DROP nascosta (multi-statement) → rifiutata, tunnel mai aperto', async () => {
    const { deps: d, openTunnel } = deps();
    await expect(executeRemoteSshDbQuery(RAW, 'SELECT 1; DROP TABLE clienti', 100, d)).rejects.toThrow(SshPolicyError);
    expect(openTunnel).not.toHaveBeenCalled();
  });

  it('lifecycle: se pgRun lancia, il tunnel viene CHIUSO comunque (finally)', async () => {
    const { deps: d, close } = deps({ pgRun: async () => { throw new Error('query boom'); } });
  ...
  it('segreto DB non risolvibile → SshPolicyError, tunnel CHIUSO', async () => {
    const { deps: d, close } = deps({ resolveSecret: async (ref) => (ref.includes('db/pw') ? null : `RES(${ref})`) });
