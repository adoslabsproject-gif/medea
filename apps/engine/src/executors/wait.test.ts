...
const ctx = (over: Partial<{ runId: string; workflowId: string; nodeId: string; abortSignal: AbortSignal }> = {}) => ({
...
describe('🚨 ensureWaitTable — auto-create idempotente', () => {
  it('🚨 prima invocazione → tabella creata', async () => {
    ...
    const tbl = sqlite.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='wait_states'").get();
...
describe('🚨 mode=timer', () => {
  it('🚨 durationMs=0 → ritorna immediato con waitedMs=0', async () => {
    ...
    const result = await waitExecutor(
      { mode: 'timer', durationMs: 0 } as never, {} as never, ctx(),
    );
  ...
  it('🚨 durationMs=50 → aspetta ~50ms', async () => {
    const start = Date.now();
    await waitExecutor(
      { mode: 'timer', durationMs: 50 } as never, {} as never, ctx(),
    );
  ...
  it('🚨 CLAMP: durationMs negativo → 0 (Math.max guard)', async () => {
    const result = await waitExecutor(
      { mode: 'timer', durationMs: -500 } as never, {} as never, ctx(),
    );
  ...
  it('🚨 default mode → "timer" se config.mode missing', async () => {
    const result = await waitExecutor(
      { durationMs: 0 } as never, {} as never, ctx(),
    );
  ...
  it('🚨🚨 EVENT-DRIVEN: resumeWait sveglia il loop SUBITO (<2s, non al fallback 5s)', async () => {
    ...
    compat.prepare = (sql: string) => {
      ...
      if (sql.includes('INSERT INTO wait_states')) {
        ...
        stmt.run = (...p: unknown[]) => {
          savedToken = p[0] as string;
          setTimeout(() => { resumeWait(savedToken, { ok: 1 }); }, 50);
          return origRun(...p);
        };
      }
      return stmt;
    ...
    const t0 = Date.now();
    const result = await waitExecutor({ mode: 'webhook', maxWaitMs: 30_000 } as never, {} as never, ctx());
...
describe('🚨 resumeWait — webhook callback resume', () => {
  it('🚨 token esistente + completed_at NULL → UPDATE + return true', async () => {
    ...
    await waitExecutor({ mode: 'timer', durationMs: 0 } as never, {} as never, ctx());
    sqlite.prepare(`INSERT INTO wait_states (token, expires_at, created_at) VALUES (?, ?, ?)`)
    ...
    const row = sqlite.prepare(`SELECT * FROM wait_states WHERE token = ?`).get('TOK-1') as {
      completed_at: number; payload_json: string;
    };
  ...
  it('🚨 token già completato → return false (no double-resume)', async () => {
    await waitExecutor({ mode: 'timer', durationMs: 0 } as never, {} as never, ctx());
    sqlite.prepare(`INSERT INTO wait_states (token, expires_at, completed_at, created_at) VALUES (?, ?, ?, ?)`)
  ...
  it('🚨 payload null → serializzato come "null" stringa', async () => {
    ...
    const row = sqlite.prepare(`SELECT payload_json FROM wait_states WHERE token = ?`).get('TOK-3') as {
      payload_json: string;
    };
  ...
  it('🚨 payload complex object → JSON-stringified', async () => {
    ...
    const row = sqlite.prepare(`SELECT payload_json FROM wait_states WHERE token = ?`).get('TOK-4') as {
      payload_json: string;
    };
