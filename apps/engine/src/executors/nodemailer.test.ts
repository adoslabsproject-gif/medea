...
describe('sendEmailExecutor — OAuth2 (XOAUTH2) inline', () => {
  ...
  it('oauthUser vuoto → fallback allo SMTP username', async () => {
    await sendEmailExecutor(
      { ...oauthConfig, oauthUser: '', username: 'fallback@gmail.com' },
      {},
      ctx,
    );
  ...
  it('🚨 authMode=oauth2 ma refreshToken mancante → throw esplicito, NESSUN invio', async () => {
    await expect(
      sendEmailExecutor({ ...oauthConfig, oauthRefreshToken: '' }, {}, ctx),
  ...
  it('🚨 authMode=oauth2 ma clientId+secret mancanti → throw elenca i campi', async () => {
    await expect(
      sendEmailExecutor({ ...oauthConfig, oauthClientId: '', oauthClientSecret: '' }, {}, ctx),
...
describe('🚨 validation guards', () => {
  it('🚨 missing host → throw all required', async () => {
    await expect(sendEmailExecutor({ ...baseConfig, host: '' }, {}, ctx)).rejects.toThrow(
      /host\/from\/to\/subject all required/u,
    );
  });

  it('🚨 missing to → throw', async () => {
    await expect(sendEmailExecutor({ ...baseConfig, to: '' }, {}, ctx)).rejects.toThrow(
      /all required/u,
    );
  });
  ...
  it('🚨 subject con {{...}} unresolved → throw', async () => {
    await expect(
      sendEmailExecutor({ ...baseConfig, subject: 'Ordine {{order.id}}' }, {}, ctx),
    ).rejects.toThrow(/unresolved \{\{\.\.\.\}\} template/u);
  });

  it('🚨 body con {{...}} unresolved → throw', async () => {
    await expect(
      sendEmailExecutor({ ...baseConfig, body: 'Hi {{user.name}}' }, {}, ctx),
...
describe('🚨 systemAccountId — credentials from vault', () => {
  it('🚨 account not found → throw esplicito', async () => {
    ...
    await expect(
      sendEmailExecutor({ ...baseConfig, systemAccountId: 'acc-1' }, {}, ctx),
  ...
  it('account password-based → override host/port/security/user/pwd', async () => {
    ...
    await sendEmailExecutor(
      { systemAccountId: 'acc-1', to: 'to@x.com', subject: 's', body: 'b', bodyType: 'text' },
      {},
      ctx,
    );
    expect(m.createTransport).toHaveBeenCalledWith(
  ...
  it('🚨 oauth2 + tokens stale → refresh + persist + send', async () => {
    ...
    m.refreshAccessToken.mockResolvedValue({
    ...
    expect(m.updateOAuthAccessToken).toHaveBeenCalledWith(
      expect.objectContaining({
        tenantId: 't1',
        accountId: 'acc-1',
        accessToken: 'ya29.NEW',
...
describe('🚨 deliverabilityCheck', () => {
  ...
  it('🚨 strict + SPF mancante → throw + sendMail NON chiamato', async () => {
    ...
    await expect(
      sendEmailExecutor(
        { ...baseConfig, from: 'strict-spf@example.com', deliverabilityCheck: 'strict' },
        {},
        ctx,
      ),
  ...
  it('warn + report.ok=false → send comunque + report in output', async () => {
    ...
    const r = await sendEmailExecutor(
      { ...baseConfig, from: 'warn-dmarc@example.com', deliverabilityCheck: 'warn' },
      {},
      ctx,
    );
    expect(m.sendMail).toHaveBeenCalled();
  ...
  it('strict + check throws → throw propagated', async () => {
    ...
    await expect(
      sendEmailExecutor(
        { ...baseConfig, from: 'strict-throw@example.com', deliverabilityCheck: 'strict' },
        {},
        ctx,
      ),
  ...
  it('warn + check throws → swallow + send', async () => {
    m.deliverabilityCheck.mockRejectedValue(new Error('DNS down'));
    await sendEmailExecutor(
      { ...baseConfig, from: 'warn-throw@example.com', deliverabilityCheck: 'warn' },
      {},
      ctx,
    );
    expect(m.sendMail).toHaveBeenCalled();
...
describe('🚨 attachments SSRF guard + parsing', () => {
  it('🚨 attachments JSON invalid → throw', async () => {
    await expect(
      sendEmailExecutor({ ...baseConfig, attachmentsJson: 'not-json' }, {}, ctx),
  ...
  it('attachments base64 → Buffer.from base64 → content', async () => {
    ...
    const call = m.sendMail.mock.calls[0]?.[0] as {
  ...
  it('🚨 attachment URL SSRF blocked → throw', async () => {
    ...
    await expect(
      sendEmailExecutor({ ...baseConfig, attachmentsJson: att }, {}, ctx),
  ...
  it('inline image → cid + contentDisposition inline', async () => {
    ...
    const call = m.sendMail.mock.calls[0]?.[0] as {
      attachments: { cid: string; contentDisposition: string }[];
    };
...
describe('🚨 headers + priority + threading', () => {
  it('headersJson valid → merged in message.headers', async () => {
    await sendEmailExecutor(
      { ...baseConfig, headersJson: JSON.stringify({ 'X-Custom': 'v1' }) },
      {},
      ctx,
    );
  ...
  it('inReplyTo + references → headers In-Reply-To + References', async () => {
    await sendEmailExecutor(
      { ...baseConfig, inReplyTo: '<msg-1@x>', references: '<msg-0@x>' },
      {},
      ctx,
    );
    const call = m.sendMail.mock.calls[0]?.[0] as { headers: Record<string, string> };
...
describe('🚨 GAP2 capstone — email accetta allegati BinaryData (resolver, verso ref-primario)', () => {
  ...
  it('🚨 allegato binary ref → risolto via readBinary, content = byte (no base64 nel JSON)', async () => {
    ...
    await sendEmailExecutor(
      { ...baseConfig, attachmentsJson: JSON.stringify([{ filename: 'doc.pdf', binary: bin }]) },
      {},
      ctxBin(readBinary),
    );
  ...
  it('🚨 allegato binary inline → byte senza reader (fallback)', async () => {
    ...
    await sendEmailExecutor(
      { ...baseConfig, attachmentsJson: JSON.stringify([{ filename: 'x.pdf', binary: bin }]) },
      {},
      ctxBin(),
    );
  ...
  it('🚨 PRECEDENZA: binary vince su base64 (forma primaria)', async () => {
    const bin = makeBinaryInline({
      mimeType: 'text/plain',
      data: Buffer.from('REAL').toString('base64'),
    });
    await sendEmailExecutor(
      {
        ...baseConfig,
        attachmentsJson: JSON.stringify([
          { filename: 'a.txt', binary: bin, base64: Buffer.from('LEGACY').toString('base64') },
        ]),
      },
      {},
      ctxBin(),
  ...
  it('🚨 REGRESSIONE: solo base64 (legacy) → content = Buffer.from(base64), invariato', async () => {
    ...
    await sendEmailExecutor(
      {
        ...baseConfig,
        attachmentsJson: JSON.stringify([{ filename: 'l.txt', base64: bytes.toString('base64') }]),
      },
      {},
      ctxBin(),
    );
