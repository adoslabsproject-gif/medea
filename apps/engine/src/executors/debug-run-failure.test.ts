...
describe('debugRunFailureExecutor — validation', () => {
  it('rejecta runId vuoto', async () => {
    await expect(
      debugRunFailureExecutor({ runId: '' }, null, ctx),
    ).rejects.toThrow(/runId obbligatorio/i);
  });

  it('rejecta run inesistente', async () => {
    mockSelect.mockResolvedValue([]);
    await expect(
      debugRunFailureExecutor({ runId: 'run_404' }, null, ctx),
...
describe('debugRunFailureExecutor — diagnosis flow', () => {
  it('run senza step error → ritorna "nessun step fallito"', async () => {
    mockSelect.mockResolvedValue([{
      id: 'r1', workflowId: 'wf1', status: 'success', input: 'null',
  ...
  it('run con error → LLM analyze + estrae fix/tests + _llm usage', async () => {
    mockSelect.mockResolvedValue([{
      id: 'r1', workflowId: 'wf1', status: 'error', input: '{"orderId":42}',
      stepsJson: JSON.stringify([
        { nodeId: 'http_call', defId: 'action_http', status: 'error',
    ...
    m.dispatch.mockImplementation(async (...args: unknown[]) => {
      ...
      return llmText({
        diagnosis: { rootCause: 'API esterna in timeout transiente', category: 'transient', confidence: 0.9 },
        suggestedFixes: [
          { type: 'retry', description: 'Aumenta retryCount=5 + backoff exponential 1s→30s', patch: { retryCount: 5 }, confidence: 0.85 },
          { type: 'config', description: 'Aumenta timeout a 60000ms', patch: { timeoutMs: 60000 }, confidence: 0.6 },
        ],
        suggestedTests: [
          { scenario: '503 + 200 → success after retry', expectation: 'output ok', mockData: { responses: [503, 200] } },
        ],
      });
    });
    const r = await debugRunFailureExecutor({ runId: 'r1' }, null, ctx);
    ...
    expect(out._llm).toEqual({ inputTokens: 250, outputTokens: 90, model: 'liara-default', provider: 'liara', fromApi: true });
  ...
  it('LLM unreachable → fallback heuristic diagnosis, output SENZA _llm (zero token)', async () => {
    mockSelect.mockResolvedValue([{
      id: 'r1', workflowId: 'wf1', status: 'error', input: 'null',
      stepsJson: JSON.stringify([
        { nodeId: 'send_email', defId: 'action_send_email', status: 'error',
  ...
  it('maxFixes limita output anche se il modello restituisce di più', async () => {
    ...
    m.dispatch.mockResolvedValue(llmText({
      ...
      suggestedFixes: Array.from({ length: 5 }, (_, i) => ({
        type: 'config', description: `fix ${String(i)}`, confidence: 0.5,
      })),
      suggestedTests: [],
    }));
  ...
  it('failedNodeId esplicito → analizza quel nodo specifico', async () => {
    mockSelect.mockResolvedValue([{
      id: 'r1', workflowId: 'wf1', status: 'partial', input: 'null',
      stepsJson: JSON.stringify([
    ...
    m.dispatch.mockResolvedValue(llmText({
      ...
      suggestedFixes: [{ type: 'guard', description: 'add validation upstream', confidence: 0.7 }],
      suggestedTests: [],
    }));
