...
import {
  ...
  buildAiAssistantUserContent,
} from '@/prompts/ai-assistant.prompt.js';
...
export function createAiAssistantRoutes(): Hono {
  ...
  app.post(
    ...
    async (c) => {
      ...
      if (!auth?.userId) {
        logger.warn(
          { tenantId },
          '[SECURITY BYOK-11] ai-assistant called without auth.userId — rejecting',
        );
      ...
      const historyForLlm = ctx.messages
        ...
        .map(
          ({ role, content }) =>
            ({ role, content }) as { role: 'user' | 'assistant'; content: string },
        );
      ...
      if (provider !== 'liara' && provider !== 'ollama' && !apiKey) {
        return c.json(
          {
            error: `Nessuna API key per ${provider}. Configurala in Settings → AI Providers o passa X-LLM-API-Key in header.`,
          },
          401,
        );
      }
      ...
      try {
        ...
        const { searchMarketplace, formatMarketplaceSuggestions } =
          await import('@/services/catalog-retrieval/marketplace-discovery.js');
        const inUseDefIds = Array.isArray(
        ...
        catalogBlock =
          formatCatalogForPrompt(retriever, retrieved) + formatMarketplaceSuggestions(marketplace);
      } catch (err) {
        logger.warn(
          { err: err instanceof Error ? err.message : String(err) },
          '[ai-assistant] catalog retrieval failed — prompt senza catalogo',
        );
      }
          ...
          if (att.kind === 'image' && att.dataBase64) {
            if (provider === 'liara') {
              ...
              attachmentIntents.push({
                type: 'analyze_image',
                args: {
                  imageBase64: att.dataBase64,
                  imagePrompt: `Analizza ${att.name}: estrai testo e dati strutturati.`,
                },
            ...
            attachmentIntents.push({
              ...
              args: {
                documentBase64: att.dataBase64,
                documentMime: att.mimeType,
      ...
      try {
        ...
        logger.warn(
          { err: err instanceof Error ? err.message : String(err) },
          '[ai-assistant] auto-intent failed',
        );
      }
      ...
      const crossSurface = buildCrossSurfaceContext(userId, body.workspaceId, {
      ...
      let chatUsage: { input: number; output: number; fromApi: boolean } = {
        input: 0,
        output: 0,
        fromApi: false,
      };
      ...
      try {
        ...
        llmText = (
          await dispatcher.dispatchLLMChatStructured(
            provider,
            apiKey,
            model,
            system,
      ...
      try {
        const { workflowCallTracker } =
          await import('../services/ai-budget/workflow-call-tracker.service.js');
        workflowCallTracker.recordChatBudget({
          provider,
          model,
          inputTokens: chatUsage.input,
          outputTokens: chatUsage.output,
          isError: llmError !== null,
        });
      } catch (e) {
        logger.warn(
          { err: e instanceof Error ? e.message : String(e) },
          '[ai-assistant] recordChatBudget failed (non-fatal)',
        );
      }
      if (llmError !== null) {
        // Context overflow → 413 con messaggio CHIARO (non 500 'Errore interno').
        const { LlmContextOverflowError, LlmProviderUnavailableError } =
          await import('../services/llm-chat.service.js');
        if (llmError instanceof LlmContextOverflowError) {
          logger.warn(
            {
              tenantId,
              estimatedTokens: llmError.estimatedTokens,
              contextWindow: llmError.contextWindow,
            },
        ...
        if (llmError instanceof LlmProviderUnavailableError) {
          logger.warn(
            { tenantId, provider: llmError.provider },
            '[ai-assistant] provider unavailable',
          );
          return c.json(
            { error: llmError.message, code: 'PROVIDER_UNAVAILABLE', provider: llmError.provider },
            503,
          );
        }
      ...
      try {
        ...
        const finalReply: z.infer<typeof AssistantReplySchema> = hasOps
          ? reply
        ...
        const workflowIdFromSnap =
          wfSnapshot &&
          typeof wfSnapshot === 'object' &&
          'id' in wfSnapshot &&
          typeof wfSnapshot.id === 'string'
        ...
        if (
          conversationService.needsCompaction(conversation.id, {
            maxContextTokens: compactionThresholdTokens,
          })
        ) {
          void Promise.resolve().then(async () => {
            try {
              const { trySummarize } =
                await import('@/services/ai-conversations/summary.service.js');
              await trySummarize(conversation.id, provider, apiKey, model, baseUrl, {
                maxContextTokens: compactionThresholdTokens,
              });
            } catch (err) {
              logger.warn(
  ...
  app.get('/ai-assistant/conversations', (c) => {
    ...
    const surface = c.req.query('surface') as
      | 'editor_chat'
      | 'wizard_scaffold'
      | 'help_chat'
      | undefined;
