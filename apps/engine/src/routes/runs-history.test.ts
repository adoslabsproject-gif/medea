...
const m = vi.hoisted(() => {
  class RunNotFoundError extends Error { override name = 'RunNotFoundError'; }
  class RunSucceededError extends Error { override name = 'RunSucceededError'; }
  class NoFailedStepError extends Error { override name = 'NoFailedStepError'; }
  class LlmResponseError extends Error {
    override name = 'LlmResponseError';
    raw: unknown;
    constructor(msg: string, raw: unknown) { super(msg); this.raw = raw; }
  }
  ...
  return {
    ...
    explain: vi.fn(),
    RunNotFoundError, RunSucceededError, NoFailedStepError,
    LlmResponseError, NoLlmProviderError,
  };
});
const { RunNotFoundError, RunSucceededError, NoFailedStepError, LlmResponseError, NoLlmProviderError } = m;
...
function insertRun(over: Partial<Record<string, unknown>> = {}): string {
  ...
  m.db!.prepare('INSERT INTO runs (id, workflow_id, tenant_id, status, trigger_type, input, started_at, steps_json) VALUES (?,?,?,?,?,?,?,?)').run(
    id, over.workflow_id ?? 'wf-1', over.tenant_id ?? 't1', over.status ?? 'success',
    over.trigger_type ?? 'manual', over.input ?? '{}', over.started_at ?? new Date().toISOString(),
    over.steps_json ?? '[]',
  );
  return id;
}
...
function buildApp(auth: Partial<AuthContext> | null, bus?: { emit: (e: unknown) => void }): Hono {
  ...
  app.use('*', async (c, next) => {
    if (auth) {
      const full = { userId: 'u', email: 'e@x', tenantId: 't1', role: 'owner', ...auth } as AuthContext;
      c.set('auth', full);
    }
    await next();
  });
...
describe('GET /runs — list + filter', () => {
  it('happy path tenant-scoped', async () => {
    ...
    const body = await res.json() as { runs: unknown[]; totalCount: number; crossTenant: boolean };
...
describe('POST /runs/bulk-delete', () => {
  ...
  it('happy path: deleted + skipped count, eventBus.emit per ciascuno', async () => {
    ...
    const res = await buildApp({ role: 'owner', tenantId: 't1' }, bus).request('/runs/bulk-delete', {
      method: 'POST', headers: { 'content-type': 'application/json' },
...
describe('DELETE /runs/:runId', () => {
  it('happy path → ok:true + eventBus.emit', async () => {
    ...
    const bus = { emit: vi.fn() };
    const res = await buildApp({ role: 'owner', tenantId: 't1' }, bus).request(`/runs/${id}`, { method: 'DELETE' });
    expect(res.status).toBe(200);
    expect(bus.emit).toHaveBeenCalledTimes(1);
  });

  it('404 quando id non esiste', async () => {
    const res = await buildApp({ role: 'owner', tenantId: 't1' }).request('/runs/fake', { method: 'DELETE' });
    expect(res.status).toBe(404);
  });

  it('🚨 cross-tenant: tenant A non può delete run di B', async () => {
    const id = insertRun({ tenant_id: 'tB' });
    const res = await buildApp({ role: 'owner', tenantId: 'tA' }).request(`/runs/${id}`, { method: 'DELETE' });
    expect(res.status).toBe(404);
  ...
  it('superadmin senza header può delete qualsiasi tenant + emit con tenant REALE', async () => {
    ...
    const bus = { emit: vi.fn() };
    const res = await buildApp({ role: 'superadmin', tenantId: 'platform' }, bus).request(`/runs/${id}`, { method: 'DELETE' });
    expect(res.status).toBe(200);
...
describe('GET /runs/:runId', () => {
  it('happy path con steps parsed', async () => {
    const id = insertRun({ tenant_id: 't1', steps_json: JSON.stringify([{ nodeId: 'n1', status: 'ok' }]) });
...
describe('POST /runs/:runId/ai-explain', () => {
  it('happy path: forward al service', async () => {
    ...
    const res = await buildApp({ role: 'owner', tenantId: 't1' }).request('/runs/run-x/ai-explain', {
      method: 'POST', headers: { 'content-type': 'application/json' },
    });
    expect(res.status).toBe(200);
  ...
  it('RunNotFoundError → 404', async () => {
    ...
    const res = await buildApp({ role: 'owner', tenantId: 't1' }).request('/runs/x/ai-explain', { method: 'POST' });
    expect(res.status).toBe(404);
  });
...
describe('DELETE /runs/:runId/ai-logs — Fase 3 (#15): cancellazione log prompt/risposta', () => {
  const llmLog = (seq: number, msg: string) => ({ ts: '2026-07-06T20:00:00.000Z', seq, level: 'info', source: 'llm', msg, fields: { kind: 'llm_prompt', text: 'segreto' } });
  const engineLog = (seq: number) => ({ ts: '2026-07-06T20:00:00.000Z', seq, level: 'info', source: 'engine', msg: 'retry 1' });
  const stepsWithLogs = () => JSON.stringify([
    { nodeId: 'agent_a', status: 'success', logs: [engineLog(1), llmLog(2, 'prompt·system (1/1)'), llmLog(3, 'risposta (1/1)')], logsTotal: 3 },
    { nodeId: 'agent_b', status: 'success', logs: [llmLog(1, 'prompt·user (1/1)')], logsTotal: 1 },
    { nodeId: 'http_c', status: 'success', logs: [engineLog(1)], logsTotal: 1 },
  ]);

  const readSteps = (runId: string): { nodeId: string; logs: { source: string; msg: string }[] }[] => {
    const row = m.db!.prepare('SELECT steps_json FROM runs WHERE id = ?').get(runId) as { steps_json: string };
  ...
  it('🚨 senza nodeId → strippa i log llm di TUTTI gli step, conserva gli altri, aggiunge marker onesto', async () => {
    ...
    const res = await buildApp({ role: 'owner', tenantId: 't1' }).request(`/runs/${runId}/ai-logs`, { method: 'DELETE' });
    expect(res.status).toBe(200);
  ...
  it('con ?nodeId= → strippa SOLO quel nodo', async () => {
    const runId = insertRun({ tenant_id: 't1', steps_json: stepsWithLogs() });
    const res = await buildApp({ role: 'owner', tenantId: 't1' }).request(`/runs/${runId}/ai-logs?nodeId=agent_b`, { method: 'DELETE' });
  ...
  it('🚨 tenant B NON può cancellare i log di tenant A → 404', async () => {
    const runId = insertRun({ tenant_id: 't1', steps_json: stepsWithLogs() });
    const res = await buildApp({ role: 'owner', tenantId: 't2' }).request(`/runs/${runId}/ai-logs`, { method: 'DELETE' });
    expect(res.status).toBe(404);
  ...
  it('run senza log llm → removed:0, steps_json NON riscritto', async () => {
    const runId = insertRun({ tenant_id: 't1', steps_json: JSON.stringify([{ nodeId: 'x', logs: [engineLog(1)] }]) });
    const res = await buildApp({ role: 'owner', tenantId: 't1' }).request(`/runs/${runId}/ai-logs`, { method: 'DELETE' });
