...
async function req(method: string, path: string, body?: unknown, tenantId = 'tenant-A'): Promise<Response> {
...
describe('🚨 GET /folders — list', () => {
  ...
  it('🚨 ritorna folders del tenant ORDERED by name', async () => {
    ...
    const res = await req('GET', '/api/v1/folders');
    const json = await res.json() as { folders: { name: string; parentId: string | null; createdAt: string }[] };
  ...
  it('🚨 mapping snake_case → camelCase (parentId, createdAt, updatedAt)', async () => {
    ...
    const res = await req('GET', '/api/v1/folders');
    const json = await res.json() as { folders: { parentId: unknown; createdAt: string; updatedAt: string }[] };
...
describe('🚨 POST /folders — create', () => {
  it('🚨 happy: 201 + id + name + parentId=null + timestamps', async () => {
    ...
    const json = await res.json() as { id: string; name: string; parentId: null; createdAt: string; updatedAt: string };
  ...
  it('🚨 SECURITY: parentId di altro tenant → 404 (no cross-tenant)', async () => {
    ...
    const tB = await req('POST', '/api/v1/folders', { name: 'B-Child', parentId: tAJson.id }, 'tenant-B');
...
describe('🚨 DELETE /folders/:id', () => {
  ...
  it('🚨 REFUSE: workflow pinnato → 409 con count', async () => {
    mockDb.sqlite!.prepare(
      'INSERT INTO workflows (id, tenant_id, folder_id) VALUES (?, ?, ?)',
...
describe('🚨 multi-tenant isolation E2E', () => {
  it('🚨 tenant A e B operano in silos completamente isolati', async () => {
    ...
    const listA = await (await req('GET', '/api/v1/folders', undefined, 'tenant-A')).json() as { folders: unknown[] };
    const listB = await (await req('GET', '/api/v1/folders', undefined, 'tenant-B')).json() as { folders: unknown[] };
