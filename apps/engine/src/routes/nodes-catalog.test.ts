...
vi.mock('@medea/engine-nodes-db', () => ({
  dbNodes: [{
    def: {
      ...
      color: '#9333ea', description: 'Run SQL', outputs: ['main'],
    },
  }],
}));
...
vi.mock('@medea/engine-nodes-integrations-italia', () => ({
  italianConnectors: [{
    def: {
      ...
      color: '#fff', description: 'Italian ERP', outputs: ['main'],
    },
  }],
}));
vi.mock('@medea/engine-nodes-ai-agents', () => ({
  aiAgentNodes: [{
    def: {
      ...
      color: '#000', description: 'AI classifier',
      actions: [
        {
          id: 'classify',
          label: 'Classify',
          ...
          aiAction: true,
          configFields: [
            { key: 'categories', label: 'Cat', type: 'text', required: true },
          ],
        },
      ],
      outputs: ['main'],
...
describe('🚨 catalog static (bundled)', () => {
  ...
  it('🚨 mapping: configFields preserved con placeholder/options/defaultValue', async () => {
    const app = makeApp();
    const res = await app.request('/api/v1/nodes');
    const json = await res.json() as { nodes: { id: string; configFields: { key: string; placeholder?: string; options?: string[] }[] }[] };
  ...
  it('🚨 mapping: actions con aiAction + category propagati', async () => {
    const app = makeApp();
    const res = await app.request('/api/v1/nodes');
    const json = await res.json() as { nodes: { id: string; actions?: { aiAction?: boolean; category?: string }[] }[] };
  ...
  it('🚨 configFields default required:false / type:"text" se omessi', async () => {
    const app = makeApp();
    const res = await app.request('/api/v1/nodes');
    const json = await res.json() as { nodes: { id: string; configFields: { required: boolean; type: string }[] }[] };
...
describe('🚨 community nodes merge', () => {
  it('🚨 listInstalled appended dinamicamente con package=community + verified', async () => {
    listInstalledMock.mockReturnValue([{
      def: {
        id: 'community_zapier', type: 'action', label: 'Zapier Webhook',
        icon: '⚡', color: '#000', description: 'Zapier',
        outputs: ['main'],
      },
      verified: true,
    }]);
    const app = makeApp();
    const res = await app.request('/api/v1/nodes');
    const json = await res.json() as { nodes: { id: string; package: string; verified?: boolean }[] };
  ...
  it('🚨 community verified=false → flag preserved', async () => {
    listInstalledMock.mockReturnValue([{
      def: {
        id: 'community_unverified', type: 'action', label: 'X',
        icon: 'x', color: '#000', description: 'X', outputs: [],
      },
      verified: false,
    }]);
    const app = makeApp();
...
describe('🚨 filters combinabili (AND)', () => {
  it('🚨 ?type=action&package=stdlib → solo action stdlib', async () => {
    ...
    const json = await res.json() as { nodes: { id: string; type: string; package: string }[]; total: number };
...
describe('🚨 schema introspection — GET /:defId/schema', () => {
  ...
  it('defId esistente → 200 con configFields + version + outputs', async () => {
    ...
    const res = await app.request('/api/v1/nodes/action_http/schema');
    expect(res.status).toBe(200);
    const body = await res.json() as { defId: string; configFields: unknown[]; version: string | null; outputs: unknown[] };
