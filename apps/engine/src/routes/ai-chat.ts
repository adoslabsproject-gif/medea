...
import { detectIntents, executeIntents, formatToolResultsForPrompt } from '@/services/chat-intent.service.js';
...
const SURFACE_PROMPTS: Record<string, string> = {
  ...
  scaffold: 'Sei **Liara**, l\'assistente AI di FlowForge (Zeli SRL) che aiuta a costruire workflow. Rispondi in italiano. Quando proponi un workflow, elenca i nodi in ordine.',
...
export function createAiChatRoutes(): Hono {
  ...
  app.post('/sessions', zValidator('json', CreateSessionSchema), (c) => {
    ...
    return c.json({ session: { id, tenantId, userId: auth.userId, surface, title: title ?? null, workflowId: workflowId ?? null } }, 201);
  });
  ...
  app.get('/sessions', (c) => {
    ...
    if (surface) { filters.push('surface = ?'); params.push(surface); }
   ...
   *      enableTools=false.
   */
  app.post('/sessions/:id/messages', llmRateLimit('ai-chat'), zValidator('json', SendMessageSchema), async (c) => {
    const auth = c.get('auth') as { userId: string } | null;
    if (!auth) return c.json({ error: 'Unauthorized' }, 401);
    const tenantId = getTenantId(c);
    const id = c.req.param('id');
    ...
    const attachmentsJson = enrichedAttachments.length > 0 ? JSON.stringify(enrichedAttachments) : null;
    sqlite.prepare(`INSERT INTO ai_chat_messages (session_id, role, content, attachments_json) VALUES (?, 'user', ?, ?)`)
    ...
    try {
      ...
      if (err instanceof NoLlmProviderError) return c.json({ error: 'no_provider', message: err.message }, 503);
      throw err;
    }
    ...
    // ─────────────────────────────────────────────────────────────────
    const ALIAS_PROMPT_ROW = 'ALIAS COMUNI (n8n-speak → defId): "code node"/"nodo code"/"function" → action_run_js (JavaScript, default come n8n) o action_run_python (se l\'utente vuole Python) · "set/edit fields" → logic_transform · "http request" → action_http · "if" → logic_if · "filtro" → action_filter · "switch" → logic_switch · "loop/split in batches" → logic_loop.\nQuando l\'utente chiede di un nodo, verifica QUI (defId, label E alias) prima di dire che non esiste.';
    try {
      ...
      logger.warn({ err: retrErr instanceof Error ? retrErr.message : String(retrErr) }, '[ai-chat] catalog retriever failed — fallback catalogo compatto');
      try {
    ...
    if (enableTools !== false) {
      ...
      if (detectedIntents.length > 0) {
        const executed = await executeIntents(detectedIntents, {
          provider: resolved.provider, apiKey: resolved.apiKey, model: resolved.model, baseUrl: resolved.baseUrl,
        });
    ...
    try {
      ...
      if (enableTools !== false) {
        const augmentedHistory = [...earlierHistory, { role: 'user' as const, content: augmentedUserMessage }];
        for (let iter = 0; iter < 3; iter++) {
          const cleaned = reply.trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```\s*$/i, '').trim();
          if (!cleaned.startsWith('{')) break;  // risposta normale, non tool call
          let toolCall: { tool?: unknown; args?: unknown };
          try { toolCall = JSON.parse(cleaned) as typeof toolCall; }
          catch { break; }  // non è valid JSON, è una risposta normale
          ...
          try {
            if (toolName === 'fetch_url') {
              ...
              toolResult = JSON.stringify({ ok: true, ...r, content: r.content.slice(0, 20_000) });
            ...
            toolResult = JSON.stringify({ ok: false, error: e instanceof Error ? e.message : String(e) });
          }
          // Ri-prompt con tool_result iniettato come messaggio user
          augmentedHistory.push({ role: 'assistant', content: cleaned });
          augmentedHistory.push({ role: 'user', content: `tool_result(${toolName}): ${toolResult}` });
      ...
      if (!session.title) {
        ...
        sqlite.prepare(`UPDATE ai_chat_sessions SET title = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ?`).run(newTitle, id);
      } else {
        sqlite.prepare(`UPDATE ai_chat_sessions SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ?`).run(id);
      }
      ...
      return c.json({ error: 'dispatch_failed', message: err instanceof Error ? err.message : String(err) }, 502);
    }
  });
 ...
 */
export function listAllSessionsForSuperadmin(filters: { tenantId?: string; userId?: string; surface?: string }): SessionRow[] {
  const { sqlite } = getDatabase();
  const where: string[] = [];
  const params: unknown[] = [];
  if (filters.tenantId) { where.push('tenant_id = ?'); params.push(filters.tenantId); }
  if (filters.userId) { where.push('user_id = ?'); params.push(filters.userId); }
  if (filters.surface) { where.push('surface = ?'); params.push(filters.surface); }
