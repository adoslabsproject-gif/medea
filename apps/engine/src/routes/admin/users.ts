...
export function registerUsersRoutes(app: Hono): void {
  app.get('/admin/users', (c) => {
    ...
    const rows = sqlite
      .prepare('SELECT id, tenant_id, email, display_name, role, enabled, created_at, last_login_at FROM users ORDER BY tenant_id, created_at DESC')
      .all() as UserRow[];
    return c.json({
      users: rows.map((r) => ({
        id: r.id,
  ...
  app.patch('/admin/users/:userId/role', zValidator('json', PromoteRoleSchema), async (c) => {
    ...
    const target = sqlite.prepare('SELECT id, email, role, tenant_id, enabled FROM users WHERE id = ?').get(userId) as { id: string; email: string; role: string; tenant_id: string; enabled: number } | undefined;
    ...
    try {
      ...
      if (e instanceof SuperadminSafetyError) return c.json({ error: e.message, reason: e.reason }, 403);
      throw e;
    }

    sqlite.prepare('UPDATE users SET role = ?, updated_at = ? WHERE id = ?').run(role, new Date().toISOString(), userId);
  ...
  app.patch('/admin/users/:userId/enabled', zValidator('json', z.object({ enabled: z.boolean() })), async (c) => {
    const auth = c.get('auth');
    const userId = c.req.param('userId');
    if (!userId) return c.json({ error: 'Bad request' }, 400);
    ...
    const target = sqlite.prepare('SELECT id, role, enabled, tenant_id FROM users WHERE id = ?').get(userId) as { id: string; role: string; enabled: number; tenant_id: string } | undefined;
    if (!target) return c.json({ error: 'User not found' }, 404);
    ...
    sqlite.prepare('UPDATE users SET enabled = ?, updated_at = ? WHERE id = ?').run(enabled ? 1 : 0, new Date().toISOString(), userId);
