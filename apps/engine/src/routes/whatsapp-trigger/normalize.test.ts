...
describe('extractWhatsAppEvents — messaggi', () => {
  it('text: estrae from/id/testo/profilo/metadata e timestamp ISO 8601', () => {
    const { messages, statuses } = extractWhatsAppEvents(metaEnvelope({
      metadata: METADATA,
      contacts: [CONTACT],
  ...
  it('interactive button_reply: id+title in interactive, title come text', () => {
    const { messages } = extractWhatsAppEvents(metaEnvelope({
      metadata: METADATA,
      messages: [textMessage({
        type: 'interactive',
        text: undefined,
        interactive: { type: 'button_reply', button_reply: { id: 'ORDINA_SOLITA', title: 'La solita 🍕' } },
      })],
    }));
  ...
  it('interactive list_reply: stessa normalizzazione del button_reply', () => {
    const { messages } = extractWhatsAppEvents(metaEnvelope({
      ...
      messages: [textMessage({
        type: 'interactive',
        text: undefined,
        interactive: { type: 'list_reply', list_reply: { id: 'PIZZA_4', title: 'Quattro Stagioni', description: 'pomodoro…' } },
      })],
    }));
  ...
  it('image con caption: media ref completo + caption come text (byte NON inclusi)', () => {
    const { messages } = extractWhatsAppEvents(metaEnvelope({
      ...
      messages: [textMessage({
        type: 'image',
        text: undefined,
        image: { id: 'MEDIA-9', mime_type: 'image/jpeg', sha256: 'abc123', caption: 'la mia fattura' },
      })],
    }));
    const m = messages[0]!;
    expect(m.media).toEqual({ id: 'MEDIA-9', mimeType: 'image/jpeg', sha256: 'abc123', caption: 'la mia fattura', filename: null });
  ...
  it('location: coordinate numeriche obbligatorie, name/address opzionali', () => {
    ...
    expect(messages[0]!.location).toEqual({ latitude: 41.9, longitude: 12.49, name: 'Casa', address: 'Via Roma 1' });
  });

  it('🚨 location con coordinate stringa (malformata) → location null, messaggio comunque estratto', () => {
    const { messages } = extractWhatsAppEvents(metaEnvelope({
      metadata: METADATA,
      messages: [textMessage({ type: 'location', text: undefined, location: { latitude: '41.9', longitude: '12.49' } })],
    }));
    expect(messages).toHaveLength(1);
  ...
  it('reaction: emoji come text', () => {
    const { messages } = extractWhatsAppEvents(metaEnvelope({
      metadata: METADATA,
      messages: [textMessage({ type: 'reaction', text: undefined, reaction: { message_id: 'wamid.X', emoji: '👍' } })],
    }));
...
describe('extractWhatsAppEvents — statuses', () => {
  it('status delivered estratto con recipient e kind=status', () => {
    const { statuses } = extractWhatsAppEvents(metaEnvelope({
      metadata: METADATA,
      statuses: [{ id: 'wamid.SENT1', status: 'delivered', timestamp: '1751810400', recipient_id: '393331234567' }],
    }));
    expect(statuses).toHaveLength(1);
    expect(statuses[0]).toMatchObject({ kind: 'status', messageId: 'wamid.SENT1', status: 'delivered', recipientId: '393331234567', phoneNumberId: 'PNID-1' });
  });
...
describe('extractWhatsAppEvents — bug-bounty payload rotti (mai throw)', () => {
  it.each([
    ...
    ['messages non-array', { entry: [{ changes: [{ field: 'messages', value: { messages: 'x' } }] }] }],
