...
const componentsArray = z.string().optional().default('').transform((raw, ctx) => {
...
export const WhatsAppSendConfigSchema = z.object({
  // ── Authentication
  phoneNumberId: z.string()
    .regex(/^[0-9]+$/, 'phoneNumberId must be digits-only (NOT the phone number itself — that\'s the recipient)')
    .min(8).max(30),
  ...
  if (cfg.mode === 'text') {
    ...
    if (!cfg.templateName) addRequired(['templateName'], 'templateName required for mode=template');
    if (!cfg.languageCode) addRequired(['languageCode'], 'languageCode required for mode=template');
  }
});
