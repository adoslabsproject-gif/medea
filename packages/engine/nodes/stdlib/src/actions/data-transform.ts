...
function asArray(input: unknown, config: Record<string, unknown>, key = 'items'): unknown[] {
  ...
  if (typeof src === 'string' && src.trim() !== '') {
    try {
      ...
      /* non-JSON string → single-item array */
    }
    return [src];
  }
...
function stringifyCsv(items: unknown[], delimiter: string, includeHeader: boolean): string {
  ...
  const headers = Array.from(
    items.reduce<Set<string>>((set, it) => {
  ...
  if (includeHeader && headers.length > 0)
    lines.push(headers.map((h) => csvCell(h, delimiter)).join(delimiter));
  for (const it of items) {
    if (isPlainObject(it))
      lines.push(headers.map((h) => csvCell(it[h], delimiter)).join(delimiter));
...
const arrayExecutor: NodeExecutor = async (config, input) => {
  ...
  switch (op) {
    ...
    case 'pick': {
      const fields = str(config.fields)
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
      result = items.map((it) => {
  ...
  if (items === input) {
    if (declaredIdx !== undefined) {
      declaredItems = declaredIdx.map((orig) => ({
        ...toExecutionItem(items[orig]),
        pairedItem: lineage.from(orig),
      }));
...
const jsonExecutor: NodeExecutor = async (config, input) => {
  ...
  const source: unknown =
    typeof rawSrc === 'string'
      ? (() => {
          try {
            return JSON.parse(rawSrc) as unknown;
          } catch {
            return rawSrc;
          }
        })()
  ...
  switch (op) {
    ...
    case 'set': {
      ...
      try {
        value = JSON.parse(str(config.value));
      } catch {
        /* tieni stringa */
      }
    ...
    case 'pick': {
      ...
      for (const p of paths) {
        ...
        if (v !== undefined) obj[p.split('.').pop()!] = v;
      }
      result = obj;
      break;
    }
    case 'omit': {
      ...
      if (isPlainObject(source))
        for (const [k, v] of Object.entries(source)) if (!drop.has(k)) obj[k] = v;
      result = obj;
      break;
    }
    case 'merge': {
      let other: unknown = {};
      try {
        other = JSON.parse(str(config.value, '{}'));
      } catch {
        throw new Error('action_json: "valore da unire" non è JSON valido');
      }
...
export const jsonNode: NodeModule = {
  def: {
    ...
    configFields: [
      ...
      {
        key: 'paths',
        label: 'Percorsi (separati da virgola)',
        type: 'text',
        required: false,
