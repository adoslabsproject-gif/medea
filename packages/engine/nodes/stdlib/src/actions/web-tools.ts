...
export const fetchUrlNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Scarica una pagina e ne restituisce il TESTO leggibile in `content`, non l\'HTML grezzo. `truncated` a vero significa che la pagina e` stata tagliata: quello che c\'era dopo non e` in `content`.',
      fields: [
        { name: 'url', type: 'string', desc: 'L\'indirizzo chiesto.' },
        { name: 'finalUrl', type: 'string', desc: 'Dove e` arrivato dopo i reindirizzamenti: puo` essere diverso da `url`.' },
        { name: 'status', type: 'number', desc: 'Il codice HTTP della risposta.' },
        { name: 'title', type: 'string', desc: 'Il titolo della pagina.' },
        { name: 'description', type: 'string', desc: 'La descrizione dichiarata dalla pagina.' },
        { name: 'content', type: 'string', desc: 'Il testo leggibile della pagina, ripulito dai tag.' },
        { name: 'contentType', type: 'string', desc: 'Il tipo di contenuto restituito dal server.' },
        { name: 'truncated', type: 'boolean', desc: 'Vero se la pagina e` stata tagliata perche` troppo lunga.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const webSearchNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'I risultati stanno in `results`, e ognuno ha titolo, indirizzo e frammento: si scorrono con un `logic_loop`. Nessun risultato non e` un errore — lista vuota e `count` a 0.',
      fields: [
        { name: 'query', type: 'string', desc: 'La ricerca eseguita.' },
        { name: 'provider', type: 'string', desc: 'Il motore interpellato.' },
        { name: 'results', type: 'array', desc: 'I risultati, ognuno con titolo, indirizzo e frammento di testo.' },
        { name: 'count', type: 'number', desc: 'Quanti risultati.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
