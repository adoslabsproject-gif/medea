...
export const memoryNoteNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'I campi cambiano con l\'operazione. LEGGENDO escono `exists`, `value`, `usedDefault` e `operation`. SCRIVENDO escono anche `updatedAt` ed `expiresAt`. MODIFICANDO escono in piu` `oldValue` e `changed`. CANCELLANDO `value` e` null. ELENCANDO escono `keys`, `count` e `pattern` — e NON `value`. `exists` falso con `usedDefault` vero significa che il valore letto e` il ripiego, non un dato salvato.',
      fields: [
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'exists', type: 'boolean', desc: 'Se la nota esisteva davvero.' },
        { name: 'value', type: 'string|null', desc: 'Il valore della nota. Null dopo una cancellazione.' },
        { name: 'usedDefault', type: 'boolean', desc: 'Vero se il valore viene dal ripiego e non dalla memoria.' },
        { name: 'oldValue', type: 'string|null', desc: 'Il valore precedente. Solo modificando.' },
        { name: 'changed', type: 'boolean', desc: 'Se il valore e` davvero cambiato. Solo modificando.' },
        { name: 'updatedAt', type: 'number', desc: 'Quando e` stata scritta, in millisecondi dal 1970.' },
        { name: 'expiresAt', type: 'number', desc: 'Quando scade. Presente se e` stata data una durata.' },
        { name: 'keys', type: 'array', desc: 'I nomi delle note trovate. Solo elencando.' },
        { name: 'count', type: 'number', desc: 'Quante note. Solo elencando.' },
        { name: 'pattern', type: 'string', desc: 'Il filtro applicato all\'elenco. Solo elencando.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
