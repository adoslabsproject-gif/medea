...
const domainArray = jsonStringOf<unknown[]>((p) => {
  if (!Array.isArray(p))
    throw new ValidationError(`odoo_rpc: domain must be a JSON array of triples`, {
      field: 'domain',
    });
  return p;
}, 'domain (array)');
...
const fieldsArray = jsonStringOf<string[]>((p) => {
  if (!Array.isArray(p))
    throw new ValidationError(`odoo_rpc: fields must be a JSON array of strings`, {
      field: 'fields',
    });
  for (const x of p)
    if (typeof x !== 'string')
      throw new ValidationError(`odoo_rpc: every field must be a string`, { field: 'fields' });
  return p as string[];
...
const idsArray = jsonStringOf<number[]>((p) => {
  if (!Array.isArray(p))
    throw new ValidationError(`odoo_rpc: recordIds must be a JSON array of positive integers`, {
      field: 'recordIds',
    });
  const out: number[] = [];
  for (const x of p) {
    const n =
      typeof x === 'number' ? x : typeof x === 'string' && /^\d+$/.test(x) ? parseInt(x, 10) : NaN;
    if (!Number.isInteger(n) || n <= 0)
      throw new ValidationError(`odoo_rpc: every recordId must be a positive integer`, {
        field: 'recordIds',
      });
    out.push(n);
  }
...
const positionalArray = jsonStringOf<unknown[]>((p) => {
  if (!Array.isArray(p))
    throw new ValidationError(`odoo_rpc: positional must be a JSON array`, { field: 'positional' });
  return p;
...
export const OdooRpcConfigSchema = z
  .object({
    // ── Authentication
    baseUrl: httpsUrl,
  ...
  .superRefine((cfg, ctx) => {
    ...
    switch (cfg.operation) {
      ...
      case 'call_method':
        if (!cfg.methodName)
          addRequired(['methodName'], 'methodName is required for operation=call_method');
        break;
    }
  });
