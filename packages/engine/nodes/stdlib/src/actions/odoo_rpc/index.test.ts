...
import { OdooFaultError, __clearOdooAuthCacheForTests } from '../../lib/odoo/xml-rpc-client.js';
...
function boolRsp(b: boolean): Response {
  return rsp(
    `<?xml version="1.0"?><methodResponse><params><param><value><boolean>${b ? '1' : '0'}</boolean></value></param></params></methodResponse>`,
  );
}
      ...
      const members = Object.entries(it)
        .map(([k, v]) => {
          ...
          if (typeof v === 'number') valueXml = `<value><int>${v}</int></value>`;
            ...
            valueXml = `<value><boolean>${v ? '1' : '0'}</boolean></value>`;
...
describe('odooRpcExecutor — operation=search_read', () => {
  it('returns the decoded array + count', async () => {
    mockedFetch.mockResolvedValueOnce(authOk(2)).mockResolvedValueOnce(
      arrayOfStructsRsp([
...
describe('odooRpcExecutor — operation=create', () => {
  it('returns the new id and exposes createdId', async () => {
    mockedFetch.mockResolvedValueOnce(authOk(2)).mockResolvedValueOnce(intRsp(42));
    const out = await odooRpcExecutor(
      {
        ...BASE_CFG,
        operation: 'create',
...
describe('odooRpcExecutor — operation=write', () => {
  it('returns success=true on a boolean true response', async () => {
    mockedFetch.mockResolvedValueOnce(authOk(2)).mockResolvedValueOnce(boolRsp(true));
    const out = await odooRpcExecutor(
      {
        ...BASE_CFG,
        operation: 'write',
...
describe('odooRpcExecutor — pipelineSteps', () => {
  it('emits authenticate + execute_kw steps', async () => {
    mockedFetch.mockResolvedValueOnce(authOk(3)).mockResolvedValueOnce(intRsp(99));
    const out = await odooRpcExecutor(
      {
        ...BASE_CFG,
        operation: 'create',
...
describe('odooRpcExecutor — error mapping', () => {
  ...
  it('throws AbortedError when context.abortSignal is already aborted', async () => {
    ...
    ctrl.abort();
    await expect(
      odooRpcExecutor({ ...BASE_CFG, operation: 'search_read' }, null, {
        ...ctx,
