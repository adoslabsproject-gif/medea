...
export const gmailNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Tre forme, secondo l\'operazione. MANDANDO o modificando escono `id`, `threadId` e `labelIds`. CERCANDO escono `count`, `resultSizeEstimate` e `messages`. LEGGENDO un messaggio esce tutto: mittente, destinatari, oggetto, corpo e allegati. `ok` c\'e` sempre.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se l\'operazione e` riuscita.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'id', type: 'string', desc: 'Il messaggio interessato. Non c\'e` cercando.' },
        { name: 'threadId', type: 'string', desc: 'La conversazione a cui appartiene.' },
        { name: 'labelIds', type: 'array', desc: 'Le etichette del messaggio.' },
        { name: 'count', type: 'number', desc: 'Quanti messaggi ha riportato. Solo cercando.' },
        { name: 'resultSizeEstimate', type: 'number', desc: 'La stima di Gmail sul totale dei risultati: puo` essere molto piu` grande di `count`.' },
        { name: 'messages', type: 'array', desc: 'I messaggi trovati. Solo cercando.' },
        { name: 'from', type: 'string', desc: 'Il mittente. Solo leggendo un messaggio.' },
        { name: 'to', type: 'string', desc: 'I destinatari. Solo leggendo.' },
        { name: 'cc', type: 'string', desc: 'I destinatari in copia. Solo leggendo.' },
        { name: 'subject', type: 'string', desc: 'L\'oggetto. Solo leggendo.' },
        { name: 'date', type: 'string', desc: 'La data del messaggio. Solo leggendo.' },
        { name: 'snippet', type: 'string', desc: 'L\'anteprima che mostra Gmail. Solo leggendo.' },
        { name: 'bodyText', type: 'string', desc: 'Il corpo testuale. Solo leggendo.' },
        { name: 'bodyHtml', type: 'string', desc: 'Il corpo HTML. Solo leggendo.' },
        { name: 'attachments', type: 'array', desc: 'Gli allegati del messaggio. Solo leggendo.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
