...
describe('action_number', () => {
  it('round half-away + epsilon: 1.005→1.01, 8.575→8.58', async () => {
    expect((await out(num, { operation: 'round', value: '1.005', decimals: '2' })).result).toBe(
      1.01,
    );
    expect((await out(num, { operation: 'round', value: '8.575', decimals: '2' })).result).toBe(
      8.58,
    );
  });
  ...
  it('🚨 round con decimals abnorme (500) → niente NaN (clamp a 100)', async () => {
    ...
    const r = (await out(num, { operation: 'round', value: '3.14159', decimals: '500' }))
      .result as number;
  ...
  it('clamp vincola tra min e max', async () => {
    expect(
      (await out(num, { operation: 'clamp', value: '150', min: '0', max: '100' })).result,
    ).toBe(100);
    expect((await out(num, { operation: 'clamp', value: '-5', min: '0', max: '100' })).result).toBe(
      0,
    );
  });
  ...
  it('🚨 input non numerico → throw esplicito', async () => {
    await expect(num({ operation: 'round', value: 'pippo' }, undefined, ctx)).rejects.toThrow(
      /non numerico/,
    );
  });
  it('🚨 operazione sconosciuta → throw', async () => {
    await expect(num({ operation: 'nope', value: '1' }, undefined, ctx)).rejects.toThrow(
      /sconosciuta/,
    );
  });
