...
describe('action_set_fields', () => {
  it('imposta campi annidati con conversione di tipo', async () => {
    const r = await setFields({ source: '{}', assignments: '{"cliente.stato":"attivo","eta":"30","vip":"true"}' }, undefined, ctx);
  ...
  it('rename sposta un campo', async () => {
    const r = await setFields({ source: '{"Email":"x@y.it"}', renameFields: '{"Email":"email"}' }, undefined, ctx);
    const res = (r.output as { result: Record<string, unknown> }).result;
    expect(res.email).toBe('x@y.it');
    expect(res).not.toHaveProperty('Email');
  });
  it('remove elimina i campi', async () => {
    const r = await setFields({ source: '{"a":1,"password":"x"}', removeFields: 'password' }, undefined, ctx);
    expect((r.output as { result: Record<string, unknown> }).result).not.toHaveProperty('password');
  });
  it('keepOnly = whitelist totale', async () => {
    const r = await setFields({ source: '{"a":1,"b":2}', assignments: '{"c":"3"}', keepOnly: 'true' }, undefined, ctx);
...
describe('action_coalesce', () => {
  it('sceglie il primo valore presente', async () => {
    const r = await coalesce({ source: '{"a":"","b":"valore","c":"altro"}', paths: 'a,b,c' }, undefined, ctx);
  ...
  it('usa il default se nessuna sorgente ha valore', async () => {
    const r = await coalesce({ source: '{"a":null,"b":""}', paths: 'a,b', default: 'Cliente' }, undefined, ctx);
    const o = r.output as { result: unknown; from: string; found: boolean };
  ...
  it('stringa vuota saltata solo se treatEmptyAsMissing', async () => {
    const r = await coalesce({ source: '{"a":""}', paths: 'a', treatEmptyAsMissing: 'false', default: 'x' }, undefined, ctx);
...
describe('action_filter', () => {
  ...
  it('AND di più regole, branch kept', async () => {
    const r = await filter({ items: JSON.stringify(orders), conditions: JSON.stringify({ combinator: 'AND', rules: [{ field: 'total', op: 'gt', value: 100 }, { field: 'region', op: 'equals', value: 'Lazio' }] }) }, undefined, ctx);
  ...
  it('OR combina le regole', async () => {
    const r = await filter({ items: JSON.stringify(orders), conditions: JSON.stringify({ combinator: 'OR', rules: [{ field: 'total', op: 'lt', value: 60 }, { field: 'region', op: 'equals', value: 'Lazio' }] }) }, undefined, ctx);
  ...
  it('operatore regex su dot-path', async () => {
    ...
    const r = await filter({ items: JSON.stringify(data), conditions: JSON.stringify({ rules: [{ field: 'user.email', op: 'not_contains', value: 'gmail' }] }) }, undefined, ctx);
    expect((r.output as { keptCount: number }).keptCount).toBe(1);
  });
  it('operatore "in" lista', async () => {
    const r = await filter({ items: JSON.stringify(orders), conditions: JSON.stringify({ rules: [{ field: 'region', op: 'in', value: 'Lazio,Veneto' }] }) }, undefined, ctx);
    expect((r.output as { keptCount: number }).keptCount).toBe(2);
  });
  it('nessuna regola → tiene tutto', async () => {
    const r = await filter({ items: JSON.stringify(orders), conditions: JSON.stringify({ rules: [] }) }, undefined, ctx);
    expect((r.output as { keptCount: number }).keptCount).toBe(3);
  });
});

describe('action_html_extract', () => {
  const html = '<html><head><title>Pagina &amp; Test</title><meta name="description" content="desc"><meta property="og:image" content="img.jpg"></head><body><h1>Titolo</h1><h2>Sub</h2><p>Ciao <a href="https://zeli.it">link</a></p><img src="/a.png"><script>var x=1</script></body></html>';
  ...
  it('links → href + text', async () => {
    ...
    expect((r.output as { result: { href: string; text: string }[] }).result[0]).toEqual({ href: 'https://zeli.it', text: 'link' });
  });
  ...
  it('headings con livello', async () => {
    ...
    expect((r.output as { result: { level: number; text: string }[] }).result).toEqual([{ level: 1, text: 'Titolo' }, { level: 2, text: 'Sub' }]);
  });
  it('images src', async () => {
...
describe('action_markdown', () => {
  it('heading + bold + italic + link', async () => {
    const r = await markdown({ markdown: '# Titolo\n\nTesto **forte** e *enfasi* e [link](https://zeli.it)' }, undefined, ctx);
...
describe('action_diff', () => {
  it('deep diff oggetti: added/removed/changed', async () => {
    const r = await diff({ mode: 'object', a: '{"nome":"Anna","eta":30,"citta":"Roma"}', b: '{"nome":"Anna","eta":31,"tel":"123"}' }, undefined, ctx);
    const o = r.output as { added: Record<string, unknown>; removed: Record<string, unknown>; changed: Record<string, { from: unknown; to: unknown }>; equal: boolean };
  ...
  it('diff annidato dot-path', async () => {
    const r = await diff({ mode: 'object', a: '{"u":{"nome":"A"}}', b: '{"u":{"nome":"B"}}' }, undefined, ctx);
    expect((r.output as { changed: Record<string, { from: unknown; to: unknown }> }).changed['u.nome']).toEqual({ from: 'A', to: 'B' });
  });
...
describe('action_mock_data', () => {
  it('genera N record secondo lo schema', async () => {
    const r = await mock({ schema: '{"nome":"fullName","mail":"email"}', count: '5' }, undefined, ctx);
  ...
  it('seed → output riproducibile (deterministico)', async () => {
    const a = (await mock({ schema: '{"n":"fullName"}', count: '3', seed: 'fisso' }, undefined, ctx)).output as { items: unknown[] };
    const b = (await mock({ schema: '{"n":"fullName"}', count: '3', seed: 'fisso' }, undefined, ctx)).output as { items: unknown[] };
    expect(a.items).toEqual(b.items);
  });
  it('seed diversi → output diversi', async () => {
    const a = (await mock({ schema: '{"n":"fullName"}', count: '5', seed: 'A' }, undefined, ctx)).output as { items: unknown[] };
    const b = (await mock({ schema: '{"n":"fullName"}', count: '5', seed: 'B' }, undefined, ctx)).output as { items: unknown[] };
    expect(a.items).not.toEqual(b.items);
  });
  it('uuid valido + phone italiano', async () => {
    const r = await mock({ schema: '{"id":"uuid","tel":"phone"}', count: '1', seed: 's' }, undefined, ctx);
  ...
  it('schema vuoto → default (id, nome, email, citta)', async () => {
    ...
    expect(Object.keys((r.output as { items: Record<string, unknown>[] }).items[0]!)).toEqual(['id', 'nome', 'email', 'citta']);
  });
});
