...
describe('action_url', () => {
  it('parse scompone tutte le parti + query come oggetto', async () => {
    const r = await out({
      operation: 'parse',
      url: 'https://api.x.it:8443/v1/o?code=ABC&x=1#frag',
    });
  ...
  it.each(['%', '%ZZ', '%E0%A4%A', 'abc%'])(
    '🚨 decode di percent-encoding malformato "%s" → errore CHIARO (no URIError grezzo)',
    async (bad) => {
      // MUTATION: senza il try/catch, decodeURIComponent lancia URIError non gestito.
      await expect(url({ operation: 'decode', url: bad }, undefined, ctx)).rejects.toThrow(
        /percent-encoding non valida/,
      );
    },
  );
  ...
  it('🚨 operazione sconosciuta → throw', async () => {
    await expect(url({ operation: 'nope', url: 'https://x.it' }, undefined, ctx)).rejects.toThrow(
      /sconosciuta/,
    );
  });
});
