...
function asSentinel(output: unknown): { status: number; contentType: string; body: string; headers: Record<string, string> } {
  ...
  return p as { status: number; contentType: string; body: string; headers: Record<string, string> };
}

describe('action_api_response — success', () => {
  it('envelope { ok:true, data } + status 200 + content-type json', async () => {
    const r = await api({ mode: 'success', data: '{"id":1,"nome":"Anna"}', statusSuccess: '200' }, undefined, ctx);
  ...
  it('status 201 Created', async () => {
    const r = await api({ mode: 'success', data: '{"created":true}', statusSuccess: '201' }, undefined, ctx);
    expect(asSentinel(r.output).status).toBe(201);
  });
  it('meta paginazione inclusa nell\'envelope', async () => {
    const r = await api({ mode: 'success', data: '[1,2,3]', meta: '{"page":1,"total":3}' }, undefined, ctx);
...
describe('action_api_response — error (codice → status)', () => {
  it('not_found → 404 + envelope error', async () => {
    const r = await api({ mode: 'error', errorCode: 'not_found', errorMessage: 'Ordine inesistente' }, undefined, ctx);
    const s = asSentinel(r.output);
    expect(s.status).toBe(404);
    expect(JSON.parse(s.body)).toEqual({ ok: false, error: { code: 'not_found', message: 'Ordine inesistente' } });
  });
  it('validation_error → 422 con details strutturati', async () => {
    const r = await api({ mode: 'error', errorCode: 'validation_error', errorMessage: 'Input non valido', errorDetails: '{"field":"email","reason":"formato"}' }, undefined, ctx);
    const s = asSentinel(r.output);
  ...
  it('rate_limited → 429, server_error → 500, unauthorized → 401', async () => {
    expect(asSentinel((await api({ mode: 'error', errorCode: 'rate_limited' }, undefined, ctx)).output).status).toBe(429);
    expect(asSentinel((await api({ mode: 'error', errorCode: 'server_error' }, undefined, ctx)).output).status).toBe(500);
    expect(asSentinel((await api({ mode: 'error', errorCode: 'unauthorized' }, undefined, ctx)).output).status).toBe(401);
  });
...
describe('action_api_response — CORS + headers', () => {
  it('CORS abilitato aggiunge gli header standard', async () => {
    const r = await api({ mode: 'success', data: '{}', cors: 'true', corsOrigin: 'https://app.x.it', corsCredentials: 'true' }, undefined, ctx);
  ...
  it('header personalizzati passano', async () => {
    const r = await api({ mode: 'success', data: '{}', headers: '{"Cache-Control":"no-store","X-Api-Version":"1"}' }, undefined, ctx);
    const h = asSentinel(r.output).headers;
    expect(h['Cache-Control']).toBe('no-store');
    expect(h['X-Api-Version']).toBe('1');
  });
  it('guardia anti header-injection: valori con CRLF scartati', async () => {
    const r = await api({ mode: 'success', data: '{}', headers: '{"X-Bad":"a\\r\\nSet-Cookie: evil=1","X-Ok":"v"}' }, undefined, ctx);
    const h = asSentinel(r.output).headers;
  ...
  it('guardia anti header-injection: NOME header non-token (CRLF nella chiave) scartato', async () => {
    ...
    const r = await api({ mode: 'success', data: '{}', headers: '{"X-Bad\\r\\nSet-Cookie":"evil","X-Ok":"v"}' }, undefined, ctx);
    const h = asSentinel(r.output).headers;
  ...
  it('guardia anti header-injection: corsOrigin con CRLF strippato', async () => {
    const r = await api({ mode: 'success', data: '{}', cors: 'true', corsOrigin: 'https://ok.it\r\nX-Injected: 1' }, undefined, ctx);
    const h = asSentinel(r.output).headers;
