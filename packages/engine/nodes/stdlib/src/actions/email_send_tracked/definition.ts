...
export const emailSendTrackedNodeDef: NodeDef = {
  ...
  configFields: [
    // ────────── Sender ──────────
    {
      ...
      required: false,
      help:
        'Scegli un account configurato in Settings → Email accounts (con Gmail OAuth ' +
    ...
    // ────────── Envelope ──────────
    {
      ...
      help:
        'Indirizzo "From" mostrato al destinatario. Se vuoto e usi un account di sistema, ' +
    ...
    },
    {
      ...
      help:
        'Uno o più indirizzi separati da virgola. Il pixel di apertura è UNICO per ' +
    ...
    },
    {
      ...
      help:
        'Massimo 998 caratteri (limite RFC). I caratteri non-ASCII vengono codificati ' +
    ...
    // ────────── Body ──────────
    {
      ...
      defaultValue: 'html',
      help:
        'HTML = formattazione + immagini + tracking pixel. Text = solo testo, pixel non iniettato ' +
    ...
    },
    {
      ...
      help:
        'HTML completo o frammento (senza <html>/<body> — vengono aggiunti automaticamente). ' +
    ...
    // ────────── Tracking identity ──────────
    {
      ...
      help:
        'ID del lead nel database. Tipicamente arriva da un nodo db_query upstream. ' +
    ...
    },
    {
      ...
      help:
        'Stringa libera che raggruppa tutti gli invii della stessa campagna. ' +
        'Usata per gli analytics "open-rate per campagna". ' +
        'Convenzione consigliata: <brand>-<anno>-<week>-<segmento>-<area>.',
    },
    {
      key: 'sendId',
      label: 'Send ID (opzionale)',
      type: 'text',
      required: false,
      placeholder: 'lasciare vuoto = auto-generato',
      help:
        'Identifica univocamente questo singolo invio. Se vuoto viene generato un uuid. ' +
    ...
    {
      key: 'trackOpens',
      label: 'Traccia aperture (pixel)',
      type: 'boolean',
      required: false,
      defaultValue: 'true',
      help:
        'On: inietta un img 1x1 GIF trasparente che chiama il runtime quando il client ' +
    ...
    {
      key: 'trackClicks',
      label: 'Traccia click (URL rewrite)',
      type: 'boolean',
      required: false,
      defaultValue: 'true',
      help:
        'On: ogni <a href> viene riscritto in un redirect tracciato. Spegni quando il body ' +
        'contiene link sensibili (es. password reset) che NON vuoi loggare.',
    },
    {
      key: 'trackingBaseUrl',
      label: 'URL base tracking',
      type: 'text',
      required: false,
      placeholder: 'https://mio-tenant.app.automazionezeli.com',
      help:
        'Dominio del tuo runtime FlowForge. Il pixel diventa <baseUrl>/api/track/open/<token> e ' +
    ...
    {
      key: 'clickWhitelist',
      label: 'Domini permessi nei link',
      type: 'chip-list',
      required: false,
      placeholder: 'example.com, mio-dominio.com',
      help:
        'Lista di domini (suffix-match) per cui i link vengono riscritti col tracking. ' +
    ...
    {
      key: 'requireConsent',
      label: 'Richiede consenso GDPR upstream',
      type: 'boolean',
      required: false,
      defaultValue: 'true',
      help:
        "On (default): l\\'invio fallisce se l\\'input upstream non ha `consentVerified=true`. " +
    ...
    {
      key: 'sampleRate',
      label: 'Frazione invio (0–1)',
      type: 'number',
      required: false,
      defaultValue: '1',
      help:
        'A/B test ratio. 1 = invia a tutti (default). 0.5 = al 50%. ' +
    ...
    // ────────── Knobs ──────────
    {
      ...
      defaultValue: '30000',
      help:
        'Quando il server SMTP non risponde entro N ms, il nodo fallisce. ' +
