...
export const openapiNodeDef: NodeDef = {
  ...
  configFields: [
    { key: 'specJson', label: 'Spec OpenAPI 3.0 (JSON)', type: 'json', required: true, language: 'json',
      help: 'La spec del provider in JSON. Da servers[] viene preso il base URL; da paths le operations disponibili.' },
    { key: 'operationId', label: 'Operation ID', type: 'text', required: true, placeholder: 'getUser',
      help: 'L\'operationId dell\'endpoint da chiamare (come nella spec). Se manca nella spec, usa "METHOD /path" (es. "GET /users").' },
    { key: 'baseUrl', label: 'Base URL (override)', type: 'text', required: false, placeholder: 'https://api.example.com/v1',
      help: 'Sovrascrive il server della spec. Vuoto = primo servers[].url della spec.' },
    { key: 'paramsJson', label: 'Parametri (JSON)', type: 'json', required: false, language: 'json',
    ...
    { key: 'bodyJson', label: 'Request body (JSON)', type: 'json', required: false, language: 'json',
    ...
    { key: 'authHeader', label: 'Header di autenticazione', type: 'text', required: false, placeholder: 'Authorization',
      help: 'Nome dell\'header auth (es. Authorization, X-API-Key). Vuoto = nessuna auth.' },
    { key: 'authValue', label: 'Valore autenticazione', type: 'secret', required: false, placeholder: 'Bearer sk_live_…',
