...
describe('action_datetime', () => {
  ...
  it('add giorni', async () => {
    const r = await dt(
      { operation: 'add', date: '2026-01-01T00:00:00Z', amount: '30', unit: 'days' },
      undefined,
      ctx,
    );
  ...
  it('🚨 add mesi CLAMPa al giorno valido (no overflow): 31 gen +1 mese → FEBBRAIO, non marzo', async () => {
    const r = await dt(
      { operation: 'add', date: '2026-01-31T12:00:00Z', amount: '1', unit: 'months' },
      undefined,
      ctx,
    );
  ...
  it('🚨 add mesi su anno bisestile: 31 gen 2024 +1 mese → 29 feb', async () => {
    const r = await dt(
      { operation: 'add', date: '2024-01-31T12:00:00Z', amount: '1', unit: 'months' },
      undefined,
      ctx,
    );
  ...
  it('🚨 add anni clampa il 29 feb: 29 feb 2024 +1 anno → 28 feb 2025 (non 1 marzo)', async () => {
    const r = await dt(
      { operation: 'add', date: '2024-02-29T12:00:00Z', amount: '1', unit: 'years' },
      undefined,
      ctx,
    );
  ...
  it('add mesi caso normale (giorno valido) → preservato: 15 gen +1 mese → 15 feb', async () => {
    const r = await dt(
      { operation: 'add', date: '2026-01-15T12:00:00Z', amount: '1', unit: 'months' },
      undefined,
      ctx,
    );
  ...
  it('subtract sottrae', async () => {
    const r = await dt(
      { operation: 'subtract', date: '2026-06-10T12:00:00Z', amount: '10', unit: 'days' },
      undefined,
      ctx,
    );
  ...
  it('parse epoch millisecondi (13 cifre)', async () => {
    const r = await dt(
      { operation: 'format', date: '1735689600000', preset: 'iso' },
      undefined,
      ctx,
    );
    expect((r.output as { formatted: string }).formatted).toBe('2025-01-01T00:00:00.000Z');
  ...
  it('format weekday in italiano', async () => {
    const r = await dt(
      { operation: 'format', date: '2026-06-06T10:00:00Z', preset: 'date', locale: 'it-IT' },
      undefined,
      ctx,
    );
  ...
  it('format relative → "fa" per il passato', async () => {
    ...
    const r = await dt(
      { operation: 'format', date: past, preset: 'relative', locale: 'it-IT' },
      undefined,
      ctx,
    );
  ...
  it('data non valida → throw', async () => {
    await expect(dt({ operation: 'format', date: 'non-una-data' }, undefined, ctx)).rejects.toThrow(
      /non valida/,
    );
  });
  it('"now" come stringa data', async () => {
    ...
    expect(
      Math.abs(new Date((r.output as { formatted: string }).formatted).getTime() - Date.now()),
    ).toBeLessThan(2000);
  });
});
