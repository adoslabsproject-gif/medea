...
const htmlExtractExecutor: NodeExecutor = async (config, input) => {
  ...
  switch (op) {
    ...
    case 'links': {
      ...
      while ((m = re.exec(html)) !== null) links.push({ href: decodeEntities(m[1]!), text: stripTags(m[2]!) });
    ...
    case 'meta': {
      ...
      return { output: { result: meta, count: Object.keys(meta).length }, durationMs: Date.now() - startedAt };
    }
    case 'headings': {
      const headings: { level: number; text: string }[] = [];
      const re = /<h([1-6])\b[^>]*>([\s\S]*?)<\/h\1>/gi;
      let m: RegExpExecArray | null;
      while ((m = re.exec(html)) !== null) headings.push({ level: Number(m[1]), text: stripTags(m[2]!) });
      return { output: { result: headings, count: headings.length }, durationMs: Date.now() - startedAt };
    }
    default:
...
export const htmlExtractNode: NodeModule = {
  def: {
    ...
    configFields: [
      {
        key: 'operation', label: 'Cosa estrarre', type: 'select', required: true, defaultValue: 'text',
      ...
      {
        key: 'html', label: 'HTML', type: 'expression', required: false, defaultValue: 'input',
...
const markdownExecutor: NodeExecutor = async (config, input) => {
  const startedAt = Date.now();
  const md = str(config.markdown !== undefined && str(config.markdown) !== '' ? config.markdown : input);
...
export const markdownNode: NodeModule = {
  def: {
    ...
    configFields: [
      {
        key: 'markdown', label: 'Markdown', type: 'textarea', required: false, defaultValue: 'input',
