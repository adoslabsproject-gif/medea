...
const FIRST = ['Marco', 'Giulia', 'Luca', 'Anna', 'Francesco', 'Sara', 'Matteo', 'Chiara', 'Davide', 'Elena', 'Alessandro', 'Martina'];
const LAST = ['Rossi', 'Russo', 'Ferrari', 'Esposito', 'Bianchi', 'Romano', 'Colombo', 'Ricci', 'Marino', 'Greco', 'Bruno', 'Gallo'];
const CITIES = ['Roma', 'Milano', 'Napoli', 'Torino', 'Palermo', 'Genova', 'Bologna', 'Firenze', 'Bari', 'Catania', 'Verona', 'Venezia'];
const COMPANIES = ['Tecnologie', 'Servizi', 'Soluzioni', 'Group', 'Consulting', 'Digital', 'Industrie', 'Sistemi'];
const LOREM = 'lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt ut labore et dolore magna aliqua enim ad minim veniam quis nostrud'.split(' ');
...
function genValue(type: string, rng: () => number): unknown {
  ...
  switch (type) {
    ...
    case 'email': return `${first.toLowerCase()}.${last.toLowerCase()}@esempio.it`;
    case 'phone': return `+39 3${Math.floor(rng() * 9) + 1}${String(Math.floor(rng() * 9000000) + 1000000)}`;
    case 'company': return `${last} ${pick(COMPANIES)}`;
    case 'city': return pick(CITIES);
    case 'word': return pick(LOREM);
    case 'sentence': return `${pick(LOREM)} ${pick(LOREM)} ${pick(LOREM)} ${pick(LOREM)} ${pick(LOREM)}`;
    case 'paragraph': return Array.from({ length: 4 }, () => Array.from({ length: 8 }, () => pick(LOREM)).join(' ')).join('. ');
    case 'integer': return Math.floor(rng() * 1000);
    case 'number': return Math.round(rng() * 100000) / 100;
    ...
    case 'uuid': {
      ...
      for (let i = 0; i < 36; i += 1) {
        s += i === 8 || i === 13 || i === 18 || i === 23 ? '-'
          : i === 14 ? '4'
          : i === 19 ? hex[8 + Math.floor(rng() * 4)]
          : hex[Math.floor(rng() * 16)];
      }
      return s;
    }
    case 'date': return new Date(Date.now() + (rng() - 0.5) * 2 * 365 * 86400000).toISOString();
    case 'pastDate': return new Date(Date.now() - rng() * 365 * 86400000).toISOString();
    case 'futureDate': return new Date(Date.now() + rng() * 365 * 86400000).toISOString();
    default: return null;
  }
}
...
const mockExecutor: NodeExecutor = async (config) => {
  ...
  const baseSeed = seedRaw !== '' ? [...seedRaw].reduce((h, c) => (h * 31 + c.charCodeAt(0)) | 0, 7) : Math.floor(Math.random() * 2 ** 31);
  ...
  if (rawSchema && typeof rawSchema === 'object') schema = rawSchema as Record<string, string>;
    ...
    try { schema = JSON.parse(rawSchema) as Record<string, string>; } catch { schema = {}; }
  }
  if (Object.keys(schema).length === 0) schema = { id: 'uuid', nome: 'fullName', email: 'email', citta: 'city' };
  ...
  return { output: { items, count: items.length, seed: seedRaw || null }, durationMs: Date.now() - startedAt };
};
...
export const mockDataNode: NodeModule = {
  def: {
    ...
    configFields: [
      {
        key: 'schema', label: 'Schema (campo → tipo)', type: 'key-value', required: false,
        help: 'Ogni riga: nome del campo → tipo (fullName, email, phone, city, uuid, integer, pastDate, …). Vuoto = schema di default.',
      },
      {
        key: 'count', label: 'Quanti record', type: 'number', required: false, defaultValue: '10',
        help: 'Da 1 a 1.000.',
      },
      {
        key: 'seed', label: 'Seed (riproducibilità)', type: 'text', required: false,
