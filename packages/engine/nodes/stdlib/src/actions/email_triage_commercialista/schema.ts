...
const optionalRecord = z.string().max(2_000)
  .refine((s) => { if (s === '') return true; try { JSON.parse(s); return true; } catch { return false; } },
    { message: 'must be valid JSON' })
  .transform((s) => s === '' ? undefined : JSON.parse(s) as Record<string, unknown>)
  .optional();

export const EmailTriageCommercialistaConfigSchema = z.object({
