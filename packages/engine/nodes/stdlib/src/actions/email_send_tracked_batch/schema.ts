...
export const BatchRecipientSchema = z.object({
  ...
  templateVars: z.record(z.string(), z.union([z.string(), z.number(), z.boolean(), z.null()])).optional(),
...
export const EmailSendTrackedBatchConfigSchema = z.object({
  // ── Account / transport ──
  systemAccountId: trimmed.min(1).optional()
  ...
  // ── Envelope template ──
  from: trimmed.email().max(254).optional()
    .or(z.literal('').transform(() => undefined)),
  /** Replied-to. */
  replyTo: trimmed.email().max(254).optional()
    .or(z.literal('').transform(() => undefined)),
...
export const EmailSendTrackedBatchInputSchema = z.object({
  recipients: z.array(BatchRecipientSchema).max(MAX_BATCH_RECIPIENTS).optional(),
