...
export const emailSendTrackedBatchNodeDef: NodeDef = {
  ...
  configFields: [
    // ────────── Sender ──────────
    {
      ...
      required: false,
      help:
        'Account configurato in Settings → Email accounts. Se OAuth Gmail, il token viene ' +
    ...
    {
      key: 'body',
      label: 'Corpo template (HTML)',
      type: 'textarea',
      required: true,
      help:
        'Body template HTML. I link verso domini in `clickWhitelist` vengono tracciati; ' +
    ...
    },
    {
      ...
      required: false,
      help:
        'Lista di domini (suffix-match) per il tracking link. Vuoto = solo trackingBaseUrl. ' +
    ...
    {
      key: 'recipients',
      label: 'Lista destinatari (JSON inline)',
      type: 'json',
      required: false,
      help:
        'Array `[{leadId, to, fromAddress?, templateVars?, sendId?}, …]`. Se vuoto, il nodo ' +
    ...
    // ────────── Throttling ──────────
    {
      ...
      defaultValue: '60',
      help:
        'Cap superiore di mail/h. Gmail Workspace: 60-100/h per account è il safe spot. ' +
    ...
    },
    {
      ...
      defaultValue: '0.2',
      help:
        'Variazione casuale del delay tra send (0 = cadenza fissa, 0.5 = ±50%). 0.2 default per ' +
        'apparire "umani" agli antispam senza spaccare il rate.',
    },
    {
      key: 'maxAttempts',
      label: 'Tentativi per destinatario',
      type: 'number',
      required: false,
      defaultValue: '3',
    ...
    },
    {
      ...
      defaultValue: '5000',
      help:
        'Backoff esponenziale: backoffBaseMs · 2^(attempt-1), cap 60s. ' +
        '5_000 → tentativi a 5s, 10s, 20s.',
    },
    {
      key: 'budgetMs',
      label: 'Budget totale (ms, opzionale)',
      type: 'number',
      required: false,
      help:
        'Tempo massimo del batch. Oltre questo, i destinatari residui escono con requeued=true ' +
