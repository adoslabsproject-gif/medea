...
export const communityGoogleSheetsNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Leggendo, le righe stanno in `rows`. Scrivendo, il numero da guardare e` `updatedCells`: a zero significa che non ha scritto niente, e il nodo riesce lo stesso.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'data', type: 'object', desc: 'La risposta di Google, per i dettagli.' },
        { name: 'rows', type: 'array', desc: 'Le righe lette dal foglio.' },
        { name: 'count', type: 'number', desc: 'Quante righe.' },
        { name: 'updatedCells', type: 'number', desc: 'Quante celle ha scritto. Zero significa nessuna scrittura.' },
        { name: 'spreadsheetId', type: 'string', desc: 'Il foglio su cui ha operato.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityDiscordNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Mandando tramite webhook Discord non restituisce l\'identificativo del messaggio: `messageId` e `channelId` restano null. Non e` un errore — e` come funziona quel canale.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se il messaggio e` stato accettato.' },
        { name: 'messageId', type: 'string|null', desc: 'Il messaggio creato. Null mandando tramite webhook.' },
        { name: 'channelId', type: 'string|null', desc: 'Il canale. Null mandando tramite webhook.' },
        { name: 'sentAt', type: 'string', desc: 'Quando e` stato mandato, in ISO 8601.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityAirtableNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'I campi cambiano con l\'operazione: elencando escono `records` e `count`, leggendo o scrivendo esce `record`, cancellando esce `deleted`. `ok` c\'e` sempre.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'records', type: 'array', desc: 'Le schede trovate. Solo elencando.' },
        { name: 'count', type: 'number', desc: 'Quante schede. Solo elencando.' },
        { name: 'record', type: 'object', desc: 'La scheda letta o appena scritta.' },
        { name: 'deleted', type: 'boolean', desc: 'Vero dopo una cancellazione riuscita.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityTrelloNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'I campi cambiano con l\'operazione: sulla singola scheda esce `card`, elencando escono `cards` o `lists` con `count`.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'card', type: 'object', desc: 'La scheda letta o creata.' },
        { name: 'cards', type: 'array', desc: 'Le schede trovate. Solo elencando.' },
        { name: 'lists', type: 'array', desc: 'Le colonne trovate. Solo elencandole.' },
        { name: 'count', type: 'number', desc: 'Quanti elementi ha riportato.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityCalendlyNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'I campi cambiano con l\'operazione: elencando gli appuntamenti escono `events` e `count`, elencando gli invitati esce `invitees`, sul singolo escono `event` o `invitee`, annullando esce `cancelled`.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'events', type: 'array', desc: 'Gli appuntamenti trovati.' },
        { name: 'count', type: 'number', desc: 'Quanti elementi ha riportato.' },
        { name: 'invitees', type: 'array', desc: 'Gli invitati di un appuntamento.' },
        { name: 'event', type: 'object', desc: 'Il singolo appuntamento.' },
        { name: 'invitee', type: 'object', desc: 'Il singolo invitato.' },
        { name: 'cancelled', type: 'boolean', desc: 'Vero dopo un annullamento riuscito.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityTypeformNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: '`count` sono le risposte TORNATE, `total` quante ne esistono per quel modulo: se differiscono la lettura e` parziale.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'forms', type: 'array', desc: 'I moduli trovati. Solo elencandoli.' },
        { name: 'form', type: 'object', desc: 'Il singolo modulo, con le sue domande.' },
        { name: 'responses', type: 'array', desc: 'Le risposte lette.' },
        { name: 'count', type: 'number', desc: 'Quante risposte sono tornate.' },
        { name: 'total', type: 'number', desc: 'Quante ne esistono in tutto: maggiore di `count` significa lettura parziale.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityShopifyNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Oltre a `ok` e `operation`, i campi dipendono dall\'operazione: sugli ordini escono `order` e `orderId`, sui prodotti escono `products`, `count`, `product` e `productId`.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'order', type: 'object|null', desc: 'L\'ordine letto o creato, come lo restituisce Shopify.' },
        { name: 'orderId', type: 'number|null', desc: 'L\'ordine creato.' },
        { name: 'products', type: 'array', desc: 'I prodotti trovati.' },
        { name: 'count', type: 'number', desc: 'Quanti prodotti.' },
        { name: 'product', type: 'object|null', desc: 'Il prodotto letto o creato.' },
        { name: 'productId', type: 'number|null', desc: 'Il prodotto creato.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityMailchimpNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Oltre a `ok` e `operation`, i campi dipendono dall\'operazione: iscrivendo escono `memberId`, `status` ed `email`; leggendo esce `member`; assegnando un\'etichetta escono `tagged`, `email` e `tag`.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'memberId', type: 'string|null', desc: 'L\'iscritto creato o aggiornato.' },
        { name: 'status', type: 'string|null', desc: 'Lo stato dell\'iscrizione: iscritto, in attesa di conferma, cancellato.' },
        { name: 'email', type: 'string', desc: 'L\'indirizzo su cui ha operato.' },
        { name: 'member', type: 'object', desc: 'L\'iscritto letto, come lo restituisce Mailchimp.' },
        { name: 'tagged', type: 'boolean', desc: 'Vero dopo l\'assegnazione di un\'etichetta.' },
        { name: 'tag', type: 'string', desc: 'L\'etichetta assegnata.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityTwilioNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: '`status` e` lo stato al momento dell\'invio — di norma «in coda»: la consegna vera arriva dopo, per webhook. `messageSid` serve a correlarla.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'messageSid', type: 'string|null', desc: 'Il messaggio creato: serve a correlare la conferma di consegna.' },
        { name: 'status', type: 'string|null', desc: 'Lo stato al momento dell\'invio, non la consegna.' },
        { name: 'to', type: 'string', desc: 'Il numero destinatario.' },
        { name: 'from', type: 'string', desc: 'Il numero mittente.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communitySendgridNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: '`accepted` dice che SendGrid ha preso in carico la email, non che sia arrivata: la consegna vera arriva dopo, per webhook.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'accepted', type: 'boolean', desc: 'Se SendGrid ha preso in carico il messaggio. Non e` la consegna.' },
        { name: 'to', type: 'string', desc: 'Il destinatario.' },
        { name: 'from', type: 'string', desc: 'Il mittente.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityAsanaNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Oltre a `ok` e `operation`, i campi dipendono da cosa si e` chiesto: creando un\'attivita` escono `taskId` e `url`, leggendola esce `task`, commentando esce `commentId`.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'taskId', type: 'string|null', desc: 'L\'attivita` creata. Solo creando.' },
        { name: 'url', type: 'string|null', desc: 'Il collegamento all\'attivita`. Solo creando.' },
        { name: 'task', type: 'object|null', desc: 'L\'attivita` letta, come la restituisce Asana. Solo leggendo.' },
        { name: 'commentId', type: 'string|null', desc: 'Il commento aggiunto. Solo commentando.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const communityDropboxNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Oltre a `ok` e `operation`, i campi dipendono dall\'operazione: elencando escono `entries`, `count` e `hasMore`; caricando escono `id` e `path`; leggendo esce `metadata`; condividendo esce `sharedUrl`; cancellando esce `deleted`.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'entries', type: 'array', desc: 'Il contenuto della cartella. Solo elencando.' },
        { name: 'count', type: 'number', desc: 'Quanti elementi in questa pagina.' },
        { name: 'hasMore', type: 'boolean', desc: 'Vero se c\'e` altro da leggere oltre questa pagina.' },
        { name: 'id', type: 'string|null', desc: 'Il file caricato.' },
        { name: 'path', type: 'string', desc: 'Il percorso del file.' },
        { name: 'metadata', type: 'object', desc: 'I dati del file, come li restituisce Dropbox.' },
        { name: 'sharedUrl', type: 'string|null', desc: 'Il collegamento di condivisione. Solo condividendo.' },
        { name: 'deleted', type: 'boolean', desc: 'Vero dopo una cancellazione riuscita.' },
      ],
    },
    vendor: 'flowforge',
...
export const communityBoxNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Oltre a `ok` e `operation`, i campi dipendono dall\'operazione: elencando escono `entries`, `count` e `totalCount`; caricando escono `id` e `name`; leggendo esce `item`; cancellando esce `deleted`.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'entries', type: 'array', desc: 'Il contenuto della cartella. Solo elencando.' },
        { name: 'count', type: 'number', desc: 'Quanti elementi in questa pagina.' },
        { name: 'totalCount', type: 'number', desc: 'Quanti in tutto, secondo Box: puo` superare `count`.' },
        { name: 'id', type: 'string|null', desc: 'L\'elemento creato o caricato.' },
        { name: 'name', type: 'string', desc: 'Il nome dell\'elemento.' },
        { name: 'item', type: 'object', desc: 'L\'elemento letto, come lo restituisce Box.' },
        { name: 'deleted', type: 'boolean', desc: 'Vero dopo una cancellazione riuscita.' },
      ],
    },
    vendor: 'flowforge',
...
export const communityGcsNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Oltre a `ok`, `operation` e `bucket`, i campi dipendono dall\'operazione: elencando escono `objects`, `count` e `nextPageToken`; leggendo escono `metadata` e `mediaLink`; cancellando escono `deleted` e `objectName`.',
      fields: [
        { name: 'ok', type: 'boolean', desc: 'Se la chiamata e` riuscita.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'bucket', type: 'string', desc: 'Il contenitore su cui ha operato.' },
        { name: 'objects', type: 'array', desc: 'Gli oggetti trovati. Solo elencando.' },
        { name: 'count', type: 'number', desc: 'Quanti oggetti in questa pagina.' },
        { name: 'nextPageToken', type: 'string|null', desc: 'Da dove riprendere l\'elenco. Null quando non c\'e` altro.' },
        { name: 'metadata', type: 'object', desc: 'I dati dell\'oggetto.' },
        { name: 'mediaLink', type: 'string|null', desc: 'Il collegamento diretto al contenuto.' },
        { name: 'deleted', type: 'boolean', desc: 'Vero dopo una cancellazione riuscita.' },
        { name: 'objectName', type: 'string', desc: 'L\'oggetto cancellato.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
