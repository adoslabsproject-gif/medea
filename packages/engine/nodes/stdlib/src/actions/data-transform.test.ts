...
describe('action_csv', () => {
  ...
  it('round-trip parse→stringify preserva i dati', async () => {
    ...
    const parsed = (await csv({ operation: 'parse', text: original }, undefined, ctx)).output as { rows: unknown[] };
    const back = (await csv({ operation: 'stringify', items: JSON.stringify(parsed.rows) }, undefined, ctx)).output as { csv: string };
  ...
  it.each(['=HYPERLINK("http://evil")', '+1+1', '-cmd', '@SUM(A1)'])(
    ...
    async (payload) => {
      ...
      const r = await csv({ operation: 'stringify', items: JSON.stringify([{ f: payload }]) }, undefined, ctx);
      const out = (r.output as { csv: string }).csv;
  ...
  it('🚨 i NUMERI negativi NON vengono neutralizzati (resta "-5", non "\'-5")', async () => {
    const r = await csv({ operation: 'stringify', items: JSON.stringify([{ n: -5 }]) }, undefined, ctx);
    const out = (r.output as { csv: string }).csv;
    expect(out.split('\n')[1]).toBe('-5');
  });
});
...
describe('action_json — prototype pollution (CWE-1321)', () => {
  it('set con path __proto__.x NON contamina Object.prototype', async () => {
    await json({ operation: 'set', path: '__proto__.polluted', value: '"yes"', source: '{}' }, undefined, ctx);
  ...
  it('merge con {"__proto__":{...}} NON contamina Object.prototype', async () => {
    ...
    await json({ operation: 'merge', source: '{"a":1}', value: '{"__proto__":{"polluted":"yes"}}' }, undefined, ctx);
    expect(({} as Record<string, unknown>).polluted).toBeUndefined();
    delete (Object.prototype as Record<string, unknown>).polluted;
  });
});
...
describe('action_array', () => {
  ...
  it('unique by key deduplica', async () => {
    const r = await array({ operation: 'unique', key: 'name', items: JSON.stringify(people) }, undefined, ctx);
    expect((r.output as { count: number }).count).toBe(2);
  });
  it('sort numeric-aware desc', async () => {
    const r = await array({ operation: 'sort', key: 'id', direction: 'desc', items: JSON.stringify(people) }, undefined, ctx);
  ...
  it('sort stringhe è numeric-aware (10 dopo 9)', async () => {
    const r = await array({ operation: 'sort', items: JSON.stringify(['item9', 'item10', 'item2']) }, undefined, ctx);
    expect((r.output as { result: string[] }).result).toEqual(['item2', 'item9', 'item10']);
  });
  it('group per campo → { valore: [items] }', async () => {
    const r = await array({ operation: 'group', key: 'city', items: JSON.stringify(people) }, undefined, ctx);
  ...
  it('chunk spezza in blocchi', async () => {
    const r = await array({ operation: 'chunk', size: '2', items: JSON.stringify([1, 2, 3, 4, 5]) }, undefined, ctx);
    expect((r.output as { result: number[][] }).result).toEqual([[1, 2], [3, 4], [5]]);
  });
  it('flatten profondità 1', async () => {
    const r = await array({ operation: 'flatten', items: JSON.stringify([[1, 2], [3, [4]]]) }, undefined, ctx);
    expect((r.output as { result: unknown[] }).result).toEqual([1, 2, 3, [4]]);
  });
  it('pick proietta i campi dot-path', async () => {
    const r = await array({ operation: 'pick', fields: 'name', items: JSON.stringify(people) }, undefined, ctx);
    expect((r.output as { result: Record<string, unknown>[] }).result[0]).toEqual({ name: 'Anna' });
  });
  it('slice con indici negativi (ultimi N)', async () => {
    const r = await array({ operation: 'slice', start: '-2', items: JSON.stringify([1, 2, 3, 4]) }, undefined, ctx);
...
describe('action_json', () => {
  ...
  it('get dot-path annidato', async () => {
    const r = await json({ operation: 'get', path: 'data.user.email', source: JSON.stringify(obj) }, undefined, ctx);
    expect((r.output as { result: unknown }).result).toBe('a@b.it');
  });
  it('get percorso inesistente → undefined (no crash)', async () => {
    const r = await json({ operation: 'get', path: 'data.x.y.z', source: JSON.stringify(obj) }, undefined, ctx);
    expect((r.output as { result: unknown }).result).toBeUndefined();
  });
  it('set è immutabile e crea gli intermedi', async () => {
    const base = { a: 1 };
    const r = await json({ operation: 'set', path: 'b.c', value: '42', source: JSON.stringify(base) }, undefined, ctx);
  ...
  it('omit rimuove i campi sensibili', async () => {
    const r = await json({ operation: 'omit', paths: 'token', source: JSON.stringify(obj) }, undefined, ctx);
    expect((r.output as { result: Record<string, unknown> }).result).not.toHaveProperty('token');
  });
  it('pick whitelist', async () => {
    const r = await json({ operation: 'pick', paths: 'data.role, token', source: JSON.stringify(obj) }, undefined, ctx);
    expect((r.output as { result: Record<string, unknown> }).result).toEqual({ role: 'admin', token: 'secret' });
  });
  it('merge in profondità', async () => {
    const r = await json({ operation: 'merge', value: '{"data":{"role":"user"},"extra":1}', source: JSON.stringify(obj) }, undefined, ctx);
    const res = (r.output as { result: { data: { role: string; user: unknown }; extra: number } }).result;
  ...
  it('merge con JSON non valido → throw', async () => {
    await expect(json({ operation: 'merge', value: '{bad', source: '{}' }, undefined, ctx)).rejects.toThrow(/JSON/);
  });
  it('flatten oggetto annidato → chiavi dot', async () => {
    const r = await json({ operation: 'flatten', source: JSON.stringify({ a: { b: { c: 1 } }, d: 2 }) }, undefined, ctx);
    expect((r.output as { result: Record<string, unknown> }).result).toEqual({ 'a.b.c': 1, d: 2 });
  });
  it('keys di primo livello', async () => {
    const r = await json({ operation: 'keys', source: JSON.stringify({ x: 1, y: 2 }) }, undefined, ctx);
...
describe('🚨 action_array — dichiarazione lineage (GAP #2)', () => {
  ...
  it('reverse: mapping speculare N-1-i', async () => {
    ...
    expect((r.items as Declared).map((it) => it.pairedItem)).toEqual([{ item: 2 }, { item: 1 }, { item: 0 }]);
  });
