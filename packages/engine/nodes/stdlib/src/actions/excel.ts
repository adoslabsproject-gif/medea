...
export const xlsxParseNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: '`truncated` a vero significa che il tetto di righe ha fermato la lettura: `rows` non contiene tutto il foglio. `totalRows` e` quante ne ha lette, non quante ne ha il file.',
      fields: [
        { name: 'rows', type: 'array', desc: 'Le righe lette, con le intestazioni come nomi di campo.' },
        { name: 'sheetName', type: 'string', desc: 'Il foglio da cui ha letto.' },
        { name: 'totalRows', type: 'number', desc: 'Quante righe ha letto.' },
        { name: 'columns', type: 'array', desc: 'I nomi delle colonne trovate.' },
        { name: 'columnCount', type: 'number', desc: 'Quante colonne.' },
        { name: 'headerRow', type: 'number', desc: 'Quale riga ha usato come intestazione.' },
        { name: 'emptyCellsCount', type: 'number', desc: 'Quante celle vuote ha incontrato: un numero alto segnala un foglio irregolare.' },
        { name: 'truncated', type: 'boolean', desc: 'Vero se il tetto di righe ha interrotto la lettura.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.1.0',
...
export const xlsxBuildNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Il foglio esce come riferimento binario in `binary`, da passare tale e quale a un allegato o a un `action_file_write`. `path` c\'e` SOLO se in configurazione e` stato chiesto di salvarlo anche su disco.',
      fields: [
        { name: 'binary', type: 'object', desc: 'Il riferimento ai byte del foglio, pronto da allegare.' },
        { name: 'fileName', type: 'string', desc: 'Il nome del file prodotto.' },
        { name: 'sheetName', type: 'string', desc: 'Il nome del foglio.' },
        { name: 'rowsWritten', type: 'number', desc: 'Quante righe ha scritto.' },
        { name: 'sizeBytes', type: 'number', desc: 'La dimensione del file in byte.' },
        { name: 'contentType', type: 'string', desc: 'Il tipo MIME del foglio.' },
        { name: 'path', type: 'string', desc: 'Dove l\'ha salvato su disco. Presente solo se il salvataggio era richiesto.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.1.0',
