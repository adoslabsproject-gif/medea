...
export const emailMxNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Controlla il DOMINIO, non la casella: un dominio valido non garantisce che l\'indirizzo esista. `ok` e` `confidence` confrontata con la soglia in configurazione, ed e` il campo pensato per il `logic_if` subito dopo.',
      fields: [
        { name: 'mx_valid', type: 'boolean', desc: 'Se il dominio ha record MX validi.' },
        { name: 'mx_records', type: 'array', desc: 'I server di posta trovati.' },
        { name: 'using_a_fallback', type: 'boolean', desc: 'Vero se ha ripiegato sul record A perche` non c\'erano MX: consegna possibile ma debole.' },
        { name: 'disposable', type: 'boolean', desc: 'Se il dominio e` di quelli usa e getta.' },
        { name: 'role_based', type: 'boolean', desc: 'Se l\'indirizzo e` di ruolo (info@, sales@) e non di una persona.' },
        { name: 'confidence', type: 'number', desc: 'Il punteggio di affidabilita`, da 0 a 100.' },
        { name: 'reason', type: 'string', desc: 'Perche` quel punteggio.' },
        { name: 'ok', type: 'boolean', desc: 'Se il punteggio supera la soglia scelta. E` il campo su cui diramare.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
