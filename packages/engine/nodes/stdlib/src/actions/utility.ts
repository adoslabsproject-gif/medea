...
export const textTemplateNode: NodeModule = {
  def: {
    ...
    outputs: ['text', 'length'],
    outputContract: {
      notes: 'Il testo viene TAGLIATO oltre un tetto di caratteri, e in quel caso `truncated` diventa vero: chi genera una email o un documento da qui deve guardarlo, perche` il testo tagliato non da` nessun errore.',
      fields: [
        { name: 'text', type: 'string', desc: 'Il testo con le variabili sostituite.' },
        { name: 'length', type: 'number', desc: 'Quanti caratteri ha.' },
        { name: 'truncated', type: 'boolean', desc: 'Vero se e` stato tagliato perche` troppo lungo.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const jsonExtractNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: '`value` cambia forma con la modalita`: prendendo il primo risultato e` il valore, prendendoli tutti e` una lista, contando e` un numero. Nessun risultato NON e` un errore: esce il valore di ripiego, oppure null, e `matchCount` a 0.',
      fields: [
        { name: 'value', type: 'object|array|string|number|null', desc: 'Quello che il percorso ha trovato. La forma dipende dalla modalita` scelta.' },
        { name: 'matchCount', type: 'number', desc: 'Quanti risultati ha trovato il percorso. Zero significa che non ha trovato niente.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
export const dateFormatNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'I pezzi della data — anno, mese, giorno, ora — sono calcolati nel FUSO scelto in configurazione, mentre `iso` e `unix` sono in UTC: confrontarli fra loro senza saperlo porta a errori di un giorno.',
      fields: [
        { name: 'formatted', type: 'string', desc: 'La data nel formato chiesto.' },
        { name: 'iso', type: 'string', desc: 'La data in ISO 8601, in UTC.' },
        { name: 'unix', type: 'number', desc: 'I secondi dal 1970.' },
        { name: 'year', type: 'number', desc: 'L\'anno nel fuso scelto.' },
        { name: 'month', type: 'number', desc: 'Il mese, da 1 a 12.' },
        { name: 'day', type: 'number', desc: 'Il giorno del mese.' },
        { name: 'weekday', type: 'string', desc: 'Il giorno della settimana, per esteso.' },
        { name: 'hour', type: 'number', desc: 'L\'ora, da 0 a 23.' },
        { name: 'minute', type: 'number', desc: 'I minuti.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
