...
export const janitorCleanupNode: NodeModule = {
  def: {
    ...
    branching: true,
    outputContract: {
      notes: 'In prova (`dryRun`) NON modifica niente: i conteggi dicono cosa AVREBBE fatto. Un fallimento su una singola regola non ferma il nodo — esce `success` falso con `error`, e le altre regole proseguono.',
      fields: [
        { name: 'mode', type: 'string', desc: 'Se ha girato su tutte le regole o su una sola.' },
        { name: 'cycleId', type: 'string', desc: 'Il ciclo di pulizia, per ritrovarlo nei rapporti.' },
        { name: 'ruleId', type: 'string', desc: 'La regola eseguita. Solo eseguendone una sola.' },
        { name: 'success', type: 'boolean', desc: 'Se la regola e` andata a buon fine.' },
        { name: 'error', type: 'string|null', desc: 'Cosa e` andato storto. Null se e` filato tutto.' },
        { name: 'rulesExecuted', type: 'number', desc: 'Quante regole ha eseguito.' },
        { name: 'rulesSkippedLock', type: 'number', desc: 'Quante saltate perche` gia` in esecuzione altrove.' },
        { name: 'rulesSkippedDisabled', type: 'number', desc: 'Quante saltate perche` spente.' },
        { name: 'rowsDetected', type: 'number', desc: 'Quante righe problematiche ha trovato.' },
        { name: 'rowsRepaired', type: 'number', desc: 'Quante ne ha sistemate. Zero in prova.' },
        { name: 'rowsQuarantined', type: 'number', desc: 'Quante ne ha messe da parte.' },
        { name: 'rowsSkipped', type: 'number', desc: 'Quante ne ha lasciate stare.' },
        { name: 'bySeverity', type: 'object', desc: 'Il conto delle righe per gravita`.' },
        { name: 'dryRun', type: 'boolean', desc: 'Vero se era una prova e non ha modificato niente.' },
        { name: 'durationMs', type: 'number', desc: 'Quanto e` durata la pulizia.' },
        { name: 'reports', type: 'array', desc: 'Un rapporto per regola eseguita.' },
        { name: 'report', type: 'object', desc: 'Il rapporto della singola regola. Solo eseguendone una sola.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
