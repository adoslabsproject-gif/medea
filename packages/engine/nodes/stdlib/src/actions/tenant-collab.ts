...
export const tenantCollabNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: '`delivered` e` il campo su cui fidarsi: `status` da solo puo` essere un codice d\'errore. `idempotencyKey` garantisce che una consegna ripetuta non venga contata due volte dall\'altra parte.',
      fields: [
        { name: 'delivered', type: 'boolean', desc: 'Se la consegna e` riuscita. E` il campo su cui diramare.' },
        { name: 'status', type: 'number', desc: 'Il codice HTTP della risposta.' },
        { name: 'response', type: 'object|string', desc: 'La risposta di chi ha ricevuto.' },
        { name: 'correlationId', type: 'string', desc: 'Il riferimento con cui seguire lo scambio da entrambe le parti.' },
        { name: 'idempotencyKey', type: 'string', desc: 'La chiave che impedisce di contare due volte una consegna ripetuta.' },
        { name: 'attempts', type: 'number', desc: 'Quanti tentativi sono serviti.' },
        { name: 'signed', type: 'boolean', desc: 'Se la richiesta e` stata firmata.' },
        { name: 'targetHost', type: 'string', desc: 'A chi e` stata consegnata.' },
