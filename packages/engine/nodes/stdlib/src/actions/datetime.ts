...
export const datetimeNode: NodeModule = {
  def: {
    ...
    outputs: ['default'],
    outputContract: {
      notes: 'Quattro forme, una per operazione, e non hanno campi in comune oltre a `iso`. ORA: `iso`, `epochMs`, `epochSec`. DIFFERENZA: `value`, `rounded`, `unit`, `absMs` — e NON `iso`. SOMMA o sottrazione: `iso` e `epochMs`. FORMATO: `formatted`, `iso`, `epochMs`, `weekday`.',
      fields: [
        { name: 'iso', type: 'string', desc: 'L\'istante in ISO 8601. Con tutte le operazioni tranne la differenza.' },
        { name: 'epochMs', type: 'number', desc: 'I millisecondi dal 1970.' },
        { name: 'epochSec', type: 'number', desc: 'I secondi dal 1970. Solo con l\'ora corrente.' },
        { name: 'formatted', type: 'string', desc: 'La data nel formato chiesto. Solo formattando.' },
        { name: 'weekday', type: 'string', desc: 'Il giorno della settimana. Solo formattando.' },
        { name: 'value', type: 'number', desc: 'La differenza nell\'unita` scelta, con i decimali. Solo calcolando una differenza.' },
        { name: 'rounded', type: 'number', desc: 'La stessa differenza troncata all\'intero.' },
        { name: 'unit', type: 'string', desc: 'In che unita` e` espressa la differenza.' },
        { name: 'absMs', type: 'number', desc: 'La differenza in millisecondi, sempre positiva.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
