...
const cryptoExecutor: NodeExecutor = async (config, input) => {
  ...
  switch (op) {
    ...
    case 'hmac-verify': {
      ...
      const valid =
        expectedBuf.length === computed.length && c.timingSafeEqual(expectedBuf, computed);
      return {
        output: {
          valid,
          result: computed.toString(encoding),
          operation: op,
          algorithm: algo,
          encoding,
        },
    ...
    case 'encode':
      result = Buffer.from(value, 'utf8').toString(
        str(config.targetFormat, 'base64') as BufferEncoding,
      );
      break;
    case 'decode':
      result = Buffer.from(value, str(config.sourceFormat, 'base64') as BufferEncoding).toString(
        'utf8',
      );
      break;
    case 'random': {
  ...
  return {
    output: { result, operation: op, algorithm: algo, encoding },
    durationMs: Date.now() - startedAt,
  };
};
...
export const cryptoNode: NodeModule = {
  def: {
    ...
    configFields: [
      {
        ...
        required: true,
        defaultValue: 'hash',
        options: ['hash', 'hmac', 'hmac-verify', 'encode', 'decode', 'random'],
      ...
      {
        key: 'expected',
        label: 'Firma attesa (da verificare)',
        type: 'expression',
        required: false,
    ...
    outputContract: {
      notes: 'Verificando una firma esce anche `valid`, ed e` quello il campo su cui decidere: `result` c\'e` comunque e contiene il valore calcolato, che da solo non dice se il confronto e` andato bene.',
      fields: [
        { name: 'result', type: 'string', desc: 'Il valore calcolato — impronta, firma o cifratura — nella codifica scelta.' },
        { name: 'valid', type: 'boolean', desc: 'Se la firma corrisponde. Presente SOLO verificando.' },
        { name: 'operation', type: 'string', desc: 'L\'operazione eseguita.' },
        { name: 'algorithm', type: 'string', desc: 'L\'algoritmo usato.' },
        { name: 'encoding', type: 'string', desc: 'In che codifica e` scritto `result`.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
const uuidExecutor: NodeExecutor = async (config) => {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    switch (version) {
      ...
      case 'ulid':
        id = ulidLikeFrom(c.randomBytes(16));
        break;
      case 'short':
        id = c.randomBytes(8).toString('base64').replace(/[+/=]/g, '').slice(0, 10);
        break;
      case 'nanoid':
        id = c.randomBytes(16).toString('base64url').slice(0, 21);
        break;
      default:
        id = c.randomUUID();
    }
  ...
  return {
    output: count === 1 ? { id: ids[0], ids } : { ids },
    durationMs: Date.now() - startedAt,
  };
};
...
export const uuidNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Chiedendone UNO escono sia `id` sia `ids`; chiedendone piu` d\'uno esce solo `ids` — `id` NON c\'e`. Un\'espressione che legge sempre `id` si rompe appena si alza il numero.',
      fields: [
        { name: 'id', type: 'string', desc: 'L\'identificativo generato. Presente SOLO quando se ne chiede uno.' },
        { name: 'ids', type: 'array', desc: 'Tutti gli identificativi generati. Presente sempre.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
...
const jwtExecutor: NodeExecutor = async (config, input) => {
  ...
  if (op === 'decode') {
    const token =
      config.token !== undefined && str(config.token) !== '' ? str(config.token) : str(input);
    ...
    if (!headerR.ok || !payloadR.ok)
      throw new Error('action_jwt: token non decodificabile (header o payload non sono JSON)');
    return {
      output: { header: headerR.value, payload: payloadR.value, verified: false },
      durationMs: Date.now() - startedAt,
    };
  }
  ...
  if (op === 'sign') {
    ...
    if (!parsed.ok)
      throw new Error('action_jwt: il payload da firmare non è un oggetto JSON valido');
    ...
    return {
      output: { token: `${header}.${body}.${sig}`, payload },
      durationMs: Date.now() - startedAt,
    };
  }
  ...
  const expected = b64url(
    c.createHmac(hashAlgo, secret).update(`${parts[0]}.${parts[1]}`).digest(),
  );
  ...
  if (!payloadR.ok) {
    return {
      output: {
        ...
        expired: false,
        payload: null,
        error: 'payload non-JSON (token malformato)',
      },
      durationMs: Date.now() - startedAt,
    };
  }
  ...
  return {
    output: { verified: valid && notExpired, signatureValid: valid, expired: !notExpired, payload },
    durationMs: Date.now() - startedAt,
  };
};
...
export const jwtNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Tre forme diverse. FIRMANDO escono `token` e `payload`. VERIFICANDO escono `verified`, `signatureValid`, `expired` e `payload` — e badare a `verified`, non a `signatureValid`: una firma giusta su un token SCADUTO da` `signatureValid` vero e `verified` falso. DECODIFICANDO senza chiave escono `header`, `payload` e `verified` sempre falso, perche` non ha controllato niente.',
      fields: [
        { name: 'token', type: 'string', desc: 'Il token firmato. Solo firmando.' },
        { name: 'payload', type: 'object|null', desc: 'Il contenuto del token. Null se non si e` potuto leggere.' },
        { name: 'header', type: 'object', desc: 'L\'intestazione del token. Solo decodificando.' },
        { name: 'verified', type: 'boolean', desc: 'Vero solo se la firma e` giusta E il token non e` scaduto. E` il campo su cui fidarsi.' },
        { name: 'signatureValid', type: 'boolean', desc: 'Se la sola firma corrisponde, scadenza a parte. Solo verificando.' },
        { name: 'expired', type: 'boolean', desc: 'Se il token e` scaduto. Solo verificando.' },
        { name: 'error', type: 'string', desc: 'Cosa non ha funzionato nella lettura del token. Presente solo in quel caso.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
