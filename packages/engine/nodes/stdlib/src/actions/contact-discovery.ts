...
export const contactDiscoveryNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: 'Gira per il sito di un dominio cercando i recapiti. Non trovare niente NON e` un errore: `has_emails` resta falso e `reason_if_empty` dice perche`. `cache_hit` a vero significa che non ha visitato niente — la risposta viene da un giro precedente.',
      fields: [
        { name: 'emails', type: 'array', desc: 'Gli indirizzi trovati sul sito.' },
        { name: 'primary_email', type: 'string|null', desc: 'Il piu` promettente fra quelli. Null se non ne ha trovati.' },
        { name: 'source_page', type: 'string|null', desc: 'La pagina in cui ha trovato l\'indirizzo principale.' },
        { name: 'pages_visited', type: 'number', desc: 'Quante pagine ha aperto.' },
        { name: 'paths_tried', type: 'array', desc: 'I percorsi tentati: contatti, chi siamo, e simili.' },
        { name: 'domain', type: 'string', desc: 'Il dominio esaminato.' },
        { name: 'took_ms', type: 'number', desc: 'Quanto ci ha messo.' },
        { name: 'cache_hit', type: 'boolean', desc: 'Vero se la risposta viene dalla memoria e non da una visita.' },
        { name: 'has_emails', type: 'boolean', desc: 'Se ha trovato almeno un indirizzo. E` il campo su cui diramare.' },
        { name: 'reason_if_empty', type: 'string|null', desc: 'Perche` non ha trovato niente. Null quando ha trovato qualcosa.' },
        { name: 'company_name', type: 'string|null', desc: 'Il nome dell\'azienda letto dal sito.' },
        { name: 'description', type: 'string|null', desc: 'La descrizione dichiarata dal sito.' },
        { name: 'site_language', type: 'string|null', desc: 'La lingua del sito.' },
        { name: 'content_text', type: 'string', desc: 'Il testo della pagina principale, ripulito.' },
        { name: 'og_image', type: 'string|null', desc: 'L\'immagine di anteprima dichiarata dal sito.' },
        { name: 'content', type: 'string', desc: 'Il contenuto della pagina, per chi deve analizzarlo a valle.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.1.0',
