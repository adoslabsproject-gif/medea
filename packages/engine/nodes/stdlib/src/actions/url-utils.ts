...
const urlExecutor: NodeExecutor = async (config, input) => {
  ...
  if (op === 'parse') {
    let u: URL;
    try { u = new URL(raw); } catch { throw new Error(`action_url: URL non valido "${raw}"`); }
    const query: Record<string, string> = {};
    u.searchParams.forEach((val, k) => { query[k] = val; });
    return {
      output: {
        protocol: u.protocol.replace(':', ''), host: u.host, hostname: u.hostname, port: u.port,
        pathname: u.pathname, query, hash: u.hash.replace('#', ''), origin: u.origin, href: u.href,
      },
  ...
  if (op === 'setQuery' || op === 'removeQuery') {
    ...
    if (rawParams && typeof rawParams === 'object') params = rawParams as Record<string, unknown>;
      ...
      try { params = JSON.parse(rawParams) as Record<string, unknown>; } catch { /* ignora */ }
    }
    if (op === 'setQuery') for (const [k, val] of Object.entries(params)) u.searchParams.set(k, str(val));
    else for (const k of Object.keys(params)) u.searchParams.delete(k);
    return { output: { href: u.href, query: Object.fromEntries(u.searchParams) }, durationMs: Date.now() - startedAt };
  }

  if (op === 'build') {
    const base = str(config.base);
    let u: URL;
    try { u = new URL(base); } catch { throw new Error(`action_url: base non valida "${base}"`); }
  ...
  if (op === 'encode') return { output: { result: encodeURIComponent(raw) }, durationMs: Date.now() - startedAt };
  if (op === 'decode') {
    ...
    try {
      ...
      throw new Error(`action_url: impossibile decodificare "${raw}" (sequenza percent-encoding non valida)`);
    }
  }
...
export const urlNode: NodeModule = {
  def: {
    ...
    configFields: [
      ...
      {
        key: 'url', label: 'URL', type: 'expression', required: false, defaultValue: 'input',
        help: 'L\'URL su cui operare (o il valore per encode/decode). Vuoto = input del nodo.',
        showIf: { field: 'operation', in: ['parse', 'setQuery', 'removeQuery', 'encode', 'decode'] },
      },
      {
        key: 'base', label: 'URL base', type: 'expression', required: false,
        placeholder: 'https://api.esempio.it',
        help: 'L\'origine + eventuale percorso base da cui partire.',
        showIf: { field: 'operation', equals: 'build' },
      },
      {
        key: 'path', label: 'Percorso', type: 'text', required: false,
        placeholder: '/v1/ordini',
        help: 'Il percorso da impostare sull\'URL base.',
        showIf: { field: 'operation', equals: 'build' },
      },
      {
        key: 'params', label: 'Parametri query (JSON)', type: 'json', required: false,
