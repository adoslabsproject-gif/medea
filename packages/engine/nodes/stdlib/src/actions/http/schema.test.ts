...
describe('HttpConfigSchema — cross-field authMode↔credenziale', () => {
  ...
  it('authMode=bearer con token → OK', () => {
    expect(
      HttpConfigSchema.safeParse({ ...base, authMode: 'bearer', bearerToken: '{{secrets.tok}}' })
        .success,
    ).toBe(true);
  });
  ...
  it('🚨 authMode=bearer con token SOLO spazi → REJECT (trim)', () => {
    expect(
      HttpConfigSchema.safeParse({ ...base, authMode: 'bearer', bearerToken: '   ' }).success,
    ).toBe(false);
  });
  ...
  it('🚨 authMode=custom senza customAuthHeaderValue → REJECT', () => {
    expect(
      HttpConfigSchema.safeParse({ ...base, authMode: 'custom', customAuthHeaderName: 'X-Foo' })
        .success,
    ).toBe(false);
  });
  ...
  it('authMode=basic con solo password → OK (user opzionale)', () => {
    expect(
      HttpConfigSchema.safeParse({ ...base, authMode: 'basic', basicPass: 'pw' }).success,
    ).toBe(true);
  });
  ...
  const oauthOk = {
    authMode: 'oauth2',
    oauth2TokenUrl: 'https://id.example.com/token',
    oauth2ClientId: 'cid',
  ...
  it('🚨 authMode=oauth2 senza tokenUrl → REJECT (path oauth2TokenUrl)', () => {
    ...
    expect(r.success).toBe(false);
    if (!r.success)
      expect(r.error.issues.some((i) => i.path.includes('oauth2TokenUrl'))).toBe(true);
  });
  ...
  it('🚨 authMode=oauth2 senza clientId → REJECT', () => {
    ...
    expect(r.success).toBe(false);
    if (!r.success)
      expect(r.error.issues.some((i) => i.path.includes('oauth2ClientId'))).toBe(true);
  });
  ...
  it('🚨 authMode=oauth2 senza clientSecret → REJECT', () => {
    ...
    expect(r.success).toBe(false);
    if (!r.success)
      expect(r.error.issues.some((i) => i.path.includes('oauth2ClientSecret'))).toBe(true);
  });
  ...
  it('oauth2AuthStyle default = header; accetta body', () => {
    ...
    expect(
      HttpConfigSchema.safeParse({ ...base, ...oauthOk, oauth2AuthStyle: 'body' }).success,
    ).toBe(true);
    expect(
      HttpConfigSchema.safeParse({ ...base, ...oauthOk, oauth2AuthStyle: 'bogus' }).success,
    ).toBe(false);
  });
...
describe('HttpConfigSchema — tipi + default', () => {
  it('🚨 secret non-stringa (bearerToken numerico) → REJECT (era z.unknown, ora string)', () => {
    expect(
      HttpConfigSchema.safeParse({ ...base, authMode: 'bearer', bearerToken: 12345 }).success,
    ).toBe(false);
  });
  ...
  it('headersJson accetta sia stringa JSON sia record (difensivo), non throwa', () => {
    expect(HttpConfigSchema.safeParse({ ...base, headersJson: '{"X-Foo":"bar"}' }).success).toBe(
      true,
    );
    expect(HttpConfigSchema.safeParse({ ...base, headersJson: { 'X-Foo': 'bar' } }).success).toBe(
      true,
