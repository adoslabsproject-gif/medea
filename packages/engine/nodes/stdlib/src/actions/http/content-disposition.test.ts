...
describe('R7 — filename* RFC 5987 (charset/lang)', () => {
  it('🚨 UTF-8 con language tag → value estratto e decodato', () => {
    expect(
      filenameFromContentDisposition("attachment; filename*=UTF-8'it'fattura%20mese.pdf"),
    ).toBe('fattura mese.pdf');
  });
  ...
  it('🚨 charset NON UTF-8 (iso-8859-1) → value comunque estratto (non ignorato)', () => {
    ...
    expect(filenameFromContentDisposition("attachment; filename*=iso-8859-1'en'rates.txt")).toBe(
      'rates.txt',
    );
  });

  it('UTF-8 senza language → simbolo decodato', () => {
    expect(filenameFromContentDisposition("attachment; filename*=UTF-8''%E2%82%AC.pdf")).toBe(
      '€.pdf',
    );
  });
