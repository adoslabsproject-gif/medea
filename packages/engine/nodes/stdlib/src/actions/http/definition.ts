...
export const httpNodeDef: NodeDef = {
  ...
  configFields: [
    ...
    },
    {
      ...
      type: 'text',
      required: false,
      showIf: { field: 'authMode', equals: 'basic' },
    },
    {
      key: 'basicPass',
    ...
    },
    {
      ...
      type: 'secret',
      required: false,
      help: 'Il token segreto da mettere dopo "Bearer ".',
      showIf: { field: 'authMode', equals: 'bearer' },
    },
    {
      ...
      placeholder: 'X-API-Key  oppure  X-Auth-Token',
      showIf: { field: 'authMode', equals: 'apikey-header' },
    },
    {
      key: 'apiKeyValue',
    ...
    {
      key: 'customAuthHeaderName',
      label: 'Nome header custom',
      type: 'text',
      required: false,
      defaultValue: 'Authorization',
      showIf: { field: 'authMode', equals: 'custom' },
    },
    {
      key: 'customAuthHeaderValue',
      label: 'Valore header custom',
      type: 'secret',
      required: false,
      showIf: { field: 'authMode', equals: 'custom' },
    },
    // ── OAuth2 client_credentials
    {
      ...
      help: 'URL del token endpoint OAuth2 (grant client_credentials).',
    ...
    {
      key: 'oauth2Scope',
      label: 'Scope (opzionale)',
      type: 'text',
      required: false,
      placeholder: 'read:data write:data',
      help: 'Scope OAuth2 separati da spazio. Vuoto = nessuno scope richiesto.',
      showIf: { field: 'authMode', equals: 'oauth2' },
    },
    {
      key: 'oauth2AuthStyle',
      label: 'Invio credenziali client',
      type: 'select',
      required: false,
      options: ['header', 'body'],
      defaultValue: 'header',
      help: 'header = HTTP Basic sul token endpoint (default, RFC 6749). body = client_id/secret nel form.',
      showIf: { field: 'authMode', equals: 'oauth2' },
    },
    ...
    {
      key: 'paginationMaxPages',
      label: 'Max pagine',
      type: 'number',
      required: false,
      defaultValue: '10',
      help: 'Limite di sicurezza — interrompe il loop anche se ci sono altre pagine.',
      showIf: {
        field: 'paginationMode',
        in: ['page-number', 'offset-limit', 'cursor', 'link-header'],
      },
    },
    {
      key: 'paginationPageSize',
      label: 'Items per pagina (limit)',
      type: 'number',
      required: false,
      defaultValue: '50',
      showIf: { field: 'paginationMode', in: ['page-number', 'offset-limit'] },
    },
    {
      key: 'paginationItemsField',
      label: 'Campo array nella risposta',
      type: 'text',
      required: false,
      placeholder: 'es. data, items, results',
      help: "Se valorizzato, concatena response[campo]. Se vuoto, concatena l'intera response.",
      showIf: {
        field: 'paginationMode',
        in: ['page-number', 'offset-limit', 'cursor', 'link-header'],
      },
    ...
    {
      key: 'paginationPageParam',
      label: 'Nome parametro pagina',
      type: 'text',
      required: false,
      defaultValue: 'page',
      showIf: { field: 'paginationMode', equals: 'page-number' },
    },
    {
      key: 'paginationStartPage',
    ...
    {
      key: 'paginationLimitParam',
      label: 'Nome parametro limit',
      type: 'text',
      required: false,
      defaultValue: 'limit',
      showIf: { field: 'paginationMode', in: ['page-number', 'offset-limit'] },
    ...
    {
      key: 'paginationOffsetParam',
      label: 'Nome parametro offset',
      type: 'text',
      required: false,
      defaultValue: 'offset',
      showIf: { field: 'paginationMode', equals: 'offset-limit' },
    },
    {
      key: 'paginationStartOffset',
      label: 'Offset iniziale',
      type: 'number',
      required: false,
      defaultValue: '0',
      showIf: { field: 'paginationMode', equals: 'offset-limit' },
    ...
    {
      key: 'paginationCursorParam',
      label: 'Nome parametro cursor',
      type: 'text',
      required: false,
      defaultValue: 'cursor',
      showIf: { field: 'paginationMode', equals: 'cursor' },
    },
    {
      key: 'paginationCursorField',
      label: 'Campo next cursor nella risposta',
      type: 'text',
      required: false,
      defaultValue: 'next_cursor',
      help: 'Es. next_cursor, nextPage, paging.next.cursor',
      showIf: { field: 'paginationMode', equals: 'cursor' },
    },
    ...
    },
    {
      ...
      required: false,
      defaultValue: '0',
      help: 'Quante volte ritentare in caso di errore (5xx o codici configurati). 0 = no retry. Ignorato se "Chi gestisce il retry" = workflow/none.',
    },
    {
      key: 'retryInitialDelayMs',
      label: 'Delay iniziale (ms)',
      type: 'number',
      required: false,
      defaultValue: '500',
      help: 'Pausa prima del primo retry. Si moltiplica per il fattore ad ogni tentativo.',
      showIf: { field: 'retryCount', truthy: true },
    },
    {
      key: 'retryBackoffFactor',
      label: 'Fattore backoff esponenziale',
      type: 'number',
      required: false,
      defaultValue: '2',
      help: '2 = raddoppia ogni retry (500ms, 1000ms, 2000ms, ...). 1 = delay costante.',
      showIf: { field: 'retryCount', truthy: true },
    },
    {
      key: 'retryOnStatus',
      label: 'Status da ritentare (csv)',
      type: 'text',
      required: false,
      defaultValue: '429,500,502,503,504',
      placeholder: '429,500,502,503,504',
      help: 'Codici HTTP che fanno scattare il retry. Errori di network/timeout sono sempre ritentati.',
      showIf: { field: 'retryCount', truthy: true },
    },
    ...
    },
    {
      ...
      required: false,
      defaultValue: '30000',
      help: 'Default 30s. Aumenta per API lente (es. report generation).',
    },
    {
      key: 'maxResponseMb',
      label: 'Max risposta (MB)',
      type: 'number',
      required: false,
      defaultValue: '50',
      help: 'Cap dimensione risposta (anti-OOM). Default 50 MB, max 500. Se superato (via content-length o in streaming) il nodo si ferma con errore invece di saturare la memoria.',
    },
    {
      key: 'throwOnError',
      label: 'Errore su status 4xx/5xx',
      type: 'boolean',
      required: false,
      defaultValue: 'true',
