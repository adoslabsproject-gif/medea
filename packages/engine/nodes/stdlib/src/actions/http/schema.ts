...
export const HttpAuthModeSchema = z
  .enum(['none', 'basic', 'bearer', 'apikey-header', 'custom', 'oauth2'])
  .default('none');
...
export const HttpBodyTypeSchema = z
  .enum(['none', 'json', 'form-urlencoded', 'multipart', 'raw-text', 'raw-binary-base64', 'binary'])
  .default('json');

export const HttpResponseFormatSchema = z.enum(['auto', 'json', 'text', 'binary']).default('auto');

export const HttpPaginationModeSchema = z
  .enum(['none', 'page-number', 'offset-limit', 'cursor', 'link-header'])
  .default('none');
  ...
  .superRefine((c, ctx) => {
    ...
    if (c.authMode === 'basic' && empty(c.basicUser) && empty(c.basicPass)) {
      ctx.addIssue({
        ...
        path: ['basicUser'],
        message: 'authMode=basic richiede username e/o password',
      });
    }
    if (c.authMode === 'bearer' && empty(c.bearerToken)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['bearerToken'],
        message: 'authMode=bearer richiede un token',
      });
    }
    if (c.authMode === 'apikey-header' && empty(c.apiKeyValue)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['apiKeyValue'],
        message: 'authMode=apikey-header richiede il valore della API key',
      });
    }
    if (c.authMode === 'custom' && empty(c.customAuthHeaderValue)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['customAuthHeaderValue'],
        message: "authMode=custom richiede il valore dell'header",
      });
    }
    if (c.authMode === 'oauth2') {
      if (empty(c.oauth2TokenUrl)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['oauth2TokenUrl'],
          message: 'authMode=oauth2 richiede il token endpoint URL',
        });
      }
      if (empty(c.oauth2ClientId)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['oauth2ClientId'],
          message: 'authMode=oauth2 richiede il client_id',
        });
      }
      if (empty(c.oauth2ClientSecret)) {
        ctx.addIssue({
          ...
          message: 'authMode=oauth2 richiede il client_secret',
        });
      }
    }
  });
