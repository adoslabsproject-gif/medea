...
describe('action_crypto', () => {
  it('hash sha256 = valore noto', async () => {
    ...
    expect((r.output as { result: string }).result).toBe(createHash('sha256').update('hello').digest('hex'));
  });
  it('hmac deterministico con stessa chiave', async () => {
    const a = await crypto({ operation: 'hmac', algorithm: 'sha256', key: 'k', value: 'msg' }, undefined);
    const b = await crypto({ operation: 'hmac', algorithm: 'sha256', key: 'k', value: 'msg' }, undefined);
  ...
  describe('hmac-verify — verifica firma webhook timing-safe', () => {
    it('firma corretta → valid:true (round-trip con hmac)', async () => {
      const sig = (await crypto({ operation: 'hmac', algorithm: 'sha256', key: 'whsec', value: 'body' }, undefined)).output as { result: string };
      const r = await crypto({ operation: 'hmac-verify', algorithm: 'sha256', key: 'whsec', value: 'body', expected: sig.result }, undefined);
      expect((r.output as { valid: boolean }).valid).toBe(true);
    });
    it('firma errata → valid:false (no throw)', async () => {
      const r = await crypto({ operation: 'hmac-verify', algorithm: 'sha256', key: 'whsec', value: 'body', expected: 'deadbeef' }, undefined);
    ...
    it('firma di lunghezza diversa → valid:false SENZA throw (timingSafeEqual non lancia)', async () => {
      ...
      const r = await crypto({ operation: 'hmac-verify', algorithm: 'sha256', key: 'whsec', value: 'body', expected: 'ab' }, undefined);
      expect((r.output as { valid: boolean }).valid).toBe(false);
    });
    it('chiave diversa → valid:false (firma non combacia)', async () => {
      const sig = (await crypto({ operation: 'hmac', algorithm: 'sha256', key: 'right', value: 'body' }, undefined)).output as { result: string };
      const r = await crypto({ operation: 'hmac-verify', algorithm: 'sha256', key: 'wrong', value: 'body', expected: sig.result }, undefined);
      expect((r.output as { valid: boolean }).valid).toBe(false);
    });
    it('base64 encoding round-trip', async () => {
      const sig = (await crypto({ operation: 'hmac', algorithm: 'sha256', key: 'k', value: 'm', encoding: 'base64' }, undefined)).output as { result: string };
      const r = await crypto({ operation: 'hmac-verify', algorithm: 'sha256', key: 'k', value: 'm', encoding: 'base64', expected: sig.result }, undefined);
      expect((r.output as { valid: boolean }).valid).toBe(true);
    });
    it('richiede la chiave', async () => {
      await expect(crypto({ operation: 'hmac-verify', value: 'x', expected: 'y' }, undefined)).rejects.toThrow(/chiave/);
    });
  });
  it('encode/decode base64 round-trip', async () => {
    const enc = await crypto({ operation: 'encode', targetFormat: 'base64', value: 'ciào' }, undefined);
    const dec = await crypto({ operation: 'decode', sourceFormat: 'base64', value: (enc.output as { result: string }).result }, undefined);
  ...
  it('usa l\'input del nodo se value è vuoto', async () => {
    ...
    expect((r.output as { result: string }).result).toBe(createHash('sha256').update('fromInput').digest('hex'));
  });
});
...
describe('action_uuid', () => {
  it('v4 ha il formato UUID', async () => {
    ...
    expect((r.output as { id: string }).id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);
  });
...
describe('action_jwt', () => {
  it('sign → verify round-trip valido', async () => {
    const signed = (await jwt({ operation: 'sign', algorithm: 'HS256', secret: 's3cr3t', payload: '{"sub":"u1"}', expiresIn: '3600' }, undefined)).output as { token: string };
    const v = (await jwt({ operation: 'verify', algorithm: 'HS256', secret: 's3cr3t', token: signed.token }, undefined)).output as { verified: boolean; payload: { sub: string } };
    expect(v.verified).toBe(true);
    expect(v.payload.sub).toBe('u1');
  });
  it('verify con secret sbagliato → non verificato (timing-safe)', async () => {
    const signed = (await jwt({ operation: 'sign', secret: 'right', payload: '{}' }, undefined)).output as { token: string };
    const v = (await jwt({ operation: 'verify', secret: 'wrong', token: signed.token }, undefined)).output as { verified: boolean; signatureValid: boolean };
    expect(v.verified).toBe(false);
    expect(v.signatureValid).toBe(false);
  });
  it('token scaduto → expired=true (exp nel passato)', async () => {
    const past = Math.floor(Date.now() / 1000) - 100;
    const signed = (await jwt({ operation: 'sign', secret: 's', payload: JSON.stringify({ exp: past }) }, undefined)).output as { token: string };
    const v = (await jwt({ operation: 'verify', secret: 's', token: signed.token }, undefined)).output as { verified: boolean; expired: boolean };
  ...
  it('decode legge il payload senza verificare (verified:false)', async () => {
    const signed = (await jwt({ operation: 'sign', secret: 's', payload: '{"role":"admin"}' }, undefined)).output as { token: string };
    const d = (await jwt({ operation: 'decode', token: signed.token }, undefined)).output as { payload: { role: string }; verified: boolean };
  ...
  it('verify su token con payload NON-JSON → verified:false, NON lancia (anti-DoS)', async () => {
    ...
    const v = (await jwt({ operation: 'verify', secret: 's', token: malformed }, undefined)).output as { verified: boolean; signatureValid: boolean; payload: unknown };
    expect(v.verified).toBe(false);
    expect(v.signatureValid).toBe(false);
  ...
  it('decode su token con parti non-JSON → errore PULITO (no JSON.parse criptico)', async () => {
    ...
    await expect(jwt({ operation: 'decode', token: bad }, undefined)).rejects.toThrow(/non decodificabile/);
  });
  it('sign con payload JSON invalido → errore pulito', async () => {
    await expect(jwt({ operation: 'sign', secret: 's', payload: '{payload rotto' }, undefined)).rejects.toThrow(/non.*JSON valido/);
  });
  it('verify su garbage totale (non 3 parti) → throw controllato, no crash', async () => {
    await expect(jwt({ operation: 'verify', secret: 's', token: 'spazzatura' }, undefined)).rejects.toThrow(/malformato/);
  });
});
