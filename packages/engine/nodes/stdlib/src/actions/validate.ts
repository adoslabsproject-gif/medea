...
function normalizePhoneIt(raw: string): { valid: boolean; e164: string | null } {
  ...
  if (v.startsWith('+'))
    return { valid: /^\+\d{8,15}$/.test(v), e164: /^\+\d{8,15}$/.test(v) ? v : null };
...
const validateExecutor: NodeExecutor = async (config, input) => {
  ...
  const value = str(
    config.value !== undefined && str(config.value) !== '' ? config.value : input,
  ).trim();
  let valid = false;
  ...
  switch (type) {
    ...
    case 'url':
      ...
      break;
    case 'piva':
      valid = validatePIva(value);
      normalized = value.replace(/\s/g, '');
      break;
    case 'codice_fiscale':
      valid = validateCodiceFiscale(value);
      normalized = value.toUpperCase().replace(/\s/g, '');
      break;
    case 'iban':
      valid = validateIban(value);
      normalized = value.toUpperCase().replace(/\s/g, '');
      break;
    case 'phone_it': {
      const r = normalizePhoneIt(value);
      valid = r.valid;
      normalized = r.e164;
      break;
    }
  ...
  return {
    output: { valid, normalized, type, value },
    branch: valid ? 'valid' : 'invalid',
