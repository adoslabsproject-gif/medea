...
export const emailPersonalizeNode: NodeModule = {
  def: {
    ...
    outputContract: {
      notes: '`success` va guardato prima di usare `snippet`: quando il modello non trova appigli nella pagina il nodo riesce lo stesso, ma il testo non e` utilizzabile e `reason` dice perche`.',
      fields: [
        { name: 'snippet', type: 'string', desc: 'La frase personalizzata da mettere nella email.' },
        { name: 'evidence_quote', type: 'string', desc: 'Il passaggio della pagina su cui si e` basata: serve a controllare che non se la sia inventata.' },
        { name: 'confidence', type: 'number', desc: 'Quanto e` sicuro, da 0 a 1.' },
        { name: 'success', type: 'boolean', desc: 'Se la frase e` utilizzabile. Da controllare prima di mandarla.' },
        { name: 'reason', type: 'string', desc: 'Perche` non ci e` riuscito, quando non ci riesce.' },
        { name: 'llm_provider', type: 'string', desc: 'Il fornitore interpellato.' },
        { name: 'llm_model', type: 'string', desc: 'Il modello usato.' },
        { name: 'llm_usage', type: 'object', desc: 'Il consumo di token. Presente quando il provider lo riporta.' },
      ],
    },
    vendor: 'flowforge',
    version: '1.0.0',
