...
describe('action_html_select', () => {
  ...
  it('estrai list di .tag', async () => {
    ...
    expect((r.output as { fields: { tags: string[] } }).fields.tags).toEqual([
      'red',
      'blue',
      'green',
    ]);
  ...
  it('errore se selectorsJson vuoto', async () => {
    ...
    ).rejects.toThrow(/selectorsJson required/);
  });
});
...
describe('action_script_var_extract', () => {
  ...
  it('estrai window.video.id come number', async () => {
    ...
    expect((r.output as { extracted: Record<string, unknown> }).extracted['window.video.id']).toBe(
      1234,
    );
  });
  ...
  it('estrai token con quote doppie (extractKey cerca nel blocco intero)', async () => {
    ...
    expect(
      (r.output as { extracted: Record<string, unknown> }).extracted['window.masterPlaylist.token'],
    ).toBe('abc-123-xyz');
  });
  ...
  it('estrai filename con quote singole', async () => {
    ...
    expect(
      (r.output as { extracted: Record<string, unknown> }).extracted['window.video.filename'],
    ).toBe('movie.mp4');
  });
  ...
  it('variabile inesistente → null', async () => {
    ...
    expect(
      (r.output as { extracted: Record<string, unknown> }).extracted['window.ghost'],
    ).toBeNull();
  });
