...
describe('buildVisionMessages', () => {
  it('struttura: system + user con image+text', () => {
    ...
    const userContent = msgs[1]?.content as { type: string; image_url?: { url: string }; text?: string }[];
  ...
  it('mimeType jpeg → data URL ha jpeg', () => {
    const msgs = buildVisionMessages({ screenshotBase64: 'xyz', prompt: 'p', mimeType: 'image/jpeg' });
  ...
  it('schema → text prompt include "SCHEMA JSON TARGET" + schema text', () => {
    const msgs = buildVisionMessages({ screenshotBase64: 'a', prompt: 'p', schemaJson: '{"title":"string"}' });
...
describe('visionExtractNode.executor — input validation', () => {
  it('screenshotBase64 vuoto → throw', async () => {
    ...
    await expect(visionExtractNode.executor({ prompt: 'p' }, null, ctx)).rejects.toThrow(/screenshotBase64 required/);
  });
...
describe('visionExtractNode.executor — happy path', () => {
  it('Risposta JSON pura → extracted parsed', async () => {
    mockedFetch.mockResolvedValue({
      ok: true, status: 200,
      json: async () => ({ choices: [{ message: { content: '{"title":"Prodotto X","price":99}' } }] }),
...
describe('visionExtractNode.executor — retry logic', () => {
  it('500 + 200 → 2 attempts, success', async () => {
    mockedFetch
      .mockResolvedValueOnce({ ok: false, status: 500, text: async () => 'overload' } as unknown as Response)
  ...
  it('3 x 500 → throw "after 3 attempts"', async () => {
    mockedFetch.mockResolvedValue({ ok: false, status: 500, text: async () => 'srv' } as unknown as Response);
  ...
  it('400 → NO retry (not 5xx)', async () => {
    mockedFetch.mockResolvedValue({ ok: false, status: 400, text: async () => 'bad req' } as unknown as Response);
...
describe('visionExtractNode.executor — request shape', () => {
  it('messages + max_tokens + response_format json_object', async () => {
    ...
    const body = JSON.parse((mockedFetch.mock.calls[0]![1] as { body: string }).body) as { model: string; max_tokens: number; response_format: { type: string }; temperature: number };
  ...
  it('maxTokens clamp [64, 8192]', async () => {
    ...
    const body = JSON.parse((mockedFetch.mock.calls[0]![1] as { body: string }).body) as { max_tokens: number };
...
describe('VisionExtractOutputSchema', () => {
  it('valida output shape correttamente (incl. _llm Fase 2 #14)', () => {
    const valid = {
      extracted: { x: 1 }, rawResponse: 'r', modelUsed: 'm', latencyMs: 100, attempts: 1,
      parseError: null, schemaValidationError: null,
  ...
  it('rejected se manca _llm (l\'usage è parte del contratto output)', () => {
    expect(() => VisionExtractOutputSchema.parse({ extracted: {}, rawResponse: 'r', modelUsed: 'm', latencyMs: 0, attempts: 1, parseError: null, schemaValidationError: null })).toThrow();
  });

  it('rejected se manca rawResponse', () => {
    expect(() => VisionExtractOutputSchema.parse({ extracted: {}, modelUsed: 'm', latencyMs: 0, attempts: 1, parseError: null, schemaValidationError: null })).toThrow();
  });
});
...
describe('visionExtractNode — gateway metered (Fase 2 #14)', () => {
  ...
  const run = async (config: Record<string, unknown>) => {
    if (!visionExtractNode.executor) throw new Error('executor mancante');
    return visionExtractNode.executor({ screenshotBase64: FAKE_SCREENSHOT, prompt: 'p', ...config }, null, ctx);
  };
  ...
  it('🚨 sentinelle legacy (endpoint :5004 + model Qwen2.5) salvate in config → gateway, model omesso', async () => {
    mockedFetch.mockResolvedValue(okBody({ model: 'liara', usage: { prompt_tokens: 500, completion_tokens: 20 } }));
    const res = await run({ endpoint: 'http://localhost:5004/v1/chat/completions', model: 'Qwen2.5-VL-7B-Instruct' });
    ...
    expect(out._llm).toEqual({ inputTokens: 500, outputTokens: 20, model: 'liara', provider: 'liara', fromApi: true });
    expect(out.modelUsed).toBe('liara');
  });

  it('endpoint BYOK custom → guard pieno (no allowedHosts), provider=custom', async () => {
    mockedFetch.mockResolvedValue(okBody());
    const res = await run({ endpoint: 'https://api.openai.com/v1/chat/completions', apiKey: 'sk-x', model: 'gpt-4o' });
    const [url, opts] = mockedFetch.mock.calls[0] as [string, { allowedHosts?: string[]; headers: Record<string, string> }];
