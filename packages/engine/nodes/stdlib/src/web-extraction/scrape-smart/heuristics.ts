...
export function detectThinHtml(html: string): ThinHtmlSignals {
  if (!html) {
    return { isThin: true, bodyContentLength: 0, hasNoScriptWarning: false, hasEmptyAppContainer: false, scriptVsContentRatio: 0 };
  }
  ...
  const hasNoScriptWarning = /<noscript[^>]*>[\s\S]*?(javascript|enable)[\s\S]*?<\/noscript>/i.test(html);
  const hasEmptyAppContainer = /<div\s+id=["'](?:root|app|__next|svelte|nuxt)["'][^>]*>\s*<\/div>/i.test(html);

  const scriptCount = (html.match(/<script[^>]*>[\s\S]*?<\/script>/gi) ?? []).length;
  const scriptVsContentRatio = bodyContentLength > 0 ? scriptCount / Math.max(1, bodyContentLength / 100) : scriptCount;
  ...
  return { isThin, bodyContentLength, hasNoScriptWarning, hasEmptyAppContainer, scriptVsContentRatio };
}
...
export interface AntiBotSignals {
  ...
  vendor: 'cloudflare' | 'akamai' | 'datadome' | 'perimeterx' | 'recaptcha' | 'hcaptcha' | 'cloudfront' | 'incapsula' | 'unknown' | null;
  evidence: string;
}

const ANTI_BOT_PATTERNS: { vendor: AntiBotSignals['vendor']; re: RegExp }[] = [
  { vendor: 'cloudflare', re: /(cf-ray|cloudflare|cf_clearance|checking your browser|__cf_chl_|cf-mitigated|just a moment\.\.\.|attention required.*cloudflare)/i },
  { vendor: 'akamai', re: /(akamai|akamai\.com\/bot|akamai bot manager|reference #18\.[a-f0-9]+)/i },
  { vendor: 'datadome', re: /(datadome|geo\.captcha-delivery\.com|dd_cookie_test)/i },
  { vendor: 'perimeterx', re: /(perimeterx|px-cdn|_px3|please verify you are a human|enable javascript and cookies)/i },
