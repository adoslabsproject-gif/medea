...
export async function runPipeline(cfg: PipelineConfig): Promise<PipelineResult> {
  ...
  if (cfg.forceStage && cfg.forceStage !== 'fetch_simple') {
    steps.push({
      ...
      status: 'skipped',
      durationMs: 0,
      evidence: `forceStage=${cfg.forceStage}`,
    });
  } else {
    const t0 = Date.now();
    try {
  ...
  if (cfg.skipUpgrade && html) {
    ...
    return {
      finalStage: 'fetch_simple',
      html,
      screenshotBase64: null,
      finalUrl,
      statusCode,
      steps,
      needsVision: vis.needsVision,
      visionReason: vis.reason,
    };
  }
  ...
  const shouldRender =
    !stopAtFetch &&
    (cfg.forceStage === 'browser_render' || (adaptive && thin.isThin && !antiBot.isChallenged));
  if (shouldRender) {
    if (!cfg.browserEndpoint) {
      steps.push({
        ...
        status: 'skipped',
        durationMs: 0,
        error: 'no browserEndpoint configured',
      });
    } else {
      const t0 = Date.now();
      try {
        ...
        steps.push({
          ...
          status: 'success',
          durationMs: Date.now() - t0,
          evidence: `thin: ${thin.bodyContentLength.toString()}b`,
        });
      } catch (err) {
        steps.push({
          stage: 'browser_render',
  ...
  const shouldStealth =
    !stopBeforeStealth &&
    (cfg.forceStage === 'browser_stealth' ||
      cfg.forceStage === 'vision_extract' ||
      (adaptive && antiBot2.isChallenged));
  if (shouldStealth) {
    if (!cfg.stealthEndpoint) {
      steps.push({
        stage: 'browser_stealth',
        status: 'skipped',
        durationMs: 0,
        error: 'no stealthEndpoint configured',
      });
    } else {
      const t0 = Date.now();
      try {
        const wantScreenshot =
          cfg.forceStage === 'vision_extract' || detectVisuallyOnly(html).needsVision;
        const r = await callStealth(
          cfg.stealthEndpoint,
          cfg.stealthApiKey ?? '',
          cfg.url,
          wantScreenshot,
        ...
        steps.push({
          ...
          status: 'success',
          durationMs: Date.now() - t0,
          evidence: antiBot2.vendor ?? 'forced',
        });
      } catch (err) {
        steps.push({
          stage: 'browser_stealth',
