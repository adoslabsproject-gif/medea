...
describe('scrapeSmartNode.executor — validation', () => {
  it('url vuoto → throw', async () => {
    ...
    await expect(scrapeSmartNode.executor({ prompt: 'p' }, null, ctx)).rejects.toThrow(
      /url required/,
    );
  });
  ...
  it('prompt vuoto → throw', async () => {
    if (!scrapeSmartNode.executor) throw new Error('exec mancante');
    await expect(scrapeSmartNode.executor({ url: 'https://t.com' }, null, ctx)).rejects.toThrow(
      /prompt required/,
    );
  });
  ...
  it('schemaJson invalido → throw', async () => {
    if (!scrapeSmartNode.executor) throw new Error('exec mancante');
    await expect(
      scrapeSmartNode.executor(
        { url: 'https://t.com', prompt: 'p', schemaJson: '{bad' },
        null,
        ctx,
      ),
...
describe('scrapeSmartNode.executor — single page happy path', () => {
  it('fetch HTML rich → LLM extract → output structured', async () => {
    ...
    mockedFetch.mockResolvedValueOnce({
      ...
      status: 200,
      json: async () => ({
        choices: [{ message: { content: '{"title":"Product XYZ","price":49.99}' } }],
      }),
    } as unknown as Response);

    if (!scrapeSmartNode.executor) throw new Error('exec mancante');
    const res = await scrapeSmartNode.executor(
      { url: 'https://t.com', prompt: 'estrai titolo e prezzo' },
      null,
      ctx,
    );
    const out = res.output as {
      ...
      pages: { finalStage: string; extracted: unknown }[];
    };
  ...
  it('anti-bot cloudflare → upgrade a stealth', async () => {
    ...
    mockedFetch
      ...
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ html: stealthHtml, finalUrl: 'https://x.com' }),
      } as unknown as Response)
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
...
describe('scrapeSmartNode.executor — pagination', () => {
  it('maxPages=3 + rel="next" trovato → follow 3 pagine', async () => {
    const p1 =
      '<html><body>page1 ' + 'x'.repeat(600) + '<a rel="next" href="/p/2">Next</a></body></html>';
    const p2 =
      '<html><body>page2 ' + 'x'.repeat(600) + '<a rel="next" href="/p/3">Next</a></body></html>';
    ...
    mockedFetch.mockImplementation(async (url, _opts) => {
      ...
      if (urlStr.includes('/v1/chat/completions')) {
        return {
          ...
          status: 200,
          json: async () => ({
            choices: [{ message: { content: `{"page":${(++fetchCount).toString()}}` } }],
          }),
        } as unknown as Response;
      }
      // fetch HTML
      if (urlStr.endsWith('/p1') || urlStr.endsWith('/page-1'))
        return { ok: true, status: 200, url: urlStr, text: async () => p1 } as unknown as Response;
      if (urlStr.endsWith('/p/2'))
        return { ok: true, status: 200, url: urlStr, text: async () => p2 } as unknown as Response;
      if (urlStr.endsWith('/p/3'))
        return { ok: true, status: 200, url: urlStr, text: async () => p3 } as unknown as Response;
      return { ok: true, status: 200, url: urlStr, text: async () => p1 } as unknown as Response;
    });
  ...
  it('maxPages=1 → no pagination', async () => {
    const html =
      '<html><body>x' + 'a'.repeat(600) + '<a rel="next" href="/p/2">N</a></body></html>';
    mockedFetch
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
    ...
    if (!scrapeSmartNode.executor) throw new Error('exec mancante');
    const res = await scrapeSmartNode.executor(
      { url: 'https://x.com', prompt: 'p', maxPages: 1 },
      null,
      ctx,
    );
  ...
  it('next URL === currentUrl → break (no infinite loop)', async () => {
    const html =
      '<html><body>' +
      'x'.repeat(600) +
      '<a rel="next" href="https://x.com/p1">N</a></body></html>';
    ...
    if (!scrapeSmartNode.executor) throw new Error('exec mancante');
    const res = await scrapeSmartNode.executor(
      { url: 'https://x.com/p1', prompt: 'p', maxPages: 10, pageDelayMs: 0 },
      null,
      ctx,
    );
  ...
  it('1 pagina → extracted singolare', async () => {
    mockedFetch
      .mockResolvedValueOnce({
        ...
        status: 200,
        url: 'https://x.com',
        text: async () => '<html><body>' + 'x'.repeat(600) + '</body></html>',
      } as unknown as Response)
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
