...
const executor: NodeExecutor = async (config, _input, _context) => {
  ...
  $('AdaptationSet').each((_, el) => {
    ...
    $as.find('Representation').each((__, repEl) => {
      ...
      const id = $r.attr('id'); if (id) rep.id = id;
      const mime = $r.attr('mimeType') ?? $as.attr('mimeType'); if (mime) rep.mimeType = mime;
      const codecs = $r.attr('codecs') ?? $as.attr('codecs'); if (codecs) rep.codecs = codecs;
      const w = $r.attr('width'); if (w) rep.width = Number(w);
      const h = $r.attr('height'); if (h) rep.height = Number(h);
      const bw = $r.attr('bandwidth'); if (bw) rep.bandwidth = Number(bw);
      const fr = $r.attr('frameRate'); if (fr) rep.frameRate = fr;
      const sr = $r.attr('audioSamplingRate'); if (sr) rep.audioSamplingRate = sr;
      reps.push(rep);
    });
    const aset: AdaptationSet = { representations: reps };
    const asId = $as.attr('id'); if (asId) aset.id = asId;
    const ct = $as.attr('contentType'); if (ct) aset.contentType = ct;
    const m = $as.attr('mimeType'); if (m) aset.mimeType = m;
    const lang = $as.attr('lang'); if (lang) aset.lang = lang;
    adaptationSets.push(aset);
  });

  const videoSets = adaptationSets.filter((a) => a.contentType === 'video' || (a.mimeType ?? '').startsWith('video'));
  const audioSets = adaptationSets.filter((a) => a.contentType === 'audio' || (a.mimeType ?? '').startsWith('audio'));
  const textSets = adaptationSets.filter((a) => a.contentType === 'text' || (a.mimeType ?? '').startsWith('text'));
...
export const dashProbeNode: NodeModule = {
  def: {
    ...
    outputs: ['type', 'profiles', 'totalDurationSec', 'minBufferTime', 'adaptationSets', 'counts', 'url'],
  },
  executor,
};
