...
describe('action_browser_render', () => {
  ...
  it('throw se endpoint non configurato (env+config entrambi vuoti)', async () => {
    await expect(browserRenderNode.executor!({ url: 'https://x.io' }, null, CTX)).rejects.toThrow(
  ...
  it('viewport clamp: width 100 → 320 (min)', async () => {
    ...
    global.fetch = vi.fn(async (_url, opts: RequestInit | undefined) => {
      ...
      return new Response(
        JSON.stringify({ html: '<ok/>', finalUrl: 'https://x.io', cookies: [] }),
        { status: 200 },
      );
    });
    await browserRenderNode.executor!(
      {
        url: 'https://x.io',
        endpoint: 'http://browser.local:3000',
        viewportWidth: 100,
        viewportHeight: 100,
      },
      null,
...
describe('action_cloudflare_solver', () => {
  ...
  it('throw se endpoint vuoto', async () => {
    await expect(
      cloudflareSolverNode.executor!({ url: 'https://x.io' }, null, CTX),
  ...
  it('throw se FlareSolverr risponde status != ok', async () => {
    global.fetch = vi.fn(
      async () =>
        new Response(JSON.stringify({ status: 'error', message: 'challenge failed' }), {
          status: 200,
        }),
    );
    await expect(
      cloudflareSolverNode.executor!(
        {
          url: 'https://x.io',
...
describe('action_user_agent_rotate', () => {
  ...
  it('pool bot → include Googlebot/FlowForge', async () => {
    ...
    expect(
      Array.from(seen).some(
        (ua) => ua.includes('Googlebot') || ua.includes('bingbot') || ua.includes('FlowForge'),
      ),
    ).toBe(true);
  });
  ...
  it('strategy deterministic + seedField → stesso UA per stessa key', async () => {
    ...
    expect((r1.output as { userAgent: string }).userAgent).toBe(
      (r2.output as { userAgent: string }).userAgent,
    );
  });
  ...
  it('pool mobile contiene UA iOS/Android', async () => {
    ...
    for (let i = 0; i < 20; i++) {
      const r = await userAgentRotateNode.executor!(
        { pool: 'mobile', strategy: 'random' },
        null,
        CTX,
      );
      seen.add((r.output as { userAgent: string }).userAgent);
    }
    expect(Array.from(seen).some((ua) => ua.includes('iPhone') || ua.includes('Android'))).toBe(
      true,
    );
  });
});
