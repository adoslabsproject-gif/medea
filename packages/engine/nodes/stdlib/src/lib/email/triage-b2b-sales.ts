...
const PATTERNS: Record<Exclude<B2BSalesLabel, 'needs_human_review'>, Record<'it' | 'en' | 'de' | 'fr', PatternSet>> = {
  interested_buy: {
    it: { patterns: [
      { test: /\b(vorrei|vogliamo|desidero|desideriamo)\s+(ordin|acquist|comprare)/i, score: 6, label: 'vorrei ordinare' },
      { test: /\b(facciamo|fate)mi?\s+(un\s+)?ordin/i, score: 5, label: 'facciamo ordine' },
      { test: /\b(ord(?:ine|inate?|inato))\b/i, score: 3, label: 'ordine' },
      { test: /\b(quant(?:o|i)|qual(?:i|e))\s+(scon|prezz|cost)/i, score: 4, label: 'sconto/prezzo' },
      { test: /\b(prim[ao]?)\s+(ordin|fornit)/i, score: 4, label: 'primo ordine' },
    ] },
    en: { patterns: [
      { test: /\b(i'?d like|we'?d like|would like)\s+to\s+(order|purchase|buy)/i, score: 6, label: 'like to order' },
    ...
    de: { patterns: [
      { test: /\b(ich|wir)\s+möchte(n)?\s+(bestell|kauf|order)/i, score: 6, label: 'möchte bestellen' },
      { test: /\bbestellung\b/i, score: 4, label: 'bestellung' },
      { test: /\bangebot\b/i, score: 3, label: 'angebot' },
    ] },
    fr: { patterns: [
      { test: /\b(je|nous)\s+(souhait|voudrais|voulons)\s+(command|achete)/i, score: 6, label: 'souhaite commander' },
  ...
  interested_info: {
    it: { patterns: [
      ...
      { test: /\bcondizioni\s+(commerciali|d[ie]\s+vendita|fornitura)/i, score: 5, label: 'condizioni commerciali' },
    ...
    fr: { patterns: [
      ...
      { test: /\bconditions (commerciales|de vente)\b/i, score: 5, label: 'conditions commerciales' },
    ] },
  },
  ...
  wrong_recipient: {
    it: { patterns: [
      { test: /\bnon sono\b[^.\n]{0,40}\b(persona|referente|responsabile|interlocutor|io quell)/i, score: 6, label: 'non sono il referente' },
  ...
  out_of_office: {
    it: { patterns: [
      { test: /\b(in\s+ferie|sono\s+in\s+vacanz|fuori\s+ufficio|out\s+of\s+office|risposta\s+automatic)/i, score: 8, label: 'out of office' },
  ...
  spam: {
    ...
    it: { patterns: [
      { test: /\b(bitcoin|crypt|nft|prestiti?\s+facile|guadagn[oi]\s+facile)/i, score: 5, label: 'bitcoin/crypto' },
...
export function classifyB2BSalesReply(input: ClassifyInput): ClassifyResult {
  ...
  const lang: 'it' | 'en' | 'de' | 'fr' = input.lang && input.lang !== 'auto'
    ? input.lang
    : detectLang(text);
  ...
  const confidence = winnerScore === 0
    ? 0
    : Math.min(1, winnerScore / (winnerScore + Math.max(runnerUp, 1)));
...
function suggestActionFor(label: keyof typeof PATTERNS): SuggestedAction {
  switch (label) {
    case 'interested_buy':     return 'send_quote';
    case 'interested_info':    return 'send_catalog';
    case 'interested_tasting': return 'book_tasting';
    case 'not_interested':     return 'send_unsubscribe_confirm';
    case 'wrong_recipient':    return 'forward_to_human';
    case 'complaint':          return 'forward_to_human';
    case 'out_of_office':      return 'archive';
    case 'spam':               return 'archive';
  }
}
...
export function detectLang(text: string): 'it' | 'en' | 'de' | 'fr' {
  ...
  const score = {
    it: count(t, [' di ', ' è ', ' la ', ' il ', ' che ', ' per ', ' non ', ' con ', ' una ', ' uno ', ' grazie', ' buongiorno', ' cordiali']),
    en: count(t, [' the ', ' and ', ' to ', ' for ', ' is ', ' of ', ' not ', ' with ', ' thank', ' please', ' best ', ' regards']),
    de: count(t, [' der ', ' die ', ' das ', ' und ', ' mit ', ' nicht ', ' für ', ' ist ', ' herr', ' frau', ' grüße', ' danke']),
    fr: count(t, [' le ', ' la ', ' les ', ' et ', ' pour ', ' avec ', ' non ', ' pas ', ' merci', ' bonjour', ' cordialement']),
  };
...
function count(haystack: string, needles: string[]): number {
  ...
  for (const needle of needles) {
    let idx = 0;
    while ((idx = haystack.indexOf(needle, idx)) !== -1) { n += 1; idx += needle.length; }
  }
  return n;
}
