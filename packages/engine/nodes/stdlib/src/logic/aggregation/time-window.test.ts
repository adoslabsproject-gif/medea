...
describe('bucketStart — UTC', () => {
  it('day → mezzanotte UTC', () => {
    expect(iso(bucketStart(Date.parse('2026-05-20T12:30:00Z'), 'day', 'UTC'))).toBe(
      '2026-05-20T00:00:00.000Z',
    );
  });
  it('hour → inizio ora UTC', () => {
    expect(iso(bucketStart(Date.parse('2026-05-20T12:45:30Z'), 'hour', 'UTC'))).toBe(
      '2026-05-20T12:00:00.000Z',
    );
  });
  it('minute → inizio minuto UTC', () => {
    expect(iso(bucketStart(Date.parse('2026-05-20T12:45:30Z'), 'minute', 'UTC'))).toBe(
      '2026-05-20T12:45:00.000Z',
    );
  });
  it('month → primo del mese UTC', () => {
    expect(iso(bucketStart(Date.parse('2026-05-20T12:30:00Z'), 'month', 'UTC'))).toBe(
...
describe('bucketStart — Europe/Rome (timezone-aware)', () => {
  it('🚨 day: mezzanotte LOCALE, non UTC (estate CEST = UTC+2)', () => {
    ...
    expect(iso(bucketStart(Date.parse('2026-05-20T01:30:00Z'), 'day', 'Europe/Rome'))).toBe(
  ...
  it("🚨 day in INVERNO (CET = UTC+1) → offset diverso dall'estate", () => {
    ...
    expect(iso(bucketStart(Date.parse('2026-01-15T10:00:00Z'), 'day', 'Europe/Rome'))).toBe(
  ...
  it('🚨 DST: il giorno della transizione marzo (CET→CEST) resta un confine di mezzanotte valido', () => {
    ...
    expect(iso(bucketStart(Date.parse('2026-03-30T12:00:00Z'), 'day', 'Europe/Rome'))).toBe(
      '2026-03-29T22:00:00.000Z',
    );
  });
  it('month: primo del mese locale', () => {
    expect(iso(bucketStart(Date.parse('2026-05-20T01:30:00Z'), 'month', 'Europe/Rome'))).toBe(
...
describe('bucketStart — week (lunedì ISO vs domenica)', () => {
  ...
  it('week monday → lunedì 18/5 (UTC)', () => {
    expect(iso(bucketStart(Date.parse('2026-05-20T12:00:00Z'), 'week', 'UTC', true))).toBe(
      '2026-05-18T00:00:00.000Z',
    );
  });
  it('week sunday → domenica 17/5 (UTC)', () => {
    expect(iso(bucketStart(Date.parse('2026-05-20T12:00:00Z'), 'week', 'UTC', false))).toBe(
