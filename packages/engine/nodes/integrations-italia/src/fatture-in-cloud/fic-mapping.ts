...
export function normalizeInvoiceOutput(ficResponse: unknown): NormalizedInvoice {
  ...
  const id = data.id;
  return {
    invoiceId: typeof id === 'number' ? id : (typeof id === 'string' && id !== '' ? Number(id) : null),
...
export function normalizeClientOutput(opts: { found: boolean; created: boolean; client: unknown }): NormalizedClient {
...
export function computeItemsGrossTotal(items: readonly unknown[]): number | null {
  ...
  for (const raw of items) {
    ...
    if (gross !== undefined && Number.isFinite(Number(gross))) {
      ...
      const vatRaw = it.vat_rate ?? it.vatRate ?? (typeof it.vat === 'number' ? it.vat : vatRec.value);
...
export interface PaymentEntry { due_date: string; amount: number; status: 'not_paid' }
 ...
 */
export function buildPaymentsList(items: readonly unknown[], paymentDays: number, now?: Date): PaymentEntry[] | null {
