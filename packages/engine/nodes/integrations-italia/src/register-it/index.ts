...
const dnsExecutor: NodeExecutor = async (config, _input, _context) => {
  ...
  if (!res.ok) {
    throw new Error(
      `Register.it ${String(res.status)}: ${(await readTextTruncated(res, 8192)).text.slice(0, 300)}`,
    );
  }
...
export const registerItDomain: NodeModule = {
  def: {
    ...
    configFields: [
      {
        key: 'apiKey',
        label: 'Register.it API key',
        type: 'secret',
        required: true,
        help: 'Da pannello Register.it → API → "Crea chiave". Richiede attivazione del servizio API.',
      },
      {
        key: 'domain',
        label: 'Dominio',
        ...
        help: 'Dominio gestito dal tuo account Register.it (senza www).',
      },
      {
        key: 'recordType',
      ...
      {
        key: 'name',
        label: 'Nome record (host)',
        type: 'expression',
        required: true,
        placeholder: 'www  oppure  @  oppure  {{input.subdomain}}',
        help: '@ = dominio apex (esempio.it). Altrimenti subdomain (es. www, mail). Supporta {{espressioni}} per creazione dinamica (es. da nodo upstream).',
      },
      {
        key: 'value',
        label: 'Valore record',
        type: 'expression',
        required: true,
        placeholder: '{{input.serverIp}}',
        help: 'Formato dipende dal tipo: A=IPv4, AAAA=IPv6, CNAME=hostname, MX="priority host", TXT=stringa. Spesso dinamico (es. dopo provisioning server).',
      },
      {
        key: 'ttl',
        label: 'TTL (secondi)',
