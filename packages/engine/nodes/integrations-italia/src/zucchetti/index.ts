...
export const zucchettiPayroll: NodeModule = {
  def: {
    ...
    configFields: [
      {
        key: 'baseUrl',
        label: 'Base URL HR Go',
        type: 'text',
        required: true,
        placeholder: 'https://hrgo.azienda.it/api',
        help: "URL dell'istanza HR Go della tua azienda (endpoint negoziato col contratto Zucchetti).",
      },
      {
        key: 'apiToken',
        label: 'API token',
        type: 'secret',
        required: true,
        help: 'Token API fornito da Zucchetti. Richiesto in fase di onboarding.',
      },
      {
        key: 'companyCode',
        ...
        type: 'text',
        required: true,
        placeholder: 'es. AZ001',
        help: "Codice identificativo dell'azienda nel sistema Zucchetti (multi-tenant).",
      },
      {
        key: 'period',
        label: 'Periodo elaborazione',
        type: 'expression',
        required: true,
        placeholder: '{{$today.slice(0,7)}}',
        help: 'Mese di cedolino in formato YYYY-MM. Es. "2026-05". Dinamico via espressione: {{$today.slice(0,7)}} = mese corrente, oppure {{input.period}} dal nodo precedente.',
      },
      {
        key: 'dryRun',
        label: 'Solo simulazione (no commit)',
        type: 'boolean',
        required: false,
        defaultValue: 'true',
