    ...
    await node('agent_html_extractor')(
      {
        provider: 'openai',
        apiKey: 'k',
        instruction: "estrai il titolo dell'articolo",
        schema: '{"title":"string"}',
        maxHtmlChars: 1000,
      },
      '<html><h1>Ciao</h1></html>',
      ctx,
    );
    ...
    await node('agent_selector_inference')(
      {
        provider: 'openai',
        apiKey: 'k',
        examples: '{"price":"€42,50"}',
        preferDataAttrs: true,
        maxHtmlChars: 5000,
      },
      '<div class="p">€42,50</div>',
      ctx,
    );
    const user = sentUserContent();
...
describe("provider gemini — API key nell'header, mai in URL", () => {
  it('🚨 x-goog-api-key header; URL senza key= (anti-leak)', async () => {
    safeFetch.mockResolvedValue(
      geminiText('{"tldr":"x","bullets":[],"detailed":"y","entities":{}}'),
    );
    await node('agent_summarizer')(
      { provider: 'gemini', apiKey: 'SECRET_G', model: 'gemini-2.0-flash' },
      'testo',
      ctx,
    );
