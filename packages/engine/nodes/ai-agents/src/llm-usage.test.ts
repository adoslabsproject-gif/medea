...
const base = {
  provider: 'openai',
  model: 'gpt-4o-mini',
  sentSystem: 'S'.repeat(35),
  sentUser: 'U'.repeat(70),
  receivedText: 'R'.repeat(7),
};
...
describe('attachAgentUsage — la forma dei DATI non cambia mai', () => {
  ...
  it('_llm echeggiato dal modello viene sovrascritto dal NOSTRO metadata', () => {
    const out = attachAgentUsage({ a: 1, _llm: 'fasullo dal modello' }, usage) as Record<
      string,
      unknown
    >;
    expect(out._llm).toEqual(usage);
