...
function makeProviderExecutor(providerId: string): NodeExecutor {
  return async (config, input, _context) => {
    ...
    const system = typeof config.systemPrompt === 'string' ? config.systemPrompt : 'You are a helpful assistant.';
    ...
    const finalUser = userPrompt.includes('{{input}}')
      ? userPrompt.replace(/\{\{input\}\}/g, typeof input === 'string' ? input : JSON.stringify(input ?? ''))
      : (userPrompt || (typeof input === 'string' ? input : JSON.stringify(input)));

    let text: string;
    switch (providerId) {
      case 'anthropic':   text = await callAnthropic(apiKey, model || 'claude-sonnet-4-5', system, finalUser); break;
      case 'openai':      text = await callOpenAI(apiKey, model || 'gpt-4o-mini', system, finalUser); break;
      case 'gemini':      text = await callGemini(apiKey, model || 'gemini-2.0-flash', system, finalUser); break;
      case 'openrouter':  text = await callOpenRouter(apiKey, model || 'anthropic/claude-sonnet-4.5', system, finalUser); break;
      case 'ollama':      text = await callOllama(baseUrl, model || 'llama3.2', system, finalUser); break;
      default:            throw new Error(`Unknown LLM provider: ${providerId}`);
    }
...
const MODEL_PRESETS: Record<string, { default: string; options: string[]; help: string; docs: string; blurb: string }> = {
  anthropic: {
  ...
  openai: {
    ...
    blurb: 'GPT (OpenAI) è il modello generalista più diffuso, ottimo per chat, generazione di testo, function calling e con il più vasto ecosistema di tooling: gpt-4o-mini offre il miglior rapporto costo/qualità per volumi alti, gpt-4o la qualità di punta quando serve il massimo.',
  },
  gemini: {
    ...
    blurb: 'Gemini (Google) è forte sul multimodale e sul contesto molto lungo (fino a 1-2M token) ed è ben integrato nell\'ecosistema Google: flash è economico e velocissimo per classificazione e riassunti su grandi volumi, 1.5-pro alza la qualità quando il compito è impegnativo.',
  },
  openrouter: {
    default: 'anthropic/claude-sonnet-4.5',
    options: ['anthropic/claude-sonnet-4.5', 'openai/gpt-4o', 'meta-llama/llama-3.1-70b-instruct', 'mistralai/mistral-large'],
    help: 'OpenRouter dà accesso a 100+ modelli con una sola key. Formato: vendor/model.',
    docs: 'openrouter.ai/docs/models',
    blurb: 'OpenRouter è un aggregatore: con UNA sola API key accedi a 100+ modelli di decine di vendor (Anthropic, OpenAI, Meta Llama, Mistral, Google e altri) usando il formato vendor/model, con routing automatico, fallback e prezzi trasparenti — ideale per confrontare modelli, ottimizzare costi o evitare il lock-in su un singolo provider.',
  },
  ollama: {
    default: 'llama3.2',
    ...
    blurb: 'Ollama esegue i modelli IN LOCALE (on-premise) senza inviare un solo byte a servizi terzi: massima privacy e zero costo per-token, a patto di avere l\'hardware adeguato — la scelta giusta per dati sensibili (sanitari, legali, PII) che per policy o GDPR non devono lasciare la tua infrastruttura.',
  },
};
...
function makeLlmNode(id: string, providerId: string, label: string, color: string): NodeModule {
  const preset = MODEL_PRESETS[providerId] ?? { default: '', options: [], help: '', docs: '', blurb: '' };
  const credentialField = providerId === 'ollama'
    ? {
        key: 'baseUrl',
...
export const aiAnthropicNode = makeLlmNode('ai_anthropic', 'anthropic', 'AI: Anthropic Claude', '#cc7855');
...
export const aiOpenRouterNode = makeLlmNode('ai_openrouter', 'openrouter', 'AI: OpenRouter (100+ models)', '#a855f7');
