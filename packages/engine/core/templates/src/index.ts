...
export const WORKFLOW_TEMPLATES: readonly WorkflowTemplate[] = [
    ...
    nodes: [
      ...
      },
      {
        ...
        config: {
          ...
          table: 'pizzeria_clienti',
          rowJson:
            '{"telefono":"{{ $node.wa_in.json.from }}","nome":{{ JSON.stringify($node.wa_in.json.profileName || "") }}}',
          onConflict: 'ignore',
        },
      },
      {
        ...
        config: {
          ...
          table: 'pizzeria_chat',
          rowJson:
            '{"telefono":"{{ $node.wa_in.json.from }}","ruolo":"cliente","messaggio":{{ JSON.stringify($node.wa_in.json.text || "") }}}',
        },
      },
      {
        id: 'info_pizzeria',
      ...
      },
      {
        ...
        config: {
          databaseId: 'pizzeria_db',
          table: 'pizzeria_menu',
          orderByJson: '[{"column":"id","direction":"asc"}]',
          limit: '50',
        },
      },
      {
      ...
      },
      {
        ...
        config: {
          ...
          table: 'pizzeria_chat',
          rowJson:
            '{"telefono":"{{ $node.wa_in.json.from }}","ruolo":"pizzeria","messaggio":{{ JSON.stringify(($node.liara.json.jsonParsed || {}).risposta || $node.liara.json.completion || "") }}}',
        },
      },
      {
        id: 'gate_ordine',
    ...
    tablesToCreate: [
      {
        ...
        seedRows: [
          {
            id: 1,
            nome: 'Pizzeria Da Esempio',
            indirizzo: 'Via Roma 1, Città',
            telefono: '+39 06 1234567',
            orari: 'mar-dom 18:30-23:00, lunedì chiuso',
            consegna_minuti: 30,
            note: 'SOSTITUISCI questi dati con quelli reali della pizzeria (DB Studio → pizzeria_info)',
          },
        ],
      },
      {
        ...
        seedRows: [
          ...
          },
          {
            ...
            ingredienti:
              'pomodoro, mozzarella, prosciutto cotto, funghi, carciofi, olive a spicchi',
            prezzo: 9.0,
            disponibile: 1,
          },
          {
          ...
          },
          {
            id: 13,
            nome: 'Boscaiola',
            ingredienti: 'mozzarella, funghi porcini, salsiccia',
            prezzo: 10.0,
            disponibile: 1,
          },
          {
            ...
            ingredienti: 'pomodoro, mozzarella, prosciutto cotto, ricotta (chiuso al forno)',
            prezzo: 9.0,
            disponibile: 1,
          },
          {
            id: 15,
            nome: 'Parmigiana',
            ingredienti: 'pomodoro, mozzarella, melanzane fritte, parmigiano',
            prezzo: 9.0,
            disponibile: 1,
          },
          {
            id: 16,
            nome: 'Crudo e Rucola',
            ingredienti: 'pomodoro, mozzarella, prosciutto crudo, rucola, scaglie di grana',
            prezzo: 10.0,
            disponibile: 1,
          },
        ],
      ...
      },
      {
        ...
        description:
          'Storico ordini per cliente (pizze in JSON) — alimenta "la solita" e i suggerimenti.',
        columns: [
          { name: 'id', type: 'integer', primaryKey: true, nullable: false },
    ...
    nodes: [
      ...
      },
      {
        ...
        config: {
          ...
          table: 'pizzeria_clienti',
          rowJson:
            '{"telefono":"{{ $node.tg_in.json.chatId }}","nome":{{ JSON.stringify($node.tg_in.json.firstName || $node.tg_in.json.username || "") }}}',
          onConflict: 'ignore',
        },
      },
      {
        ...
        config: {
          ...
          table: 'pizzeria_chat',
          rowJson:
            '{"telefono":"{{ $node.tg_in.json.chatId }}","ruolo":"cliente","messaggio":{{ JSON.stringify($node.tg_in.json.text || "") }}}',
        },
      },
      {
        id: 'info_pizzeria',
      ...
      },
      {
        ...
        config: {
          ...
          table: 'pizzeria_chat',
          rowJson:
            '{"telefono":"{{ $node.tg_in.json.chatId }}","ruolo":"pizzeria","messaggio":{{ JSON.stringify(($node.liara.json.jsonParsed || {}).risposta || $node.liara.json.completion || "") }}}',
        },
      },
      {
        id: 'gate_ordine',
      ...
      },
      {
        ...
        config: {
          ...
          table: 'pizzeria_ordini',
          rowJson:
            '{"telefono":"{{ $node.tg_in.json.chatId }}",' +
            '"pizze":{{ JSON.stringify(JSON.stringify($node.liara.json.jsonParsed.ordine.pizze || [])) }},' +
            '"totale":{{ $node.liara.json.jsonParsed.ordine.totale || 0 }},' +
            '"stato":"ricevuto",' +
            '"note":{{ JSON.stringify($node.liara.json.jsonParsed.ordine.note || "") }}}',
  ...
  // ─── FEATURED — primo workflow funzionante out-of-the-box ───
  {
    ...
    edges: [{ from: 'webhook', to: 'slack' }],
  },
  {
    id: 'tmpl_it_invoice_to_sdi',
    name: 'Da webhook a fattura SDI in 5 step',
    description:
      'Ricevi un webhook con dati cliente + ordine, crea il cliente su Fatture in Cloud se non esiste, emette la fattura, la trasmette al Sistema di Interscambio (SDI) e invia conferma per PEC.',
    ...
    nodes: [
      {
        ...
        x: 0,
        y: 0,
        config: { method: 'POST', authMode: 'hmac-signature' },
      },
      {
        id: 'lookup',
      ...
      },
      {
        ...
        x: 880,
        y: 0,
        config: { subject: 'Fattura trasmessa' },
      },
    ],
    edges: [
      { from: 'trig', to: 'lookup' },
  ...
  },
  {
    ...
    description:
      'Ogni nuovo messaggio PEC ricevuto viene salvato in una cartella organizzata per mittente e mese.',
    category: 'fiscalita-italia',
  ...
  },
  {
    ...
    description:
      'Every weekday at 9am, run a DB query against the user metrics table, summarize the rows with AI, and send the report by email.',
    category: 'monitoring',
    ...
    nodes: [
      {
        ...
        x: 0,
        y: 0,
        config: { cronExpression: '0 9 * * 1-5', timezone: 'Europe/Rome' },
      },
  ...
  },
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        x: 220,
        y: 0,
        config: { condition: 'input.body.ref === "refs/heads/main"' },
      },
...

  {
    ...
    description:
      'Verifica che il nodo action_http funzioni: GET httpbin.org/json + POST httpbin.org/post.',
    category: 'general',
    tags: ['test', 'http'],
    language: 'it',
    vendor: 'flowforge',
  ...
  },
  {
    ...
    description:
      'Test insert + query + update + delete su una tabella esistente. Richiede un DB FlowForge con tabella.',
    ...
    nodes: [
      { id: 'start', defId: 'trigger_manual', x: 0, y: 0, config: {} },
      {
        ...
        x: 220,
        y: 0,
        config: { rowJson: '{"date":"2026-06-01","page_views":999,"signups":99,"revenue":999.99}' },
      },
      { id: 'sel', defId: 'db_query', x: 440, y: 0, config: { limit: '10' } },
      {
        ...
        x: 660,
        y: 0,
        config: { whereJson: '{"date":"2026-06-01"}', patchJson: '{"page_views":1234}' },
      },
      {
        id: 'del',
        defId: 'db_delete',
        x: 880,
        y: 0,
        config: { whereJson: '{"date":"2026-06-01"}' },
      },
    ],
    edges: [
      { from: 'start', to: 'ins' },
  ...
  },
  {
    ...
    description:
      'Scrive un file nel namespace del tuo tenant e poi lo rilegge per verifica. I path relativi vengono risolti dentro la cartella del cliente, isolata dagli altri.',
    category: 'general',
    tags: ['test', 'file'],
    language: 'it',
    vendor: 'flowforge',
    ...
    nodes: [
      { id: 'start', defId: 'trigger_manual', x: 0, y: 0, config: {} },
      {
        ...
        config: {
          path: 'test.json',
          content: '{"hello":"flowforge","timestamp":"{{ctx.runId}}"}',
          mode: 'overwrite',
        },
      },
  ...
  },
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        config: {
          itemsExpression:
            'Object.entries(input.reduced).map(([k,v]) => ({customer: k, total: v}))',
          strategy: 'naive',
          concurrency: '1',
          errorPolicy: 'continue',
          maxItems: '100',
      ...
      },
      {
        ...
        config: {
          expression:
            '{ "msg": "Customer " & loop.item.customer & " spent " & $string(loop.item.total) }',
        },
      },
      {
        id: 'done',
        ...
        config: {
          expression:
            '{ "summary": "Processed " & $string(input.iterations) & " customers in " & $string(input.totalDurationMs) & "ms" }',
        },
      },
    ],
    edges: [
  ...
  },
  {
    ...
    description:
      'Trigger manual con array di oggetti → trasforma con JSONata (somma revenue, conta righe).',
    category: 'data-pipeline',
    ...
    nodes: [
      { id: 'start', defId: 'trigger_manual', x: 0, y: 0, config: {} },
      {
        ...
        y: 0,
        config: {
          expression: '{ "totalRevenue": $sum(input.rows.revenue), "count": $count(input.rows) }',
        },
      },
    ],
  ...
  },
  {
    ...
    description:
      'Manual trigger con un testo italiano → traduce in EN, classifica sentiment, estrae entità nominate.',
    category: 'ai-orchestration',
    tags: ['test', 'AI', 'liara'],
    language: 'it',
    vendor: 'flowforge',
    ...
    nodes: [
      { id: 'start', defId: 'trigger_manual', x: 0, y: 0, config: {} },
      {
        ...
        x: 220,
        y: 0,
        config: { provider: '', extraContext: 'Translate from Italian to English.' },
      },
  ...
  },
  {
    ...
    description:
      'Form HTML auto-generato (nome+email+messaggio) → AI extract → salva in tabella contatti.',
    category: 'crm',
    tags: ['test', 'form', 'lead'],
    language: 'it',
    vendor: 'flowforge',
    ...
    nodes: [
      {
        ...
        config: {
          title: 'Contattaci',
          fieldsJson:
            '[{"key":"name","label":"Nome","type":"text","required":true},{"key":"email","label":"Email","type":"email","required":true},{"key":"message","label":"Messaggio","type":"textarea","required":true}]',
          submitLabel: 'Invia',
        },
      },
      {
        ...
        config: {
          rowJson:
            '{"name":"{{form.output.body.name}}","email":"{{form.output.body.email}}","message":"{{form.output.body.message}}"}',
        },
      },
    ],
    ...
    nodes: [
      {
        ...
        x: 0,
        y: 0,
        config: { ops: 'insert', pollIntervalSec: '5' },
      },
      { id: 'sum', defId: 'agent_summarizer', x: 220, y: 0, config: {} },
      {
        ...
        x: 440,
        y: 0,
        config: { path: 'logs/db-changes.log', mode: 'append' },
      },
    ],
    edges: [
      { from: 'sub', to: 'sum' },
  ...
  },
  {
    ...
    description:
      'Monitora /tmp/flowforge-watch/ → quando arriva un file, lo legge e logga il contenuto.',
    category: 'general',
    ...
    nodes: [
      {
        ...
        x: 0,
        y: 0,
        config: { directory: 'watch', events: 'add', debounceMs: '500' },
      },
      {
        id: 'read',
  ...
  },
  {
    ...
    description:
      'Agente Liara con accesso a rag_search e db_query — fa Q&A iterativo sul tuo DB e knowledge base.',
    category: 'ai-orchestration',
    ...
    nodes: [
      { id: 'start', defId: 'trigger_manual', x: 0, y: 0, config: {} },
      {
        ...
        x: 220,
        y: 0,
        config: { provider: '', maxIterations: '5' },
      },
    ],
...

  {
    ...
    description:
      "Quando arriva un'email da un fornitore con conferma ordine, AI estrae prodotti/quantità/prezzi e inserisce nella tabella ordini. Salva anche l'email originale per audit.",
    category: 'data-pipeline',
    ...
    nodes: [
      {
        ...
        config: {
          ...
          mailbox: 'INBOX',
          pollIntervalSec: '60',
          filterSubject: '(ordine|conferma|order)',
        },
      },
      {
        id: 'is_supplier',
        ...
        x: 220,
        y: 0,
        config: { condition: '/fornitore|supplier|orders@/i.test(input.from)' },
      },
      {
        id: 'extract',
        ...
        config: {
          provider: '',
          extraContext:
            "Estrai dall'email: lista prodotti (nome, codice SKU, quantità, prezzo_unitario), totale, data consegna prevista, numero ordine. Restituisci JSON con campi: order_number, supplier_email, total, expected_delivery, items[].",
        },
      },
      {
        id: 'save_order',
        ...
        config: {
          table: 'orders',
          rowJson:
            '{"order_number":"{{extract.output.order_number}}","supplier_email":"{{inbox.output.from}}","total":{{extract.output.total}},"expected_delivery":"{{extract.output.expected_delivery}}","items_json":"{{extract.output.items}}","received_at":"{{inbox.output.date}}","raw_subject":"{{inbox.output.subject}}"}',
        },
      },
      {
        id: 'archive',
        ...
        y: 140,
        config: {
          path: 'archive/orders/{{extract.output.order_number}}.eml',
          content: '{{inbox.output.raw}}',
          mode: 'overwrite',
        },
      },
    ],
  ...
  },
  {
    ...
    description:
      'Form pubblico sul tuo sito (nome, email, azienda, messaggio) → AI qualifica il lead (hot/warm/cold) + estrae industry + size → inserisce nel DB + alert Slack se HOT.',
    category: 'crm',
    ...
    nodes: [
      ...
      },
      {
        ...
        config: {
          provider: '',
          extraContext:
            'Classifica il lead: hot (vuole comprare entro 30gg), warm (interessato, 1-3 mesi), cold (esplora). Output JSON: {level, reasoning, industry, urgency_score}.',
        },
      },
      {
        id: 'save',
        ...
        config: {
          table: 'leads',
          rowJson:
            '{"name":"{{form.output.body.name}}","email":"{{form.output.body.email}}","company":"{{form.output.body.company}}","role":"{{form.output.body.role}}","company_size":"{{form.output.body.company_size}}","message":"{{form.output.body.message}}","level":"{{qualify.output.level}}","industry":"{{qualify.output.industry}}","urgency":{{qualify.output.urgency_score}}}',
        },
      },
      {
        id: 'is_hot',
        ...
        x: 660,
        y: 0,
        config: { condition: 'input.level === "hot"' },
      },
      {
        id: 'alert',
        ...
        config: {
          action: 'send_message',
          channel: '#sales',
          text: '🔥 HOT LEAD: {{form.output.body.name}} da {{form.output.body.company}} ({{form.output.body.company_size}}) — {{qualify.output.reasoning}}',
        },
      },
    ],
    edges: [
  ...
  },
  {
    ...
    description:
      'Quando ricevi un webhook con dati ordine cliente (es. da Shopify/WooCommerce/sito custom), crea o trova il cliente su Fatture in Cloud, emette fattura, invia PEC di conferma al cliente.',
    category: 'fiscalita-italia',
    ...
    nodes: [
      ...
      { id: 'invoice', defId: 'italia_fatture_in_cloud_invoice', x: 440, y: 0, config: {} },
      {
        ...
        x: 660,
        y: 0,
        config: { subject: 'La tua fattura {{invoice.output.invoice_number}}' },
      },
    ],
    edges: [
      { from: 'wh', to: 'client' },
  ...
  },
  {
    ...
    description:
      'Quando arriva una PEC, AI classifica (fattura/contratto/comunicazione legale/altro), salva il PDF allegato in archivio strutturato + log nel DB per audit GDPR.',
    category: 'fiscalita-italia',
    ...
    nodes: [
      ...
      },
      {
        ...
        config: {
          provider: '',
          extraContext:
            'Classifica: fattura | contratto | comunicazione_legale | altro. Output JSON: {category, sender_type, urgency}.',
        },
      },
      {
        id: 'archive',
        ...
        y: 0,
        config: {
          path: 'archive/pec/{{classify.output.category}}/{{pec.output.message_id}}.eml',
          mode: 'overwrite',
        },
      },
      {
        ...
        config: {
          table: 'pec_audit',
          rowJson:
            '{"message_id":"{{pec.output.message_id}}","from":"{{pec.output.from}}","subject":"{{pec.output.subject}}","category":"{{classify.output.category}}","urgency":"{{classify.output.urgency}}","received_at":"{{pec.output.date}}"}',
        },
      },
    ],
    edges: [
  ...
  },
  {
    ...
    description:
      'Ogni mattina alle 8, query del DB ordini di ieri, AI genera insight (top prodotti, anomalie, trend), invia email HTML al management.',
    category: 'monitoring',
    ...
    nodes: [
      {
        ...
        x: 0,
        y: 0,
        config: { cronExpression: '0 8 * * 1-5', timezone: 'Europe/Rome' },
      },
      { id: 'q', defId: 'db_query', x: 220, y: 0, config: { table: 'orders' } },
      {
        ...
        config: {
          provider: '',
          extraContext:
            'Genera report vendite: totale, top 3 prodotti, top 3 clienti, eventuali anomalie. Italiano, professionale. Output HTML.',
        },
      },
      {
        id: 'email',
        ...
        config: {
          ...
          subject: 'Report vendite — {{cron.output.timestamp}}',
          body: '{{analyze.output.report}}',
        },
      },
    ],
    edges: [
  ...
  },
  {
    ...
    description:
      'Webhook ordine completato → attendi 7 giorni (delay) → AI genera email personalizzata di follow-up basata sul prodotto comprato → invia SMTP.',
    category: 'crm',
    ...
    nodes: [
      ...
      { id: 'wait', defId: 'logic_delay', x: 220, y: 0, config: { durationMs: '604800000' } },
      {
        ...
        config: {
          provider: '',
          extraContext:
            'Componi email di follow-up personale (italiano, tono amichevole) per cliente {{wh.output.body.customer_name}} che ha comprato {{wh.output.body.product}}. Chiedi feedback + offri sconto 10%.',
        },
      },
      {
        id: 'send',
        ...
        y: 0,
        config: {
          to: '{{wh.output.body.customer_email}}',
          subject: 'Come va con {{wh.output.body.product}}?',
          body: '{{compose.output.detailed}}',
        },
      },
    ],
    edges: [
      { from: 'wh', to: 'wait' },
      { from: 'wait', to: 'compose' },
  ...
  },
  {
    ...
    description:
      'Ogni notte alle 3, esporta tutta la tabella ordini in JSON, salva su disco con timestamp + log Slack.',
    category: 'devops',
    ...
    nodes: [
      {
        ...
        x: 0,
        y: 0,
        config: { cronExpression: '0 3 * * *', timezone: 'Europe/Rome' },
      },
      { id: 'dump', defId: 'db_query', x: 220, y: 0, config: {} },
      {
        ...
        y: 0,
        config: {
          path: 'backup/db-{{cron.output.timestamp}}.json',
          content: '{{dump.output}}',
          mode: 'overwrite',
        },
      },
      {
        ...
        config: {
          action: 'send_message',
          channel: '#devops',
          text: '✅ Backup {{dump.output.rowCount}} righe completato',
        },
      },
    ],
    edges: [
  ...
  },
  {
    ...
    description:
      'Webhook da GitHub quando viene aperta una issue → AI determina priorità (P0/P1/P2) e team owner → crea task corrispondente su Linear.',
    category: 'devops',
    ...
    nodes: [
      { id: 'wh', defId: 'trigger_webhook', x: 0, y: 0, config: { method: 'POST' } },
      {
        ...
        config: {
          provider: '',
          extraContext:
            'Classifica issue GitHub: priority (P0=critico/P1=alto/P2=medio/P3=basso), team (frontend/backend/infra/security), estimated_hours. Output JSON.',
        },
      },
      {
        id: 'linear',
        defId: 'community_linear',
        x: 440,
        y: 0,
        config: {
          action: 'create_issue',
          title: '[{{triage.output.priority}}] {{wh.output.body.issue.title}}',
          description:
            '{{wh.output.body.issue.body}}\n\n📊 AI triage: {{triage.output.team}} · est. {{triage.output.estimated_hours}}h\n🔗 GitHub: {{wh.output.body.issue.html_url}}',
        },
      },
    ],
    edges: [
      { from: 'wh', to: 'triage' },
      { from: 'triage', to: 'linear' },
    ],
  },
  ...
  // ─── Sprint 2026-06-06 E2: 6 template enterprise top-richiesta ─────────
  {
    ...
    description:
      'Cron giornaliero alle 8: pesca lead nuovi da Odoo, calcola lead score (intent + budget + fit), aggiorna scheda CRM, notifica account manager se score≥80.',
    category: 'crm',
    ...
    nodes: [
      {
        ...
        x: 0,
        y: 0,
        config: { cronExpression: '0 8 * * *', timezone: 'Europe/Rome' },
      },
      {
        id: 'fetch',
        ...
        config: {
          ...
          domainJson:
            '[["stage_id.is_won","=",false],["create_date",">=","{{cron.output.dayMinus1}}"]]',
        },
      },
      { id: 'score', defId: 'action_lead_score', x: 440, y: 0, config: {} },
      {
        ...
        x: 660,
        y: 0,
        config: { expression: '{{score.output.score}} >= 80' },
      },
      {
        id: 'alert',
        ...
        config: {
          channel: '#sales-hot',
          text: '🔥 Hot lead {{fetch.output.name}} — score {{score.output.score}}/100',
        },
      },
    ],
    edges: [
  ...
  },
  {
    ...
    description:
      'Ogni mattina alle 7, calcola KPI business (fatturato, nuovi clienti, churn) interrogando il DB, formatta il report in Markdown e lo invia al canale Telegram del management.',
    category: 'monitoring',
    ...
    nodes: [
      {
        ...
        x: 0,
        y: 0,
        config: { cronExpression: '0 7 * * *', timezone: 'Europe/Rome' },
      },
      {
        id: 'kpi',
        defId: 'db_query',
        ...
        y: 0,
        config: {
          sql: "SELECT COUNT(*) FILTER (WHERE created_at >= NOW() - INTERVAL '1 day') AS new_customers, SUM(amount) FILTER (WHERE date = CURRENT_DATE) AS revenue_today FROM orders",
        },
      },
      {
        id: 'fmt',
  ...
  },
  {
    ...
    description:
      'Ogni 6 ore controlla 20 URL di prodotti competitor, estrae prezzo via CSS selector, confronta con storico in DB. Se variazione >10% manda alert email al product manager.',
    category: 'monitoring',
    ...
    nodes: [
      {
        ...
        x: 0,
        y: 0,
        config: { cronExpression: '0 */6 * * *', timezone: 'Europe/Rome' },
      },
      {
        id: 'loop',
        defId: 'logic_loop',
        ...
        y: 0,
        config: {
          itemsExpression: '["https://competitor1.com/p/foo","https://competitor2.com/p/bar"]',
          strategy: 'naive',
          concurrency: '2',
          rateLimitPerMin: '60',
        },
      ...
      {
        id: 'fetch',
        defId: 'web_fetch_advanced',
        x: 440,
        y: 0,
        config: { url: '{{loop.item}}', method: 'GET', userAgent: 'rotate' },
      },
      {
        id: 'price',
        defId: 'html_select',
        x: 660,
        y: 0,
        config: { selector: '.price, [itemprop="price"]', attribute: 'textContent' },
      },
      {
        id: 'diff',
        defId: 'logic_if',
        ...
        y: 0,
        config: {
          expression: 'Math.abs({{price.output.value}} - {{previous}}) / {{previous}} > 0.1',
        },
      },
      {
        id: 'alert',
        ...
        config: {
          to: 'pm@zeli.it',
          subject: '🚨 Price change su {{loop.item}}',
          body: 'Vecchio: {{previous}}\nNuovo: {{price.output.value}}',
        },
      },
    ],
    edges: [
  ...
  },
  {
    ...
    description:
      'Webhook da modulo "Esporta i miei dati" (art. 20 GDPR): query tutti i record collegati alla email, costruisce ZIP firmato HMAC e invia link download protetto.',
    category: 'general',
    ...
    nodes: [
      ...
      },
      {
        ...
        y: 0,
        config: {
          sql: 'SELECT row_to_json(t) FROM (SELECT * FROM users u LEFT JOIN orders o ON o.user_id=u.id WHERE u.email = $1) t',
          params: '["{{wh.output.body.email}}"]',
        },
      },
      {
        id: 'fmt',
        ...
        x: 440,
        y: 0,
        config: { template: '{{ JSON.stringify(pull.output.rows, null, 2) }}' },
      },
      {
        id: 'save',
        defId: 'action_file_write',
        ...
        y: 0,
        config: {
          path: 'gdpr-exports/{{wh.output.body.email}}/{{wh.output.timestamp}}.json',
          content: '{{fmt.output}}',
          mode: 'overwrite',
        },
      },
      {
        ...
        y: 0,
        config: {
          to: '{{wh.output.body.email}}',
          subject: 'I tuoi dati (GDPR art. 20)',
          body: 'Trovi il download protetto qui: {{save.output.signedUrl}}\n\nIl link scade dopo 7 giorni come da policy.',
        },
      },
    ],
    edges: [
  ...
  },
  {
    ...
    description:
      'Webhook da Zoom/Teams quando termina una call, scarica audio MP3, lo passa al summarizer AI per estrarre decisioni + action items, crea task in Linear assegnati per owner.',
    category: 'ai-orchestration',
    ...
    nodes: [
      ...
      {
        id: 'sum',
        defId: 'action_video_summarizer',
        x: 220,
        y: 0,
        config: {
          url: '{{wh.output.body.recording_url}}',
      ...
      },
      {
        ...
        y: 0,
        config: {
          itemsExpression: '{{sum.output.actionItems}}',
          strategy: 'naive',
          concurrency: '3',
        },
      },
    ...
    edges: [
      { from: 'wh', to: 'sum' },
      { from: 'sum', to: 'loop' },
      { from: 'loop', to: 'task', sourceHandle: 'body' },
    ],
  },
  {
    ...
    description:
      'Webhook quando un pagamento Stripe fallisce: retry automatico con backoff, AI compone email personalizzata per il cliente, se non risponde in 5 giorni triggera workflow di churn save (sconto + call).',
    category: 'crm',
    ...
    nodes: [
      ...
      { id: 'wait1', defId: 'logic_delay', x: 220, y: 0, config: { durationMs: '86400000' } },
      {
        ...
        y: 0,
        config: {
          url: 'https://api.stripe.com/v1/charges/{{wh.output.body.charge_id}}/capture',
          method: 'POST',
        },
      },
      {
        ...
        x: 660,
        y: 0,
        config: { expression: '{{retry.output.status}} === "succeeded"' },
      },
      {
        id: 'compose',
        ...
        config: {
          tone: 'empathetic',
          context:
            'Cliente {{wh.output.body.customer.name}} ha pagamento fallito su Stripe. Offri sconto 20% + opzione metodo pagamento alternativo.',
        },
      },
      {
        id: 'send',
        ...
        y: 60,
        config: {
          to: '{{wh.output.body.customer.email}}',
          subject: 'Possiamo aiutarti con il pagamento?',
          body: '{{compose.output.body}}',
        },
      },
    ],
    edges: [
    ...
    nodes: [
      {
        ...
        x: 40,
        y: 340,
        config: { cronExpression: '0 10 * * 1-5', timezone: 'Europe/Rome' },
      },
      {
        id: 'cerca',
        ...
        config: {
          seedPrompt:
            'PERSONALIZZA: descrivi qui il tuo cliente target, es. "ristoranti e cocktail bar di fascia alta in Italia"',
          country: 'IT',
          maxResults: '10',
          resultsPerQuery: '10',
          queryExpansionCount: '6',
        },
      },
      {
        id: 'loop',
        ...
        y: 340,
        config: {
          itemsExpression: 'input.companies',
          strategy: 'naive',
          batchSize: '1',
          concurrency: '1',
          errorPolicy: 'continue',
      ...
      },
      {
        ...
        config: {
          ...
          respectRobots: 'true',
          followSitemap: 'true',
          preferredLocalParts: 'info,commerciale,prenotazioni,booking,contatti,sales',
          minEmailConfidence: '0.5',
        },
      },
      {
        id: 'valida',
        defId: 'action_email_validate_mx',
        x: 1000,
        y: 340,
        config: {
          email: '{{ $node.contatti.json.primary_email }}',
          minConfidence: '0.5',
        },
      },
      {
        ...
        config: {
          ...
          threshold: '20',
          customPositiveJson:
            '[{"keyword":"PERSONALIZZA-settore","weight":30},{"keyword":"contatti","weight":10}]',
          customNegativeJson:
            '[{"keyword":"magazine","weight":90},{"keyword":"rivista","weight":90},{"keyword":"wikipedia","weight":100},{"keyword":"classifica","weight":40}]',
        },
      },
      {
        id: 'dedup',
        ...
        config: {
          ...
          limit: '1',
          filtersJson:
            '[{"column":"email","op":"eq","value":"{{ $node.contatti.json.primary_email }}"}]',
        },
      },
      {
        id: 'optout',
      ...
      },
      {
        ...
        config: {
          condition:
            '$node.contatti.json.has_emails === true && $node.valida.json.mx_valid === true && $node.score.json.send_recommended === true && $node.dedup.json.rowCount === 0 && $node.optout.json.rowCount === 0',
        },
      },
      {
        id: 'scrivi',
      ...
      },
      {
        ...
        config: {
          ...
          subject:
            '{{ ($node.scrivi.json.jsonParsed || {}).oggetto || ("Proposta commerciale per " + item.title) }}',
          bodyType: 'html',
          body:
      ...
      },
      {
        ...
        config: {
          ...
          onConflict: 'ignore',
          rowJson:
            '{"azienda":{{ JSON.stringify(item.title || "") }},"url":{{ JSON.stringify(item.url || "") }},"email":{{ JSON.stringify($node.contatti.json.primary_email || "") }},"score":{{ $node.score.json.score || 0 }},"oggetto":{{ JSON.stringify(($node.scrivi.json.jsonParsed || {}).oggetto || "") }}}',
        },
      },
    ],
    edges: [
      { from: 'trigger', to: 'cerca' },
      { from: 'cerca', to: 'loop' },
      { from: 'loop', to: 'contatti' },
      { from: 'contatti', to: 'valida' },
      { from: 'valida', to: 'score' },
      { from: 'score', to: 'dedup' },
      { from: 'dedup', to: 'optout' },
      { from: 'optout', to: 'gate' },
      { from: 'gate', to: 'scrivi', fromPort: 'true' },
      { from: 'scrivi', to: 'invia' },
      { from: 'invia', to: 'registra' },
    ],
    tablesToCreate: [
      {
        databaseId: 'crm_db',
  ...
  // ─── Gestione opt-out email (STOP) — companion del lead-gen ─────────
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        config: {
          condition:
            '/\\bstop\\b/i.test(($node.imap.json.text || "") + " " + ($node.imap.json.subject || ""))',
        },
      },
      {
        id: 'blocca',
        ...
        config: {
          ...
          onConflict: 'ignore',
          rowJson:
            '{"email":{{ JSON.stringify($node.imap.json.from || "") }},"motivo":"risposta STOP"}',
        },
      },
    ],
    edges: [
      { from: 'imap', to: 'is_stop' },
      { from: 'is_stop', to: 'blocca', fromPort: 'true' },
    ],
    tablesToCreate: [
      {
        databaseId: 'crm_db',
