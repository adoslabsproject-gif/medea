...
const streamTrigger: TriggerSpec = {
  ...
  label: 'Prezzo live',
  mode: 'stream',
  connect: async (_config, _ctx, emit) => {
    emit({ price: 42 });
    return () => {
      /* teardown */
    };
...
describe('trigger() + validateTriggerSpec', () => {
  ...
  it('polling valido / stream valido → nessun throw', () => {
    expect(() => {
      validateTriggerSpec(pollingTrigger);
    }).not.toThrow();
    expect(() => {
      validateTriggerSpec(streamTrigger);
    }).not.toThrow();
  });

  it('mode=polling senza poll() → throw', () => {
    expect(() => {
      validateTriggerSpec({ id: 'x', label: 'X', mode: 'polling' });
    }).toThrow(/poll\(\)/u);
  });

  it('mode=stream senza connect() → throw', () => {
    expect(() => {
      validateTriggerSpec({ id: 'x', label: 'X', mode: 'stream' });
    }).toThrow(/connect\(\)/u);
  });

  it('id non valido → throw', () => {
    expect(() => {
      validateTriggerSpec({ ...pollingTrigger, id: 'bad id!' });
    }).toThrow(/non valido/u);
  });
});
...
describe('defineCommunityNode con triggers', () => {
  ...
  it('rifiuta un trigger incoerente (polling senza poll)', () => {
    expect(() => defineCommunityNode(node([{ id: 'x', label: 'X', mode: 'polling' }]))).toThrow(
      /poll\(\)/u,
    );
  });
  ...
  it('il lifecycle connect() emette e ritorna teardown', async () => {
    const emit = vi.fn();
    const teardown = await streamTrigger.connect!(
      {},
      { tenantId: 't', workflowId: 'w', nodeId: 'n', state: {} },
      emit,
    );
...
describe('compile() — trigger polling supportato, stream rifiutato', () => {
  it("compile() con un trigger POLLING → nodedef.triggers + bridge nell'executor", () => {
    ...
    expect(out.nodedef.triggers?.[0]).toMatchObject({
      id: 'new_order',
      mode: 'polling',
      pollIntervalSec: 30,
    });
  ...
  it('poll bridge: trigger id sconosciuto → throw esplicito', async () => {
    ...
    await expect(
      execute(
        { __ff_trigger_poll: 'does_not_exist' },
        { state: {} },
        { tenantId: 't', workflowId: 'w', nodeId: 'n' },
      ),
