...
describe('renderCreateViewSql', () => {
  it('CREATE VIEW <q name> AS <select>;', () => {
    expect(renderCreateViewSql({ name: 'clienti_attivi', select: 'SELECT * FROM clienti WHERE attivo = 1' }, dq))
  ...
  it('rimuove un ; finale del SELECT (anti statement-break smuggling)', () => {
    expect(renderCreateViewSql({ name: 'v', select: 'SELECT 1 ;  ' }, dq)).toBe('CREATE VIEW "v" AS SELECT 1;');
  });

  it('usa il quoting INIETTATO dal dialetto (mysql backtick, mssql bracket)', () => {
    expect(renderCreateViewSql({ name: 'v', select: 'SELECT 1' }, bt)).toBe('CREATE VIEW `v` AS SELECT 1;');
    expect(renderCreateViewSql({ name: 'v', select: 'SELECT 1' }, br)).toBe('CREATE VIEW [v] AS SELECT 1;');
  });

  it('quoting difensivo: un " nel nome viene raddoppiato (no break dell\'identificatore)', () => {
    const q = (id: string): string => `"${id.replace(/"/gu, '""')}"`;
    expect(renderCreateViewSql({ name: 'a"b', select: 'SELECT 1' }, q)).toBe('CREATE VIEW "a""b" AS SELECT 1;');
  });
});
