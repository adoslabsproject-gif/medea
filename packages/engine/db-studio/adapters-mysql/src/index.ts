...
function renderAction(a: MigrationAction): string {
  switch (a.kind) {
    case 'create_table':
      return `CREATE TABLE IF NOT EXISTS ${quoteIdent(a.table.name)} (\n  ${a.table.columns.map(renderColumn).join(',\n  ')}\n) ENGINE=InnoDB;`;
    case 'drop_table':
      return `DROP TABLE IF EXISTS ${quoteIdent(a.tableName)};`;
    case 'rename_table':
      return `RENAME TABLE ${quoteIdent(a.from)} TO ${quoteIdent(a.to)};`;
    case 'add_column':
      return `ALTER TABLE ${quoteIdent(a.tableName)} ADD COLUMN ${renderColumn(a.column)};`;
    case 'drop_column':
      return `ALTER TABLE ${quoteIdent(a.tableName)} DROP COLUMN ${quoteIdent(a.columnName)};`;
    case 'rename_column':
      return `ALTER TABLE ${quoteIdent(a.tableName)} RENAME COLUMN ${quoteIdent(a.from)} TO ${quoteIdent(a.to)};`;
    case 'alter_column':
      return `-- ALTER COLUMN ${a.columnName} on ${a.tableName} — MySQL requires MODIFY/CHANGE; generated manually`;
    case 'add_relation': {
    ...
    case 'drop_relation':
      return `-- drop relation ${a.relationId} (run ALTER TABLE ... DROP FOREIGN KEY manually)`;
    case 'add_index': {
...
function filterToFragment(f: QueryFilter, paramIdx: number): { sql: string; params: unknown[] } {
  ...
  switch (f.op) {
    ...
    case 'lte':
      return { sql: `${c} <= ?`, params: [f.value] };
    case 'like':
      return { sql: `${c} LIKE ?`, params: [f.value] };
    case 'isNull':
      return { sql: `${c} IS NULL`, params: [] };
    case 'notNull':
...
export class MysqlAdapter implements IDatabaseAdapter {
  ...
  async query<T = Record<string, unknown>>(spec: QuerySpec): Promise<QueryResult<T>> {
    ...
    const whereClause = fragments.length
      ? `WHERE ${fragments.map((f) => f.sql).join(' AND ')}`
      : '';
  ...
  async introspect(): Promise<Table[]> {
    ...
    for (const row of rows as Record<string, string>[]) {
      ...
      tables.push({
        id: tableName,
        name: tableName,
        columns: (cols as { Field: string; Type: string; Null: string; Key: string }[]).map(
          (c) => ({
