...
const orders: Table = {
  ...
  columns: [
    ...
    { id: 'total', name: 'total', type: 'integer', constraints: { nullable: false, unique: false, primaryKey: false } },
  ],
  indexes: [],
};
...
describe('SqliteAdapter — VIEW', () => {
  ...
  async function queryFails(table: string): Promise<boolean> {
    try { await adapter.query({ table }); return false; } catch { return true; }
  }

  it('create_view → SQL CREATE VIEW + la view è interrogabile e riflette i dati base', async () => {
    await seed();
    const action: MigrationAction = { kind: 'create_view', view: { name: 'big_orders', select: 'SELECT id, total FROM orders WHERE total >= 100' } };
  ...
  it('la view è DINAMICA: nuovi dati nella tabella base appaiono nella view', async () => {
    await seed();
    await adapter.applyMigration([{ kind: 'create_view', view: { name: 'big_orders', select: 'SELECT * FROM orders WHERE total >= 100' } }]);
  ...
  it('preview NON esegue: la view non esiste dopo previewMigration', async () => {
    await seed();
    const sql = await adapter.previewMigration([{ kind: 'create_view', view: { name: 'v_prev', select: 'SELECT 1 AS x' } }]);
  ...
  it('drop_view rimuove la view (idempotente: drop due volte non lancia)', async () => {
    ...
    await expect(adapter.applyMigration([{ kind: 'drop_view', viewName: 'big_orders' }])).resolves.toBeDefined();
  });
});
