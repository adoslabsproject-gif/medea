...
describe('buildPostgresUrl — include la password (FIX 2026-06-14)', () => {
  it('hostname+port+db+user+password → url con :password@ (auth funzionante)', () => {
    const url = buildPostgresUrl(conn({ hostname: 'ff-db-postgres-x', port: 5432, database: 'tenant_db', username: 'ff_app', passwordSecretRef: 'P4ss-w0rd_~test' }));
  ...
  it('password con caratteri speciali → percent-encoded (no url rotto)', () => {
    const url = buildPostgresUrl(conn({ hostname: 'h', port: 5432, database: 'd', username: 'u', passwordSecretRef: 'a@b:c/d?e' }));
  ...
  it('username senza password → solo user@ (back-compat)', () => {
    expect(buildPostgresUrl(conn({ hostname: 'h', port: 5432, database: 'd', username: 'u' }))).toBe('postgres://u@h:5432/d');
  });
  ...
  it('sslMode → query param', () => {
    expect(buildPostgresUrl(conn({ hostname: 'h', port: 5432, database: 'd', username: 'u', passwordSecretRef: 'p', sslMode: 'require' }))).toContain('?sslmode=require');
  });
});
...
describe('PostgresAdapter', () => {
  it('rejects connect when no URL/hostname provided', async () => {
    ...
    await expect(adapter.connect({
      id: 'd', tenantId: 'default', name: 'X',
      connection: { engine: 'postgres', embedded: false },
      tables: [], relations: [], createdAt: '2026-05-19T00:00:00Z', updatedAt: '2026-05-19T00:00:00Z',
    })).rejects.toThrow();
  });
    ...
    const preview = await adapter.previewMigration([
      {
        ...
        table: {
          ...
          columns: [
            { id: 'id', name: 'id', type: 'uuid', constraints: { primaryKey: true, nullable: false, unique: false } },
  ...
  it('rejects invalid identifiers (SQL injection guard)', async () => {
    const adapter = new PostgresAdapter();
    await expect(adapter.previewMigration([
      { kind: 'drop_table', tableName: 'drop_me; DROP TABLE users;--' },
    ])).rejects.toThrow();
  });
...
describe('PostgresAdapter — N19 audit: executeRaw readOnly gate (source inspection)', () => {
  ...
  it('source uses classifyStatement per-statement loop for readOnly check', () => {
    ...
    expect(adapterSource).toMatch(/kind\s*!==\s*['"]select['"]\s*&&\s*kind\s*!==\s*['"]explain['"]/);
  });
