...
function renderAction(a: MigrationAction): string {
  switch (a.kind) {
    case 'create_table':
      return `IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = '${a.table.name}') CREATE TABLE ${quoteIdent(a.table.name)} (\n  ${a.table.columns.map(renderColumn).join(',\n  ')}\n);`;
    case 'drop_table':
      return `IF EXISTS (SELECT * FROM sys.tables WHERE name = '${a.tableName}') DROP TABLE ${quoteIdent(a.tableName)};`;
    case 'rename_table':
      return `EXEC sp_rename '${a.from}', '${a.to}';`;
    case 'add_column':
      return `ALTER TABLE ${quoteIdent(a.tableName)} ADD ${renderColumn(a.column)};`;
    case 'drop_column':
      return `ALTER TABLE ${quoteIdent(a.tableName)} DROP COLUMN ${quoteIdent(a.columnName)};`;
    // SQL Server rinomina le colonne via sp_rename (no ALTER RENAME COLUMN).
    case 'rename_column':
      return `EXEC sp_rename '${a.tableName.replace(/'/gu, "''")}.${a.from.replace(/'/gu, "''")}', '${a.to.replace(/'/gu, "''")}', 'COLUMN';`;
    case 'alter_column':
      return `-- ALTER COLUMN ${a.columnName} on ${a.tableName} — MSSQL requires ALTER COLUMN syntax; generated manually`;
    case 'add_relation': {
      const fk = `${a.relation.fromTable}_${a.relation.fromColumn}_fk`;
      const cascade =
        a.relation.onDelete === 'cascade'
          ? 'CASCADE'
          : a.relation.onDelete === 'set null'
            ? 'SET NULL'
            : 'NO ACTION';
    ...
    case 'drop_relation':
      return `-- drop relation ${a.relationId}`;
    case 'add_index': {
    ...
    case 'drop_index':
      return `DROP INDEX ${quoteIdent(a.indexName)} ON ${quoteIdent(a.tableName)};`;
    ...
    case 'create_view':
      return renderCreateViewSql(a.view, quoteIdent);
    case 'drop_view':
      return renderDropViewSql(a.viewName, quoteIdent);
  }
}
...
export class MssqlAdapter implements IDatabaseAdapter {
  ...
  async connect(database: Database): Promise<void> {
    ...
    this.pool = await sql.connect({
      ...
      options: {
        ...
        trustServerCertificate: conn.sslMode === 'require',
      },
  ...
  async applyMigration(
    ...
    for (const a of actions) {
      ...
      else if (a.kind === 'drop_table' || a.kind === 'add_column' || a.kind === 'drop_column')
  ...
  async query<T = Record<string, unknown>>(spec: QuerySpec): Promise<QueryResult<T>> {
    ...
    const text =
      `SELECT ${select} FROM ${quoteIdent(spec.table)} ${where} ${orderClause} OFFSET ${offset.toString()} ROWS FETCH NEXT ${limit.toString()} ROWS ONLY`.trim();
  ...
  async update(
    ...
    setCols.forEach((c) => {
      req.input(`set_${c}`, patch[c]);
    });
    whereCols.forEach((c) => {
      req.input(`w_${c}`, where[c]);
    });
  ...
  async executeRaw(text: string, opts: RawQueryOptions = {}): Promise<RawQueryResult> {
    ...
    try {
      for (let i = 0; i < statements.length; i++) {
        ...
        const limited =
          isSelect && opts.rowLimit !== undefined
        ...
        if (isSelect) {
          ...
          const colsMeta = (
            result.recordset as unknown as
              | { columns?: Record<string, { name?: string; type?: { name?: string } }> }
              | undefined
          )?.columns;
          const columns = colsMeta
            ? Object.entries(colsMeta).map(([k, v]) => ({
                name: v?.name ?? k,
                ...(v?.type?.name ? { type: v.type.name } : {}),
              }))
            : rows[0]
          ...
          lastRows = [];
          lastColumns = [];
          lastAffected = result.rowsAffected[0] ?? 0;
          breakdown.push({
            index: i,
            kind,
            rowCount: 0,
