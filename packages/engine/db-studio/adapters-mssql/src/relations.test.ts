...
const FK_ROWS = [
  ...
  { from_table: 'orders', from_column: 'shop_id', to_table: 'shops', to_column: 'id', on_delete: 'SET_NULL' }, // *_desc underscore
];
...
function adapterWithPool(rows: unknown[]): { adapter: MssqlAdapter; query: () => string } {
  ...
  (adapter as unknown as { pool: unknown }).pool = {
    request: () => ({ query: (sql: string) => { captured = sql; return Promise.resolve({ recordset: rows }); } }),
  };
...
describe('MssqlAdapter.introspectRelations', () => {
  it('mappa le FK + onDelete con underscore (SET_NULL → "set null")', async () => {
    ...
    expect(rels[0]).toMatchObject({ fromTable: 'orders', fromColumn: 'user_id', toTable: 'users', toColumn: 'id', onDelete: 'cascade' });
...
describe('MssqlAdapter — rename_column', () => {
  it('genera sp_rename (SQL Server non ha ALTER RENAME COLUMN)', async () => {
    const sql = await new MssqlAdapter().previewMigration([{ kind: 'rename_column', tableName: 'users', from: 'name', to: 'full_name' }]);
