...
function newPool(): string {
  return `test-${randomUUID()}`;
}
async function inflightCount(pool: string): Promise<number> {
  return client.zcard(`liara:adm:${pool}:inflight`);
}
async function cleanup(pool: string): Promise<void> {
  await client.del(
    `liara:adm:${pool}:inflight`,
    `liara:adm:${pool}:queue`,
    `liara:adm:${pool}:qmeta`,
  );
}
...
describe.skipIf(!available)('redis-queue su Redis reale', () => {
  ...
  it('cap pieno → coda (pos/ahead), poi release → ri-tenta → ammesso + tolto da coda/qmeta', async () => {
    ...
    const q = await tryAdmit(
      redis,
      pool,
      baseInput({ requestId: 'r2', tenantId: 'tX', nowMs: Date.now() + 1 }),
    );
    ...
    const r2 = await tryAdmit(
      redis,
      pool,
      baseInput({ requestId: 'r2', tenantId: 'tX', nowMs: Date.now() + 2 }),
    );
  ...
  it('anti-flood per-tenant: cap raggiunto → rejected tenant_flood; altro tenant passa', async () => {
    ...
    await tryAdmit(
      redis,
      pool,
      baseInput({ requestId: 'q1', tenantId: 'T', maxPerTenantQueue: 2, nowMs: Date.now() + 1 }),
    );
    await tryAdmit(
      redis,
      pool,
      baseInput({ requestId: 'q2', tenantId: 'T', maxPerTenantQueue: 2, nowMs: Date.now() + 2 }),
    );
    const flood = await tryAdmit(
      redis,
      pool,
      baseInput({ requestId: 'q3', tenantId: 'T', maxPerTenantQueue: 2, nowMs: Date.now() + 3 }),
    );
    expect(flood).toMatchObject({ outcome: 'rejected', rejectReason: 'tenant_flood' });
    const other = await tryAdmit(
      redis,
      pool,
      baseInput({
        requestId: 'q4',
  ...
  it('una sequenza mista produce gli STESSI esiti via Lua e via decideAdmission', async () => {
    ...
    const steps: { requestId: string; tenantId: string }[] = [
      { requestId: 'a', tenantId: 't1' },
      { requestId: 'b', tenantId: 't1' },
      { requestId: 'c', tenantId: 't2' },
      { requestId: 'd', tenantId: 't2' },
    ...
    for (const s of steps) {
      ...
      expect({
        ...
        ahead: lua.ahead,
        rejectReason: lua.rejectReason,
      }).toEqual({
        outcome: pure.outcome,
        position: pure.position,
        ahead: pure.ahead,
        rejectReason: pure.rejectReason,
      });
    }
    await cleanup(pool);
  });
...
describe('redis-queue — disponibilità Redis', () => {
  it(
    available
      ? 'Redis reale disponibile → integrazione eseguita'
      : 'Redis NON disponibile → integrazione SALTATA (set TEST_REDIS_URL)',
    () => {
