...
interface FakeRedis extends RedisLike { _z: Map<string, Map<string, number>>; _h: Map<string, Map<string, string>>; }
function makeFakeRedis(): FakeRedis {
  const z = new Map<string, Map<string, number>>();
  const h = new Map<string, Map<string, string>>();
  const zmap = (k: string): Map<string, number> => { let m = z.get(k); if (!m) { m = new Map(); z.set(k, m); } return m; };
  const hmap = (k: string): Map<string, string> => { let m = h.get(k); if (!m) { m = new Map(); h.set(k, m); } return m; };
  function zaddRaw(key: string, args: (string | number)[]): void {
    let i = 0; let xx = false;
    if (args[0] === 'XX') { xx = true; i = 1; }
    const m = zmap(key);
    for (; i + 1 < args.length; i += 2) { const s = Number(args[i]); const mem = String(args[i + 1]); if (xx && !m.has(mem)) continue; m.set(mem, s); }
  }
  const redis: FakeRedis = {
    _z: z, _h: h,
    async zadd(key, ...args) { zaddRaw(key, args); return 1; },
    async zrem(key, ...members) { const m = zmap(key); for (const x of members) m.delete(x); return 1; },
    async hdel(key, ...fields) { const m = hmap(key); for (const f of fields) m.delete(f); return 1; },
    async eval(_script, _numKeys, ...args) {
      const a = args as string[];
      const [inflightK, queueK, qmetaK, rid, tenant, now, lease, maxc, maxq, maxpt] = a;
      const inflight = zmap(inflightK!); const queue = zmap(queueK!); const qmeta = hmap(qmetaK!);
      const state: PoolState = {
        active: [...inflight.entries()].map(([id, score]) => ({ id, expiresAtMs: score })),
        queue: [...queue.entries()].sort((x, y) => (x[1] - y[1]) || (x[0] < y[0] ? -1 : 1))
          .map(([id, score]) => ({ id, enqueuedAtMs: score, tenantId: qmeta.get(id) ?? '' })),
      };
      const d = decideAdmission(state, {
        requestId: rid!, tenantId: tenant!, nowMs: Number(now), leaseMs: Number(lease),
        maxConcurrent: Number(maxc), maxQueueDepth: Number(maxq), maxPerTenantQueue: Number(maxpt),
      });
      // applica il nuovo stato (come farebbe la Lua)
      inflight.clear(); for (const l of d.state.active) inflight.set(l.id, l.expiresAtMs);
      queue.clear(); qmeta.clear();
      for (const w of d.state.queue) { queue.set(w.id, w.enqueuedAtMs); qmeta.set(w.id, w.tenantId); }
      return d.outcome === 'rejected' ? [d.outcome, d.rejectReason] : [d.outcome, d.position, d.ahead];
    },
  };
  return redis;
}
...
const POOL = 'vllm';
const OPTS = {
  pool: POOL, maxConcurrent: 1, leaseMs: 30_000, heartbeatMs: 1_000_000,
  pollMs: 10, maxQueueDepth: 50, maxPerTenantQueue: 5, maxWaitMs: 60_000,
};
...
describe('ammissione diretta', () => {
  it('slot libero → admitted; release() ferma l\'heartbeat e libera lo slot', async () => {
    ...
    const handle = await admit({ ...OPTS, redis, requestId: 'r1', tenantId: 't1', now: () => 1000, schedule });
...
describe('coda → ammesso quando si libera uno slot', () => {
  it('emette onQueue(position) finché bloccato, poi entra appena lo slot si libera', async () => {
    // occupa l'unico slot
    await tryAdmit(redis, POOL, { requestId: 'blocker', tenantId: 't0', nowMs: 1, leaseMs: 30_000, maxConcurrent: 1, maxQueueDepth: 50, maxPerTenantQueue: 5 });
    const positions: { position: number; ahead: number }[] = [];
    let t = 100;
    // al primo sleep, libera il blocker → il poll successivo ammette r1
    const sleep = vi.fn(async () => { redis._z.get('liara:adm:vllm:inflight')!.delete('blocker'); });
    const handle = await admit({
      ...OPTS, redis, requestId: 'r1', tenantId: 't1',
      now: () => t, sleep, schedule: noopSchedule,
      onQueue: (p) => { positions.push(p); t += 50; },
    });
...
describe('abort del client (SSE chiuso)', () => {
  it('signal già abortito → AdmissionAbortError e NESSUNO slot/coda lasciato', async () => {
    ...
    ac.abort();
    await expect(admit({ ...OPTS, redis, requestId: 'r1', tenantId: 't1', signal: ac.signal, now: () => 1, schedule: noopSchedule }))
  ...
  it('abort MENTRE in coda → esce dalla coda (cleanup) e lancia AbortError', async () => {
    await tryAdmit(redis, POOL, { requestId: 'blocker', tenantId: 't0', nowMs: 1, leaseMs: 30_000, maxConcurrent: 1, maxQueueDepth: 50, maxPerTenantQueue: 5 });
    const ac = new AbortController();
    const sleep = vi.fn(async () => { ac.abort(); }); // aborta durante l'attesa
    await expect(admit({ ...OPTS, redis, requestId: 'r1', tenantId: 't1', signal: ac.signal, now: () => 1, sleep, schedule: noopSchedule }))
      .rejects.toBeInstanceOf(AdmissionAbortError);
...
describe('backpressure', () => {
  it('coda piena → AdmissionOverloadError(queue_full)', async () => {
    ...
    await tryAdmit(redis, POOL, { requestId: 'blocker', tenantId: 't0', nowMs: 1, leaseMs: 30_000, maxConcurrent: 1, maxQueueDepth: 2, maxPerTenantQueue: 9 });
    await tryAdmit(redis, POOL, { requestId: 'q1', tenantId: 't0', nowMs: 2, leaseMs: 30_000, maxConcurrent: 1, maxQueueDepth: 2, maxPerTenantQueue: 9 });
    await tryAdmit(redis, POOL, { requestId: 'q2', tenantId: 't0', nowMs: 3, leaseMs: 30_000, maxConcurrent: 1, maxQueueDepth: 2, maxPerTenantQueue: 9 });
    const err = await admit({ ...OPTS, redis, requestId: 'r1', tenantId: 't1', maxQueueDepth: 2, now: () => 4, schedule: noopSchedule }).catch((e: unknown) => e);
...
describe('timeout di attesa', () => {
  it('superato maxWaitMs in coda → AdmissionTimeoutError + cleanup', async () => {
    await tryAdmit(redis, POOL, { requestId: 'blocker', tenantId: 't0', nowMs: 1, leaseMs: 1e9, maxConcurrent: 1, maxQueueDepth: 50, maxPerTenantQueue: 5 });
    let t = 0;
    const sleep = vi.fn(async () => { t += 1000; }); // ogni poll avanza il clock
    const err = await admit({
      ...OPTS, redis, requestId: 'r1', tenantId: 't1', maxWaitMs: 1500,
      now: () => t, sleep, schedule: noopSchedule,
    }).catch((e: unknown) => e);
